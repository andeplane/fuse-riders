...
export const RULES = "fuse-p2p-48"; // 48: fixed catalog obstacle variants on every scenery map; circular rocks across collision paths, radius-zero shell walls. 47: a member may log its own presence: `PRESENCE false` about itself steps it away (its page is hidden) — its seat, round place and rating stay, its controls are neutral and its own entries unread, and it drops out of the succession order — and `PRESENCE true` about itself brings it back; folds and watchers carry the `away` mark. 46: the room passes to the rider in the next seat, not the lowest member id: succession ranks connected human riders by seat. 45: generation- and match-scoped ready votes start games and rematches deterministically. 44 was reserved by the hidden-tab policy PR, which ships as 47.

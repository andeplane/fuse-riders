 ...
 *
 * `foldPlayerEntries` is the only caller, and calls them with the ids the log carries; that is what every replica
 * agrees on. Ids are what make the commands safe to derive from a stream that can repeat or reorder: a press that is
 * not newer than the newest one seen is a repeat, and a release or cancel that does not name the held gesture is
 * stale. Both are no-ops. Gesture ids start at 1 (`isEntry` refuses 0), so 0 can mean "none held".
 */

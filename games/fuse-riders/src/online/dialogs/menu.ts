import { createConfirm } from "fuse-ui";
...
export interface MenuDialogOptions {
  solo: boolean;
  /** This device created the room, so it may close it for everyone (END ROOM) as well as leave it. */
  canEnd: () => boolean;
  playerId: () => string;
  ...
  statsHidden: () => boolean;
  toggleStats: () => void;
  /** Leaves the room (or, `ending`, closes it for everyone) and goes back to the main menu. */
  leave: (ending: boolean) => Promise<void>;
  document?: Document;
}
...
/**
 * EXIT (solo) or ROOM: leave (the next seat takes over as host) or, for the room's creator, END ROOM for everyone;
 * the session standings and, in a room, the link diagnostics.
 */
export function createMenuDialog(
  ...
  return {
    ...
    open() {
      const canEnd = options.canEnd() && !solo;
      const {
        question,
        choices,
        confirm: leave,
      ...
      const end = node("button", "END ROOM", "exit-end", doc);
      end.hidden = !canEnd;
      ...
      const endOrLeave = async (ending: boolean) => {
        ...
        await options.leave(ending);
      };
      const content: Node[] = [question, choices];
      ...
      if (!solo) {
        ...
        // Each opening builds its own report, so a menu closed and reopened within the second stops the old refresh
        // too: it ends once its `<pre>` is no longer the one on screen.
        const refresh = setInterval(() => {
          if (dialogs.current() !== "menu" || !body.contains(diagnostics)) {

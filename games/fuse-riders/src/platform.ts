 ...
 * account's totals and career, and rivalries from eliminations, plus the account rules every game shares (a rider
 * name and a Fuse Riders avatar). Attestation, settlement, Elo and storage are the platform's. The room service
 * registers it beside the other games (`service/history.ts`), as it does Pig's (`games/dice/src/platform.ts`).
 * Node-only through `fuse-platform`: browser code imports only its types.
 */

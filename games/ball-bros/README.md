...
- **W / Space** or up arrow: launch your starting ball after the countdown. Unlaunched balls release automatically after three seconds.
- Touch: left, launch and right buttons. Movement can be held while launching.
...
The `ball-bros-1` golden in `tests/engine.test.ts` records a complete deterministic five-bot match. Change the game rules version and review/refresh its hash for intended physics changes. Fuse Riders' rules and golden remain unchanged.

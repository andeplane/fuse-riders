...
import { FAST } from "./fixtures/dice.js";
import { DiceMesh, type TestDiceRuntime } from "./fixtures/mesh.js";

type DiceWorld = World<DiceRoom, DiceEntry, DiceView, DiceEvent, DiceSettings>;
 ...
 */
function replica(self = "a"): DiceWorld {
  const world = new World(diceGame, createRoom("m0", FAST), "a", self);
...
const kinds = (events: readonly WorldEvent<DiceEvent>[]) =>
...
test("the same log folds to the same room and hash on every replica, however the packets batch it", () => {
  ...
  assert.equal(diceGame.hash(whole.state), diceGame.hash(trickled.state));
  ...
  // A different match id seeds different dice.
  const other = new World(diceGame, createRoom("m0", FAST), "a", "a");
  ...
  other.advance(300);
  assert.notEqual(diceGame.hash(other.state), diceGame.hash(whole.state));
});
...
test("a late HOLD stamped before the timer ran out undoes a replica's speculative auto-hold and the bot's next roll", () => {
  ...
  const roll: DiceEntry = [1, 5, ROLL, 1],
    hold: DiceEntry = [2, 44, HOLD, 1];
  ...
  assert.notEqual(diceGame.hash(late.state), diceGame.hash(onTime.state));
  ...
  late.advance(120);
  assert.equal(diceGame.hash(late.state), diceGame.hash(onTime.state));
...
test("three phones and a bot play Pig over dropped, duplicated and reordered packets and agree on every roll", () => {
  ...
  const mesh = new DiceMesh("a", { turnTicks: 60, display: false });
  const ids = ["a", "b", "c"];

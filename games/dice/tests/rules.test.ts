...
test("a roll adds to the turn total and restarts the timer; a 1 busts, loses the total and passes the turn", () => {
  ...
  assert.deepEqual(diceView(room).lastRoll, { id: "a", value: 5, n: 2 });
  rig(room, 1);
  assert.deepEqual(fold(room, { a: [[ROLL, 1]] }), [
...
test("hold banks the turn total; reaching 50 wins the round, and two round wins take the match", () => {
  ...
  assert.equal(diceView(room).roundWinnerId, "a");
  assert.equal(diceView(room).timerTicks, 0);
  assert.equal(diceView(room).currentId, undefined);
  assert.deepEqual(
  ...
  assert.equal(room.turn, "b");
  assert.deepEqual(room.scores, { a: 0, b: 0 });
  assert.equal(diceView(room).roundWinnerId, undefined);
  ...
  assert.equal(diceView(room).winnerId, "a");
  assert.equal(diceGame.stage(room), "over");
  const result = matchResult(room)!;
...
test("the turn timer holds for an idle player; a HOLD stamped at the deadline tick comes first", () => {
  ...
  assert.equal(diceView(idle).timerTicks, 1);
...
test("a late joiner plays from the next pass, and a match counts only the seats that played", () => {
  ...
  assert.equal(diceView(room).players.length, 3);
});
...
test("two bots play a whole match to a winner, the same way every time", () => {
  ...
  assert.equal(diceGame.hash(play()), diceGame.hash(room));
  const result = matchResult(room)!;
...
test("settings, bot ids and names go through the shared management entries and helpers", () => {
  ...
  assert.deepEqual(diceGame.settings(room), { turnTicks: 100, display: true });
  assert.equal(diceGame.seating.sharedScreen(room.settings), true);
  assert.deepEqual(diceGame.seating.soloSettings(room.settings), {
    turnTicks: 100,
    display: false,
  });
  ...
  assert.equal(diceGame.seating.botId(room, new Set()), "bot:2");
  assert.equal(diceGame.seating.botId(room, new Set(["bot:2"])), "bot:3");
  assert.equal(diceGame.seating.botName(2), "Bot 3");
  ...
  assert.equal(diceGame.seat(room, "a")?.name, "Ada");
  assert.equal([...diceGame.members(room)].length, 2);
  assert.deepEqual(diceGame.scope(room), { matchId: "m1", round: 1 });
});
...
test("a new room's view is an empty lobby", () => {
  const room: DiceRoom = createRoom("m0", DEFAULT_SETTINGS);
  const view = diceView(room);

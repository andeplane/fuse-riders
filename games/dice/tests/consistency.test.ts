...
import { FAST, fold, rig, runTo, started } from "./fixtures/dice.js";

const { encode, decode } = diceGame.checkpoint;
const roundTrip = (room: DiceRoom) =>
...
test("a seat that joins mid-round and banks points keeps the room decodable", () => {
  ...
  assert.equal(diceGame.hash(decoded), diceGame.hash(room));
...
test("the entry boundary and the checkpoint accept the same match ids", () => {
  ...
  for (const matchId of [
    ...
    const entry = isDiceEntry([1, 1, ACTION, "start", matchId]);
  ...
  assert.equal(isDiceEntry([1, 1, ACTION, "start", "a b"]), false);
});
...
test("members come in slot then id order, before and after a checkpoint", () => {
  ...
  const ids = (from: DiceRoom) =>
    [...diceGame.members(from)].map((seat) => seat.id);
...
test("settings logged during a match take effect at the next match", () => {
  ...
  assert.equal(diceView(room).turnTicks, FAST.turnTicks);
  assert.equal(
    diceGame.settings(room).turnTicks,
    100,

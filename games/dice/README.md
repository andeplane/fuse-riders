...
| `src/game/game.ts`       | `diceGame`, the `RollbackGame` the netcode runs, and `diceView`, what the page renders                                    |
...
| `src/platform.ts`        | The `GameRegistration` for `fuse-platform`: per-player stats and account totals (exported as `dice/platform`)             |
| `src/app/`               | The page: `presenter.ts` (view to screen, pure), `session.ts`, `reports.ts`, `runtime.ts` (ROLL/HOLD) and `main.ts` (DOM) |
| `index.html`             | The page's entry; Vite builds it to `dist/dice/index.html`                                                                |
| `tests/`                 | Rules, checkpoint, rollback over lossy links, the presenter, reports, session and runtime                                 |

The page renders every frame from the view, never from events: a rollback can change a roll after it was shown, and the netcode deduplicates events by position, so only the view carries the correction. Dice come from a generator in the room state, so every device rolls the same numbers, and any member could compute the next roll. That fits the trust model: every member is trusted with the shared log.

Checks: `npx tsx --test games/dice/tests/*.test.ts tests/dice-service.test.ts`, and the browser smoke `ONLINE_URL=http://localhost:8890/ npx tsx scripts/dice-smoke.ts` (`BROWSER=webkit` for WebKit).
...
This copies this folder to `games/snake-eyes` with every id and name renamed (`"dice"` becomes `"snake-eyes"`, `diceGame` becomes `snakeEyesGame`, `dice.css` becomes `snakeEyes.css`). The copy is Pig under the new name, with its tests passing. Then:

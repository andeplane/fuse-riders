...
const load = (n: string) => parseTrack(JSON.parse(readFileSync(`games/fuse-drivers/tracks/${n}.tmj`, 'utf8')), n);
...
test('the parser accepts every committed track and variant', () => {
  const names = readdirSync('games/fuse-drivers/tracks').filter((f) => f.endsWith('.tmj')).map((f) => f.replace('.tmj', ''));
...
test('entering the deck from its entry side sets onBridge; the under lane does not; leaving clears it', () => {
  ...
  for (let i = 0; i < 40 && r.state.trucks[0].onBridge; i++) r = step(r.state, [NEUTRAL_INPUT], sidewinder);
  assert.equal(r.state.trucks[0].onBridge, false);
  ...
  assert.ok(Math.abs(ur.state.trucks[0].y - cy) < 40, 'under-lane truck stays in its lane through the crossing');
});
...
test('a deck truck is held by the railings and never drops onto the under lane sideways', () => {
  ...
  assert.ok(t.onBridge || t.y > bridge.y1, 'either still on the deck or exited at the bottom');
  assert.ok(t.x > bridge.x0 && t.x < bridge.x1, `x ${t.x} stayed between the railings`);
});
...
test('a drone zaps from its own position, once per second per truck, and dies with its owner', () => {
  ...
  const d: Drone = { id: 1, owner: 0, launchedTick: 100, zaps: 0, lastZapTick: [0, 0] };
  ...
  const again = stepDrones([{ ...d, lastZapTick: [0, 100] }], [owner, near], 110);
  assert.equal(again.hits.length, 0);
  assert.equal(stepDrones([d], [{ ...owner, respawnAtTick: 200 }, near], 100).drones.length, 0);
});
...
test('an oil slick makes the truck slide but keeps the tile underneath working', () => {
  ...
  const pad = (() => { for (let i = 0; i < refinery.surface.length; i++) if (refinery.surface[i] === 'boost') return { x: (i % refinery.cols) * 32 + 16, y: Math.floor(i / refinery.cols) * 32 + 16 }; throw new Error('no pad'); })();
  s = { ...s, trucks: [at(s.trucks[0], pad.x, pad.y, 0, { speed: 0 })], oils: [{ id: 9, owner: 0, x: pad.x, y: pad.y, droppedTick: 100 }] };
  const r = step(s, [NEUTRAL_INPUT], refinery);
  assert.ok(r.state.trucks[0].oilUntilTick > r.state.tick);
  assert.ok(r.state.trucks[0].padUntilTick > r.state.tick, 'the boost pad still fires under the slick');
});

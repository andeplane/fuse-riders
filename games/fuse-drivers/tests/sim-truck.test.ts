...
const run = (
  ...
  ticks: number,
  surface: "dirt" | "toxic" | "boost" | "mud" = "dirt",
  from = 0,
) => {
  let rng = 1;
...
test("turns 270 degrees in 30 ticks at rest", () => {
  ...
  for (let i = 0; i < 30; i++) {
    const [n, r] = stepTruck(
      { ...t, speed: 0 },
      { ...NEUTRAL_INPUT, right: true },
      "dirt",
      i,
      rng,
    );
    total += Math.atan2(
      Math.sin(n.heading - t.heading),
      Math.cos(n.heading - t.heading),
    );
    t = n;
    rng = r;
  }
...
test("drift enters on tick 5 of a held turn at speed and boosts after 8 ticks, not 7", () => {
  ...
  assert.equal(
    run(drifted8, {}, 1, "dirt", 53).boostUntilTick,
    53 + config.truck.boostTicks,
  );
});
...
test("oil offsets movement direction but never heading, and not while spun out", () => {
  ...
  const [spun] = stepTruck(
    { ...base, spinUntilTick: 100 },
    NEUTRAL_INPUT,
    "dirt",
    40,
    1,
...
test("heading never becomes -0", () => {
  const [t] = stepTruck(
    { ...createTruck(0, 0, 0, -0), airborneUntilTick: 5 },
    NEUTRAL_INPUT,
    "dirt",
    0,
    1,
...
test("nitro on a boost pad yields x1.5, not x2.25", () => {
  ...
  const both = stepTruck(
    { ...base, nitroUntilTick: 100, padUntilTick: 100 },
    NEUTRAL_INPUT,
    "dirt",
    40,
    1,
...
test("surface multiplies displacement instantly and nitro ignores it", () => {
  ...
  const nitroToxic = stepTruck(
    { ...base, nitroUntilTick: 100 },
    NEUTRAL_INPUT,
    "toxic",
    40,
    1,
...
test("spin-out and airborne block steering; brake reverses from a stop", () => {
  ...
  assert.equal(
    stepTruck(spun, { ...NEUTRAL_INPUT, right: true }, "dirt", 0, 1)[0].heading,
    0,
  );

...
test("a scripted five-race series accumulates points and money and ranks by points then money", () => {
  ...
  assert.equal(
    new Set(s.tracks.slice(0, 3)).size,
    3,
    "no repeats until the pool is exhausted",
  );
  for (let r = 0; r < 5; r++) {
    const state = {
      ...createRace(track, r, Array(3).fill(statsFor(NO_LEVELS))),
      placements: [2, 0, 1],
    };
    state.trucks[2] = { ...defined(state.trucks[2]), kills: 1 };
    s = applyRace(s, state);
  }
  assert.equal(s.raceIndex, 5);
  assert.deepEqual(
    standings(s).map((d) => d.slot),
    [2, 0, 1],
  );
  const spent = {
    ...s,
    drivers: s.drivers.map((d) => ({
      ...d,
      points: 10,
      money: d.slot === 2 ? 0 : d.money,
    })),
  };
  assert.equal(
    defined(standings(spent)[0]).slot,
    2,
    "tie-break is money earned, not money left",
  );
  assert.equal(defined(s.drivers[2]).points, 5 * config.points[0]);
  assert.equal(
    defined(s.drivers[2]).money,
    5 * (config.prize[0] + config.killBonus),
  );
  assert.equal(defined(s.drivers[1]).money, 5 * config.prize[2]);
});
...
test("bots spend until nothing is affordable", () => {
  ...
  assert.ok(
    Math.abs(
      statsFor({ ...NO_LEVELS, tires: 5 }).turnRateHigh - (270 * Math.PI) / 180,
    ) < 1e-9,
  );
});

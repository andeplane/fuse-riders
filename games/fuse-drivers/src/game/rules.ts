 ...
 * fuseDrivers come from the seeded generator in the room (`rng`): every member can compute the rolls to come, which the trust
 * model allows, since every member is trusted with the shared log.
 */
export const ROLL = 0,
  HOLD = 1;
export type FuseDriversAction = typeof ROLL | typeof HOLD;
...
export type PlayEntry = [
  seq: number,
  tick: number,
  kind: FuseDriversAction,
  turn: number,
];
export type FuseDriversEntry = ManagementEntry<FuseDriversSettings> | PlayEntry;
...
export function parseSettings(raw: unknown): FuseDriversSettings | undefined {
  ...
  const { turnTicks, display } = raw as Partial<FuseDriversSettings>;
  if (
    keys.length !== 2 ||
...
export function rollDie(room: FuseDriversRoom): number {
...
export function createRoom(matchId: string, settings: FuseDriversSettings): FuseDriversRoom {
  return {
    tick: 0,
    matchId,
    round: 1,
...
export function botDelay(room: FuseDriversRoom): number {
  return 12 + ((room.turnNo * 7 + room.turnTotal) % 9);
}

function beginTurn(room: FuseDriversRoom, id: string, tick: number): void {
  room.turn = id;
  room.turnNo++;
  room.turnTotal = 0;
...
function passTurn(room: FuseDriversRoom, tick: number): void {
...
function remember(room: FuseDriversRoom): void {
  for (const seat of players(room))
    room.roster[seat.id] = { name: seat.name, slot: seat.slot };
}
function startRound(room: FuseDriversRoom, tick: number): void {
  room.stage = "running";
  room.scores = {};
  room.roundWinner = "";
  room.lastRoll = 0;
...
function resetMatch(room: FuseDriversRoom, matchId: string): void {
...
function leader(room: FuseDriversRoom): string {
...
export function botEntry(
  room: FuseDriversRoom,
  tick: number,
): [FuseDriversAction, number] | undefined {
...
export function foldTick(
  ...
  if (room.stage === "between" && tick >= room.resumeAt) {
    ...
    if (!seat || seat.watcher) passTurn(room, tick);
      ...
      const entries: [FuseDriversAction, number][] = seat.bot

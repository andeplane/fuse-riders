...
function closestSegment(
  ...
  for (let k = 0; k < count; k++) {
    ...
    if (d < bestD) {
      bestD = d;
      best = i;
      bestAlong = t * Math.sqrt(len2);
    }
  }
  return { i: best, along: bestAlong };
}

const segmentLength = (wp: Point[], i: number) => {
  const a = wp[i % wp.length]!,
    b = wp[(i + 1) % wp.length]!;
  return hypot(b.x - a.x, b.y - a.y) || 1;
};
const tangentOf = (wp: Point[], i: number) => {
  const a = wp[i % wp.length]!,
    b = wp[(i + 1) % wp.length]!;
...
function wantsItem(state: RaceState, t: Truck): boolean {
  ...
  const others = state.trucks.filter(
    (o) =>
      o.slot !== t.slot &&
      !o.respawnAtTick &&
      !o.finishedTick &&
      state.tick >= o.invulnerableUntilTick,
  );
  const rel = (o: Truck) => ({
    d: hypot(o.x - t.x, o.y - t.y),
    a: Math.abs(wrapAngle(atan2(o.y - t.y, o.x - t.x) - t.heading)),
  });
  switch (t.item) {
    case "missile":
      return others.some((o) => {
        const r = rel(o);
        return (
          (r.d < 500 && r.a < config.items.missile.lockCone) ||
          (r.d < 300 && r.a > Math.PI / 2)
        );
      });
    case "mine":
    case "oil":
      return others.some((o) => {
        const r = rel(o);
        return r.d < 300 && r.a > Math.PI / 2;
      });
    case "shield":
      return t.lockedUntilTick > state.tick;
    case "emp":
      return (
        others.filter((o) => rel(o).d <= config.items.emp.range).length >= 2
      );
    case "nitro":
    case "drone":
      return true;
...
export function botInput(
    ...
    return [
      {
        ...NEUTRAL_INPUT,
        nitro:
          state.tick >= state.countdownEndTick - config.truck.rocketStartWindow,
      },
      memory,
    ];
  }
  ...
  const vertexOf = (c: number) => {
    const m = cps[(c + n) % n]!.mid;
    let best = 0;
    wp.forEach((p, i) => {
      if (
        hypot(p.x - m.x, p.y - m.y) <
        hypot(wp[best]!.x - m.x, wp[best]!.y - m.y)
      )
        best = i;
    });
    return best;
  };
  const from = vertexOf(t.checkpoint - 1),
    to = vertexOf(t.checkpoint);
  const { i: segment, along } = closestSegment(
    wp,
    t,
    (from - 1 + wp.length) % wp.length,
    ((to - from + wp.length) % wp.length) + 2,
  );
  ...
  const visible = (p: Point) => {
    const len = hypot(p.x - t.x, p.y - t.y) || 1,
      ox = (-(p.y - t.y) / len) * flank,
      oy = ((p.x - t.x) / len) * flank;
    return [0, 1, -1].every(
      (s) =>
        !track.walls.some(
          (w) =>
            ...
            !(w.deck && !t.onBridge) &&
            crosses(
              { x: t.x + ox * s, y: t.y + oy * s },
              { x: p.x + ox * s, y: p.y + oy * s },
              w,
            ),
        ),
    );
  ...
  for (let d = Math.max(80, 0.35 * t.speed); d >= 30; d /= 2) {
    ...
    const nx = -sin(look.tangent),
      ny = cos(look.tangent);
    target = {
      x: look.p.x + nx * memory.lateral,
      y: look.p.y + ny * memory.lateral,
    };
  ...
  for (
    let k = 1, d = segmentLength(wp, segment) - along;
    (d < STRAIGHT_LENGTH || d < brakeLook) && k < wp.length;
    d += segmentLength(wp, segment + k), k++
  ) {
  ...
  const airborneOrSpun =
    state.tick < t.airborneUntilTick || state.tick < t.spinUntilTick;
    ...
    itemAlt:
      ...
      !state.trucks.some(
        (o) =>
          o.slot !== t.slot &&
          !o.respawnAtTick &&
          hypot(o.x - t.x, o.y - t.y) < 500 &&
          Math.abs(wrapAngle(atan2(o.y - t.y, o.x - t.x) - t.heading)) <
            config.items.missile.lockCone,
      ),
  };

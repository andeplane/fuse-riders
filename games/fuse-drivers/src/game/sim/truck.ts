import { BASE_STATS, config, DT, type SurfaceKind, type TruckStats } from './config.js';
 ...
 */
export function stepTruck(t: Truck, input: TruckInput, surface: SurfaceKind, tick: number, rng: number): [Truck, number] {
  ...
  let turnHeldTicks = dir !== 0 && dir === t.turnDir ? t.turnHeldTicks + 1 : dir !== 0 ? 1 : 0;
  ...
  if (canSteer) {
    ...
    if (driftDir !== 0) {
      if (!input.left && !input.right) {
        ...
        const opposite = (driftDir === -1 && input.right) || (driftDir === 1 && input.left);
        heading += driftDir * turnRate * c.driftTurnMul * (opposite ? c.driftOppositeMul : 1) * DT;
        driftTicks += 1;
      }
    } else {
      heading += dir * turnRate * DT;
      if (dir !== 0 && turnHeldTicks > c.driftEnterTicks && speedFrac >= c.driftMinSpeedFrac && surf.drift) {
        driftDir = dir;
        driftTicks = 0;
      }
    }
  ...
  let mul = Math.max(1, nitroActive ? c.nitroMul : 1, tick < t.padUntilTick ? c.boostPadMul : 1, tick < boostUntilTick ? c.boostMul : 1);

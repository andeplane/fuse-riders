...
try {
  for (let index = 0; index < 6; index++) {
    ...
    await page.evaluate(([code, token]) => (globalThis as unknown as { startMesh(code: string, token: string): void }).startMesh(code, token), [room.code, tokens[index]!]);
  }
  await waitReady('links');
  const ready = await Promise.all(pages.map(page => mesh<Snapshot>(page, 'mesh.snapshot()')));
  assert.ok(ready.every(peer => peer.stats.direct === 5 && peer.stats.relayed === 0), 'fifteen direct links');
  ...
  assert.ok(final.every(peer => peer.errors.length === 0), JSON.stringify(final.map(peer => peer.errors)));
  assert.deepEqual(errors, []);
  report.peers = final.map(peer => ({ id: peer.id, links: peer.links.length, received: peer.received, stats: peer.stats }));
  report.passed = true;
  ...
  report.diagnostics = await Promise.all(pages.map(page => mesh<Snapshot>(page, 'mesh.snapshot()').catch(() => 'unavailable')));
  throw error;
} finally {
  for (const page of pages) await mesh(page, 'mesh.stop()').catch(() => {});
  await writeFile('artifacts/p2p-mesh-browser.json', JSON.stringify(report, null, 2));

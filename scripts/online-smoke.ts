...
try{
  ...
  await guest.getByPlaceholder('Your name').fill('Guest');await guest.getByRole('button',{name:/^Change avatar/}).click();await guest.getByRole('button',{name:'Fox',exact:true}).click();await guest.getByRole('button',{name:'JOIN AS PLAYER',exact:true}).click();

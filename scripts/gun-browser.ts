...
try {
  ...
  tv.on('websocket', socket => socket.on('framereceived', frame => {
    const message = JSON.parse(frame.payload.toString()) as { type: string; tick?: number; state?: { bombs: { shell?: { gun?: boolean } }[] } };
    if (message.state?.bombs.some(bomb => bomb.shell?.gun)) tracerDelivered = true;
  ...
  startMatch(app.game); app.advance(COUNTDOWN_TICKS);
  const [shooter, target, spare] = [...app.game.players.values()].sort((a, b) => a.slot - b.slot);
  ...
  await waitFor(() => { app.advance(1); return app.game.shots.length > 0; });
  assert.equal(target!.alive, false, 'headshot resolved while the pointer remains down');
  assert.equal(app.game.shots.length, 1); assert.equal(app.game.blasts.length, 0);
  if (app.game.tick % 2) app.advance(1);
  await waitFor(() => deliveredTick >= app.game.tick);
  assert.equal(tracerDelivered, true, 'authenticated TV receives the gun tracer');
  await tv.locator('canvas.arena[data-renderer-status=ready]').waitFor();
  await mkdir('artifacts', { recursive: true });
  await tv.screenshot({ path: `artifacts/gun-${process.env.BROWSER ?? 'chrome'}.png` });
  ...
  console.log('Gun browser passed: pointer-down headshot, one shot, no blast, snapshot delivered, screenshot captured, tracer expired.');
} finally {

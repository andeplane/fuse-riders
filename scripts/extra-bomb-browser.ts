  ...
  for (const mode of ["webgl", "phaser-canvas"] as const)
    for (const themeId of ["neon-pixel", "clean-neon"] as const) {
      const result = await page.evaluate(
        async ({ mode, themeId }) => {
          ...
          const snapshot = {
            ...fixture,
            boundaryInset: 0,
            players: fixture.players.slice(0, 3).map((p, i) => ({
              ...p,
              name: ["TWO", "FOUR", "MAX + FIVE"][i]!,
              x: 240 + i * 510,
              y: 450,
              angle: -Math.PI / 2,
              extraBombs: [1, 3, 8][i]!,
          ...
          let badgePixels = 0;
          for (let y = 650; y < 670; y++)
            for (let x = 795; x < 825; x++) {
          ...
          if (badgePixels < 5)
            throw Error(`Missing gold +1 badge: ${badgePixels}`);
          Reflect.set(window, "disposeExtraBomb", () => {
            arena.destroy();
            canvas.remove();
          });
          return { badgePixels };
      ...
      await page.screenshot({
        path: `artifacts/extra-bomb-${mode}-${themeId}.png`,
      });
      await page.evaluate(() => {
        (Reflect.get(window, "disposeExtraBomb") as () => void)();
      });

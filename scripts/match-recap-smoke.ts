...
async function inside(page: Page, locator: Locator): Promise<void> {
  ...
  assert.ok(box && box.x >= -1 && box.y >= -1 && box.x + box.width <= view.width + 1 && box.y + box.height <= view.height + 1, `Element outside ${view.width}x${view.height}: ${JSON.stringify(box)}`);
}
async function assertRecapLayout(page: Page): Promise<{ podium: number; awards: number; totals: number; rows: number; comparisonScrolls: boolean }> {
...
try {
  for (const viewport of [{ width: 1280, height: 800 }, { width: 844, height: 390 }]) {
    ...
    const waitFor = async (predicate: () => boolean, timeoutMs: number, what: string) => { const deadline = Date.now() + timeoutMs; while (!predicate()) { assert.ok(Date.now() < deadline, `timed out waiting for ${what}`); await page.waitForTimeout(50); } };
    try {
      ...
      await page.screenshot({ path: `artifacts/match-recap-failure-${tag}.png` }).catch(() => {});
      results.push({ browser: browserName, viewport, passed: false, error: String(error), errors, lastSnapshots: snapshots.slice(-10) });
      throw error;
    } finally {
      await context.close();
      await writeFile(`artifacts/match-recap-smoke-${browserName}.json`, JSON.stringify({ identity, results }, null, 2));
    }
  }

...
for (const theme of ["neon-pixel", "clean-neon"]) {
  for (const [name, body] of Object.entries(artwork)) {
    const gradients = Object.entries({
      metal: ["#f4fcff", "#8eaec7", "#36546c"],
      gold: ["#fff7bf", "#ffd051", "#ed861b"],
      orange: ["#ffdc79", "#ff8b36", "#e93665"],
      green: ["#c3ff88", "#58dc71", "#159377"],
      violet: ["#f0c8ff", "#b375ed", "#6540a6"],
      cyan: ["#c3ffff", "#4edce9", "#217dba"],
      pink: ["#ffdbf5", "#f86cbd", "#993c85"],
    })
      .map(
        ([id, colors]) =>
          `<linearGradient id="${id}" x1="0" y1="0" x2=".7" y2="1">${colors.map((color, index) => `<stop offset="${index / 2}" stop-color="${color}"/>`).join("")}</linearGradient>`,
      )
      .join("\n    ");
  ...
  <g transform="scale(.5)">${body}
    ...
    const used = new Set(
      [...body.matchAll(/url\(#([a-z]+)\)/g)].map((match) => match[1]),
    );
    await writeFile(
      new URL(`../public/themes/${theme}/${name}.svg`, import.meta.url),
      svg
        .replace(
          /<(linearGradient|radialGradient) id="([a-z]+)"[\s\S]*?<\/\1>/g,
          (definition, _tag, id) => (used.has(id) ? definition : ""),
        )
        .replace(/^ +\n/gm, ""),
    );
  }

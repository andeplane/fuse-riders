...
function score(game: Awaited<ReturnType<typeof createGameServer>>['game']) {
  ...
  return (game.portalPairs.length > 0 ? 100 : 0) + gunBombs * 40 + game.bombs.size * 15 + alive * 2 + (game.blasts.length > 0 ? 20 : 0);
}
...
try {
  for (let attempt = 0; attempt < 3 && bestScore < 175; attempt++) {
    ...
    try {
      ...
      for (let i = 0; i < 5; i++) await page.getByRole('button', { name: 'ADD AI', exact: true }).click();
      await page.getByRole('button', { name: 'START RACE', exact: true }).click();
      for (let ticks = 0; app.game.phase !== 'matchOver' && ticks < 6000 && bestScore < 175; ticks++) {
        app.advance(1);
        ...
        if (current > bestScore) {
          ...
          bestScore = current;
          const bombPositions = [...app.game.bombs.values()].map(b => `(${b.x | 0},${b.y | 0}${b.shell?.gun ? ',gun' : ''})`).join(' ');
          const portalPositions = app.game.portalPairs.map(p => p.gates.map(g => `(${g.x | 0},${g.y | 0})`).join('-')).join(' ');
          console.log(`attempt ${attempt} tick ${app.game.tick}: new best score ${bestScore} (bombs ${app.game.bombs.size} ${bombPositions}, portals ${app.game.portalPairs.length} ${portalPositions}, blasts ${app.game.blasts.length})`);
        }
      }
  ...
  console.log(`Captured a real AI-played match, best action score ${bestScore}: docs/gameplay-phaser.png`);

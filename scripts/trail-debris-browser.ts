...
try {
  ...
  for (const mode of ["webgl", "phaser-canvas"] as const) {
    ...
    const removed = await page.evaluate(async (mode) => {
      ...
      for (let p = 0; p < 3; p++) {
        ...
        Object.assign(rider, {
          ...
          y: 160 + p * 220,
          angle: 0,
          trail: Array.from({ length: 60 }, (_, i) => ({
            x1: 520 + i * 7,
            x2: 527 + i * 7,
            y1: 330 + p * 100 + Math.sin(i * 0.16) * 35,
            y2: 330 + p * 100 + Math.sin((i + 1) * 0.16) * 35,
            createdTick: 120 + i,
            expiresAtTick: 400,
          })),
        });
      ...
      Object.assign(game, {
        ...
        round: 1,
        phase: "playing",
        roundStartedTick: 190,
        boundaryInset: 35,
        nextPickupSpawnTick: 9999,
      });
      game.bombs.set(99, {
        id: 99,
        ownerId: "p0",
      ...
      const removed =
        before.players.reduce((n, p) => n + p.trail.length, 0) -
        after.players.reduce((n, p) => n + p.trail.length, 0);
      ...
      Reflect.set(window, "debrisTailCheck", () => {
        ...
        for (let i = 0; i < moving.length; i += 4)
          if (
            Math.abs(moving[i]! - cleared[i]!) +
              Math.abs(moving[i + 1]! - cleared[i + 1]!) +
              Math.abs(moving[i + 2]! - cleared[i + 2]!) >
            70
          )
            changed++;
        paint(tail, 2200);
        const expired = pixels();
        if (!cleared.every((value, i) => value === expired[i]))
          throw Error("Debris survived expiry");
        arena.destroy();
        return changed;
      });
      return removed;
    ...
    await page.screenshot({
      path: `artifacts/trail-debris-${mode}-${browserName}.png`,
    });
    const changed = await page.evaluate(() =>
      (Reflect.get(window, "debrisTailCheck") as () => number)(),
    );
    assert.ok(
      changed > 50,
      `${mode}: visible debris must outlast the explosion, move, and clear`,
    );

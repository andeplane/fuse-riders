...
async function rapidNavigation(browser:Browser,browserName:string){
 ...
 page.on('pageerror',e=>errors.push(e.stack??e.message));page.on('requestfailed',r=>{if(r.resourceType()==='xhr')cancelled++;});
...
for(const [browserName,type] of [['chrome',chromium],['webkit',webkit]] as const){
 ...
 try{for(const viewport of [{width:320,height:568},{width:390,height:844},{width:844,height:390}]){
  ...
  page.on('pageerror',e=>errors.push(e.stack??e.message));page.on('request',r=>{if(new URL(r.url()).pathname.startsWith('/api/'))api.push(new URL(r.url()).pathname);});page.on('websocket',()=>sockets++);

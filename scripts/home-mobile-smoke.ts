...
async function rapidNavigation(browser:Browser,browserName:string){
 const context=await browser.newContext({viewport:{width:320,height:568}}),page=await context.newPage();page.setDefaultTimeout(smokeTimeout(20000));const errors:string[]=[];let delayed=0,cancelled=0;
...
for(const [browserName,type] of [['chrome',chromium],['webkit',webkit]] as const){
 ...
 try{for(const viewport of [{width:320,height:568},{width:390,height:844},{width:844,height:390}]){
  const context=await browser.newContext({viewport,isMobile:true,hasTouch:true});const page=await context.newPage();page.setDefaultTimeout(smokeTimeout(20000));const errors:string[]=[],api:string[]=[];let sockets=0,rtcTotal=0;
  ...
  try{
   ...
   for(const control of ['.landing-brand','.landing-top .audio-controls summary','.landing-audio','.landing-mute'])await inside(page,page.locator(control));
   const guide=page.getByRole('region',{name:'POWER-UPS'});assert.equal(await guide.getByRole('listitem').count(),POWERUP_GUIDE.length,'power-up guide lists every pickup');

...
const root = new URL("../artifacts/", import.meta.url).pathname;
...
for (const [name, type] of [
  ...
  try {
    ...
    result.release = await release.json();
    // ?analytics=0 sticks for the browser context, so the whole smoke — room included — stays out of the production project.
    await host.goto("https://andeplane.github.io/fuse-riders/?analytics=0");
    await host
    ...
    await host.locator(".mobile-rotate-gate").waitFor({ state: "visible" });
    await host.setViewportSize({ width: 844, height: 390 });
    await host.locator(".mobile-rotate-gate").waitFor({ state: "hidden" });
    result.countdownReceived = true;
    ...
    await host.waitForFunction(
      () =>
        document.querySelector("canvas")?.dataset.renderer === "phaser-webgl",
    );
    result.metrics = await guest.locator("#app").getAttribute("data-metrics");
    result.renderer = await host
      .locator("canvas")
      .getAttribute("data-renderer");
    await host.screenshot({ path: `${root}/public-phaser-${name}.png` });
    result.screenshot = `artifacts/public-phaser-${name}.png`;
    await host.waitForFunction(
      () =>
        / · [1-9] wins/.test(
          document.querySelector(".online-roster")?.textContent ?? "",
        ),
      {},
      { timeout: 25000 },
    );
    result.replicatedScoring = true;
    assert.ok(
      await host.evaluate(
        () => document.documentElement.scrollWidth <= innerWidth + 1,
      ),
      "phone host fits viewport",
    );
    result.phoneHost = {
      createdInPortrait: true,
      playedInLandscape: true,
      createdJoinedStarted: true,
    };
    if (await host.locator(".mobile-tools-toggle").isVisible())
      await host.locator(".mobile-tools-toggle").click();
    await host
    ...
    assert.equal(await host.getByLabel("Match length").inputValue(), "2");
    await host
      .getByRole("button", { name: "CONFIGURE POWERUPS", exact: true })
      .click();
    assert.equal(
    ...
    assert.deepEqual(result.savedPreferences, {
      mode: "shared",
      length: 2,
      powerWeight: 0,
    });
    result.settingsRestoredInLobby = true;
    ...
    await display.waitForFunction(() => {
      const image = document.querySelector(".shared-lobby img");
    ...
    assert.equal(await display.locator(".online-arena").isVisible(), true);
    result.separateTvVisible = true;
    if (await host.locator(".mobile-tools-toggle").isVisible())
      await host.locator(".mobile-tools-toggle").click();
    await host
      .getByRole("button", { name: "BACK TO LOBBY", exact: true })
      .click();
    await display.locator(".shared-lobby").waitFor({ state: "visible" });
    result.phoneHostControlsTv = true;
    await display.screenshot({ path: `${root}/public-shared-tv-${name}.png` });
    result.pass = true;
  } catch (e) {
    result.pass = false;
    ...
    for (const [i, context] of browser.contexts().entries())
      for (const page of context.pages()) {
        result[`body${i}`] = await page
          .locator("body")
          .innerText()
          .catch(() => "<closed>");
        await page
          .screenshot({ path: `${root}/public-failure-${name}-${i}.png` })
          .catch(() => {});
      }
  } finally {
    await browser.close();
    ...
    if (errors.length || requests.length) result.pass = false;
    results.push(result);
    await writeFile(

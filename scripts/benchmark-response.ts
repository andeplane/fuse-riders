...
import { headingResponse,renderedTickContinuity,type ResponseFrame,type ResponsePointer,type ResponseResult } from '../src/online/response-measurement';
...
interface Capture {clocks:unknown[];frames:ResponseFrame[];pointers:ResponsePointer[];actorId:string;truncated:number}
declare global {interface Window {__responseBench:Capture}}
function install(hidden:boolean){
 const state:Capture={clocks:[],frames:[],pointers:[],actorId:'',truncated:0};window.__responseBench=state;
 ...
 window.addEventListener('fuse-benchmark',event=>{const detail=(event as CustomEvent).detail;if(detail.kind==='response-clock'){if(state.clocks.length>=8000){state.clocks.shift();state.truncated++;}state.clocks.push(detail);}if(detail.kind==='snapshot')state.actorId=detail.playerId??state.actorId;if(detail.kind==='response-render'){if(state.frames.length>=12000){state.frames.shift();state.truncated++;}state.frames.push(detail);}});
...
const duration=Number(process.env.BENCH_SECONDS??60);if(!Number.isFinite(duration)||duration<30||duration>180)throw new Error('BENCH_SECONDS must be 30–180');
const minimumEligible=duration>=120?50:10;
...
try{
 const pages=[];for(let i=0;i<3;i++){const context=await browser.newContext({viewport:i===1?{width:844,height:390}:{width:1280,height:720},hasTouch:i===1});context.setDefaultTimeout(20000);await context.addInitScript({content:`(()=>{const __name=(fn)=>fn;(${install.toString()})(${i===0});})();`});const page=await context.newPage();page.on('pageerror',e=>errors.push(e.message));pages.push(page);}
 ...
 for(const [page,name] of [[host,'Response host'],[guest,'Response actor']] as const){if(page===guest)await page.goto(invite.href);await page.getByPlaceholder('Your name').fill(name);await page.getByRole('button',{name:'JOIN AS PLAYER',exact:true}).click();await host.locator(':is(.online-roster,.room-riders):visible').getByText(name,{exact:false}).waitFor();}
 invite.searchParams.set('display','1');await tv.goto(invite.href);await tv.locator(':is(.online-roster,.room-riders):visible').getByText('Response actor',{exact:false}).waitFor();
 await host.getByRole('button',{name:'ROOM SETTINGS',exact:true}).click();const dialog=host.getByRole('dialog');await host.getByRole('button',{name:'CONFIGURE POWERUPS',exact:true}).click();for(const input of await dialog.locator('label input[type="number"]').all())await input.fill('0');await host.getByRole('button',{name:'← BACK TO ROOM SETTINGS',exact:true}).click();await host.getByRole('button',{name:'SAVE SETTINGS',exact:true}).click();
...
const summaries=Object.fromEntries((['local','tv'] as const).map(view=>{const results=attempts.flatMap(a=>a[view]?[a[view]!]:[]);const distribution=quantiles(results.filter(r=>r.status==='changed').map(r=>r.latencyMs!));const secondary=attempts.flatMap(a=>a.oneDegree?[a.oneDegree[view]]:[]);return[view,{...distribution,rejected:results.filter(r=>r.status==='rejected').length,timeouts:results.filter(r=>r.status==='timeout').length,reasons:results.filter(r=>r.reason).map(r=>r.reason),candidateLimitMs:view==='local'?33:100,passed:results.every(r=>r.status!=='timeout')&&distribution.count>=minimumEligible&&distribution.p95!==null&&distribution.p95<=(view==='local'?33:100),oneDegree:{...quantiles(secondary.filter(r=>r.status==='changed').map(r=>r.latencyMs!)),rejected:secondary.filter(r=>r.status==='rejected').length,timeouts:secondary.filter(r=>r.status==='timeout').length}}];}));
const actorId=attempts.find(a=>a.pointer)?.pointer?.actorId??'';
const continuity=Object.fromEntries((['local','tv'] as const).map(view=>[view,renderedTickContinuity(captures[view==='local'?1:2]?.frames??[],actorId)]));
const fixedPreinputFrames=(trials:Attempt[],view:'local'|'tv')=>trials.flatMap(a=>a.pointer?(a[view]?.frames??[]).filter(f=>f.epochAt>=a.pointer!.epochAt-200&&f.epochAt<=a.pointer!.epochAt):[]);
const sampledPreinputContinuity=Object.fromEntries((['local','tv'] as const).map(view=>[view,renderedTickContinuity(fixedPreinputFrames(attempts,view),actorId)]));
let baselineContinuity:unknown;try{const prior=JSON.parse(await readFile('docs/online/evidence/response-02a982f.json','utf8')) as {revision:string;attempts:Attempt[]};baselineContinuity={revision:prior.revision,method:'Only fixed200ms pre-pointer windows are comparable; prior artifact lacks full-run frames. Large gaps include unsampled spaces between attempts.',views:Object.fromEntries((['local','tv'] as const).map(view=>[view,renderedTickContinuity(fixedPreinputFrames(prior.attempts,view),prior.attempts.find(a=>a.pointer)?.pointer?.actorId??'')]))};}catch(error){baselineContinuity={error:error instanceof Error?error.message:String(error)};}
...
await mkdir('artifacts',{recursive:true});await writeFile('artifacts/response-benchmark.json',JSON.stringify({date:new Date().toISOString(),revision,assets,durationSeconds:duration,minimumEligible,method:'Trusted DOM pointer to first actual submitted Phaser heading departure >=0.0001 rad; secondary >=1 degree. Two visible renderers: guest844x390 landscape with touch capability and TV1280x720; host renderer hidden. Same machine performance.timeOrigin clocks, direct RTC without impairment. Not physical touch-to-photon, scanout, phone hardware, or WAN qualification.',renderers,errors,failure,summaries,continuity,sampledPreinputContinuity,baselineContinuity,attempts,clockObservations:captures.map(c=>c.clocks),responseRenderFrames:captures.slice(1).map(c=>c.frames),truncated:captures.map(c=>c.truncated),passed},null,2)+'\n');console.log(JSON.stringify({report:'artifacts/response-benchmark.json',summaries,passed}));if(!passed)process.exitCode=1;

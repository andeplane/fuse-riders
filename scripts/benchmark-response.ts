...
await mkdir('artifacts',{recursive:true});await writeFile('artifacts/response-benchmark.json',JSON.stringify({date:new Date().toISOString(),revision,assets,durationSeconds:duration,method:'Trusted DOM pointer to first actual submitted Phaser heading departure >=0.0001 rad; secondary >=1 degree. Two visible renderers: guest390x844 and TV1280x720; host renderer hidden. Same machine performance.timeOrigin clocks, direct RTC without impairment. Not physical touch-to-photon, scanout, phone hardware, or WAN qualification.',renderers,errors,failure,summaries,attempts,clockObservations:captures.map(c=>c.clocks),truncated:captures.map(c=>c.truncated),passed},null,2)+'\n');console.log(JSON.stringify({report:'artifacts/response-benchmark.json',summaries,passed}));if(!passed)process.exitCode=1;

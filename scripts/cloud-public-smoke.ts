...
import { isAuthorityGrant, type AuthorityGrant } from '../src/online/authority.js';
...
class Peer {
 ...
 constructor(url:string,origin:string,private timeout:number){this.socket=new WebSocket(url,{origin,handshakeTimeout:timeout});this.socket.on('message',raw=>{try{this.frames.push(JSON.parse(raw.toString()));}catch{}this.notify();});this.socket.on('close',code=>{this.closed=code;this.notify();});this.socket.on('error',()=>{});}
...
export async function runPublicSmoke(options:PublicSmokeOptions):Promise<PublicSmokeReport>{
 ...
 try{
  ...
  checked('complete');report.passed=true;

...
for(const [name,type] of [['chrome',chromium],['webkit',webkit]] as const){
 ...
 await page.addInitScript(()=>{localStorage.setItem('fuse-riders-room-settings-v1',JSON.stringify({version:1,mode:'devices',match:'wins',length:3,map:'classic',weights:{}}));window.addEventListener('fuse-benchmark',event=>{void Reflect.get(window,'recordKeyboardSnapshot')((event as CustomEvent).detail);});});
 ...
 try{
  ...
  await page.keyboard.down('Space');await fire.getByText('RELEASE!',{exact:true}).waitFor({state:'attached'});await wait(()=>actor().bombChargeStartedTick!==undefined);await page.getByRole('button',{name:/^(ROOM|EXIT)$/}).click();await page.keyboard.up('Space');await page.getByRole('button',{name:'CLOSE',exact:true}).click();const cancelTick=latest().tick;await wait(()=>latest().tick>=cancelTick+6);assert.equal(latest().authorityScope,cancelScope);assert.ok(actor().alive);assert.equal(actor().bombReadyAtTick,beforeCancel,'opening modal must cancel, not fire');
  await page.getByRole('button',{name:'BACK TO LOBBY',exact:true}).click();await page.getByRole('button',{name:'ROOM SETTINGS',exact:true}).click();const length=page.getByLabel('Match length');await length.fill('12');await length.press('ArrowLeft');await length.press('Backspace');assert.equal(await length.inputValue(),'2','editable input must retain ordinary arrow/delete editing');await page.keyboard.press('Space');assert.ok(!(await fire.textContent())?.includes('RELEASE'),'editable Space must not charge');

...
try {
  ...
  assert.ok(
    arena && arena.height > 997 * 0.8,
    "desktop arena should use over 80% of viewport height",
  );
  ...
  const driving = keyboardShortcuts({
    mac,
    canConfigure: true,
    solo: true,
  }).find((group) => group.title === "Driving")!;
  ...
  assert.notEqual(
    after,
    before,
    "arrow input must steer the rider during gameplay",
  );
  await page.keyboard.down("Space");
  ...
  await page.waitForFunction(
    () =>
      document.querySelector('[aria-keyshortcuts="Space"]')?.textContent !==
      "RELEASE!",
  );
  ...
  for (const viewport of [
    { width: 2048, height: 1178 },
    { width: 1723, height: 997 },
    ...
    const layout = await (
      await page.waitForFunction(() => {
        ...
        const roster = document
          .querySelector(".online-roster")!
          .getBoundingClientRect();
        const actions = document
          .querySelector(".online-host")!
          .getBoundingClientRect();
        return {
          ui:
          ...
          arena: { width: r.width, height: r.height, y: r.y },
          fitted: {
            width: canvas.width * scale,
            height: canvas.height * scale,
          },
          bar: { height: bar.height, bottom: bar.bottom },
          rosterX: roster.x,
          rosterWidth: roster.width,
          arenaRight: r.right,
          actionsY: actions.y,
          overflow:
      ...
      .evaluateAll((elements) =>
        elements.map((element) => {
          ...
          return {
            ...
            fits: score.left >= label.left && score.right <= label.right + 1,
          };
        }),
      );
    ...
    assert.ok(
      scores.every(
        (score) => score.fits && /\d+ PTS · \+\d+/.test(score.text ?? ""),
      ),
      "long rider names must not truncate match points or the round award",
    );
    assert.ok(
      layout.arena.y >= layout.bar.bottom &&
        layout.arena.y <= layout.bar.bottom + 5 * layout.ui,
      `Arena/bar placement: ${JSON.stringify(layout)}`,
    );
    // The UI scales with the viewport above 1920x1080, so the bar's padding and gap allowances scale with it.
    assert.ok(
      layout.arena.height >=
        viewport.height - layout.bar.height - 13 * layout.ui,
    );
    if (viewport.width === 2048) {
      assert.ok(
        layout.fitted.width > (viewport.width - layout.rosterWidth) * 0.97,
        "reference screen uses at least 97% of the space left of the standings",
      );
      assert.ok(
        layout.rosterX >= layout.arenaRight - 1,
        "standings sit in their own column right of the arena",
      );
  ...
  assert.ok(
    phonePads && phonePads.height >= 64,
    "phone retains large touch pads",
  );

  ...
  for (const mode of ['webgl', 'phaser-canvas'] as const) {
    const report = await page.evaluate(async mode => {
      ...
      for (const theme of ['neon-pixel', 'clean-neon'] as const) {
        paint(base, theme); const baseline = read();
        const areas: number[] = [];
        for (const tick of [100, 102.4, 105.2, 107.5]) {
          paint({ ...base, tick, blasts: [blast] }, theme);
          const pixels = read(); let bright = 0;
          for (let i = 0; i < pixels.length; i += 4) {
            if (pixels[i]! > 160 && pixels[i + 1]! > 70 && pixels[i + 2]! < 180 && pixels[i]! > baseline[i]! + 50) bright++;
          }
          areas.push(bright);
        }
        // Repeated snapshots and expired blasts must not restart or retain a flash.
        paint({ ...base, tick: 102.4, blasts: [blast] }, theme); const first = read();
        paint({ ...base, tick: 102.4, blasts: [blast] }, theme); const repeat = read();
        if (!first.every((value, i) => value === repeat[i])) throw Error(`${mode}: repeated blast changed`);
        paint({ ...base, tick: 108, blasts: [blast] }, theme); const expired = read();
        if (!baseline.every((value, i) => value === expired[i])) throw Error(`${mode}: expired blast remained visible`);
        results.push({ theme, brightPixels: areas });
      }
      // Top: one explosion at 25/125/225/325 ms. Bottom: five bombs at the same age.
      paint({ ...base, tick: 100.5, blasts: [
        ...[108, 106, 104, 102].map((expiresAtTick, i) => ({ ...blast, circle: { x: 200 + i * 400, y: 270, radius: 150 }, expiresAtTick })),
        ...[42, 43, 44, 45, 46].map((bombId, i) => ({ bombId, circle: { x: 160 + i * 320, y: 650, radius: 130 }, expiresAtTick: 106 })),
      ] }, 'neon-pixel');
      const captions = document.createElement('div');
      captions.style.cssText = 'position:fixed;inset:0;pointer-events:none;color:#a5bfff;font:16px monospace;letter-spacing:2px';
      for (const [text, top] of [['POP / BLOOM / BREAK / CLEAR', 65], ['FIVE SIMULTANEOUS BOMBS · DIFFERENT IDS, SAME AGE', 475]] as const) {
    ...
    for (const result of report.results) {
      ...
      assert.ok(bloom! > pop! * 2, `${mode}/${result.theme}: bloom must visibly expand`);
      assert.ok(breaking! < bloom! && clear! < breaking!, `${mode}/${result.theme}: circles must visibly contract`);
    }
    reports.push(report);
    await page.screenshot({ path: `artifacts/blast-${mode}-${browserName}.png` });
    await page.evaluate(() => { (Reflect.get(window, 'disposeBlastCheck') as () => void)(); });
  }
  await page.goto(`http://127.0.0.1:${address.port}/?solo=1&benchmark=1`);
  await page.evaluate(() => window.addEventListener('fuse-benchmark', event => {
    const sample = (event as CustomEvent<{ kind: string; event?: { type: string } }>).detail;
    if (sample.kind === 'event' && sample.event?.type === 'explosion') document.body.dataset.blastObserved = 'true';
  }));
  await page.locator('canvas[data-renderer="phaser-webgl"]').waitFor();
  await page.waitForFunction(() => document.body.dataset.blastObserved === 'true', undefined, { timeout: 30000 });
  await page.screenshot({ path: `artifacts/blast-solo-${browserName}.png` });
  assert.deepEqual(errors, []);
  console.log(JSON.stringify({ reports, soloExplosionObserved: true, errors }, null, 2));
} finally {

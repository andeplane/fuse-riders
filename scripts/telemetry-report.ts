...
type Event = Record<string, any> & { kind: string; id: string; role: string; wall: number; at: number };
...
for (const e of events) { const key = `${e.role}:${e.id}`; byDevice.set(key, [...(byDevice.get(key) ?? []), e]); }
const hosts = events.filter(e => e.role === 'host' && e.kind === 'ingest');
...
for (const [key, list] of byDevice) {
  ...
  const recv = list.filter(e => e.kind === 'recv'), rtts = recv.map(e => e.rtt).filter((r): r is number => typeof r === 'number');
  const seconds = Math.max(1, (list.at(-1)!.wall - list[0]!.wall) / 1000);
  console.log(`== ${key}  ${list[0]!.ua ?? ''}`);
  ...
  if (list[0]!.role === 'host') {
    ...
    for (const input of inputs) { const folded = hosts.find(h => h.from === list[0]!.id && h.lastSeq >= input.seq && h.wall >= input.wall - 2000); if (folded) latencies.push(folded.wall - input.wall); }

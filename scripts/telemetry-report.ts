...
for (const e of events) { const key = `${e.device.role}:${e.device.id}`; byDevice.set(key, [...(byDevice.get(key) ?? []), e]); }
...
for (const [key, list] of byDevice) {
  ...
  if (list[0]!.device.role === 'host') {
    ...
    for (const input of inputs) { const folded = hosts.find(h => h.from === list[0]!.device.id && h.lastSeq >= input.seq && h.wall >= input.wall - 2000); if (folded) latencies.push(folded.wall - input.wall); }

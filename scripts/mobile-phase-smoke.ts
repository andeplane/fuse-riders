...
for(const [name,type] of [['chrome',chromium],['webkit',webkit]] as const){const browser=await type.launch({headless:true});const context=await browser.newContext({viewport:{width:844,height:390},isMobile:true,hasTouch:true});const page=await context.newPage();page.setDefaultTimeout(smokeTimeout(20000));const errors:string[]=[];page.on('pageerror',e=>errors.push(e.stack??e.message));
 ...
 try{await page.goto(new URL('?solo=1',base).href);await page.locator('.mobile-play').waitFor();await page.waitForFunction(()=>document.querySelector('canvas')?.dataset.renderer?.startsWith('phaser-'));
  ...
  // Host cue: lobby and matchOver open the tools overlay for the host so START RACE / REMATCH / MAIN MENU are in view.
  assert.equal(await page.locator('.mobile-tools-open').count(),1,'host lobby opens the tools overlay');assert.equal(await page.locator('.online-roster:visible>span').count(),5,'lobby roster reachable behind MENU');assert.equal(await page.locator('.shared-lobby').isVisible(),false);await page.getByRole('button',{name:'START RACE',exact:true}).waitFor({state:'visible'});
  await page.screenshot({path:`artifacts/mobile-phase-lobby-menu-${name}.png`});await tools(false);assert.equal(await hintsOpacity(),0,'no hints in the lobby');seen.lobby=await capture('lobby');
  await tools(true);await page.getByRole('button',{name:'START RACE',exact:true}).click();await notice(/^READY/);assert.equal(await page.locator('.mobile-tools-open').count(),0,'countdown closes the tools overlay');assert.ok(await hintsOpacity()>0,'hints visible at countdown start');seen.countdown=await capture('countdown');

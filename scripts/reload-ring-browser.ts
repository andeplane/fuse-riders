    ...
    const results = await page.evaluate(async mode => {
      ...
      for (const theme of ['neon-pixel', 'clean-neon'] as const) {
        ...
        const equal = (a: Uint8Array, b: Uint8Array, message: string) => {
          if (!a.every((v, i) => v === b[i])) throw Error(`${mode}/${theme}: ${message}`);
        };
        const ready = paint({ ...base, players: [{ ...player, bombReadyAtTick: 0 }] });
        ...
        const fractional = paint({ ...base, tick: halfway.tick + .75 });
        if (fractional.every((v, i) => v === half[i])) throw Error('Fractional cooldown does not move');

...
import { themes } from "../games/fuse-riders/src/render/themes.js";
...
const names = [
  "bomb",
  ...POWERUP_GUIDE.map((entry) => `pickup-${entry.type}`),
  "flame",
];
const labels = ["BOMB", ...POWERUP_GUIDE.map((entry) => entry.name), "FLAME"];
const themeList = (Object.keys(themes) as Array<keyof typeof themes>).map(
  (id) => ({ id, heading: themes[id].label.toUpperCase() }),
);
...
try {
  ...
  await page.route("**/sprite-showcase.html", (route) =>
    route.fulfill({
      contentType: "text/html",
      body: "<!doctype html><html><head></head><body><main></main></body></html>",
    }),
  );
  await page.goto(`http://127.0.0.1:${address.port}/sprite-showcase.html`);
  await page.evaluate(
    async ({ names, labels, themeList }) => {
      ...
      *{box-sizing:border-box}body{margin:0;background:#040d1e;color:#ebf5ff;font-family:system-ui;padding:44px}
      ...
      root.innerHTML =
        "<h1>Power-ups, polished.</h1><p>Smooth neon sprites · enlarged detail above, 22px and 34px sizes below</p>";
  ...
  await page.screenshot({
    path: `${output}/${browserName}-sprites.png`,
    fullPage: true,
  });
    ...
    for (const { id: themeId } of themeList) {
      await page.evaluate(
        async ({ backend, themeId, types }) => {
          ...
          fixture.pickups = types.map((type, index) => ({
            id: index,
            type,
            x: 110 + index * spacing,
            y: 780,
          ...
          fixture.bombs = fixture.bombs
            .filter((bomb) => !bomb.shell)
            .slice(0, 3)
            .map((bomb, index) => ({
              ...bomb,
              x: 450 + index * 250,
              y: 250,
              explodeAtTick: 43 + index * 8,
            }));
          arena.render(fixture, 1000, themes[themeId], "sprite-showcase");
          canvas.dataset.ready = "true";
          window.addEventListener("showcase-dispose", () => arena.destroy(), {
            once: true,
          });
        },
  ...
  console.log(
    `Decoded all ${names.length * themeList.length} SVG assets; captured every theme in WebGL and Canvas (${browserName}): ${output}`,
  );
} finally {
  await browser.close();
  await server.close();

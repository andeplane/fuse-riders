...
const newest = (directory: string): number =>
  readdirSync(directory, { withFileTypes: true, recursive: true })
    ...
    .map(
      (item) => statSync(join(item.parentPath ?? directory, item.name)).mtimeMs,
    )
    .reduce((latest, at) => Math.max(latest, at), 0);
if (newest("src") > newest("dist"))
  console.warn(
    "WARNING: dist is older than src — run `npx vite build` or this smoke tests the previous bundle.",
  );
...
const waitFor = async (predicate: () => boolean, detail: string) => {
  ...
  while (!predicate()) {
    if (Date.now() > until) throw new Error(`Timeout: ${detail}`);
...
try {
  ...
  host.on("websocket", (socket) =>
    socket.on("framereceived", (frame) => {
      const message: { type?: string; tick?: number } = JSON.parse(
        frame.payload.toString(),
      );
      if (message.type === "snapshot" && typeof message.tick === "number")
        hostSnapshotTick = message.tick;
    }),
  );
  const advanceDelivered = async (count: number) => {
    ...
    for (let remaining = count; remaining > 0; remaining -= 2) {
      ...
      await waitFor(
        () => hostSnapshotTick >= tick,
        `host snapshot delivery at tick ${tick}`,
      );
    }
  };
  ...
  await host
    .getByRole("button", { name: "TV audio enabled · test sound", exact: true })
    .waitFor();
  ...
  const PLAYING =
    "(() => { const e = document.querySelector('audio.game-music'); return Boolean(e && e.currentSrc && !e.paused); })()";
  await host.waitForFunction(PLAYING, undefined, {
  ...
  await host.waitForFunction(
    "document.querySelector('audio.game-music').paused === true",
  );
  await host.getByRole("button", { name: "Mute music", exact: true }).click();
  ...
  const legendNames = await host
    .locator(".pickup-legend li b")
    .allTextContents();
  assert.deepEqual(
    legendNames,
    POWERUP_GUIDE.filter((entry) => entry.spawnsByDefault).map(
      (entry) => entry.name,
    ),
  ...
  assert.ok(
    legendNames.includes("POWER") && !legendNames.includes("STAR"),
    `legend copy looks wrong: ${legendNames.join(", ")}`,
  );
  assert.ok(
    (
      await host.locator(".pickup-legend img").first().getAttribute("src")
    )?.includes("/themes/neon-pixel/pickup-power.svg"),
  );
  ...
  const defaultPickups = POWERUP_GUIDE.filter(
    (entry) => entry.spawnsByDefault,
  ).length;
  for (const themeId of ["clean-neon", "neon-pixel"]) {
    await host.getByRole("combobox").selectOption(themeId);
    const legendSources = await host
      .locator(".pickup-legend img")
      .evaluateAll((images) =>
    ...
    assert.equal(
      ...
      defaultPickups,
      `legend icons found: ${legendSources.length}, expected ${defaultPickups}`,
    );
    for (const source of legendSources)
      assert.ok(
        source.includes(`/themes/${themeId}/`),
        `legend icon ${source} ignores theme ${themeId}`,
      );
  }

  // A fresh token delivered as a hash-only navigation must be consumed and re-authenticated.
  const recoveredHost = await browser.newPage({
    viewport: { width: 1200, height: 800 },
  });
  monitor(recoveredHost);
  ...
  await recoveredHost.evaluate((token) => {
    location.hash = token;
  }, app.hostToken);
  await recoveredHost.getByText("HOST ONLINE", { exact: true }).waitFor();
  assert.equal(
    new URL(recoveredHost.url()).hash,
    "",
    "replacement host fragment removed from URL",
  );
  ...
  for (let i = 0; i < 5; i++) {
    const context = await browser.newContext({
      viewport:
        i === 0 ? { width: 390, height: 844 } : { width: 844, height: 390 },
      isMobile: true,
      hasTouch: true,
    });
    ...
    assert.equal(
      await phone.locator(".join-screen .avatar-option").count(),
      10,
    );
    const avatarLabels = ["Robot", "Cat", "Fox", "Alien", "Astronaut"];
    await phone
      .getByRole("button", { name: avatarLabels[i], exact: true })
      .click();
    if (i === 0) {
    ...
    await phone
      .getByPlaceholder("Rider name")
      .fill(["Ada", "Bo", "Cy", "Dee", "Eli"][i]);
    await phone.getByRole("button", { name: "JOIN THE GRID" }).click();
    await waitFor(
      () =>
        [...app.game.players.values()].some(
          (player) =>
            player.name === ["Ada", "Bo", "Cy", "Dee", "Eli"][i] &&
            player.avatarId ===
              ["dragon", "cat", "fox", "alien", "astronaut"][i],
        ),
      "chosen avatar reaches server",
    );
  ...
  for (let i = 0; i < phones.length; i++) {
    const color = await phones[i]!.locator(".controller-shell").evaluate((el) =>
      (el as HTMLElement).style.getPropertyValue("--player-color"),
    );
    assert.equal(
      color,
      [...app.game.players.values()].find((player) => player.slot === i)!.color,
    );
    controllerColors.push(
      await phones[i]!.locator("[data-control=left]").evaluate(
        (el) => getComputedStyle(el).backgroundImage,
      ),
    );
  }
  assert.equal(
    new Set(controllerColors).size,
    5,
    "each phone has its own rider-colored controls",
  );
  const startBounds = (await host
    .getByRole("button", { name: "START RACE", exact: true })
    .boundingBox())!;
  assert.ok(
    startBounds.y >= 0 && startBounds.y + startBounds.height <= 960,
    "start button fits the TV viewport",
  );
  ...
  app.advance(60);
  await phones[0]
    .getByRole("button", { name: "Drop bomb" })
  ...
  await host
    .locator(".score-card .avatar-portrait[data-avatar-id=slime]")
    .waitFor();
  await host
    .locator(".seat-marker .avatar-portrait[data-avatar-id=slime]")
    .waitFor({ state: "attached" });
  await waitFor(
    () =>
      [...app.game.players.values()].find((p) => p.slot === 0)!.avatarId ===
      "slime",
    "live avatar selection reaches TV state",
  );
  assert.equal(String(app.game.phase), "playing");
  await host.locator(".announcement.hidden").waitFor({ state: "attached" });
  const slideLeft = await phones[0]
    .getByRole("button", { name: "Turn left" })
    .boundingBox();
  const slideRight = await phones[0]
    .getByRole("button", { name: "Turn right" })
    .boundingBox();
  assert.ok(slideLeft && slideRight);
  await phones[0].mouse.move(10, 10);
  await phones[0].mouse.down();
  await phones[0].mouse.move(
    slideLeft.x + slideLeft.width / 2,
    slideLeft.y + slideLeft.height / 2,
  );
  await phones[0].locator("[data-control=left].active").waitFor();
  await phones[0].mouse.move(
    slideRight.x + slideRight.width / 2,
    slideRight.y + slideRight.height / 2,
  );
  await phones[0].locator("[data-control=right].active").waitFor();
  assert.equal(
    await phones[0].locator("[data-control=left].active").count(),
    0,
  );
  await phones[0].mouse.move(10, 10);
  ...
  for (const phone of phones) {
    assert.ok(
      await phone.getByRole("button", { name: "Turn left" }).isVisible(),
    );
    assert.ok(
      await phone.getByRole("button", { name: "Turn right" }).isVisible(),
    );
    const overflow = await phone.evaluate(() => ({
      x: document.documentElement.scrollWidth > innerWidth,
      y: document.documentElement.scrollHeight > innerHeight,
    }));
    assert.equal(overflow.x, false, "phone no horizontal overflow");
    assert.equal(
      overflow.y,
      false,
      "phone no vertical scroll while controlling",
    );
  }
  // Real DOM pointer handlers -> WebSocket -> authoritative simulation.
  const rider = [...app.game.players.values()][1];
  const initialAngle = rider.angle;
  ...
  assert.notEqual(
    rider.angle,
    initialAngle,
    "phone steers authoritative rider",
  );
  await phones[1].mouse.up();
  await new Promise((r) => setTimeout(r, 50));
  const releasedAngle = rider.angle;
  app.advance(2);
  assert.equal(rider.angle, releasedAngle, "pointer release neutralizes");
  // Real browser capture cleanup: interrupted contact must never trap the next press.
  await left.evaluate((button) =>
    button.addEventListener("pointerdown", (event) => {
      (button as HTMLElement).dataset.lastPointer = String(
  ...
  for (const interruption of [
    ...
    await phones[1].mouse.move(box.x + box.width / 2, box.y + box.height / 2);
    await phones[1].mouse.down();
    await phones[1].mouse.move(
      box.x + box.width / 2 + 2,
      box.y + box.height / 2,
    );
    assert.equal(
      await left.evaluate((button) => button.classList.contains("active")),
      true,
    );
    ...
    await left
      .locator('xpath=self::*[not(contains(@class, "active"))]')
      .waitFor();
    await phones[1].mouse.up();
    await phones[1].mouse.move(box.x + box.width / 2, box.y + box.height / 2);
    await phones[1].mouse.down();
    ...
    assert.equal(
      await left.evaluate((button) => button.classList.contains("active")),
      true,
      `${interruption}: next press works`,
    );
    await phones[1].mouse.up();
  }
  const poweredRider = [...app.game.players.values()].find(
    (player) => player.slot === 0,
  )!;
  poweredRider.powerPickups = 0;
  app.game.pickups.push({
    id: 9_001,
    type: "power",
    x: poweredRider.x,
    y: poweredRider.y,
    expiresAtTick: app.game.tick + 100,
  });
  app.advance(2);
  await phones[0].getByText("◆ 1", { exact: true }).waitFor();
  assert.equal(
    app.game.pickups.some((pickup) => pickup.id === 9_001),
    false,
    "power pickup consumed authoritatively",
  );
  app.game.pickups.push({
    id: 9_002,
    type: "star",
  ...
  app.advance(2);
  await phones[0].getByText(/STAR · [0-9.]+s/).waitFor();
  assert.equal(
    app.game.pickups.some((pickup) => pickup.id === 9_002),
    false,
    "star pickup consumed authoritatively",
  );
  app.game.pickups.push({
    id: 9_003,
    type: "beer",
  ...
  app.advance(2);
  await phones[1].getByText(/WOBBLE · [0-9.]+s/).waitFor();
  assert.equal(
    app.game.pickups.some((pickup) => pickup.id === 9_003),
    false,
    "beer pickup consumed authoritatively",
  );
  assert.equal(
    poweredRider.drunkUntilTick,
    0,
    "beer collector is immune to own pickup",
  );
  app.game.pickups.push({
    id: 9_020,
    type: "ink",
  ...
  app.advance(2);
  await phones[1].getByText(/INK · [0-9.]+s/).waitFor();
  ...
  app.advance(2);
  await phones[0].getByText("◆ 1 · B×2", { exact: true }).waitFor();
  ...
  assert.equal(
    app.game.pickups.some((pickup) => pickup.id === 9_041),
    false,
    "shorter-fuse pickup consumed authoritatively",
  );
  app.game.pickups.push({
    id: 9_004,
    type: "triple",
  ...
  app.advance(2);
  await phones[0].getByText("TRIPLE · ARMED", { exact: true }).waitFor();
  assert.equal(
    app.game.pickups.some((pickup) => pickup.id === 9_004),
    false,
    "triple pickup consumed authoritatively",
  );
  app.game.pickups.push({
    id: 9_008,
    type: "five",
  ...
  app.advance(2);
  await phones[0].getByText("FIVE · ARMED", { exact: true }).waitFor();
  app.game.pickups.push({
    id: 9_030,
    type: "target",
    x: poweredRider.x,
  ...
  app.advance(1);
  assert.ok(
    poweredRider.bombTarget!.x > initialTarget.x + 250,
    "drag moves the authoritative TV target",
  );
  ...
  assert.equal(app.game.bombs.size, 0);
  assert.equal(targetBlast.circle.x, finalTarget.x);
  assert.equal(targetBlast.circle.y, finalTarget.y);
  assert.equal(
    targetBlast.circle.radius,
    powerBlastRadius(poweredRider.powerPickups) * 0.7,
  );
  assert.equal(poweredRider.targetBombArmed, false);
  assert.equal(poweredRider.fiveShotArmed, true);
  ...
  await phones[0].evaluate(() => {
    const seen = { launched: false };
    Reflect.set(window, "__launchSeen", seen);
    ...
    const observer = new MutationObserver((records) => {
      if (
        records.some(
          (record) =>
            !(record.oldValue ?? "").split(/\s+/).includes("launching") &&
            (record.target as Element).classList.contains("launching"),
        )
      )
        seen.launched = true;
    });
    for (const bomb of document.querySelectorAll(".bomb"))
  ...
  assert.equal(
    app.game.bombs.size,
    6,
    "Five overrides Triple and stacks with Extra Bomb",
  );
  await phones[0].waitForFunction(
    () =>
      (Reflect.get(window, "__launchSeen") as { launched: boolean }).launched,
    undefined,
  ...
  app.advance(2);
  await phones[0]
    .getByText("GREEN SHELL · HOLD + RELEASE", { exact: true })
    .waitFor();
  await phones[0].getByRole("button", { name: "Drop bomb" }).tap();
  await new Promise((r) => setTimeout(r, 50));
  app.advance(2);
  ...
  assert.equal(
    app.game.bombs.size,
    1 + poweredRider.extraBombs,
    "Shell fans out with Extra Bomb",
  );
  assert.ok([...app.game.bombs.values()].every((bomb) => bomb.shell));
  ...
  assert.equal(
    app.game.bombs.size,
    1 + poweredRider.extraBombs,
    "Gun fans out with Extra Bomb",
  );
  assert.ok(
    [...app.game.bombs.values()].every((bomb) => bomb.shell?.gun === true),
  );
  ...
  await host.screenshot({ path: "artifacts/gun-projectile.png" });
  app.game.bombs.clear();
  app.game.pickups.push({
    id: 9_011,
    type: "power",
  ...
  app.advance(2);
  await phones[0].getByText("◆ 2 · B×2", { exact: true }).waitFor();
  ...
  app.advance(2);
  await phones[0].getByText("◆ 2 · B×2 · GRIP", { exact: true }).waitFor();
  assert.equal(poweredRider.grip, true);
  assert.equal(
    app.game.pickups.some((pickup) => pickup.id === 9_012),
    false,
  );
  app.game.pickups.push({
    id: 9_013,
  ...
  app.advance(2);
  assert.equal(
    app.game.pickups.some((pickup) => pickup.id === 9_013),
    true,
    "repeat GRIP stays available for another rider",
  );
  ...
  app.advance(2);
  await phones[0].getByText(/^NITRO · ×4 · \d\.\ds$/).waitFor();
  assert.equal(
    poweredRider.nitroUntilTicks.length,
    2,
    "both Nitros are their own deadlines",
  );
  ...
  app.advance(2);
  await phones[0].getByText("SHIELD · READY", { exact: true }).waitFor();
  assert.equal(
    app.game.pickups.some((pickup) => pickup.id === 9_006),
    false,
    "shield pickup consumed authoritatively",
  );
  app.game.pickups.push({
    id: 9_007,
    type: "portal",
  ...
  app.advance(2);
  assert.equal(
    app.game.pickups.some((pickup) => pickup.id === 9_007),
    false,
    "portal pickup consumed authoritatively",
  );
  assert.equal(
    app.game.portalPairs.length,
    1,
    "portal pickup opens an authoritative gate pair",
  );
  ...
  for (const player of app.game.players.values()) {
    player.trail = [];
    if (player !== poweredRider) {
      player.x = app.game.width / 2;
      player.y = 80 + player.slot * 35;
      player.angle = 0;
    }
  }
  app.game.portalPairs[0]!.gates = [
    { x: app.game.width * 0.3, y: app.game.height / 2, halfLength: 100 },
    { x: app.game.width * 0.7, y: app.game.height / 2, halfLength: 100 },
  ];
  const entryGate = app.game.portalPairs[0]!.gates[0];
  poweredRider.x = entryGate.x + 15;
  poweredRider.y = entryGate.y;
  poweredRider.angle = Math.PI;
  poweredRider.portalCooldownUntilTick = 0;
  poweredRider.portalGraceUntilTick = 0;
  assert.equal(poweredRider.alive, true, "transit fixture rider remains alive");
  app.advance(2);
  assert.ok(
    poweredRider.portalCooldownUntilTick > app.game.tick,
    "gate transit starts authoritative cooldown",
  );
  assert.ok(
    poweredRider.portalGraceUntilTick > app.game.tick,
    "gate transit starts authoritative phase grace",
  );
  assert.ok(
    poweredRider.x > app.game.width / 2,
    "rider arrives at linked gate",
  );
  ...
  for (const phone of phones) {
    ...
    assert.equal(
      overflow.x,
      false,
      "power-up phone has no horizontal overflow",
    );
    assert.equal(overflow.y, false, "power-up phone has no vertical overflow");
  }
  await phones[0].screenshot({ path: "artifacts/phone-powerups-portrait.png" });
  await phones[1].screenshot({
    path: "artifacts/phone-powerups-landscape.png",
  });
  ...
  assert.equal(
    await host
      ...
      .isVisible(),
    true,
    "fullscreen stays visible during a match",
  );
  assert.equal(
    await host
      .getByRole("button", { name: "Fullscreen", exact: true })
  ...
  const previousIds = [...app.game.players.keys()];
  await phones[0].reload();
  await phones[0].locator(".controls:not(.hidden)").waitFor();
  assert.deepEqual([...app.game.players.keys()], previousIds);
  ...
  for (let round = 0; round < 5 && app.game.phase !== "matchOver"; round++) {
    ...
    if (app.game.phase === "roundOver")
      await advanceDelivered(app.game.phaseEndsAtTick! - app.game.tick);
    if (app.game.phase === "countdown")
      await advanceDelivered(app.game.phaseEndsAtTick! - app.game.tick);
    assert.equal(
      String(app.game.phase),
      "playing",
      "the next round reaches active play",
    );
    for (const id of previousIds)
      if (id !== winner) eliminatePlayer(app.game, id);
    await advanceDelivered(2);
  }
  ...
  assert.equal(
    await host.locator(".comparison-row:not(.comparison-header)").count(),
    5,
  );
  ...
  await host
    .getByText("ROUND POINTS // +1 PER OPPONENT OUTLASTED", { exact: false })
    .waitFor();
  await phones[0].getByText(/#1 · \+5 · 25PTS/).waitFor();
  await host.getByRole("button", { name: "Close leaderboard" }).click();
  const matchId = app.game.matchId;
  await host.getByRole("button", { name: "REMATCH" }).click();
  await waitFor(() => app.game.matchId !== matchId, "new match scope");
  app.advance(2);
  assert.equal(app.game.phase, "countdown");
  assert.ok([...app.game.players.values()].every((p) => p.roundWins === 0));
  await phones[0].getByText("PTS · 0", { exact: true }).waitFor();
  assert.equal(
    app.game.leaderboard.get(winner!)!.totalScoreUnits,
    25 * 60,
    "session points survive while the phone shows fresh match points",
  );
  ...
  assert.deepEqual(
    [...app.game.players.keys()],
    previousIds,
    "menu preserves connected phone identities",
  );
  ...
  console.log(
    "Browser smoke passed: host token recovery, pickups, leaderboard, TV, five phones, controls, themes, reconnect and rematch.",
  );
} finally {
  await browser.close();
  await app.close();

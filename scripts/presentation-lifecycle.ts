import { mountArenaPresentation, type PresentationDependencies } from '../src/client/phaser/presentation.js';
...
export async function checkPresentationLifecycle(): Promise<void> {
  ...
  function harness() {
    ...
    const presentation = mountArenaPresentation(canvas, replacement => { canvas = replacement; }, {
      loadArena: () => { const load = deferred<typeof module>(); loads.push(load); return load.promise; },
      schedule: (delay, callback) => { const timer = { delay, callback }; timers.add(timer); return () => { timers.delete(timer); }; },
    });
    return {
      ...
      retry: () => {
        const button = document.querySelector<HTMLButtonElement>('.graphics-status:not([hidden]) button');
        check(button && !button.hidden, 'Missing visible retry button'); button.click();
      },
      destroy: () => { presentation.destroy(); presentation.destroy(); canvas.remove(); check(timers.size === 0, 'Disposal leaked deadlines'); check(!document.querySelector('.graphics-status'), 'Disposal leaked status'); },
    };
  }
  {
    const h = harness();
    check(h.loads.length === 0, 'Hidden controller eagerly loads Phaser');
    h.render(); h.destroy(); h.loads[0]!.resolve(h.module); await flush();
    check(h.arenas.length === 0, 'Import after disposal created an arena');
  }
  {
    const h = harness(); h.render(); h.expire(10000);
    check(h.canvas().dataset.rendererStatus === 'failed', 'Module download had no deadline');
    h.retry(); h.loads[1]!.resolve(h.module); await flush();
    const arena = h.arenas[0]!; arena.ready.resolve(); await flush();
    const draws = arena.draws;
    h.loads[0]!.reject(Error('Late failed download')); await flush();
    check(h.canvas().dataset.renderer === 'phaser-canvas' && arena.destroys === 0, 'Stale import failure destroyed retry');
    check(draws === 1, 'Retry did not repaint stored snapshot without another frame'); h.destroy();
  }
  {
    const h = harness(); h.render(); h.expire(10000); h.retry();
    h.loads[0]!.resolve(h.module); await flush();
    check(h.arenas.length === 0, 'Timed-out download created a stale arena'); h.destroy();
    h.loads[1]!.reject(Error('Disposed import')); await flush();
  }
  for (const failure of ['import', 'create', 'ready'] as const) {
    ...
    await flush(); check(h.canvas().dataset.rendererStatus === 'failed', `${failure} failure did not offer retry`);
    check(h.timers.size === 0, `${failure} failure leaked timers`); h.destroy();
  }
  {
    const h = harness(); h.render(); h.loads[0]!.resolve(h.module); await flush();
    const old = h.arenas[0]!; old.rejectOnDestroy = false; h.expire(10000);
  ...
  }
  {
    const h = harness(); h.render(); h.loads[0]!.resolve(h.module); await flush();
    const arena = h.arenas[0]!; arena.ready.resolve(); await flush();
    arena.status?.('context-lost'); check(h.timers.size === 1, 'Lost context has no deadline');
    arena.status?.('restored'); check(Number(h.timers.size) === 0, 'Restoration did not cancel deadline');
    check(arena.draws === 2, 'Restoration did not repaint a paused view');
    arena.failPaint = true; h.render(); check(h.canvas().dataset.rendererStatus === 'failed', 'Paint exception escaped failure UI');
    h.render(); check(arena.destroys === 1, 'Failed frames repeatedly destroyed arena'); h.destroy();
  }
  {
    const h = harness(); h.render(); h.loads[0]!.resolve(h.module); await flush();
    const arena = h.arenas[0]!; h.destroy(); await flush();
    check(arena.destroys === 1, 'Pending startup disposal was not idempotent');
    arena.status?.('context-lost'); check(h.timers.size === 0, 'Disposed context event scheduled recovery');
  }
}

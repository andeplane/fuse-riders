...
type PullRequest = { number: number; title: string; body: string | null; merged_at: string | null; updated_at: string; html_url: string; user: { login: string }; additions?: number; deletions?: number };
...
async function mergedPullRequests(since: Date): Promise<PullRequest[]> {
  ...
  return merged.sort((a, b) => (b.additions! + b.deletions!) - (a.additions! + a.deletions!));
}
...
async function highlightBullets(prs: PullRequest[]): Promise<Map<number, string>> {
  // prs is sorted biggest diff first.
  const candidates = prs.slice(0, 3).filter(pr => (pr.additions ?? 0) + (pr.deletions ?? 0) >= HIGHLIGHT_MIN_LINES);
  if (candidates.length === 0) return new Map();
  const list = candidates.map(pr => `#${pr.number} (+${pr.additions}/-${pr.deletions} lines): ${pr.title}\n${(pr.body ?? '').slice(0, 1500)}`).join('\n\n');
  ...
  for (const line of reply.split('\n')) {
    ...
    if (match && match[2]!.trim().toUpperCase() !== 'SKIP') bullets.set(Number(match[1]), `- ${match[2]!.trim()}`);
  }
  return bullets;
}
...
async function summarize(prs: PullRequest[]): Promise<string> {
  ...
  const body = rest.length === 0 ? '' : await chatCompletion(
  ...
  const highlightSection = highlights.size === 0 ? '' : `*Notable this batch*\n${[...highlights.values()].join('\n')}\n\n`;

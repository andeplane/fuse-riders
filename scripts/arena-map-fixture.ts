...
import { ARENA_MAPS, generateObstacles, OBSTACLE_WALL_MARGIN } from '../src/shared/arena-map.js';
...
try {
  ...
  for (let i = 0; i < 3; i++) addPlayer(app.game, { id: `p${i}`, name: `P${i + 1}`, avatarId: AVATARS[i * 2]!.id, slot: i, color: colors[i] });
  startMatch(app.game);
  ...
  for (const [index, player] of [...app.game.players.values()].entries()) {
    Object.assign(player, { x: 120, y: 160 + index * 300, angle: 0, alive: true, trail: [], invulnerableUntilTick: Number.MAX_SAFE_INTEGER });
  }
  ...
  for (const map of ARENA_MAPS) {
    ...
    app.game.obstacles = generateObstacles({
      map, random: () => { seed = (seed * 1103515245 + 12345) >>> 0; return seed / 0x1_0000_0000; },

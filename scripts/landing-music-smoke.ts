...
const track = (page: Page) =>
  page.evaluate(() => {
    ...
    return (
      element && {
        src: new URL(element.src, location.href).pathname,
...
try {
  ...
  await page.goto(url);
  await page.waitForSelector(".landing-audio");
  await settle();
  ...
  assert.equal(
    await label(),
    "♫ MUSIC ON",
    "an untapped page reads the setting, which is on",
  );
  assert.equal(
    await stored(),
    null,
    "nothing is stored before the visitor chooses",
  );
  ...
  assert.equal(
    playing.paused,
    false,
    "music plays on the landing page, with no room and no match",
  );
  assert.ok(
    playing.volume > 0 && playing.volume <= 1,
    `music has a real volume, got ${playing.volume}`,
  );
  ...
  assert.ok(
    button.y >= bar.y - 1 && button.y + button.height <= bar.y + bar.height + 1,
    "the toggle sits inside the top bar",
  );
  assert.ok(
    button.x > bar.x + bar.width / 2,
    "the toggle sits on the right, like the room header SETTINGS button",
  );
  assert.ok(
    button.height >= 40,
    `the toggle is a real touch target, got ${button.height}px`,
  );
  assert.equal(await label(), "♫ MUSIC ON");
  ...
  assert.ok(
    ...
    `alt-tabbing never rewinds the track: ${before.time} -> ${after.time}`,
  );
  ...
  await page.locator(".landing-audio").click();
  await settle();
  assert.equal(await label(), "♫ MUSIC OFF");
  assert.equal(
    await page.locator(".landing-audio").getAttribute("data-muted"),
    "true",
  );
  assert.equal(
    await page.locator(".landing-audio").getAttribute("aria-pressed"),
    null,
    "the label states the state, so aria-pressed would contradict it",
  );
  ...
  assert.ok(
    back.time >= off.time,
    `toggling never rewinds the track: ${off.time} -> ${back.time}`,
  );
  ...
  await page.locator(".landing-mute").click();
  await settle();
  assert.equal(await mute(), "🔇 SOUND OFF");
  assert.equal(await label(), "♫ MUSIC OFF");
  ...
  page.on("request", (request) => {
    if (/\/music\/.*\.m4a$/.test(request.url()))
      muted.push(new URL(request.url()).pathname);
  });
  await page.reload();
  await page.waitForSelector(".landing-audio");
  await settle();
  assert.equal(
    await label(),
    "♫ MUSIC OFF",
    "a reload keeps the choice, and the label reports it before any gesture",
  );
  assert.deepEqual((await stored()).muted, { music: true, effects: false });
  await page.mouse.click(720, 700);
  await settle();
  ...
  assert.deepEqual(
    muted,
    [],
    `a muted page fetches no track, got ${JSON.stringify(muted)}`,
  );

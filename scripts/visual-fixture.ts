...
try {
  ...
  routes.forEach((points, index) => {
    ...
    player.trail = segments;
    player.x = previous[0];
    player.y = previous[1];
    const last = segments.at(-1)!;
    player.angle = Math.atan2(last.y2 - last.y1, last.x2 - last.x1);
    player.alive = true;
    player.roundWins = index % 4;
  });
  for (const [id, x, y] of [
    [1, 405, 145],
    [2, 1040, 355],
    ...
    app.game.bombs.set(id, {
      ...
      launchedTick: 940,
      landsAtTick: 946,
      explodeAtTick: 980 + id * 5,
      blastRange: 150,
      flightPath: Array.from({ length: 7 }, () => ({ x, y, angle: 0 })),
    });
  app.game.blasts.push({
    bombId: 5,
    ownerId: "p0",
    circle: { x: 510, y: 340, radius: 150 },
    expiresAtTick: 968,
  });
  app.game.pickups = [
  ...
  app.game.portalPairs = [
    {
      id: "fixture-gates",
      gates: [
        { x: 1300, y: 180, halfLength: 140 },
        { x: 900, y: 720, halfLength: 140 },
      ],
      expiresAtTick: 1150,
    },
    {
      id: "fixture-gates-2",
      gates: [
        { x: 260, y: 300, halfLength: 110 },
        { x: 700, y: 620, halfLength: 110 },
      ],
      expiresAtTick: 1150,
    },
  ];
  const flightPath = createVolleyFlightPaths({ x: 700, y: 700 }, 0, 300, {
    minX: 27,
    minY: 27,
    maxX: 1573,
    maxY: 873,
    ...
    const sample = await page.evaluate(async () => {
      ...
      await new Promise<void>((resolve) => {
        function measure(now: number) {
          ...
          previous = now;
          if (now - start < 5000) requestAnimationFrame(measure);
          else resolve();
        }
      ...
      return {
        ...
        fps: (intervals.length * 1000) / (previous - start),
  ...
  await page.getByRole("combobox").selectOption("clean-neon");
  await page.waitForTimeout(300);
  await page.screenshot({ path: "artifacts/clean-neon-visual.png" });
  console.log(
    "Saved controlled art comparison fixtures for both themes. These are renderer fixtures, not live match evidence.",
  );
} finally {
  await browser.close();
  await app.close();

  ...
  for (const mode of ["webgl", "phaser-canvas"] as const) {
    const result = await page.evaluate(async (mode) => {
      ...
      const offset = (x: number, y: number) =>
        ((mode === "webgl" ? 899 - y : y) * 1600 + x) * 4;
      const before = paint();
      const samples = players.map((p) => {
        let gold = 0,
          identity = 0;
        const rgb = [1, 3, 5].map((start) =>
          parseInt(p.color.slice(start, start + 2), 16),
        );
        for (let y = 410; y < 435; y++)
          for (let x = p.x - 130; x < p.x + 130; x++) {
            const o = offset(x, y);
            if (
              before[o]! > 240 &&
              before[o + 1]! > 200 &&
              ...
              gold++;
            if (
              rgb.every((v, channel) => Math.abs(before[o + channel]! - v) < 15)
            )
              identity++;
          }
        if (gold < 5 || identity < 5)
          throw Error(
            `Missing gold count or identity color: ${p.name}, ${gold}, ${identity}`,
          );
      ...
      let labelChanges = 0,
        avatarChanges = 0;
      for (let y = 410; y < 480; y++)
        for (let x = 270; x < 530; x++) {
          const o = offset(x, y);
          const changed = [0, 1, 2].some(
            (channel) => before[o + channel] !== after[o + channel],
          );
          if (changed && y < 435) labelChanges++;
          if (changed && y >= 435) avatarChanges++;
        }
      if (labelChanges === 0)
        throw Error("First pickup did not update the visible count");
      if (avatarChanges !== 0)
        throw Error(
          "Pickup changed the avatar/reload area: progress ring must be absent",
        );
      players[0]!.powerPickups = 0;
      paint();
      const local = paint(players[0]!.id);
      let localGold = 0,
        localWhite = 0;
      for (let y = 410; y < 430; y++)
        for (let x = 270; x < 530; x++) {
          const o = offset(x, y);
          if (local[o]! > 240 && local[o + 1]! > 200 && local[o + 2]! < 110)
            localGold++;
          if ([0, 1, 2].every((channel) => local[o + channel]! > 240))
            localWhite++;
        }
      if (localGold < 5 || localWhite < 5)
        throw Error("Local YOU label must retain its gold Power count");
      Reflect.set(window, "disposePowerCheck", () => {
        arena.destroy();
        canvas.remove();
      });
    ...
    await page.evaluate(() => {
      (Reflect.get(window, "disposePowerCheck") as () => void)();
    });

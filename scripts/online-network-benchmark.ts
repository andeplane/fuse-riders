...
interface Injection {
  ...
  windows:Record<string,number>; active:boolean; frameMs:number[];
}
...
function installImpairment({profile,seed,host}:{profile:Profile;seed:number;host:boolean}) {
  const state:Injection={attempted:0,delivered:0,dropped:0,expired:0,reordered:0,bytes:0,queueBytes:0,maxQueueBytes:0,relayAttempts:0,blockedUntil:0,windows:{},active:false,frameMs:[]};
  ...
  for(const profile of selected){
    ...
    try{
      for(let i=0;i<6;i++){
        ...
        await context.addInitScript(installImpairment,{profile,seed:12345+i,host:i===0});
        const page=await context.newPage();pages.push(page);page.on('pageerror',error=>errors.push(error.message));
      }
      const host=pages[0]!;await host.goto(base);await host.getByRole('button',{name:'CREATE ROOM',exact:true}).click();await host.waitForURL(/room=/);

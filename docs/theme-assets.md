...
The baseline follows the [neon/pixel gameplay reference](gameplay-concepts/06-neon-pixel-hybrid.png): a dark navy arena, crisp silhouettes, cyan/magenta/orange accents, and bright blast sparks. Assets live in `public/themes/`; both `neon-pixel` and `clean-neon` provide all sixteen SVG files below. Bombs, flames and all thirteen pickups use smooth rounded artwork with shaded surfaces and bright edge highlights to match the interface. That artwork is intentionally shared by both themes (only the dark body shading tint differs slightly); rider, trail and arena styling remain theme-specific.

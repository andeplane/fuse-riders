...
- **A rollback is split too.** It restores the snapshot and re-runs as far as the window allows; the rest is owed to the next passes (`World.frontier` is the tick it had reached), which finish it before anything new and without the stall bound, since those ticks were reached once already. The world keeps showing the frames it had until the re-run reaches them, then shows the re-run's, so the picture never jumps back. A late entry during a re-run either restarts it from an earlier snapshot or is folded when the re-run gets there, so a burst of late packets costs one re-run, not one each. Nothing else reads a half-finished re-run as final: its hashes and snapshots exist only up to where it has got, and `servable()` serves a valid earlier state. A hidden member finishes owed re-runs (they are history) but starts no new ticks.
- **`BEHIND_STEPS` = 400.** "Behind" is the log-tick backlog times the step count the world's state runs at now (`RoomRuntime.behind`). That fetches a snapshot at the CPU cost the old 400-tick limit had when a tick was one step: 20 s behind at 1×, 6.7 s (133 log ticks) in the fast phase, the same wall time as under the old 3× clock. Counting log ticks would let a fast backlog cost 1,200 steps, 7–10 s of CPU on the developer's Mac, before resyncing. It is an estimate; a backlog that spans a phase change is misjudged by at most three.
...
| ------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------- |
| Catch-up, 119 fast log ticks behind (6 s hidden)  | worst pass 193–198 ms, 16 passes, 2.2 s CPU | worst pass 57 ms (p95 51 ms), 66 passes, 2.5 s CPU       |
...
| Rollback of 42 fast log ticks (126 steps)         | one 710–728 ms handler                      | 48–62 ms in the handler, then 20 passes of at most 50 ms |

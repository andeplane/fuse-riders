...
- Wandering walls and trains are **permanent**: a blast does not clear them and the overtime walls do not crush them. A map is its movers; a railway with nothing on it is the classic arena.
...
- **Checkpoint guard.** `motion` is admitted in its two shapes only; a `wall` may span the board (half extents to half the board), other kinds stay bounded to a quarter; a rail car is admitted only on the `trains` map, on a loop the map has, within its length.

...
`step()` was one function of about 970 lines whose sections talked to each other through some twenty locals. It is now a loop over an ordered list, `PHASES` in `src/engine/sim/pipeline.ts`. Every commit of the stage is `[hash-identical]`: `RULES` is left as `main` has it (`fuse-p2p-32` when the stage was written, `fuse-p2p-34` since the refresh onto main at 9479799) and `tests/fixtures/golden-hashes.json` is only ever main's own recording. Nothing a player, a peer or a checkpoint can see has changed.
...
| `src/shared/`                                                                               | What is not simulation: `protocol`, `avatars`, `uuid`, `duration-text`, `rating`, `elo`, `career-stats`, `firebase-config`                                                          |
...
The old `step` checked for play once, after the countdown; the loop checks before every `playing` phase. That is the same thing today because only `resolveRound`, the last phase, ever leaves `playing`. It would not be for a phase appended after `resolveRound`: on the tick that ends a round it would be skipped. Keep `resolveRound` last, or give such a phase `when: "always"` and its own guard.
...
Known limit: the guard reads imports, not writes. A phase could still `state.shots.push(…)`, mutate `state.matchStats.get(id)` in place, or call a recorder re-exported by a module outside `sim/`, and no test would object. It catches the mistake that is easy to make by accident, not one made on purpose.
...
- **The known throw sources are gone.** The per-tick match-stats recorders threw on a rider the match never seated and on a distance that is not one. Only a damaged state can produce either, but a throw there left the tick half-done. They now tolerate: the rider is not counted, the distance is dropped (`match-stats.ts`). `finalizeMatchStatsRound` skips an unseated id the same way and keeps its other two rejections (duplicate participant ids, a winner who is not a participant); the decided round's rating standings in `resolve-round.ts` read the seat from the rider when the statistics have none, rather than dereferencing a missing entry at round end. The tolerance is silent; a once-per-match warning through the runtime's reporting path belongs to the follow-up. The exposure is small: `codec/checkpoint.ts` rejects any wire state in which a living rider or a bomb's owner lacks a statistics entry, so such a state cannot arrive from a peer — and for the same reason a room whose replicas all carried that damage could not be joined.

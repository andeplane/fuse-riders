...
The repo holds two games: Fuse Riders (`games/fuse-riders`) and Pig, a small dice game (`games/dice`). They share six packages: the networking (`packages/fuse-network-fe`, `-be`, `-protocol`), the rollback netcode (`fuse-netcode`), the account, history and Elo backend (`fuse-platform`) and the menus and styles (`fuse-ui`). Pig proves that the contract really is game-agnostic. Fuse Riders' screens are built on `fuse-ui`; what stays its own is game content: the attract loop, recap content, room settings fields and standings. This note sets out the layout, the contract a game implements and the order of the work.
...
- **Routes.** The existing `/api/...` history routes are Fuse Riders'. `/api/games/<gameId>/...` addresses any registered game with the same routes, the public match feed (`/api/matches`) included. `/api/me` and its `PUT` (the username) are the shared account under either form; the profile it returns carries that game's rating and totals. An unregistered `gameId` is `404 Unknown game`.
...
`packages/fuse-ui` owns: the neon/pixel tokens (colours, font, spacing, z-index scale), the element factory, and the landing card, join by code (with REJOIN), radio group, name entry (with a second way in, an account name and a slot), picker (avatars), lobby, lobby shell with its QR invite and roster, status line and toast, dialog shell with views, confirm, key list, account dialog, controller row and phone layout. A game supplies its settings fields, its table or arena, its recap content and its renderers for totals, match rows and the leaderboard. See [its README](../../packages/fuse-ui/README.md); Pig's page is built from it alone.
...
- **What Fuse Riders uses.** Every screen: the landing page (hero built with `el`, mode radio group, join by code with REJOIN), the dialogs (landing SETTINGS, the in-room menu's views: settings, room, results, avatar, voice, radio and shortcuts), the account panel (`createAccountDialog`, with Fuse's match rows as the match results view), the lobby shell with its invite card, rider list and WATCHING list, the join form (name entry with the avatar picker in its slot), the controller row and the phone layout. It loads `components.css`; its dialogs and join form carry the `fui-*` classes beside its own, and its online stylesheets read the spacing tokens. What stays in the game: the attract canvas (it runs the engine and renderer), the recap and standings content, the room settings fields and the screen decision (`room-screen.ts`).

Still to do: the room settings dialog's body and the recap's layout could become `fuse-ui` parts once a second game needs them.

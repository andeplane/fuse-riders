...
games/
  fuse-riders/        src/: engine/, render/, client/, online/, shared/; tests/ with the golden hashes (built); its page is
                      the root index.html, and its registration still sits in service/history.ts
...
Built in `packages/fuse-platform` ([README](../../packages/fuse-platform/README.md)); `service/history.ts` is Fuse Riders' registration and the service's `Platform`.

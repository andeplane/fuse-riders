...
All of #254 is `[hash-identical]`: `RULES` is still `fuse-p2p-32` and the golden fixtures are untouched.
...
| `openEdges`                         | Whether the board wraps right now                                                                                 | `edgesOpen(view)` from `arena-map.ts`                                                                                                                                                                                             |
...
| Path                                            | Holds                                                                                                                                                  |
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `src/render/phaser/`                            | The scene (`arena.ts`), its lifecycle state machine (`presentation.ts`), trail cache and tips, one-shot effect transitions, backing-size observation   |
...
`ArenaScene.paint` builds one `Frame` and calls `drawBackground`, `drawFloor`, `drawTrails`, `drawTransitions`, `drawPickups`, `drawPortals`, `drawBombs`, `drawFields`, `drawBlasts`, `drawDebris`, `drawRiders`, `drawInk`. That order is the z-order within each Graphics layer.
...
`npx tsx scripts/render-parity.ts` replays the golden recording, takes the view at 31 moments chosen to cover every kind of thing the scene draws, renders each after a `reset()` at a fixed clock on WebGL and Canvas in both themes, and prints a hash of the pixels. The file resolves its paths at run time, so it can be copied onto an older revision. Between `14e1452` (the renderer in `src/client/`, reading engine constants) and this work, 124 of 124 frames are identical. It draws single frames, so it does not cover what needs two (death sparks, rubble, debris launch, the interpolated tip); those are covered by the unit tests of `effects`, `trail-debris`, `trails` and `present`.

...
| Layering                        | `tests/layer-boundaries.test.ts` pins every cross-layer import against `tests/fixtures/layer-allowlist.json`; `tests/ci-manifest.test.ts` pins the CI lists against `scripts/ci-manifest.json`                                              |

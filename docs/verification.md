...
| Engine behaviour is unchanged   | `games/fuse-riders/tests/golden-hash.test.ts` compares the state hash on every tick of a recording that exercises every mechanic; a refactor that was meant to change nothing fails here ([engine safety net](design/engine-safety-net.md)) |
| Layering                        | `tests/layer-boundaries.test.ts` fails any cross-layer import — since #254 the migration allowlist `tests/fixtures/layer-allowlist.json` is empty; `tests/ci-manifest.test.ts` pins the CI lists against `scripts/ci-manifest.json`         |

...
Desert uses a warm copper floor with sand ripples, stepped pyramids with shaded sandstone faces, and flowering cacti. Forest uses a cool jade floor with grass/moss marks, clustered tree crowns, berry bushes and fallen trunks with growth rings, moss and flowers. City uses a plum floor with inset paving and buildings viewed from above: parapets, roof access hatches, fan housings and service conduits replace facade-style window grids. Small timber crates remain in the city. These low-contrast ground marks are decorative; the existing grid and rider colors remain unchanged.

Ground detail is baked into the existing background texture only when map, size or visual style changes. The renderer passes the map to the obstacle painter so the shared rock footprint can look like a desert pyramid or a forest trunk using the existing wire kinds and the footprint supplied by the simulation. Triangle faces, ellipses and rectangles use the same primitives on both backends. Procedural obstacle details remain in the cached floor pass and disappear with their snapshot obstacles. Both Canvas and WebGL use the same art. Desert pyramids use `public/maps/desert/pyramid.png`, a shared baked sandstone texture with beveled tiers and directional lighting. Their square local footprint and seeded rotation come from the shared obstacle geometry; texture sprites sit below trails and are reconciled on every snapshot, including rollback and blast removal. The procedural painter remains a loading-failure fallback and follows the same rotation. Cached shadows include obstacle geometry in their cache key. No animation timer or renderer physics is involved. Classic, wrap-around and crossed maps retain their visual style's original floor. Run `npx tsx scripts/map-styles-smoke.ts` for reproducible contact sheets in both visual styles on Canvas and WebGL (`artifacts/map-styles-*.png`).

...
WebGL trails now use a continuous triangle ribbon and a Phaser `SinglePipeline` shader in both visual styles. A rounded cross-section, fixed upper-left light, soft specular highlight and restrained halo give the body depth. Rounded caps appear only at actual path ends, including explosion cuts. The body uses `TRAIL_WIDTH`; the halo is cosmetic. Recorded centerline vertices, gaps, piece identities and collision rules are unchanged. Fractional tip points join the same ribbon without an internal end cap; dead and detached pieces retain 60% opacity. Phaser's built-in Canvas backend retains its existing strokes.

...
Desert uses a warm copper floor with sand ripples and layered sandstone, flowering cacti and bound timber crates. Forest uses a cool jade floor with grass/moss marks, clustered tree crowns and berry bushes. City uses a plum floor with inset paving, cyan accents and recessed building roofs. These low-contrast ground marks are decorative; the existing grid, rider colors and collision silhouettes remain unchanged.

Ground detail is baked into the existing background texture only when map, size or visual style changes. Obstacle details remain in the cached floor pass and disappear with their snapshot obstacles. Both Canvas and WebGL use the same art, with no extra textures, animation timers or simulation state. Classic, wrap-around and crossed maps retain their visual style's original floor. Run `npx tsx scripts/map-styles-smoke.ts` for reproducible contact sheets in both visual styles on Canvas and WebGL (`artifacts/map-styles-*.png`).

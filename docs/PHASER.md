...
Dead riders disappear immediately, including their avatar, heading marker, label and status auras, leaving the crash particles unobscured. Their remaining trails stay at 80% opacity on both Phaser backends until destroyed or cleared for the next round. The simulation gives these segments a non-expiring finite deadline on death. This is presentation only; trail collision and expiry remain simulation-owned. Run `npx tsx scripts/dead-rider-browser.ts` (also with `BROWSER=webkit`) to check both themes across Phaser WebGL and Canvas, including trail removal, death particles and avatar reuse.

...
Held bomb aiming uses a glowing dashed guide, paired direction chevrons, four open reticle corners and a white landing dot, without a blast-radius circle. Ordinary volleys and Target Bomb share this treatment in each rider's color; short guides omit chevrons. Landing positions, fractional charge/bounce timing and weapon rules are unchanged. Run `npx tsx scripts/bomb-preview-smoke.ts` (also with `BROWSER=webkit`) to check marker motion on both backends and generate `artifacts/bomb-aim-*.png` visual sheets.

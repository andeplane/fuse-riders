...
The target is `engine` (pure rules), `net` (simulation coordination), `render` (engine view contract only), and `app` (composition). Renaming folders alone does not establish this boundary. The engine half of that is in place: the simulation lives in `games/fuse-riders/src/engine/`, imports nothing outside itself, and runs each tick as the ordered `PHASES` list ([engine pipeline](design/engine-pipeline.md)). Since #254 the migration allowlist is empty, so the layer-boundary test fails a new cross-layer import rather than pinning it: what the log or the checkpoint guard validates against is the engine's (the rider-name rule in `engine/rider-name.ts`, the avatar ids in `engine/avatar-id.ts`), and `games/fuse-riders/src/shared/` keeps the presentation and the wire vocabulary — avatar labels and atlas cells, the game id, the account and career types, and `protocol.ts`, which re-exports the engine's own primitives rather than owning them. The render half is too ([render boundary](design/render-boundary.md)). The single tick driver and the effect/pickup/weapon registries (#253 A3, A4) and the app layer with its shared UI (#255) are still staged work.
...
`step` in [game.ts](../games/fuse-riders/src/engine/game.ts) holds no rules. It builds one `TickContext` and walks `PHASES` in [pipeline.ts](../games/fuse-riders/src/engine/sim/pipeline.ts), the single statement of tick order; with no round in play the tick ends after the clock, the expiries and the countdown. In broad strokes:

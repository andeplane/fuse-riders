...
Every death goes through one `commitDeaths` over `DeathFact`s, and no phase before `recordFacts` reads statistics, so they cannot steer an outcome. `step` returns events only; callers that need the public snapshot call `toSnapshot`. The order of `PHASES` is part of the rules: the [engine pipeline note](design/engine-pipeline.md) has the full contract, the orderings it preserves, and the behaviour kept behind a named flag for stage A4. A phase that throws surfaces as a `TickFault`; the `World` then stands on its last whole snapshot, reports the fault and resyncs from a peer rather than riding on with a half-simulated state.

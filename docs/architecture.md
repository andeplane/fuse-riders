...
| `src/online/fuse-game.ts`, `room-runtime.ts`                               | Fuse Riders behind `RollbackGame`, and its runtime subclass with the rider's controls                                                                  |
...
| `src/service/history.ts`                                                   | Fuse Riders' game registration (stats, totals, career, rivals), the served games (`EXTRA_GAME_IDS`) and `Platform`                                     |

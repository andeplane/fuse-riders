...
This is a territory RTS with tower-defence pacing: structures fight automatically; players choose expansion, infrastructure, research and concentration. No heroes, unit selection groups, creep waves or last-hitting are needed to test this identity. Simulation time always advances. Destroying a brain eliminates that player; the last surviving brain wins. Simultaneous destruction of all remaining brains is a draw.
...
- A player has one active growth job initially. Manual mode accepts a bounded queue of explicit adjacent destinations; illegal or obsolete entries are skipped with a reason. Auto mode proposes the next legal tile toward a player-selected target, displaying the next step and cost. It uses the same job, cost, timing and validation as manual growth.
- Auto growth stops at enemy contact, an unreachable target or the player's Biomass reserve floor. A manual order replaces the next automatic choice. Auto never buys research or towers. It does not repeatedly rebuild a destroyed frontier unless the player retargets it. Default automation should save clicks, not commit the whole economy invisibly.
...
For a four-corner board, use canonical axial hex coordinates clipped into an approximately rectangular arena; an offset layout is a rendering choice only. Equal map distances and resource access matter more than visual rectangle symmetry. Begin with a hand-authored test map; two-player tests occupy opposite corners, while three-player matches require separate fairness assessment rather than assuming an unused fourth corner is neutral.
...
The first headless scenario should exercise this exact loop: two equal brains → queue growth to opposite sides of one shared deposit → both receive partial income → extend into contact → focus charge and exchange pulses → create an alternate connection → sever and reconnect a branch → destroy a brain. A second fixture builds a legal tower ring and compares an adequately supplied tower against a starved one. Research follows only after this base loop is measurable.
...
**First engine:** deterministic hex topology, 2–4 player state, resource sharing, manual/auto growth, connectedness, conserved charge flow, basic simultaneous combat, defeat/draw, snapshot/replay and headless policies. Then add the single tower and research fork before a small web playtest using the existing room/netcode packages.

**Deferred:** fog of war, powerups, active abilities, hero-like units, multiple tower classes, procedural map generation, campaigns, ranked play and particle physics. Public information is the first mode, including the shared TV. Fog later needs an explicit trust model: hiding pixels cannot hide state already held by a peer.

The supplied visual sheet is inspiration, not binding implementation instructions. Prefer the bio-luminescent palette with the minimal variant's readable hex geometry: dark restrained terrain, recognizable brain silhouettes, bright owner-colored links, distinct deposit shapes and visible flowing charge. Electrical intensity should show local available charge; an independent marker shows selected priority so a starved Focus node cannot look fully powered. Use geometry/icons as well as color. Detailed assets and image generation can wait until the core flow is worth watching.

...
Online, an AI rider is a management entry in the creator's stream (`BOT add`/`remove` in `src/engine/apply-tick.ts`): every replica seats it and simulates it, the bot set travels inside the world snapshot a joiner installs, and a stream named after a bot is ignored, so no client can steer one. The physics/snapshot player shape does not gain a client-controlled bot flag.

...
`npm run dev:online` builds the assets and starts Wrangler on http://localhost:8787, using the local Cloudflare adapter in `worker/`. Create a room, then open its link in another browser/profile. Test WebRTC on localhost or HTTPS: ordinary remote HTTP addresses are not secure browser contexts.
...
`npm run deploy` targets the local/legacy Cloudflare adapter only. It is not the production path and runs no release gates.

...
Suggested implementation boundaries are the shared deterministic reducer/checkpoint/replay layer in `games/fuse-riders/src/shared/`; host scheduling and action acceptance in `games/fuse-riders/src/online/host-session.ts`; replication, binary codec, recovery and tick synchronization in `games/fuse-riders/src/online/`; and presentation/controls in `games/fuse-riders/src/client/`. The replica supplies the renderer's existing snapshot interface. Preserve the separate `src/server/` LAN authority and `/display`/`/controller` flows; no Phaser physics or backend gameplay simulation is introduced.

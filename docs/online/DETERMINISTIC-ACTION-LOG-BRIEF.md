...
- **Independent decoding:** each included player stream retains its absolute tick anchor and starting sequence, followed by contiguous MessagePack tick deltas. Receiving the redundant bundle must not depend on an earlier lost packet just to decode timestamps. Preserve authority/connection/match/round/control scope and stable gesture IDs.

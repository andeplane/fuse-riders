...
`.github/workflows/backend.yml` runs the same `scripts/deploy-cloud.sh` after a successful `CI` push run on this repository's `main`, so an automatic deployment obeys the same clean-checkout, current-main and green-CI gates as a manual one. It authenticates with workload identity federation and uploads `artifacts/cloud-release-<commit>.json` as run evidence. An automatic run first asks Cloud Run which commit it serves (the `commit` label the deploy script sets) and skips the build and rollout when no file between that commit and the new one is part of the backend; the list is `scripts/lib/backend-paths.ts` and covers everything `Dockerfile.cloud` copies (all of `src/` and `packages/`, so a client-only change still redeploys), the package manifests, the deploy and configuration scripts, the Firebase/Firestore configuration and the workflow itself. It compares with the served commit rather than the previous push, so a backend change whose own run failed or was superseded is deployed by the next run; an unreadable label, a served commit that is not an ancestor, or any other doubt deploys. A manual `workflow_dispatch` always deploys. A skipped run uploads no release evidence, because nothing was released. Deploying is not acceptance: the smoke field still reads **NOT YET VERIFIED**, and the documented public smoke and network gates remain manual.

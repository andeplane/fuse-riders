...
The container entrypoint is `node --import tsx service/index.ts`. It binds `PORT` on all interfaces. Required production environment:
...
The image installs production dependencies only (`pnpm install --prod --frozen-lockfile`). `tsx` executes the TypeScript entrypoint, so it is a production dependency; packages that only the browser bundle, the tests or the tooling use (Phaser, Firebase web SDK, Mixpanel, Vite, Playwright, TypeScript) are devDependencies and are not in the image. `tests/service-image-dependencies.test.ts` fails if the entrypoint comes to import a dev-only package. The image runs as the unprivileged Node user. Bundling the entrypoint to drop `tsx` from the runtime is a possible later optimization, not an excuse to change runtime dependencies without testing.

The service runs the workspace packages `fuse-network-be` (with `fuse-network-protocol`) and `fuse-platform`; `Dockerfile.cloud` copies each workspace manifest before `pnpm install --frozen-lockfile` so they are linked, and the same test checks that every workspace manifest is copied.

### Multi-game history storage

One gateway serves every registered game (`platform.gameIds` in `service/history.ts`, composed from each game's `src/platform.ts`, passed to the room service as `gameIds`). Fuse Riders uses the same collections and fields as before, plus `gameId` on new match records and rooms: `${prefix}-matches`, `${prefix}-users` with its rating fields, `${prefix}-users/{uid}/rivals` and `${prefix}-rating-claims`. New Fuse Riders match records carry `gameId: "fuse-riders"`, and a gateway from before games ignores the field. Every other game stores ratings and totals in `${prefix}-ratings` (one document per `gameId:uid`, with a `rivals` subcollection). Every game's history query, Fuse Riders' included, filters on `gameId`, so a record without one is never listed (it still parses as Fuse Riders'). **Backfill before deploying to a database that holds records from before games:** run `ROOM_COLLECTION_PREFIX=<prefix> pnpm exec tsx scripts/backfill-match-game-id.ts` (a dry run that counts), then again with `--apply`: it writes `gameId: "fuse-riders"` onto records that have none, which changes no meaning and is safe to repeat. Production's `fuse-production-matches` has been backfilled (237 records, none left without `gameId`), and every gateway since `e08470a` writes it. Rolling the image back past the filter is safe: an older revision's query uses the index without `gameId`, which `firestore.indexes.json` keeps for that reason. A second game must also wait until every serving revision stores `gameId` on rooms: a revision from before games ignores `?gameId=` and stores a room that reads as Fuse Riders', which an up-to-date revision then refuses to the other game's page. **The gateway therefore serves Fuse Riders alone by default.** `service/index.ts` adds a game only when the `EXTRA_GAME_IDS` environment variable names it (comma-separated; `dice` today), and an id without a registration stops the revision from starting. `backend.yml` passes the repository variable `EXTRA_GAME_IDS` through `scripts/deploy-cloud.sh` into the revision's environment. Set it to `dice` only after the backfill above has run with `--apply` and every serving revision stores `gameId` on rooms; clearing it and redeploying takes the game offline again, and its records and ratings stay. The local dev service (`service/dev.ts`) always serves every game. Until then the static `/dice/` page still loads: CREATE ROOM says online rooms are not open yet, and solo against bots needs no service. `firestore.indexes.json` adds the indexes those queries need: `(gameId, ranked, elo)` in both directions on `fuse-production-ratings`, `(gameId, participantUids, endedAt desc)` on `fuse-production-matches`, which Fuse Riders' history now needs too, and `(gameId, feedAt desc)` on the same collection for the public match feed (`GET /api/matches`). Apply that last one before deploying the revision that serves the feed; until it is `READY` the route fails and the MATCHES tab shows its "could not load" line with TRY AGAIN. **Backfill `feedAt` after deploying the feed to a database that holds games confirmed before it:** run `ROOM_COLLECTION_PREFIX=<prefix> pnpm exec tsx scripts/backfill-match-feed-at.ts` (a dry run that counts), then again with `--apply`. Those records carry no `feedAt`, and a Firestore query returns no document lacking the field it orders by, so EVERYONE comes back empty rather than failing while YOURS — which orders on `endedAt` — still lists them. The script stamps `feedAt` from each record's own `endedAt` under confirmation's own rule (a confirmed whole game an account owns a seat in or that two riders attested), never touches a record that already has one, and is safe to repeat. Run it after `backfill-match-game-id.ts`, since the feed filters on `gameId` before it orders. Production's `fuse-production-matches` has **not** been backfilled yet, which is why EVERYONE is empty there while YOURS lists games; update this line when it has. Configuration CD applies them before the image (see [configuration CD](CONFIGURATION-CD.md)); `gcloud firestore indexes composite list --project=andershaf-87 --database=fuse-riders` shows them `READY`. `firestore.rules` stays deny-all and covers the new collection.

### Friends, presence and invites

The same gateway serves the account-level friends routes (`/api/friends/…`, `POST /api/rooms/:code/invites`) over three more collections: `${prefix}-presence` (one document per account that has ever synced: its public id, last sync, name, head and room), `${prefix}-friends` (one document per pair) and `${prefix}-invites` (one per inviter, invitee and room, with `cleanupAt` for the TTL policy `firestore.indexes.json` declares). Each is queried on one field (`publicId`, `room.code`, `uids` array-contains, `to`), so no composite index is needed; the configuration deploy creates the invites TTL policy. A client polls `POST /api/friends/sync` every 20 seconds while signed in; the service rewrites a presence document only when something in it changed or it is a minute old (docs/design/heartbeat-write-cost.md), so an idle player costs reads alone. Design: [docs/design/friends.md](../design/friends.md).

## Backend pipeline

**A change to room capacity deploys the service first.** `ROOM_LIMITS` (`service/room-limits.ts`) lives in the service, not the client: the page offers whatever it was built with, and the room service decides what it admits. A release that raises the cap and reaches Pages before Cloud Run leaves the client offering a place the service refuses, with the service's older wording explaining why — so for such a release, confirm the Cloud Run revision carries the new commit before publishing the frontend, or accept that the extra places do not work until it does. Lowering a cap is the same in reverse, and worse: members already admitted keep their sockets. Spectators (`maxGuests` 5 → 10, [design note](../design/spectators.md)) are the first such change.

Pages publishes the live backend revision, so the path filter also includes frontend release inputs (`public/`, `index.html`, `vite.config.ts`, and the Pages workflow). An asset-only change must advance that revision too; otherwise Pages would repeatedly publish the previous assets. Documentation, tests and browser-smoke-only changes can still skip deployment.

`.github/workflows/backend.yml` runs the same `scripts/deploy-cloud.sh` after a successful `CI` push run on this repository's `main`, so an automatic deployment obeys the same clean-checkout, current-main and green-CI gates as a manual one. It authenticates with workload identity federation and uploads `artifacts/cloud-release-<commit>.json` as run evidence. An automatic run first asks Cloud Run which commit it really serves and skips the build and rollout when no file between that commit and the new one is part of the backend or the frontend release inputs above. The service's `commit` label alone does not say that: the deploy script writes it in the same update that creates the new revision, so a rollout that never becomes ready leaves the label naming a commit nobody is served. The label therefore counts only when the latest created revision is the latest ready one and carries 100% of the traffic; a failed or unfinished rollout, a traffic split or a manual rollback all count as doubt, and the next run deploys, as every push did before the filter. The one exception protects against rollback: on doubt the filter also reads the revision that carries 100% of the traffic (`gcloud run revisions describe`), and when that ready revision's `commit` label is strictly newer than the queued target (the target is its ancestor, for example a stale run after a local deploy of a newer commit and a failed rollout of another, or a pin to a newer revision), the automatic run skips instead of rolling production back. A split, an unreadable or unready revision, a missing label, or a commit that is not provably newer all deploy. The diff uses `--no-renames`, so a file moved out of the backend paths still counts; the list is `scripts/lib/backend-paths.ts` and covers everything `Dockerfile.cloud` copies (all of `service/`, `packages/` and `games/`, so a client-only change still redeploys), the package manifests, the deploy and configuration scripts, the Firebase/Firestore configuration and the workflow itself. It compares with the served commit rather than the previous push, so a backend change whose own run failed or was superseded is deployed by the next run; an unreadable service description, a missing label, a served commit that is not an ancestor, or any other doubt deploys unless production is provably ahead of the target. A manual `workflow_dispatch` always deploys, so an intentional rollback to an older commit works. A skipped run uploads no release evidence, because nothing was released. Deploying is not acceptance: the smoke field still reads **NOT YET VERIFIED**, and the documented public smoke and network gates remain manual.
...
`backend.yml` runs automatically only after a successful `CI` push run on this repository's `main`, resolves the newest CI-verified `main` revision (not necessarily the triggering run's, because CI runs for different commits overlap and a waiting deploy run can be replaced) and skips a target that is already live, older than the live revision, or unchanged in all release inputs (see **Backend pipeline**), so no release change is left undeployed; a manual dispatch runs only from `main`, so a stale branch cannot roll the backend back behind the published frontend. `.github/workflows/pages.yml` runs automatically only after a successful `Deploy backend` run, never beside it: the two used to start from the same CI run, and because Pages publishes in about two minutes while the Cloud Run build takes about ten, each release that added an API route left the new frontend calling a backend that did not have it yet (the browser reports the failed preflight and the 404 as a CORS error). Pages therefore publishes the revision the live gateway reports as `revision` on `/api/health` (the image's `BUILD_REVISION`), requires a successful CI push run for that exact SHA (a backend deployed locally with `LOCAL_VERIFIED_REVISION` ahead of CI therefore publishes no frontend until its CI run passes and the resulting backend run triggers Pages again), retrying the health read through a cold start, checks it out and verifies it is still on `main` before building and again before publishing. The frontend can trail the backend by a few minutes, which is the compatible direction; it can no longer lead it. A failed backend deployment publishes no frontend. Pull requests and unchecked branches cannot deploy. It can also be dispatched manually from `main` with two required inputs: `revision`, the exact 40-character SHA that was verified locally, and `local_verification`, a non-blank description of the completed local checks (no secrets). The build job refuses a dispatch whose `revision` is not on `main`, and the deploy job re-checks that before publishing. A manual dispatch is not gated on the live backend's revision, so it is the operator's job not to publish a frontend ahead of the backend it needs. A manual dispatch does not wait for or consult the CI run for that commit. It uses `pnpm build -- --base=/fuse-riders/`, injects `VITE_API_ORIGIN`, uploads one Pages artifact, and deploys that same artifact without a rebuild. The application must use the injected API origin and preserve the Pages base path; gateway deployment alone cannot repair hardcoded `/api` URLs.
...
```sh
CLOUD_RUN_ORIGIN=https://VERIFIED_SERVICE_ORIGIN pnpm exec tsx scripts/cloud-public-smoke.ts
```

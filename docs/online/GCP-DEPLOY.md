...
One gateway serves every registered game (`platform.gameIds` in `service/history.ts`, passed to the room service as `gameIds`). Fuse Riders uses the same collections and fields as before, plus `gameId` on new match records and rooms: `${prefix}-matches`, `${prefix}-users` with its rating fields, `${prefix}-users/{uid}/rivals` and `${prefix}-rating-claims`. New Fuse Riders match records carry `gameId: "fuse-riders"`; older records without it read as Fuse Riders, so nothing is backfilled, and a gateway from before games ignores the field, so rolling the image back is safe. Every other game stores ratings and totals in `${prefix}-ratings` (one document per `gameId:uid`, with a `rivals` subcollection) and queries its match history by `gameId`. Fuse Riders' history query cannot filter on a field its old records lack, so it reads up to five pages to fill one. Before a second game is live, run `ROOM_COLLECTION_PREFIX=fuse-production npx tsx scripts/backfill-match-game-id.ts` (a dry run that counts), then again with `--apply`: it writes `gameId: "fuse-riders"` onto records that have none, which changes no meaning and is safe to repeat. Once it has run, Fuse Riders' query can filter on `gameId` like every other game's (a follow-up change). A second game must also wait until every serving revision stores `gameId` on rooms: a revision from before games ignores `?gameId=` and stores a room that reads as Fuse Riders', which an up-to-date revision then refuses to the other game's page. **The gateway therefore serves Fuse Riders alone by default.** `service/index.ts` adds a game only when the `EXTRA_GAME_IDS` environment variable names it (comma-separated; `dice` today), and an id without a registration stops the revision from starting. `backend.yml` passes the repository variable `EXTRA_GAME_IDS` through `scripts/deploy-cloud.sh` into the revision's environment. Set it to `dice` only after the backfill above has run with `--apply` and every serving revision stores `gameId` on rooms; clearing it and redeploying takes the game offline again, and its records and ratings stay. The local dev service (`service/dev.ts`) always serves every game. Until then the static `/dice/` page still loads: CREATE ROOM says online rooms are not open yet, and solo against bots needs no service. `firestore.indexes.json` adds the indexes those queries need: `(gameId, ranked, elo)` in both directions on `fuse-production-ratings` and `(gameId, participantUids, endedAt desc)` on `fuse-production-matches`. Configuration CD applies them before the image (see [configuration CD](CONFIGURATION-CD.md)); nothing Fuse Riders serves depends on them, but they must be ready before a second game is registered in production. `firestore.rules` stays deny-all and covers the new collection.

...
- Rebased again on 2026-09-16 over the gravity bomb, speed boost, chain-reaction and aim-bounce settings, analytics, keyboard shortcuts and the theme switch. `checkpoint.ts` validates `gravityFields` and the new player fields as `main` does; `RULES` is `fuse-p2p-3`, moved again because rider contact is now tested at matching times rather than path against path (#186); the transport says goodbye on its links when a page leaves on purpose (#143), as `main`'s did.

...
The report is `artifacts/online-network-benchmark.json`, including revision, profiles/seeds, raw UI samples, raw animation-frame intervals, payload-byte windows, injected drops/reorders and queue counts. It is written on failure too. Do not commit token-bearing URLs or unredacted WebSocket messages.
...
An application-level read-only bounded diagnostic surface is still needed for accepted snapshot scope/match/round/tick/outcomes, processed action IDs and dispositions, accepted-state age, actual input-to-changed-pose timing, correction events and resync/queue counters. Decoded RTC packets cannot substitute for evidence that the application accepted them. Until those diagnostics and assertions exist, the harness does not claim zero stale actions, duplicate shots, contradictory confirmed outcomes, or bounded application recovery.

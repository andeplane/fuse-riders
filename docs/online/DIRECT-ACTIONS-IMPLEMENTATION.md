...
Sustained mobile-02 completed five clean matches, then exposed transient pulse readiness during the sixth rematch. Activation now retains the frozen old world and prepared candidate until all pulse bindings succeed; existing retries and deadlines remain. The independent review and all 652 tests/coverage, build and Worker typecheck pass. [Activation evidence](direct-actions-evidence/activation-readiness/manifest.json) retains the failed browser run and regression results. Sustained verification of this correction is pending.

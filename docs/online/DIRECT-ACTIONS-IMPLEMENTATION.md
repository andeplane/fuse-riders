...
## Current round and recovery refinement

[ADR045](../adr/045-deterministic-round-progression.md) moves ordinary round progression into the shared direct reducer. The absolute tick clock, stream sequences and repair queues survive across rounds; unchanged-roster next rounds send no lifecycle headers or checkpoint chunks. Pending powerup settings are validated replay state, match format stays fixed, and finalized round changes cancel controls without consuming a UI revision. Shared-TV controller status preserves exact roster identities across rounds, including disconnected retained players. Speculative new-round geometry remains hidden until that round is finalized.

The mobile-01/mobile-02 failures showed why pacing full-state transition traffic was insufficient. Mobile-03 completed three rounds but recovered during initial countdown. A separate reviewed ADR042 correction serves completeness requests from behind peers even when the coordinator/origins have already finalized that tick. Requests share bounded repair fanout; duplicate/failed certificate sends wait 250 ms while fresh frontiers retain urgent delivery. Mobile-04 then passed three rounds with six matching Chromium/WebKit outcomes and no recovery. It preceded the final certificate-pacing refinement, so current-source repeated-match qualification is being recorded separately. No deployment or physical-device acceptance is implied.

Sustained mobile-02 completed five clean matches, then exposed transient pulse readiness during the sixth rematch. Activation now retains the frozen old world and prepared candidate until all pulse bindings succeed; existing retries and deadlines remain. The independent review and all 652 tests/coverage, build and Worker typecheck pass. [Activation evidence](direct-actions-evidence/activation-readiness/manifest.json) retains the failed browser run and regression results. Sustained mobile-03 then exposed a separate lost-pause problem: switching aliases retired the unreliable pause retry before one follower received its plan. A reviewed reliable copy now queues before later management, retaining one bounded pending notice across rebinding. All 653 tests/coverage, build, Worker typecheck and native Chromium/WebKit pause ordering regressions pass. [Reliable pause evidence](direct-actions-evidence/reliable-pause/manifest.json) preserves this second failure. Sustained mobile-04 passed two matches but failed the next rematch while sending five full checkpoints: peers had different confirmed ticks despite retaining the requested simulation history. Reviewed ADR043 lookup now reconstructs and hash-checks the requested retained tick without changing old finality. All 655 tests/coverage, build and Worker typecheck pass, with two focused equivalence/runtime regressions. [Retained-reference evidence](direct-actions-evidence/retained-reference/manifest.json) preserves the failure and correction. Sustained verification of the resulting source is pending.
...
The repeated mobile-05 run exposed an explicit-rematch pause gap after its first complete match: an old simulator could exhaust its history before receiving the replacement plan. The reviewed ADR041 correction now propagates the existing authenticated coordinator pause probe into a bounded runtime pause, sends it promptly, ignores stale/duplicate notices, and preserves both published and rendered scenes while waiting. No replacement plan within five seconds still enters visible recovery. All 647 tests pass, coverage is 99.62% lines/statements, 94.35% branches and 99.23% functions, and build/Worker typecheck pass. [Evidence](direct-actions-evidence/deterministic-rounds/manifest.json) distinguishes these final-source checks from earlier browser runs; sustained final-source qualification is in progress.

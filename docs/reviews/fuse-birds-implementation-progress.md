...
| Cross-runtime rules 5   | Complete 2/3/5-player matches agree tick-for-tick in Node, Chromium and WebKit, including checkpoint restores: `/tmp/fuse-birds-rules5-replay.log`. Firefox failed before game load because its installed browser could not find its profile; no Firefox gameplay pass is claimed. |

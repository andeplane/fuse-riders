...
Rule of thumb enforced by test: **engine → nothing · net → engine · render → engine/view(+kit) · app → all.** Data the renderer needs from rules travels *in the view* (e.g. per-rider `speed`, gravity field duration, next-volley angles, a `rules` block with `tickHz`, `blastVisibleTicks`, `trailWidth`) instead of being imported as constants and recomputed.

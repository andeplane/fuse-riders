...
Context restoration remains automatic for two seconds. A load shows its progress on a bar in the graphics status card: indeterminate while the lazy module downloads and Phaser boots, neither of which reports anything, then the loader's own figure. The ten-second deadline applies to that silent phase; once the loader reports, each step forward buys another ten seconds, so a slow connection finishes and only a load that has actually stopped fails. Failed restoration or an expired deadline exposes an in-place graphics retry, which replaces the canvas and reconstructs presentation from the latest snapshot without rejoining the room. Attempts are generation-scoped so stale asynchronous completions cannot overwrite a retry. No physics, inputs or network behavior changes. The tradeoff is an interrupted view after unrecoverable graphics failure rather than continued rendering through a separately maintained scene.

## Update 2026-09-18: Phaser 4

Moved from Phaser 3.90.0 to 4.2.1 for a maintained release line, with no intended visible change. Phaser 4 removed the pipeline system the beveled trail shader used; trails are now drawn by a custom WebGL program inside a Phaser `Extern`. Phaser 4's `Mask#setMask` is Canvas-only, which turned out to match existing behavior: the arena's WebGL context has no stencil buffer, so `GeometryMask` was already a no-op on WebGL under Phaser 3 — only the Canvas backend clips the shrinking playfield, on both versions. Phaser 4's Canvas renderer is deprecated upstream but kept as the no-WebGL fallback and for `?renderer=phaser-canvas`. See [docs/design/phaser-4-migration.md](../design/phaser-4-migration.md) for the full design and verification record.

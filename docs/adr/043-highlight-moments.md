...
## Why detection belongs in `games/fuse-riders/src/shared/game.ts`
...
- Online play folds the identical deterministic state on every peer (`games/fuse-riders/src/shared/apply-tick.ts`, rewound and replayed by `games/fuse-riders/src/online/rollback.ts`). A pure detector inside `step` therefore agrees everywhere with no new packet type, survives rollback because it lives in the cloned state, and reaches the LAN TV through the `matchOver` snapshot exactly like `matchStats` does.
...
A **moment** is `{ kind, round, tick, elapsed, playerId, targetIds, value }`, recorded during `step` from observations `step` already makes: `tick` is the absolute simulation tick a replay clip would be cut around, `elapsed` the tick within the round for the card's clock, and `value` the kind's own integer measure. Scores, titles and copy are presentation (`games/fuse-riders/src/shared/match-recap.ts`), so tuning them never changes the replicated state or its hash. Thresholds are named constants in `games/fuse-riders/src/shared/moments.ts`.
...
- Both elimination sites in `step` (the movement sweep and the same-tick Gun resolution; Target Bomb shared the latter until it was removed in `fuse-p2p-40`) feed one observation record; detection runs once per tick before round resolution, so aggregates such as `multiKill` see every death of the tick.
...
- `games/fuse-riders/tests/match-recap.test.ts`: highlight ordering, dedupe, variety, copy, empty case, signature change.

...
- Online play folds the identical deterministic state on the host and on every full view (`applyTick` in `src/shared/action-log.ts`, rewound and replayed by `src/online/rollback.ts`). A pure detector inside `step` therefore agrees everywhere with no new packet type, survives rollback because it lives in the cloned state, and reaches the LAN TV through the `matchOver` snapshot exactly like `matchStats` does.
...
A **moment** is `{ kind, round, tick, playerId, targetIds, value }`, recorded during `step` from observations `step` already makes. `value` is the kind's own integer measure. Scores, titles and copy are presentation (`src/shared/match-recap.ts`), so tuning them never changes the replicated state or its hash. Thresholds are named constants in `src/shared/moments.ts`.
...
- Per-tick cost: no new geometry on ordinary ticks. Death ticks scan the victim's own trail once (`boxedIn`); blast ticks scan each surviving rider's own trail once (`bombDodge`). No allocation on ticks without a moment.
...
- Checkpoint and wire validation (`src/online/checkpoint.ts`): a bounded array, enumerated kinds, `round` in `[1, game.round]`, `tick ≤ game.tick`, protagonist and targets present in `matchStats` (riders may have left the room), targets bounded and unique, integer values; `shell.bounces` optional non-negative integer. `gameInvariants` adds "lobby ⇒ no moments". `CHECKPOINT_VERSION` → 5 and `REPLAY_RULES` → `fuse-rollback-3`; `CHECKPOINT_COMPATIBILITY` is unchanged because the version pin already refuses every stored checkpoint. As with every rules bump, all clients refresh together and rooms restart after a rollback; `docs/online/PROTOCOL.md` records it.

## Recap

`buildMatchRecap(stats, moments)` gains `highlights`: the top `RECAP_HIGHLIGHTS = 5` by presentation score (multi-kill of three or more, direct hit, double kill, boxed in, trick shot, bomb dodge, cut off, mutual destruction, own goal, in that order of weight), one entry per `(round, tick, protagonist)` so one bomb that hit a head and took two riders is one card, and at most two cards per kind and per protagonist for variety. Each card has a title, one line of copy naming the riders, and the round with the time into it. Both recap renderers (`src/client/main.ts` and `src/online/ui.ts`) render the cards with the award-card styling plus a `highlight-card` class, in their own `recap-highlights` container ahead of the awards; the recap smoke's award-card floor still holds. Empty moments render no section.

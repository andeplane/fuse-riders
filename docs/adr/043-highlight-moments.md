...
- Both elimination sites in `step` (the movement sweep and the same-tick Gun resolution; Target Bomb shared the latter until it was removed in `fuse-p2p-30`) feed one observation record; detection runs once per tick before round resolution, so aggregates such as `multiKill` see every death of the tick.

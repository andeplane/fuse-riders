...
`RULES` names the simulation rules; see `RULES` in `src/engine/apply-tick.ts` for the current value. It changes with every simulation change, so it is neither quoted here nor in the checked table. It travels in `hello` and in every snapshot chunk and payload.

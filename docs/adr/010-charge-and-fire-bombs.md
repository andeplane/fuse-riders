...
[Issue #86](https://github.com/andeplane/fuse-riders/issues/86) makes full-charge time configurable through room settings, from two to 40 ticks (0.1–2 seconds), defaulting to eight ticks (0.4 seconds). The original pace is selectable as 1.2 seconds. Preferences persist in the creator's browser and apply at the existing round boundary. The active `bombChargeTicks` travels in validated snapshots, keeping AI, fractional previews and charge indicators aligned with authority rather than pending preferences. (The LAN default this sentence used to mention went with the LAN server in [#271](https://github.com/andeplane/fuse-riders/pull/271).) The 100–400 unit range and the authority, release, cancellation, flight, fuse and cooldown rules are preserved. Current bounds live in [bomb-launch.ts](../../games/fuse-riders/src/engine/bomb-launch.ts).

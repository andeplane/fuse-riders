...
- **Director.** Moment events queue per round; when a screen sees the phase turn to `roundOver` or `matchOver`, the heaviest moment of that round (the reel's own ranking) plays after a one-second hold, if its clip exists. A screen that joined late or missed frames shows nothing and the pause simply passes. The director hands each animation frame a clip frame, the zoom and focus, the flash, and the cues (`in`, `impact`, `out`) the audio director turns into stings.
...
- `tests/rollback.test.ts`: one moment event across a rewind.

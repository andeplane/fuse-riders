...
The continuing-play verifier (`scripts/fuse-birds-continuing-check.ts`) retains ordinary action histories, sampled-state hashes and successful routes for reachable post-destruction states. Its rules-4 corpus covers 40 matches and 589 current-wind directed checks: 29 need a walk/hop route and 24 have no witness in the initial direct/walk/hop search. Expanded search found three direct and nineteen ordinary Pebble excavation routes among those 24 cases. `scripts/fuse-birds-escape-check.ts --verify-saved` independently replays the retained `games/fuse-birds/tests/fixtures/continuing-play-witnesses.json` routes against the corpus checkpoints, checking initial hashes, legal action timing, surviving shooters, target damage and final hashes. Other birds pass during excavation: these are existence witnesses, not winning strategies against resisting opponents. The corpus snapshot writer now copies terrain revision arrays, avoiding later-turn mutation of saved metadata. Both unresolved cases involve seed 3/four players at turns 32–33 near sudden death. Their classification remains open; rising water alone does not establish reachability. The verifier exits nonzero while unresolved cases remain, and this finite corpus is not a proof for every possible later state.
...
Follow-up classification of the two late corpus cases: `scripts/fuse-birds-escape-check.ts --late-comparison` verifies successful routes from earlier real turns 20/21 with identical terrain bits, bird positions, health and wind. At turns 32/33 the same ordinary excavation sequence ends after one launch because water reaches 481, above the low bird's feet at 484.996. The retained `late-water-comparison.json` documents a missed excavation opportunity before sudden death, consistent with the distinction below between reachability and a guaranteed winning move. No terrain or health is altered to obtain that comparison. The strict current-state route verifier still reports two misses; they are explained timed losses rather than successful current-state routes. Finite-corpus coverage is not a universal guarantee.

**Opening fairness:** from every assigned starting perch, every other player's starting body must be damageable in one legal shot with unlimited Pebble ammunition, without prior movement, digging, self-elimination, rare weapons or crate luck. Check directed pairs: A hitting B does not prove B can hit A. Check every permitted initial wind state, not only calm air. For five players this is 20 ordered pairs per wind state.

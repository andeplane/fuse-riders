...
On desktop, use **← / →** to steer and hold/release **Space** to charge and fire in online rooms or solo mode. Opening a menu or leaving the tab cancels held controls. On phones the lobby is a plain screen in either orientation: the room code with the join link and QR, the riders, and for the host START RACE, ADD AI, ROOM SETTINGS and TV VIEW. Once the race starts, rotate to landscape: from countdown through the match report the phone is the controller — the left, middle and right thirds of the screen steer left, charge/fire, and steer right, and the phase notice floats at the top. Introductory hints fade over the arena; shared-TV phones retain visible colored controls. **☰ MENU** opens the roster, game actions (START RACE / REMATCH / BACK TO LOBBY) and settings. Fullscreen is requested where the browser supports it. Hold **Fire** to charge a forward launch, then release. Target Bomb changes Fire into a thumb trackpad with a public aiming marker. It is available only when every connected human rider is on a phone using touch controls; keyboard, tablet and unknown devices disable it. The online lobby shows each rider’s reported device/input. AI and TV-only viewers do not affect eligibility. If a mixed-device rider joins or a phone starts using gameplay keys, existing Target pickups and armed Targets are cleared; the saved drop weight returns when the room is phone-only again. Device detection is browser-reported rather than guaranteed hardware identification. Tap **HEAD** to change avatar, including during a round. Phone colors match riders. Joiners can enter during play when a seat is available and wait for the next round.
...
Common **Power** pickups improve your main weapon for the rest of the round. Collect five to gain a level: larger blasts and faster reload, with diminishing gains at higher levels and a fixed fuse. Levels and partial progress reset each round. Rider labels start at level 1. Every rider has a gold inner ring that fills toward the next level, separate from the outer reload countdown; your controller also shows the pickup count. Special drops include Beer, Ink, Triple/Five Shot, Target Bomb, Orbit Shield, portals, shells, gun projectiles, speed boost and Singularity. Star is excluded from default drops.

For balance experiments, edit [POWER_TUNING](src/shared/power-progression.ts): `pickupsPerLevel` sets the level threshold; `spawnTicksPerRider`, `activePickupsPerRider` and `maxActivePickups` control abundance; `defaultDropWeight` sets Power's default share relative to the [special pickup weights](src/shared/pickup-weights.ts). Blast/reload limits and the diminishing-return curve live there too. Intervals use 20 Hz simulation ticks. Drops appear one at a time, about once every two seconds per living rider (bots included), with four board slots per living rider and a global ceiling of twenty. There is no time-based spawn ramp. When a rider dies, the next scheduled attempt uses the smaller population; existing drops stay until collected or expired, and new drops pause while at or above the reduced cap. Existing reloads and launched bombs retain their original values; upgrades apply to subsequent launches, including multishot/Target blasts and Singularity field size. Reload also applies to gun/shell launches, whose projectile behavior is unchanged. Integer tick rounding means reload eventually plateaus at its floor. Saved settings containing the removed Blast/Fuse types are invalidated and fall back to defaults; configure and save a new draft after updating.
...
ONLINE_URL=http://localhost:8787/ npx tsx scripts/device-profile-smoke.ts
BROWSER=webkit ONLINE_URL=http://localhost:8787/ npx tsx scripts/device-profile-smoke.ts
npx tsx scripts/device-profile-lan-smoke.ts

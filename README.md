...
| `src/online/fuse-game.ts`, `room-runtime.ts`                                 | Fuse Riders as a `RollbackGame` (entries, fold, checkpoint fields, seats, text), and its runtime with the rider's controls                                                                                                                |
| `src/render/time/present.ts`, `src/online/net-stats.ts`, `ui.ts`             | Fractional presentation with immediate local steering, the per-device link quality overlay, and the room UI                                                                                                                               |

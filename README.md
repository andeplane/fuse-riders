...
The end-of-match report (podium, totals, highlight reel, awards and rider comparison) is built by the pure [`src/engine/match-recap.ts`](src/engine/match-recap.ts) module from the authoritative `matchStats` and `moments`; the online `MATCH RESULTS` dialog renders it, so ties, empty rosters and formatting are covered once by `tests/match-recap.test.ts`. The highlight reel lists the plays worth replaying and is detected inside the shared simulation by [`src/engine/moments.ts`](src/engine/moments.ts) so every device agrees on them.

...
**Play solo or with friends.** Choose **PLAY SOLO** for an immediate local game against four AI riders, **PLAY LOCAL** for game controllers on one computer, or create a room and share its invite. Solo and local controller play use no room service or WebRTC; refreshing starts a fresh run. In a room every device simulates the game, so any rider can refresh or drop and rejoin while the others keep playing. Online gameplay requires a direct WebRTC connection; unsupported networks show a retry state. Physical-phone qualification is still pending.
...
**Game controllers on one computer:** choose **PLAY LOCAL** on the home page (or open `?solo=1&local=1`). Connect each controller and press a button while the page has focus so the browser exposes it. Select up to five players, edit their names, then choose **START LOCAL GAME**. AI fills unused seats. Standard controllers use D-pad / left stick to steer and the bottom face button to charge and fire on release; **CONFIGURE CONTROLS** supports other layouts. After closing the results, Start on a standard controller rematches. Disconnecting or opening a menu cancels held controls; reconnect the same pad and release its controls to resume. To change the roster, exit and open PLAY LOCAL again. Local controller games do not submit account results.

**Multiple keyboards or controllers in keyboard mode:** click **ADD KEYBOARD PLAYER** once per player. Defaults in **left / right / fire** order are **A/D/Space**, then **E/F/M**, then **I/G/K**. Change any binding with its dropdown; shared keys are refused so one controller cannot steer two riders. Further players default to ←/→/↓ and J/L/O. Keyboard players and Web API gamepads can share the five seats. The in-game keyboard help lists everyone's bindings. Ordinary PLAY SOLO and online keyboard controls remain arrows or A/D and Space.

Controller access uses the [browser Gamepad API](https://www.w3.org/TR/gamepad/), which can require a button press before a device appears. If detection is blocked, open the page directly in Chrome or Safari on HTTPS or localhost and use **RETRY DETECTION**. Mapping depends on the controller's connection mode and browser. Automated validation uses simulated pads; physical 8BitDo acceptance is a separate playtest. Run its browser check against a local server with `BASE_URL=http://localhost:8787 pnpm exec tsx scripts/local-gamepads-smoke.ts` (`BROWSER=webkit` also supported).
...
Open the URL it prints — **http://localhost:8787/** unless that port is taken, in which case it walks upward (8788, …) and says which one it got, like `pnpm dev`. Create a room, choose shared-screen or individual-device play, and share its short code (for example **AB42**), invite link or QR code. A shared-TV lobby keeps its QR code visible until the race starts, with the join link printed under it and a **COPY LINK** button for anyone who cannot scan. The creator can join as a player on the same phone and use **BACK TO LOBBY** and **ROOM SETTINGS**. **TV VIEW** opens a separate display role for a shared screen in a new tab. Anyone who would rather watch than race taps **JOIN AS SPECTATOR** under the join button: they take no seat, steer nothing and appear in the lobby's **WATCHING** list, and the room holds up to five of them beside its five riders. Changing your mind does not mean leaving: between rounds, **TAKE A SEAT** on your own row in the **WATCHING** list puts you in a free seat — a free one, or one a rider the room lists as offline was holding — and **WATCH** on your own rider row gives your seat up and moves you to the list. Both buttons say why when they cannot: a round is running, the side you want is full, or you are running the room and the host's device has left, which leaves nobody who can move you. Every connected human rider toggles **READY** in the lobby; when all are ready and there are at least two riders (AI included), the countdown starts automatically. After the results, every rider toggles **READY FOR REMATCH**. **NOT READY** withdraws a vote before the countdown starts. TV displays, spectators and AI need no vote. Reconnecting or changing room settings clears readiness; solo keeps its start and rematch buttons. Whoever runs the room wears a **HOST** badge on their row, and it moves: if the host's device drops, the rider in the next seat down takes over the room's controls within a few seconds and can change the settings, go back to the lobby and manage riders, so the rest of the party is not stuck waiting for someone who is not coming back. It moves straight back as soon as the host is in the room again, because the host's device still owns the room code. **☰ MENU → LEAVE ROOM** leaves the room running for everyone still in it; **END ROOM**, which only the host has, closes it for everyone. The host can also remove a rider or a watcher with the **×** on their row, between rounds for riders — the first tap asks **KICK?**, the second one does it, and whoever was removed can join again. Use HTTPS for remote-device testing; local HTTP testing does not prove Internet connectivity.
...
Room settings offer **3 ROUNDS · QUICK** and **5 ROUNDS · STANDARD**, plus a custom length of 1–20 rounds. Open **CONFIGURE POWERUPS** to adjust drop weights and **BACK TO ROOM SETTINGS** to return; every change saves as you make it, and an invalid value shows why it did not. A weight of zero disables that drop; all zero means no random drops. Defaults and preferences are stored in the creator's browser under `fuse-riders-room-settings-v2`. This storage-key reset replaces older saved room preferences with current defaults on the next visit, including enabled Nitro and Snail drops; preferences saved afterward persist normally. Match length changes apply to the next match, and pickup-weight changes apply to the next round. Clearing browser storage loses saved preferences and room credentials.

**ROOM SETTINGS → Bomb aim time (seconds)** adjusts how quickly a held bomb reaches maximum distance in solo and online rooms: 0.1–2 seconds in 0.05-second steps, default 0.4 seconds. Try 1.2 seconds for the original pace. It applies next round; the room shares one active aim time for players, AI and previews. With aim bounce on, the aiming marker moves at a steady speed through the middle 75% of its range, smoothly slowing into each end and accelerating away. It pauses for 100 ms at maximum reach to make full-range throws easier. The range and time to maximum stay the same; each full cycle gains that 100 ms pause.

Every device in a room simulates the game from one shared input log, so no browser owns the world: the creator's stream carries the room management entries (seats, settings, start, AI riders), and if the creator goes quiet for five seconds the lowest connected rider marks it absent so play continues. A refreshed creator or guest rejoins the running match with a validated snapshot from any peer; nothing is persisted locally. A tab you switch away from steps away by its own entry: it neutralises its controls, stops being waited on, and stops being asked for a world — but it keeps its seat, is placed in the next round and is still rated, so glancing at another tab no longer costs the seat. Coming back brings it straight in. These mechanisms have regression tests (`games/fuse-riders/tests/room-runtime.test.ts`, `games/fuse-riders/tests/hidden-tabs.test.ts`), while sustained recovery on real phones and networks remains unqualified.
...
Rounds are played on an **arena map**, chosen in ROOM SETTINGS. **Desert**, **Forest** and **City** share the same native-proportion buildings, crates, circular rocks and pyramids, and differ in their floors, spacing and how many of each they lay; **Classic** is the plain neon grid with nothing on it. A room's default rotates through all four, so a match visits every one of them — the obstacle-free Classic included — before it repeats any. Every map is generated per round from the match seed, so every device lays out the same board ([`arena-map.ts`](games/fuse-riders/src/engine/arena-map.ts)).

Only immunity (a Star, shield grace or portal grace) can carry a rider into scenery; if it lapses in there, that one obstacle lets the rider out again rather than killing it on the spot, and the same goes for a shell fired from inside. Scenery is otherwise solid, though judged in the rider's favour: you crash at your trail head's width, not your portrait's, and against the shape that is drawn — rocks use a full circle, while crates, buildings and pyramids use their full rectangle (`OBSTACLE_HIT_SHAPES`), so the empty corners around a rock are floor. Riding into it kills you exactly as the boundary does, and carries the same `wall` cause as the boundary (the elimination feed says “crashed” rather than “hit the wall” on a board that has scenery, because the cause alone cannot say which solid thing you hit). A bomb blast clears whatever it covers, at the blast's full radius, so blowing a hole through a row of buildings is a way through and a way to open a line on someone. Shells bounce off scenery and gun shots stop dead at it; thrown bombs fly over and land normally. Pickups and portal gates never appear on top of it, every rider starts with a clear road ahead, and the closing overtime walls crush whatever they reach. An Orbit Shield absorbs one crash and turns you away from the face you hit; Star rides straight through. AI riders steer around it under the same rules.

Two more maps change the edges instead of the ground. **Wrap-around** has no walls: riders, shells, bullets, thrown bombs and blasts that leave one side of the board arrive on the opposite one, a trail just past an edge is as solid as any other, and bots plan through the edges too. A bullet flies one board's width in all, so a shot that meets nothing still ends. Overtime brings the walls in from the very edge, and from then on the round is an ordinary walled one. **Crossed** is the classic arena under exactly the classic rules, drawn shifted by half a board: the outer wall meets in a cross in the middle of the screen, and what rides off one screen edge comes back on the other. It plays move for move like Classic and is only harder to read. Neither is part of the default rotation.

Two more put scenery that moves on the board ([`scenery-motion.ts`](games/fuse-riders/src/engine/scenery-motion.ts)). **Drifting cross** is Wrap-around with a cross of walls on it: the two walls start on the board's edges and slide across it like a screensaver logo, each turned back by the edge it reaches, so the room you are riding in keeps moving and you have to move with it, through the open edges if need be. **Trains** keeps the classic walls and runs three trains — red, yellow and blue — round two loops of track that never meet, two the same way round the outer loop and one the other way round the inner one. The rails are paint; the cars kill exactly as a rock does, and a train that has just rolled onto where you stand kills you too. Neither a bomb blast nor the closing overtime walls remove a wandering wall or a train: they are the map. Both are opt-in, like Wrap-around and Crossed, and every device runs the same trains from the same checkpoint.

A map is the room's choice and part of the simulation. The visual style (Neon Pixel / Clean Neon) stays a per-device choice on top of it, so one player can watch the same desert in either style.

On desktop, use **← / →** to steer and hold/release **Space** to charge and fire in online rooms or solo mode. Opening a menu or leaving the tab cancels held controls. On phones the lobby is a plain screen in either orientation: the room code with the join link and QR, the riders, a READY toggle for every rider, and for the host ADD AI, ROOM SETTINGS and TV VIEW. Once the race starts, Invisible full-screen controls follow the TV controller layout: portrait has the bomb in the top half and left/right steering in the two bottom quarters; landscape has left/right in the first two quarters and the bomb in the right half. White outlines trace the regions with outlined arrows and a bomb throughout the countdown, then disappear as the round starts. Shared-TV pads keep their visible artwork. The phase notice stays at the top. **☰ MENU** opens the roster, game actions (READY FOR REMATCH / BACK TO LOBBY) and settings. Fullscreen is requested where the browser supports it. Hold **Fire** to charge a forward launch, then release. While Fire is held the rider eases down to half speed to steady the aim, for at most a second, and eases back up afterwards. Phone colors match riders. Opening a room link seats you straight away: no form in between, wearing the name, colour and head you had last time, or the nearest free ones when somebody already has them. **NAME**, **AVATAR** and **COLOUR** change all three from inside the room, and **WATCH** gives your seat up for the watching list (**TAKE A SEAT** brings you back), right up until you press **READY** — which settles your rider for the round. Each rider picks a colour and a head from ten of each, and the room keeps those two unique: a choice another rider already wears is shown as theirs rather than offered. The AI riders all wear the robot head. Joiners can enter during play when a seat is available and wait for the next round.

On a computer, hold **← / →** or **A / D** to steer and hold/release **Space** for the Fire action in solo play or online rooms. Opening a dialog or switching away cancels held controls. Desktop arena views use compact controls to give the board more space; touch devices keep large pads.

Common **Power** pickups improve your main weapon and increase your maximum trail length for the rest of the round. Riders start with 8 seconds of trail (1,200 units at normal speed); each diamond adds a flat 2 seconds (300 units), reaching 16 seconds after 4 diamonds and 24 seconds after 8 diamonds. Existing active trail lasts longer immediately, so the rider grows into the extra capacity without restoring expired or destroyed segments. Trail lifetime has a defensive ceiling of 1,024 ticks (51.2 seconds) to stay within the checkpoint budget. Detached pieces and eliminated riders’ remaining trails pause for three seconds, then shrink from both ends at 37.5 units per second per end, remaining collidable until gone. Each pickup contributes immediately to larger blasts and faster reload, with diminishing returns and a fixed fuse. The count resets to zero each round. Beside every rider’s name, a gold diamond and number show the total collected; your controller shows the same count. The player-colored ring around the avatar shows reload only. Special drops include Star, Beer, Ink, Triple/Five Shot, Orbit Shield, portals, shells, gun projectiles, Nitro, Snail, Grip and Gravity. A portal pair carries shells and gun shots as well as riders; a shell gets its own re-entry cooldown, and a gun shot crosses each pair at most once, so neither can be held in a loop. Thrown bombs are unaffected, since they resolve against the point they were aimed at ([ADR 045](docs/adr/045-projectiles-through-portals.md)). Existing saved drop weights stay as configured; select CLASSIC in powerup settings to apply the current defaults.
...
**RANGE** increases maximum bomb reach for the rest of the round: **1.5× → 1.75× → 2×** (600 / 700 / 800 units). It stacks three times, leaving later drops for rivals. Minimum reach and charge timing stay unchanged; aim previews and bots use the upgraded reach. The HUD shows the current multiplier. Enabled in default drops; select CLASSIC to update existing custom weights.
...
The shared-screen TV view (**TV VIEW**, `?room=CODE&display=1`) provides audio controls, fullscreen, session scores and end-of-match statistics. Music and effects need a browser user gesture. Phone-sized Chrome/WebKit product checks pass; physical device and background/lock behavior remain separately unverified.
...
The shared deterministic simulation advances at 20 Hz and uses pinned JavaScript trigonometry so every engine folds the same state. Once every seated human rider is out of a round and a bot still rides, each shared tick steps the simulation three times instead of once until the round ends, so the game runs faster while the clock, the log and the network cadence keep their rate (`BOTS_ONLY_STEPS_PER_TICK` and `stepsPerTick` in [tuning.ts](games/fuse-riders/src/engine/tuning.ts)); the tick rules themselves do not change. Online rooms are peer-to-peer: every device that renders the world simulates it locally from one shared input log. Each member owns one stream of edge-filtered entries (steer, press, release, cancel, avatar); the creator's stream also carries the management entries (join, leave, presence, settings, start, rematch, lobby, AI riders). Every member sends one small MessagePack packet to every other member per tick and immediately on a new entry; completeness, liveness, loss and RTT are derived from that stream, and a missing entry is repaired by nack or by rotation through the retained window. A player's own input applies on the next simulation tick; other players' inputs apply one network hop later, and a late entry rolls the world back up to 40 ticks and re-simulates. Joiners and refreshed pages install a validated snapshot from any peer. See the [P2P design brief and measurements](docs/online/P2P-INPUT-LOG-BRIEF.md); `?stats=1` (or **ROOM → SHOW NETWORK STATS**) shows each device's own link quality. Gameplay never uses the backend as a relay. Failed WebRTC connections show why (STUN, signalling or ICE) in the header and under **MENU → LINK DIAGNOSTICS**; there is no TURN server, so a guest behind symmetric or carrier-grade NAT (common on cellular) may be unable to connect directly and should join the host's Wi-Fi. See [protocol notes](docs/online/PROTOCOL.md#direct-link-establishment-diagnostics-and-nat-limits-issues-12-27).

| Location                                                                                             | Responsibility                                                                                                                                                                                                                                                                                                                                                                                                            |
| ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `games/fuse-riders/src/engine/`                                                                      | Deterministic rules: the tick pipeline (`sim/`), the effect/pickup/weapon registries, pure rider-motion kernel, bounded AI controller, geometry, scores, settings, drops and the checkpoint codec (`codec/`). Imports nothing outside itself: since #254 the migration allowlist `tests/fixtures/layer-allowlist.json` is empty, so `tests/layer-boundaries.test.ts` fails a new cross-layer import instead of pinning it |
| `games/fuse-riders/src/shared/`                                                                      | What is not simulation: the wire vocabulary (`protocol.ts`, which re-exports the engine's own primitives), avatar labels and atlas cells, the game id, career/account types and the Firebase client config. The rules the log and the checkpoint guard validate against are the engine's — `engine/rider-name.ts` and `engine/avatar-id.ts` — not these                                                                   |
| `games/fuse-riders/src/render/`                                                                      | Phaser presentation (WebGL/Canvas), presentation time, themes and drawing helpers. Imports only the engine's view contract (`games/fuse-riders/src/engine/view.ts`, `view-kit.ts`)                                                                                                                                                                                                                                        |
| `games/fuse-riders/src/client/`                                                                      | Audio, avatars, replay, announcer and phone pointer controls                                                                                                                                                                                                                                                                                                                                                              |
| `games/fuse-riders/src/engine/input-log.ts`, `apply-tick.ts`                                         | Log entry types and validation, the gesture fold, and the deterministic per-tick reducer over management and player entries                                                                                                                                                                                                                                                                                               |
| `packages/fuse-netcode/`                                                                             | Game-agnostic netcode behind the `RollbackGame` contract: stream buffers with repair, the speculative world with rollback, the slewed clock, packet and snapshot codecs, and the room runtime ([README](packages/fuse-netcode/README.md))                                                                                                                                                                                 |
| `games/fuse-riders/src/online/fuse-game.ts`, `room-runtime.ts`                                       | Fuse Riders as a `RollbackGame` (entries, fold, checkpoint fields, seats, text), and its runtime with the rider's controls                                                                                                                                                                                                                                                                                                |
| `games/fuse-riders/src/render/time/present.ts`, `games/fuse-riders/src/online/net-stats.ts`, `ui.ts` | Fractional presentation with immediate local steering, the per-device link quality overlay, and the room UI                                                                                                                                                                                                                                                                                                               |
...
| `service/`                                                                                           | The room service for every game, on `fuse-network-be` and `fuse-platform`: production (`index.ts`), local development and CI (`dev.ts`), the room capacity and each game's stats registration (`history.ts`)                                                                                                                                                                                                              |
...
| `games/fuse-riders/tests/`, `tests/`, `scripts/`                                                     | Fuse Riders' deterministic tests and golden hashes; service, tooling and layering tests; browser checks and benchmark runners                                                                                                                                                                                                                                                                                             |
...
[AGENTS.md](AGENTS.md) defines engineering expectations. The [system map](docs/architecture.md) describes module ownership; verify game constants from source, and later ADRs take precedence over earlier ones. Themes stay separate from gameplay and hitboxes; see [theme assets](docs/theme-assets.md) and the [chosen graphics reference](docs/gameplay-concepts/06-neon-pixel-hybrid.png).
...
ONLINE_URL=http://localhost:8787/ pnpm exec tsx scripts/spectator-smoke.ts
...
Solo and online arenas turn 90° locally when that makes the complete board larger. Avatars, pickups and labels stay upright; scenery, trails and collision footprints turn together. Phones, tablets and narrow windows use invisible full-screen touch regions matching the TV controller: bomb above left/right in portrait, and left/right beside a half-screen bomb region in landscape. Hold the bomb to charge and release to fire. White region outlines with outlined arrow/bomb icons appear during the countdown and disappear when play starts; MENU and the HUD stay above the overlay. MENU → SETTINGS → Landscape bomb side swaps the groups. Opening the menu, changing sides or rotating cancels held input. Shared-TV controllers use the same grouping with visible opaque pads. Run `pnpm exec tsx scripts/portrait-arena-browser.ts` (also with `BROWSER=webkit`) for renderer and offline solo checks. To try it, run `pnpm exec vite --host 0.0.0.0`, open `http://localhost:5173/?solo=1&mute` (or your computer’s LAN address on a phone), play in portrait, then turn the device during play.

Shared-TV phones use **Arcade Pads**: portrait puts a red bomb pad above two blue steering pads; landscape puts both steering pads on the left half and the bomb on the right. **MENU → SETTINGS → Landscape bomb side** swaps those groups and remembers the preference on that device, without changing portrait or reversing left/right steering. Rotating or changing the setting cancels held input. The fire pad shows the current weapon with the same themed icons as the arena (Gun, Shell, Five or Triple, otherwise Bomb), while retaining charge/release and weapon-readiness feedback. Scores, round/winner announcements and automatic results stay on the TV; the phone keeps rider identity, connection status, sign-in and a compact MENU overlay with settings. Run `ONLINE_URL=http://localhost:8787/ pnpm exec tsx scripts/arcade-pads-smoke.ts` (also with `BROWSER=webkit`) to check the shared-room flow, both layouts, side preference, reloading and input cancellation. This is browser emulation, not physical-phone qualification.

The same Phaser scene stays visible behind the lobby and end-of-match overview, blurred and dimmed while those screens are open (a shared-TV controller shows no arena at all; the TV does). The end-of-match view has no outer panel or frame, and leads with the winner, final standings and match highlights. “View full stats” expands totals, the replayable highlight reel, awards and rider comparison; the footer keeps rematch and lobby actions within reach. Its data is built by the pure [`games/fuse-riders/src/engine/match-recap.ts`](games/fuse-riders/src/engine/match-recap.ts) module from the authoritative `matchStats` and `moments`; the online `MATCH RESULTS` dialog renders it, so ties, empty rosters and formatting are covered once by `games/fuse-riders/tests/match-recap.test.ts`. The highlight reel lists the plays worth replaying and is detected inside the shared simulation by [`games/fuse-riders/src/engine/moments.ts`](games/fuse-riders/src/engine/moments.ts) so every device agrees on them.
...
[CI](.github/workflows/ci.yml) has two parts. The gate a pull request waits on is `verify`, which is green only when `checks` (formatting, lint, configuration and the build, which is also the type check) and the four `unit` test shards all passed; they run side by side, so the gate is the slowest of them rather than the sum. That is five jobs on purpose — it is how many concurrent runners this account gets, and a sixth would sit queued. Coverage is not in it: `coverage` runs the same tests instrumented on a main push and on the release suite, never on a pull request, and so do the full whole-recording replay checks (see [`replay-budget.ts`](games/fuse-riders/tests/fixtures/replay-budget.ts)). The browser matrix runs on a push to main, on a manual dispatch, or on a pull request labelled `full-ci`: one parallel `smoke: …` job per entry of [`scripts/ci-manifest.json`](scripts/ci-manifest.json), each retried once with the flake reported, summed up by one `e2e` job that is green only when every smoke passed. A push to main deploys only once all of it passes; the backend then redeploys only when something it is built from changed ([`scripts/lib/backend-paths.ts`](scripts/lib/backend-paths.ts)). Run the affected local smoke when practical and report any untested browser flow. Reserve `full-ci` for explicit requests or changes whose failure would be expensive to unwind; routine PRs do not wait on the full browser matrix.

To run the whole CI suite locally in the same order and with the same env, use `scripts/ci-local.sh`. It runs the same manifest through the same runner as CI (`scripts/ci-run.ts`), so there is no second list to keep in step; to add or change a smoke, edit `scripts/ci-manifest.json`. It stops at the first failing step, prints a `PASS`/`FAIL` line with wall time per step, starts the local room service itself (log in `artifacts/room-service.log`) and always stops it on exit. The room service takes a free port, or starts its search at `PORT`, so parallel worktrees do not collide. `ONLY` runs a comma-separated subset of steps (`format`, `lint`, `config`, `build`, `unit`, `coverage`, `keyboard`, `desktop`, `online`, `voice`, `preview`, `phaser`, `home`, `landscape`, `phase`, `recap`, `shared`, `spectator`, `determinism`, `mesh`, `dice`; `core` expands to every `verify` step and `release` to the coverage gate, and a single variant such as `online-webkit` also works) and starts the room service only when a selected step needs it. Steps CI runs in both Chrome and WebKit still run both. Unlike CI, a local run does not retry a failed smoke; `pnpm exec tsx scripts/ci-run.ts --smoke <id>` runs one smoke exactly as its CI job does, retry included. The script assumes `pnpm install --frozen-lockfile` and `pnpm exec playwright install chrome chromium webkit` have run; the room-service steps serve `dist/`, so run `build` (or `core`) first:
...
Coverage thresholds in [.c8rc.json](.c8rc.json) are 95% lines/statements/functions and 85% branches across its listed modules. Those thresholds do **not** mean every browser path is covered. And a pull request does not measure them at all: as the gate above says, `coverage` runs on a main push and in the release suite, never on a pull request, so a green PR is the build, lint, configuration and the unit shards — not coverage and not the browser checks. Inspect the actual revision's result, and whether `coverage` and `e2e` ran on it at all, rather than treating this checklist as proof of passing CI.
...
and one `Miss` per shot that killed nobody, sent by the shooter's own device once each round is decided and confirmed. Local dev (any address with a port, a LAN address included) is off
...
([`rider-name.ts`](games/fuse-riders/src/engine/rider-name.ts)): 1–18 characters, trimmed, no control characters.
...
1. When the recap opens, each rider's device sends the result it computed to
   ...
   token in `X-Fuse-Identity` ([`match-report.ts`](games/fuse-riders/src/online/match-report.ts)).
...
| `POST /api/friends/sync`                 | `Authorization: Bearer <ID token>`                                           | Body `{ name?, avatarId?, room?: { code, memberId, gameId? } }`: where this device is. A room claim is proven when it changes and again about once a minute: the seat must be a live member of the room the code names now (403 otherwise), which only a device in the room can know; the claim is bound to that room's incarnation, so a reissued code is another room. Answers with the caller's public id, friends (online, and their room), requests both ways, pending invites and the signed-in members of the reported room. Presence is rewritten only when it changed or is a minute old. 400 per account per hour |
| `POST /api/friends`                      | `Authorization: Bearer <ID token>`                                           | Body `{ publicId }`: ask to be friends, or accept. 30 per account per hour; 100 friends and 50 open requests, sent or received, per account                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
[`games/fuse-riders/tests/match-report.test.ts`](games/fuse-riders/tests/match-report.test.ts) feeds stats produced by the real simulation through the
...
- **Guests never load Firebase.** [`account.ts`](games/fuse-riders/src/online/account.ts) imports the SDK on the first tap of SIGN IN,
...
The client config lives in [`games/fuse-riders/src/shared/firebase-config.ts`](games/fuse-riders/src/shared/firebase-config.ts). **None of it is secret** —
...
2. Set `authDomain: 'fuse-riders.web.app'` in [`firebase-config.ts`](games/fuse-riders/src/shared/firebase-config.ts).
...
```bash
curl -s -X POST -H 'Referer: http://localhost:8787/' -H 'Content-Type: application/json' -d '{"providerId":"google.com","continueUri":"http://localhost:8787/"}' "https://identitytoolkit.googleapis.com/v1/accounts:createAuthUri?key=$(grep -o "AIza[A-Za-z0-9_-]*" games/fuse-riders/src/shared/firebase-config.ts)"
```

...
| ---------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `games/fuse-riders/src/engine/`                                                                      | Deterministic rules: the tick pipeline (`sim/`), the effect/pickup/weapon registries, pure rider-motion kernel, bounded AI controller, geometry, scores, settings, drops and the checkpoint codec (`codec/`). Its only imports outside itself are the five edges pinned in `tests/fixtures/layer-allowlist.json` |
| `games/fuse-riders/src/shared/`                                                                      | What is not simulation: wire protocol types (`protocol.ts`), avatars, the game id, career stats, duration text and the Firebase client config. The rider-name rule is not here — it is `engine/rider-name.ts`, because the log and the checkpoint guard apply it                                                 |

...
The soundtrack is background music: it plays for as long as the page is open, starting on the landing page itself, and nothing in the game state restarts a track — rounds, matches and alt-tabbing all leave it playing, and the playlist only advances when a track ends or someone presses Next tune. Music plays through a media element so a phone's silent switch and volume keys reach it; effects stay synthesized. Mute and volume for both channels persist in `localStorage`, so the music toggle in the landing page top bar (same position as a room's `♫ AUDIO` button) keeps music off through the page load into a room. `LANDING_URL=http://127.0.0.1:5173/ npx tsx scripts/landing-music-smoke.ts` drives that flow in a real browser against `npx vite` and writes `artifacts/landing-music.png`; set `CHROMIUM_PATH` to use a specific Chromium build.

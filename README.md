...
Solo and online arenas turn 90° locally when that makes the complete board larger. Avatars, pickups and labels stay upright; scenery, trails and collision footprints turn together. Portrait phones, tablets and narrow windows use the compact MENU layout with three equal, invisible touch zones covering the full screen: left, bomb (hold to charge, release to fire), and right. Brief hints appear at the bottom; MENU stays accessible above the zones. Landscape phones also use full-screen thirds. Opening the menu or changing orientation cancels held input. Other players are unchanged; shared-TV controllers use the separate layout described below. Run `pnpm exec tsx scripts/portrait-arena-browser.ts` (also with `BROWSER=webkit`) for renderer and offline solo checks. To try it, run `pnpm exec vite --host 0.0.0.0`, open `http://localhost:5173/?solo=1&mute` (or your computer’s LAN address on a phone), play in portrait, then turn the device during play.

...
Online rooms also offer opt-in **VOICE** chat: join with your microphone or listen only, mute/deafen, choose a microphone, and adjust voice volume or silence a participant under **SETTINGS → VOICE CHAT**. Voice starts off on every device and uses the existing peer mesh with no media server. For shared TVs use one voice device or headphones. See [voice controls, design and verification](docs/online/VOICE-CHAT.md).

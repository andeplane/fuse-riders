...
| `POST /api/friends/sync`                 | `Authorization: Bearer <ID token>`                                           | Body `{ name?, avatarId?, room?: { code, memberId, gameId? } }`: where this device is. A room claim is proven when it changes: the seat must be a live member of that room (403 otherwise), which only a device in the room can know. Answers with the caller's public id, friends (online, and their room), requests both ways, pending invites and the signed-in members of the reported room. Presence is rewritten only when it changed or is a minute old. 400 per account per hour |

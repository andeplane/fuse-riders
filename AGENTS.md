...
- `src/engine/` owns deterministic rules and simulation and must not import app, net or render code; its few remaining imports of `src/shared/` are pinned exactly in `tests/fixtures/layer-allowlist.json` and shrink in #254 (`src/shared/` keeps what is not simulation: wire types, avatars, ids, the game id; the rider-name rule is `src/engine/rider-name.ts` because the log and the checkpoint guard apply it); `src/online/` owns the app and Fuse Riders' side of the netcode (`fuse-game.ts`: the game behind `RollbackGame`; `room-runtime.ts`: the rider's controls); `packages/` holds the game-agnostic libraries (`fuse-netcode`: input log, rollback, snapshots, clock and room runtime behind the `RollbackGame` contract; `fuse-network-fe`: WebRTC mesh and room client, `fuse-network-be`: room signalling service, one game per room, `fuse-network-protocol`: their shared wire contract, `fuse-platform`: accounts, match history, Elo, ratings and leaderboards for every game, keyed by `gameId`; `fuse-ui`: shared menus, components and neon CSS tokens) and must not import from `src/` or `games/`; `games/` holds the other games (`games/dice`, Pig, the template `scripts/new-game.ts` copies), which may import any package but never `src/` or another game; `src/service/` is the one deployed entry composing `fuse-network-be` and `fuse-platform` with every game's registration (`src/service/history.ts`; Cloud Run serves games beyond Fuse Riders only when `EXTRA_GAME_IDS` names them) (Cloud Run in production, `src/service/dev.ts` in-memory locally and in CI); `src/render/` owns presentation and imports only the engine's view contract (`src/engine/view.ts` and `view-kit.ts`; rule values it needs travel as data in the `WorldView`, see `docs/design/render-boundary.md`); `src/client/` owns audio, controls and the rest of the app-side UI.

...
`scripts/ci-local.sh` is the local mirror of CI (`PORT=<port> ONLY=core,<steps> scripts/ci-local.sh`); see README for the step names. See README for Chrome/WebKit LAN and online smoke commands and browser installation. Coverage is enforced by `.c8rc.json` over its named modules; retain thresholds and report excluded surfaces. Excluded today: `src/online/runtime.ts` (imported only as a type by `tests/local-runtime.test.ts`), the browser/DOM surfaces `src/online/peer-transport.ts`, `src/online/ui.ts`, `src/online/room-settings-menu.ts`, `src/online/attract.ts`, `src/online/mobile-play-layout.ts` and `src/client/main.ts`, and the Phaser/canvas renderers `src/client/phaser/arena.ts`, `phaser/presentation.ts`, `phaser/benchmark-fixture.ts`, `src/client/avatar-heads.ts`, `game-audio.ts`, `ink-renderer.ts`, `pickup-renderer.ts`, `target-renderer.ts`, `themes.ts` and `viewport-lock.ts` — none has a unit test; they are covered by browser smokes instead. Browser/Worker integration tests and physical-device checks complement coverage rather than being inferred from it. Do not claim a test passed unless its completed output was inspected for the relevant revision.

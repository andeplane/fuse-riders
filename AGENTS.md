...
- A change to engine behaviour must bump `RULES` in `src/shared/apply-tick.ts` and refresh the golden with `npx tsx scripts/update-golden-hashes.ts --record` (about 30 s) in the same commit. `tests/golden-hash.test.ts` failing without an intended behaviour change is a regression to fix, not a golden to refresh. See "When the golden fails" in `docs/design/engine-safety-net.md`.

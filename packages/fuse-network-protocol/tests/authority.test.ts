...
test("clock stops after suspension or stale samples, resuming only after fresh sample", () => {
  ...
  clock.synchronize(0, 1100);
  assert.equal(clock.permits(grant), true);
  local = STALL_MS - 100;
  assert.equal(clock.permits(grant), true, "a busy frame is not a suspension");
  local = 2 * STALL_MS;
  assert.equal(clock.permits(grant), false);
  assert.equal(clock.permits(grant), false);
  clock.synchronize(local, 1100 + local);
  assert.equal(clock.permits(grant), true);
  for (let i = 0; i < 40; i++) {
    local += 100;
    assert.equal(clock.permits(grant), true);
...
test("clock rejects bad samples and conservative intervals crossing lease boundaries", () => {
  let now = 600;
  const clock = new AuthorityClock(() => now);
  ...
  assert.notEqual(
    clock.interval(),
    undefined,
    "a slow probe widens the interval instead of refusing it",
  );
...
test("grant boundary validation rejects malformed values and exhausted epochs", () => {
  ...
  for (const value of [
    ...
    assert.equal(isAuthorityGrant(value), false);
  assert.throws(
    () => reserveAuthority(grant, "other", "h", "g", 1),
    /incarnation/,
  );
  assert.throws(() => reserveAuthority(undefined, "r", "", "g", 1));
  assert.throws(() =>
    reserveAuthority(
      { ...grant, epoch: Number.MAX_SAFE_INTEGER },
      "r",
      "h",
      "g",
      1,
  ...
  assert.equal(
    renewAuthority(grant, { ...grant, holder: "other" }, 1),
    undefined,
  );

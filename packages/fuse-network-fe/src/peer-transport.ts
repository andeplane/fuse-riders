...
import { AuthorityClock, CLOSE_AUTHORITY_REPLACED, ROOM_PROTOCOL_VERSION, isAuthorityGrant, type AuthorityGrant } from 'fuse-network-protocol';
...
export class PeerTransport implements RoomTransport {
  ...
  constructor(readonly code:string,readonly token:string,private callbacks:TransportCallbacks,options:PeerTransportOptions){
    this.apiUrl=options.apiUrl;this.maxFastBytes=options.maxFastBytes??DEFAULT_MAX_FAST_BYTES;this.relayOnly=options.disableDirect===true;this.copy={...DEFAULT_TRANSPORT_COPY,...options.copy};
  }
  ...
  connect():void {
    ...
    const url=new URL(this.apiUrl(`/api/rooms/${this.code}/ws`));url.protocol=url.protocol==='https:'?'wss:':'ws:';url.searchParams.set('token',this.token);
    ...
    ws.onmessage=async event=>{
      ...
      try {
        ...
        if(message.type==='welcome'){
          if(message.protocol!==ROOM_PROTOCOL_VERSION||typeof message.connectionId!=='string'){this.terminate(this.copy.protocolChanged);this.close();return;}
          this.received.clear();
          ...
          await this.ice.load(signal=>fetch(this.apiUrl(`/api/rooms/${this.code}/ice?token=${this.token}`),{signal}).then(response=>response.json()),AbortSignal.timeout(ICE_FETCH_TIMEOUT_MS));
          if(ws!==this.socket)return;
          ...
          if(this.id===this.hostId&&this.grant&&this.grant.holder!==this.connectionId&&this.authorityClock.permits(this.grant)){ws.close(CLOSE_AUTHORITY_REPLACED,'Authority held elsewhere');return;}
    ...
    ws.onclose=event=>{
      if(ws!==this.socket)return;
      handleRoomSocketClose(event.code,{stopped:()=>this.stopped,stop:()=>this.close(),revoked:()=>this.callbacks.revoked(),ended:()=>this.callbacks.ended(),status:this.callbacks.status,terminated:()=>this.terminate(this.copy.roomEnded),retry:()=>{this.retry=setTimeout(()=>this.connect(),1500);}});
    };
    ws.onerror=()=>ws.close();
  }
  ...
  private channel(id:string,link:Link,channel:RTCDataChannel):void {
    ...
    if(channel.label==='input'){
      ...
      channel.onmessage=event=>{if(!current())return;const data=event.data;if(!(data instanceof ArrayBuffer)||data.byteLength>this.maxFastBytes||data.byteLength===0)return;this.callbacks.fast(id,new Uint8Array(data));};
  ...
  sendFast(id:string,bytes:Uint8Array):boolean {
    if(this.stopped||!this.connections.has(id)||bytes.byteLength>this.maxFastBytes)return false;
  ...
  explain(id:string):string {
    ...
    const link=this.links.get(id);
    if(!link)return this.connections.has(id)?(this.initiator(id)?'offer not sent yet — waiting for the room service':'no offer received yet — signalling has not delivered the offer'):this.copy.hostAbsent;

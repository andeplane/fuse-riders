...
export type CandidateType =
  "host" | "srflx" | "prflx" | "relay" | "end" | "unknown";
export function candidateType(
...
export function sdpUfrags(sdp: string | undefined): string[] {
  return [...(sdp ?? "").matchAll(/^a=ice-ufrag:(\S+)/gm)].map((m) => m[1]!);
}
export function sdpFingerprint(sdp: string | undefined): string | undefined {
...
export function sameCertificate(
  previousSdp: string | undefined,
  nextSdp: string,
): boolean {
...
export function ufragMatches(
  candidate: { usernameFragment?: string | null; candidate?: string | null },
  sdp: string,
): boolean {
  const ufrag =
    candidate.usernameFragment ??
    /\sufrag\s+(\S+)/.exec(candidate.candidate ?? "")?.[1];

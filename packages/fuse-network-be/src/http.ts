...
export interface HttpExtension {
  ...
  handle(req: IncomingMessage, res: ServerResponse, clientAddress: string): Promise<boolean>;
}
...
export function createRoomServer(options: RoomHttpOptions): RoomServer {
  ...
  const server = createServer(async (req, res) => {
    ...
    if (req.method === 'OPTIONS') { res.setHeader('Access-Control-Allow-Methods', ['GET', 'POST', 'OPTIONS', ...(options.extension?.methods ?? [])].join(', ')); res.setHeader('Access-Control-Allow-Headers', ['Content-Type', 'Authorization', ...(options.extension?.headers ?? [])].join(', ')); res.writeHead(204); res.end(); return; }
    ...
    try {
      ...
      if (route?.[2] === 'end' && req.method === 'POST') { await store.end(route[1]!, req.headers.authorization?.replace(/^Bearer /, '') ?? ''); json({ ok: true }); return; }
      if (route?.[2] === 'ice') {
      ...
      if (await options.extension?.handle(req, res, options.clientAddress(req))) return;

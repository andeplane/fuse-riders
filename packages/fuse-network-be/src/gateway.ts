...
export class RoomGateway {
  ...
  async connect(code:string,token:string,socket:GatewaySocket):Promise<string>{
    return this.serial(async()=>{
      ...
      this.send(client,{type:'welcome',protocol:ROOM_PROTOCOL_VERSION,id:member.id,hostId:room.hostId,connectionId:member.connectionId,peers:Object.values(room.members).filter(p=>p.connectionId!==member.connectionId&&p.expiresAt>this.deps.now()).map(p=>({id:p.id,connectionId:p.connectionId})),grant:room.grant});
  ...
  private async handle(client:Client,raw:string):Promise<void>{
    ...
    if(!target||target.expiresAt<=this.deps.now()||(m.targetConnectionId!==undefined&&m.targetConnectionId!==target.connectionId)||target.id===client.member.id||current.members[client.member.id]?.connectionId!==client.member.connectionId)return;
  ...
  async deliver(message:RoutedMessage):Promise<void>{
    ...
    if(room.expiresAt<=this.deps.now()||room.incarnation!==message.incarnation||room.members[message.from.id]?.connectionId!==message.from.connectionId||room.members[message.to.id]?.connectionId!==message.to.connectionId||message.from.id===message.to.id)return;

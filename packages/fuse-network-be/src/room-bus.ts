...
export interface RoutedMessage {
  ...
  to: Member;
  expiresAt: number;
  wire: { type: "signal"; from: string; connectionId: string; data: unknown };
}
...
export function parseRoutedMessage(raw: unknown): RoutedMessage | undefined {
  ...
  if (
    !["id", "code", "incarnation", "destination"].every(
      (k) => typeof m[k] === "string" && m[k].length > 0 && m[k].length <= 128,
    ) ||
  ...
  const member = (v: unknown): v is Member => {
    ...
    return (
      ["id", "connectionId", "gatewayId"].every(
        (k) =>
  ...
  if (
    !member(m.from) ||
    !member(m.to) ||
    m.to.gatewayId !== m.destination ||
    !m.wire ||
    typeof m.wire !== "object"
  )
    return;
  ...
  if (
    w.type !== "signal" ||
    w.from !== m.from.id ||
    w.connectionId !== m.from.connectionId
  )
    return;

...
export class PubSubRoomBus implements RoomBus {
  ...
  constructor(
    ...
    this.topic = client.topic(topic, {
      messageOrdering: true,
      gaxOpts: { timeout: 3000, retry: null },
      batching: { maxMilliseconds: 2, maxMessages: 20, maxBytes: 128_000 },
    });
  }
  async start(
    ...
    const [subscription] = await this.topic.createSubscription(name, {
      ...
      filter: `attributes.destination = "${this.gatewayId}"`,
    ...
    subscription.on("error", (error) => {
      if (generation === this.generation && !this.stopping) failed(error);
    });
    subscription.on("message", (message: Message) => {
      ...
      const work = previous
        .then(async () => {
          if (message.data.byteLength > BUS_FRAME_MAX_BYTES) {
            message.ack();
            return;
          }
          let decoded: unknown;
        ...
        .catch((error) => {
          ...
          if (!this.stopping)
            failed(
              error instanceof Error ? error : new Error("Bus delivery failed"),
            );
        });
      this.receiving.set(key, work);
      void work.finally(() => {
        if (this.receiving.get(key) === work) this.receiving.delete(key);
      });
    });
  ...
  async publish(message: RoutedMessage): Promise<void> {
    if (!this.subscription || this.stopping)
      throw new Error("Room bus is not ready");
    const generation = this.generation;
    const data = Buffer.from(JSON.stringify(message)),
      key = `${message.incarnation}:${message.from.connectionId}:${message.to.connectionId}`;
    if (
      data.byteLength > BUS_FRAME_MAX_BYTES ||
      this.pendingBytes + data.byteLength > 2_000_000
    )
      throw new Error("Room bus backpressure limit");
    this.pendingBytes += data.byteLength;
    const previous = this.edges.get(key) ?? Promise.resolve();
    const work = previous.then(async () => {
      if (this.stopping || generation !== this.generation)
        throw new Error("Signal generation closed");
      await this.topic.publishMessage({
        data,
        attributes: { destination: message.destination },
        orderingKey: key,
      });
    });
    ...
    try {
      ...
      if (generation === this.generation) this.pendingBytes -= data.byteLength;
      if (this.edges.get(key) === work) this.edges.delete(key);
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    await subscription.delete({ timeout: 5000, retry: null }).catch((error) => {

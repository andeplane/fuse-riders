...
export interface GcpRoomServiceOptions {
  httpExtension?: (context: { store: RoomStore; firestore: Firestore; prefix: string; projectId: string }) => HttpExtension;
...
export function startGcpRoomService(options:GcpRoomServiceOptions):Server {
  ...
  const server=createRoomServer({store,gateway,extension:options.httpExtension?.({store,firestore,prefix,projectId}),allowOrigin:origin=>origins.has(origin),

...
export class FirestoreRoomDatabase implements RoomDatabase {
  ...
  constructor(
    ...
    this.maxGuests = roomGuestLimit(maxGuests);
  }
  private room(code: string) {
    return this.firestore.collection(`${this.prefix}-rooms`).doc(code);
  }
  private parse(value: unknown): RoomRecord | undefined {
    if (value === undefined) return;
    const room = parseRoomRecord(value, this.maxGuests);
    if (!room) throw new Error("Stored room schema is incompatible");
    return room;
  }
  async read(code: string): Promise<RoomRecord | undefined> {
    return this.parse((await this.room(code).get()).data());
  }
  async transact<T>(
    code: string,
    ...
    return this.firestore.runTransaction(
      async (transaction) => {
        const snapshot = await transaction.get(ref),
          next = operation(this.parse(snapshot.data()));
        if (next.room)
          transaction.set(ref, {
            ...next.room,
            cleanupAt: Timestamp.fromMillis(next.room.expiresAt),
          });
        return next.result;
      },
      { maxAttempts: 5 },
  ...
  watch(
    ...
    return this.room(code).onSnapshot((snapshot) => {
      try {
        listener(this.parse(snapshot.data()));
      } catch (error) {
        failed(
          error instanceof Error ? error : new Error("Room metadata failure"),
        );
      }
    }, failed);
  }
    ...
    return this.firestore.runTransaction(
      async (transaction) => {
        ...
        transaction.set(ref, {
          hour,
          count,
          cleanupAt: Timestamp.fromMillis(now + 3600000),
        });
        return true;
      },
      { maxAttempts: 5 },

...
export class MemoryRoomDatabase implements RoomDatabase {
    ...
    const work = this.chain.then(() => {
      ...
      if (next.room) {
        ...
        for (const watcher of [...(this.listeners.get(code) ?? [])]) {
          try {
            watcher.listener(structuredClone(next.room));
          } catch (error) {
            watcher.failed?.(
              error instanceof Error
                ? error
                : new Error("Room watcher failure"),
            );
          }
        }
      }
  ...
  watch(
    ...
    const listeners = this.listeners.get(code) ?? new Set(),
      watcher: Watcher = { listener, ...(failed ? { failed } : {}) };
    listeners.add(watcher);
    this.listeners.set(code, listeners);
    return () => {
      listeners.delete(watcher);
      if (listeners.size === 0 && this.listeners.get(code) === listeners)
  ...
  async allowance(key: string, now: number, limit: number): Promise<boolean> {
    ...
    if (!previous && this.allowances.size >= MAX_ALLOWANCE_KEYS) {
      for (const [stale, window] of this.allowances)
        if (window.hour !== hour) this.allowances.delete(stale);
...
export class LocalRoomBus implements RoomBus {
  ...
  async publish(message: RoutedMessage): Promise<void> {
    throw new Error(
      `Local room service has a single gateway; cannot route to ${message.destination}`,
    );
  }

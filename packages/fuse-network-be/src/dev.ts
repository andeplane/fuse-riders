...
import { createRoomServer, type RoomServer, type HttpExtension } from './http.js';
...
export function createDevRoomService(options: DevRoomServiceOptions = {}): DevRoomService {
  ...
  const store = new RoomStore(new MemoryRoomDatabase(), { now, id: randomUUID, maxGuests: options.maxGuests, fullMessage: options.fullMessage });
  ...
  const server = createRoomServer({
    store, gateway, now, extension: options.httpExtension?.(store),
...
export function runDevRoomService(options: DevRoomServiceOptions & { argv?: readonly string[] } = {}): DevRoomService {
  const argv = options.argv ?? process.argv;
  const argument = (name: string) => { const index = argv.indexOf(`--${name}`); return index > 0 ? argv[index + 1] : undefined; };
  ...
  const service = createDevRoomService({ ...options, ...(staticDirectory ? { staticDirectory } : {}) });
  service.server.listen(port, host, () => console.log(`Local room service: http://${host === '127.0.0.1' ? 'localhost' : host}:${port}/ (in-memory rooms${staticDirectory ? `, serving ${staticDirectory}` : ''})`));

...
import { MemoryRoomDatabase, RoomError, RoomStore, parseRoomRecord } from '../src/index.js';
...
test('configured rooms round-trip through stored metadata validation and retain a capacity bound',async()=>{
  for(const maxGuests of [0,2,5,6,10]){
    let ids=0;const store=new RoomStore(new MemoryRoomDatabase(),{now:()=>1000,id:()=>`id-${++ids}`,maxGuests});
    ...
    assert.deepEqual(parseRoomRecord(JSON.parse(JSON.stringify(room)),maxGuests),room);
    if(maxGuests>0)assert.equal(parseRoomRecord(room,maxGuests-1),undefined,'a smaller configured bound rejects the record');
    if(maxGuests>5)assert.equal(parseRoomRecord(room),undefined,'the default bound stays unchanged');
    await assert.rejects(store.admit(code,token(99),'gateway'),error=>error instanceof RoomError&&error.status===429);
    assert.deepEqual(parseRoomRecord(await store.time(code,room.members[room.hostId]!,room.grant),maxGuests),await store.get(code));
  }
});
test('invalid capacity configuration fails before accepting members or parsing metadata',()=>{
  for(const maxGuests of [-1,0.5,NaN,Infinity,Number.MAX_SAFE_INTEGER]){
    assert.throws(()=>new RoomStore(new MemoryRoomDatabase(),{now:()=>1000,id:()=> 'id',maxGuests}),RangeError);

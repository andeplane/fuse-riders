...
import { validSignal } from "../src/signal.js";
const line =
...
test("every candidate shape Chrome and Safari emit passes the signalling boundary", () => {
  ...
  for (const candidate of shapes)
    assert.equal(validSignal({ candidate }), true, JSON.stringify(candidate));
});

test("descriptions from localDescription.toJSON() pass; anything else is rejected", () => {
  assert.equal(
    validSignal({ description: { type: "offer", sdp: "v=0" } }),
    true,
  );
  assert.equal(
  ...
  for (const raw of [
    ...
    { description: { type: "offer", sdp: "v".repeat(30_001) } },
    { description: { type: "offer", sdp: "v=0", extra: 1 } },
    { candidate: null },
    { candidate: {} },
    { candidate: { candidate: 5 } },
    { candidate: { candidate: line, foo: 1 } },
    { candidate: { candidate: "c".repeat(2049) } },
    { candidate: { candidate: line, sdpMLineIndex: -1 } },
    { candidate: { candidate: line, sdpMLineIndex: 1.5 } },
    { candidate: { candidate: line, usernameFragment: 7 } },
    {

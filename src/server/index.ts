...
function proxyHeaders(
  ...
  for (const [key, value] of Object.entries(headers))
    if (
      value !== undefined &&
      !/^(cf-connecting-ip|x-real-ip|x-forwarded-.*)$/.test(key)
    )
      forwarded[key] = value;
...
function parseRange(
  ...
  const end = Math.min(
    match[1] && match[2] ? Number(match[2]) : size - 1,
    size - 1,
  );
...
function proxyRequest(
  ...
  const upstream = http.request(
    {
      ...
      method: req.method,
      path: req.url,
      headers: proxyHeaders(target, req.headers),
    },
    (upstreamResponse) => {
      res.writeHead(
  ...
  upstream.on("error", () => {
    ...
    res.end(
      JSON.stringify({
        error:
          "Room service unavailable: start it with npm run dev:online or set ROOM_API",
      }),
    );
  });
  req.pipe(upstream);
...
function proxyUpgrade(
  ...
  upstream.on("response", (upstreamResponse) => {
    socket.end(
      `HTTP/1.1 ${upstreamResponse.statusCode ?? 502} ${upstreamResponse.statusMessage ?? ""}\r\nConnection: close\r\n\r\n`,
    );
  });
...
export function controllerSnapshot(state: GameSnapshot): GameSnapshot {
  ...
  return {
    ...rest,
    players: state.players.map((player) => ({ ...player, trail: [] })),
    bombs: [],
    blasts: [],
    pickups: [],
    portalPairs: [],
...
export function lanAddress() {
  ...
  return (
    process.env.HOST_IP ||
    ips.find((x) => /^192\.168\./.test(x)) ||
    ips.find((x) => /^10\./.test(x)) ||
    ips.find((x) => /^172\.(1[6-9]|2\d|3[01])\./.test(x)) ||
    "127.0.0.1"
  );
}
export function catchUpSteps(elapsed: number) {
...
export async function createGameServer(options: ServerOptions = {}) {
  ...
  const bots = new Set<string>(),
    botController = new BotController({ random: dependencies.botRandom });
  ...
  if (roomApi && roomApi.protocol !== "http:")
    throw new Error(`ROOM_API must be an http:// URL, got ${options.roomApi}`);
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "POST" && req.url?.split("?")[0] === "/telemetry") {
      ...
      req.on("end", async () => {
        try {
          const { device, events } = JSON.parse(
          ...
          const room =
              String(device?.room ?? "none").replace(/[^A-Za-z0-9_-]/g, "") ||
              "none",
            dir = path.join(ROOT, "artifacts", "telemetry");
          await mkdir(dir, { recursive: true });
          await appendFile(
            path.join(dir, `${room}.ndjson`),
            events
              .map((event) =>
                JSON.stringify({ ...event, device, received: Date.now() }),
              )
              .join("\n") + "\n",
          );
          res.writeHead(204);
    ...
    if (req.url?.split("?")[0] === "/api/config") {
      ...
      res.end(JSON.stringify({ controllerUrl }));
      return;
    }
    if (roomApi && req.url?.startsWith("/api/")) {
      proxyRequest(roomApi, req, res);
      return;
    }
    if (vite) {
    ...
    try {
      ...
      const dist = path.resolve(
        options.buildDirectory ?? path.join(ROOT, "dist"),
      );
      const filename = path.resolve(
        dist,
        isPage ? "index.html" : "." + urlPath,
      );
    ...
    const deliver = (client: WebSocket) => {
      ...
      const payload = host
        ...
        : (compact ??= JSON.stringify({
            ...message,
            state: controllerSnapshot(message.state),
          }));
      client.send(payload);
    };
    if (ws) deliver(ws);
    else for (const client of connections.keys()) deliver(client);
  }
  function error(ws: WebSocket, code: ErrorCode) {
  ...
  function clearInputs() {
    for (const seat of seats.values()) neutral(seat);
  }
  ...
  function connectedCount() {
    return (
      [...seats.values()].filter((s) => s.socket && !s.leaving).length +
      bots.size
    );
  }
  function hostAction(
    ws: WebSocket,
    action: "start" | "nextRound" | "rematch" | "lobby",
  ) {
    if (action === "lobby") {
    ...
    if (game.phase !== required[action]) {
      error(ws, "invalid_phase");
      return;
    }
  ...
  wss.on("connection", (ws, req) => {
    ...
    ws.on("message", (data, binary) => {
      ...
      if (message.type === "ping") {
        send(ws, { type: "pong", id: message.id, sentAt: message.sentAt });
        return;
      }
      if (message.type === "hostAuth") {
        if (
          c.seat ||
          message.token.length !== hostToken.length ||
          !timingSafeEqual(Buffer.from(message.token), Buffer.from(hostToken))
        ) {
          error(ws, "unauthorized");
          return;
        }
        c.host = true;
        send(ws, { type: "hostAuthenticated" });
        snapshot(ws);
        return;
      }
      if (message.type === "hostBot") {
        ...
        if (message.action === "add") {
          if (game.players.size >= 5 || game.leaderboard.size >= 128) {
            error(ws, "full");
            return;
          }
          const slot = COLORS.findIndex(
            (_, slot) =>
          ...
          let number = 1;
          while (game.leaderboard.has(`${BOT_ID_PREFIX}${number}`)) number++;
          const id = `${BOT_ID_PREFIX}${number}`,
            difficulty = rollBotDifficulty(
              dependencies.botRandom(game.seed, id, game.tick),
            );
          addPlayer(game, {
            id,
            name: botDisplayName(BOT_NAMES[slot]!, difficulty),
            slot,
            color: COLORS[slot]!,
            avatarId: "robot",
            connected: true,
          ...
          if (!message.id || !bots.has(message.id)) {
            error(ws, "invalid_message");
            return;
          }
          removePlayer(game, message.id);
          bots.delete(message.id);
        }
        snapshot();
        return;
      }
      ...
      if (message.type === "join") {
        ...
        if (!rate || now - rate.since > 60_000) {
          ...
          joins.set(address, rate);
        }
        // Reconnects are authorized by their token and must survive brief Wi-Fi drops.
        if (!message.playerToken && ++rate.count > 5) {
          error(ws, "full");
          return;
        }
        let seat: Seat | undefined;
        if (message.playerToken) {
          seat = [...seats.values()].find(
            (s) => s.token === message.playerToken && !s.leaving,
          );
          if (!seat) {
            error(ws, "unauthorized");
            return;
          ...
          if (old) {
            ...
            old.close(4001, "Controller replaced");
          }
          neutral(seat, true);
        } else {
          ...
          const slot = COLORS.findIndex(
            (_, i) => ![...game.players.values()].some((s) => s.slot === i),
          );
          seat = {
        ...
        c.seat = seat;
        seat.socket = ws;
        setPlayerConnected(game, seat.id, true);
        send(ws, {
          type: "joined",
          playerId: seat.id,
          playerToken: seat.token,
          slot: seat.slot,
          color: COLORS[seat.slot],
          nextInputSeq: seat.seq + 1,
        });
        snapshot();
        return;
      }
      const seat = c.seat;
      if (!seat || seat.socket !== ws || seat.leaving) {
        error(ws, "unauthorized");
        return;
      }
      ...
      if (message.type === "input") {
        if (message.seq <= seat.seq) {
          error(ws, "stale");
          return;
        }
        seat.seq = message.seq;
        seat.inputTick = game.tick;
  ...
  let accumulator = 0;
  const stopLoop = options.manualTicks
    ? undefined
    : dependencies.schedule(() => {
        const now = dependencies.now();
        accumulator += (now - previousTime) * simulationTimeScale(game, bots);
        previousTime = now;
        const ticks = catchUpSteps(accumulator);
        if (ticks) {
          advance(ticks);
          accumulator = accumulator >= 300 ? 0 : accumulator - ticks * 50;
        }
      }, 10);
  ...
  return {
    ...
    hostToken,
    controllerUrl,
    hostUrl: `http://${ip}:${port}/display#${hostToken}`,
    advance,
    checkConnections,
    async close() {
      stopLoop?.();
...
if (
  ...
  try {
    app = await createGameServer({ dev, ...(roomApi ? { roomApi } : {}) });
  } catch (error) {

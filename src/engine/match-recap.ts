...
export function countList(
  entries: ReadonlyArray<readonly [label: string, count: number]>,
): string {
...
export function recapSignature(
  ...
  const figures = orderedStats(stats)
    .map((entry) =>
      [
        entry.playerId,
        ...
        entry.bombsExploded,
        entry.eliminations,
        entry.pickupsCollected,
        entry.invulnerableTicks,
        entry.wallBounces,
        entry.earlyExits,
        entry.powerPickups,
        entry.starPickups,
        ...
        entry.deathsByCause.wall,
        entry.deathsByCause.trail,
        entry.deathsByCause.explosion,
        entry.deathsByCause.rider,
      ].join(":"),
    )
    .join("|");
  return moments.length
    ? `${figures}#${moments.map((moment) => `${moment.kind}:${moment.round}:${moment.tick}:${moment.playerId}:${moment.value}`).join(",")}`
    : figures;
}
...
export function highlightScore(moment: Moment): number {
  switch (moment.kind) {
    case "multiKill":
      return moment.value >= 3 ? 100 : 60;
    case "directHit":
      return 70;
    case "boxedIn":
      return 50;
    case "trickShot":
      return 45 + 5 * Math.min(moment.value, 3);
    case "bombDodge":
      return 35;
    case "cutOff":
      return 30 + 2 * Math.max(0, CUT_OFF_MAX_AGE_TICKS - moment.value);
...
const HIGHLIGHT_COPY: Record<
  ...
  {
    icon: string;
    title: (moment: Moment) => string;
    copy: (name: string, targets: readonly string[], moment: Moment) => string;
  }
> = {
  multiKill: {
    icon: "✹",
    title: (moment) =>
      moment.value >= 4
        ...
        : moment.value === 3
          ? "TRIPLE TAP"
          : "DOUBLE TAP",
    copy: (name, targets) => `${name} TOOK OUT ${list(targets)} AT ONCE`,
  },
  directHit: {
    icon: "◎",
    title: () => "BULLSEYE",
    copy: (name, targets) => `${name} BOMBED ${list(targets)} ON THE HEAD`,
  },
  trickShot: {
    icon: "↯",
    title: () => "BANK SHOT",
    copy: (name, targets, moment) =>
      `${name} BANKED ${plural(moment.value, "BOUNCE")} INTO ${list(targets)}`,
  },
  cutOff: {
    icon: "⟋",
    title: () => "CUT OFF",
    copy: (name, targets, moment) =>
      `${name} CUT OFF ${list(targets)} · TRAIL ${(moment.value / 20).toFixed(1)}s OLD`,
  },
  boxedIn: {
    icon: "▣",
    title: () => "BOXED IN",
    copy: (name, targets) =>
      `${list(targets)} HAD NOWHERE LEFT TO GO · ${name}'S TRAIL`,
  },
  bombDodge: {
    icon: "✧",
    title: () => "OUT OF THE FIRE",
    copy: (name, targets, moment) =>
      `${name} LEFT ${list(targets)}'S BLAST ZONE ${Math.max(0, moment.value)}u CLEAR`,
  },
  ownGoal: {
    icon: "☹",
    title: () => "OWN GOAL",
    copy: (name) => `${name} BOOMED THEMSELVES`,
  },
  mutualDestruction: {
    icon: "✖",
    title: () => "EVERYBODY DIES",
    copy: (name, targets, moment) =>
      `${list([name, ...targets])} · ${plural(moment.value, "RIDER")}, ONE TICK`,
  },
};
...
export function describeMoment(
  ...
  return {
    title: text.title(moment),
    icon: text.icon,
    copy: text.copy(name(moment.playerId), moment.targetIds.map(name), moment),
    when: `ROUND ${moment.round} · ${clockText(moment.elapsed)}`,
  };
}
...
export function rankMoments(
  moments: ReadonlyArray<Moment>,
): { moment: Moment; score: number }[] {
  return moments
    .map((moment) => ({ moment, score: highlightScore(moment) }))
    .sort(
      (a, b) =>
        b.score - a.score ||
        a.moment.round - b.moment.round ||
        a.moment.tick - b.moment.tick ||
        MOMENT_KINDS.indexOf(a.moment.kind) -
          MOMENT_KINDS.indexOf(b.moment.kind),
    );
}
...
export function matchHighlights(
  ...
  for (const { moment, score } of ranked) {
    ...
    if (
      (perKind.get(moment.kind) ?? 0) >= HIGHLIGHT_VARIETY ||
      (perRider.get(moment.playerId) ?? 0) >= HIGHLIGHT_VARIETY
    )
      continue;
    ...
    reel.push({
      ...
      round: moment.round,
      tick: moment.tick,
      ...describeMoment(moment, name),
      playerId: moment.playerId,
      name: name(moment.playerId),
      color: byId.get(moment.playerId)?.color ?? "#ffffff",
      score,
    });
  }
  return reel;
...
export function podiumOrder(
  ...
  const placed = orderedStats(stats).filter(
    (entry) =>
      entry.matchPlacement >= 1 && entry.matchPlacement <= PODIUM_PLACES,
  );
  ...
  return [
    ...runners.slice(0, centerAt),
    ...champions,
    ...runners.slice(centerAt),
  ].map((entry) => ({
    ...
    placeLabel:
      entry.matchPlacement === 1 ? "♛  #1" : `#${entry.matchPlacement}`,
...
export const AWARD_DEFINITIONS: ReadonlyArray<AwardDefinition> = [
  {
    ...
    icon: "✹",
    value: (entry) => entry.bombsExploded,
    detail: (value) => `${value} ${value === 1 ? "BOMB" : "BOMBS"} BOOMED`,
  },
  {
    id: "trailblazer",
    title: "TRAILBLAZER",
    icon: "⌁",
    value: (entry) => entry.distanceUnits,
    detail: (value) => `${distanceText(value)} TRAVELLED`,
  },
  {
    id: "untouchable",
    title: "UNTOUCHABLE",
    icon: "✦",
    value: (entry) => entry.survivalTicks,
    detail: (value) => `${durationText(value)} ALIVE`,
  },
  {
    id: "collector",
    title: "COLLECTOR",
    icon: "◆",
    value: (entry) => entry.pickupsCollected,
    detail: (value) => `${value} ${value === 1 ? "POWER-UP" : "POWER-UPS"}`,
  },
  {
    id: "headhunter",
    title: "HEADHUNTER",
    icon: "☠",
    value: (entry) => entry.eliminations,
    detail: (value) => `${value} ${value === 1 ? "RIDER" : "RIDERS"} TAKEN OUT`,
  },
  {
    id: "lastStand",
    title: "LAST STAND",
    icon: "⏱",
    value: (entry) => entry.longestSurvivalTicks,
    detail: (value) => `${durationText(value)} BEST ROUND`,
  },
  {
    id: "gateCrasher",
    title: "GATE CRASHER",
    icon: "◎",
    value: (entry) => entry.portalTransits,
    detail: (value) => `${value} PORTAL ${value === 1 ? "JUMP" : "JUMPS"}`,
  },
  {
    id: "wallRider",
    title: "WALL RIDER",
    icon: "⟁",
    value: (entry) => entry.wallBounces,
    detail: (value) => `${value} WALL ${value === 1 ? "BOUNCE" : "BOUNCES"}`,
  },
];
...
export function matchAwards(
  ...
  for (const definition of AWARD_DEFINITIONS) {
    ...
    const winners = ordered
      .filter((entry) => definition.value(entry) === best)
      .map((entry) => ({
        playerId: entry.playerId,
        name: entry.name,
        color: entry.color,
...
export type ComparisonColumnKey = Exclude<
  keyof ComparisonRow,
  "playerId" | "name" | "color" | "placement" | "riderLabel" | "riderNote"
>;
...
export function comparisonRows(
  ...
  return orderedStats(stats).map((entry) => {
    ...
    return {
      ...
      pickups: entry.pickupsCollected
        ? `${entry.pickupsCollected} · ${countList([
            ["power", entry.powerPickups],
            ["star", entry.starPickups],
            ["beer", entry.beerPickups],
        ...
        : "—",
      star: entry.invulnerableTicks
        ? durationText(entry.invulnerableTicks)
        : "—",
      deaths: countList([
        ["wall", deaths.wall],
        ["trail", deaths.trail],
        ["blast", deaths.explosion],
        ["rider", deaths.rider],
      ]),
    };
  });
...
export function matchTotals(
  ...
  const sum = (pick: (entry: MatchPlayerStats) => number) =>
  ...
  return [
    {
      label: "ROUNDS",
      value: draws > 0 ? `${rounds} · ${draws} DRAWN` : String(rounds),
    },
    { label: "RIDERS", value: String(stats.length) },
    {
      label: "BOMBS",
      value: `${sum((entry) => entry.bombsExploded)}/${sum((entry) => entry.bombsPlaced)}`,
    },
    { label: "KOs", value: String(sum((entry) => entry.eliminations)) },
    { label: "CRASHES", value: String(deaths) },
    {
      label: "DISTANCE",
      value: distanceText(sum((entry) => entry.distanceUnits)),
    },
    {
      label: "POWER-UPS",
      value: String(sum((entry) => entry.pickupsCollected)),
    },
    {
      label: "PORTAL JUMPS",
      value: String(sum((entry) => entry.portalTransits)),
    },
  ];
}
...
export function buildMatchRecap(
  ...
  return {
    signature: recapSignature(stats, moments),
    podium: podiumOrder(stats),
    awards: matchAwards(stats),
    totals: matchTotals(stats),
    comparison: comparisonRows(stats),
    highlights: matchHighlights(stats, moments),
  };
}

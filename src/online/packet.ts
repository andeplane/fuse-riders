...
export interface Packet {
  ...
  sentAt: number; echoSentAt: number; echoHeld: number; clockTick: number; hash: [tick: number, hash: string] | null;
}
export interface Nack { room: number; from: string; firstMissingSeq: number }
...
export function unpackMessage(bytes: Uint8Array): unknown {
  if (bytes.byteLength > MAX_MESSAGE_BYTES) throw new Error('Message too large');
  preflight(bytes);
  return decode(bytes, { maxStrLength: MAX_MESSAGE_BYTES, maxBinLength: 0, maxArrayLength: 4096, maxMapLength: 64, maxExtLength: 0 });
}
function preflight(bytes: Uint8Array): void {
  ...
  const visit = (depth: number): void => {
    if (depth > 32 || ++nodes > 200_000) throw new Error('Message complexity limit');
    ...
    else if ([0xd9, 0xda, 0xdb].includes(tag)) { skip(length(tag === 0xd9 ? 1 : tag === 0xda ? 2 : 4)); return; }
    ...
    const count = array >= 0 ? array : map * 2; capacity += count; if (capacity > 200_000) throw new Error('Allocation limit');
...
export function encodePacket(packet: Packet): Uint8Array {
  if (packet.entries.length > MAX_PACKET_ENTRIES) throw new Error('Too many entries');
  const bytes = packMessage([PACKET_VERSION, packet.room, packet.from, packet.generation, packet.through, packet.lastSeq, packet.entries, packet.sentAt, packet.echoSentAt, packet.echoHeld, packet.clockTick, packet.hash]);
...
export function decodePacket(bytes: Uint8Array): { packet: Packet } | { nack: Nack } | undefined {
  ...
  const [, room, from, generation, through, lastSeq, entries, sentAt, echoSentAt, echoHeld, clockTick, hash] = value;
  if (!uint32(room) || !memberId(from) || !uint32(generation) || !uint32(through) || !uint32(lastSeq) || !Array.isArray(entries) || entries.length > MAX_PACKET_ENTRIES || !entries.every(isEntry)) return;
  if (!uint32(sentAt) || !uint32(echoSentAt) || !uint32(echoHeld) || !finite(clockTick) || clockTick < 0) return;
  if (hash !== null && !(Array.isArray(hash) && hash.length === 2 && uint32(hash[0]) && typeof hash[1] === 'string' && /^[0-9a-f]{16}$/.test(hash[1]))) return;
  return { packet: { room, from, generation, through, lastSeq, entries: entries as Entry[], sentAt, echoSentAt, echoHeld, clockTick, hash: hash as Packet['hash'] } };
}

export function roomHash(incarnation: string): number {
  let hash = 0x811c9dc5;
  for (let index = 0; index < incarnation.length; index++) hash = Math.imul(hash ^ incarnation.charCodeAt(index), 0x01000193) >>> 0;
  return hash;
}
...
export const wrapDelta = (later: number, earlier: number): number => (later - earlier) | 0;

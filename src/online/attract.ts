...
export async function startAttract(initialCanvas: HTMLCanvasElement, toggle: HTMLButtonElement): Promise<()=>void> {
  const world = new World(createRoomState(crypto.randomUUID(), defaultRoomSettings()), 'attract', 'attract');
  const log = world.stream('attract', 1);
  for (let slot = 0; slot < 5; slot++) log.append(1, [BOT, 'add', `bot:${slot + 1}`, `AI ${BOT_NAMES[slot]}`, slot]);
  log.append(2, [ACTION, 'start', crypto.randomUUID()]);
  ...
  const frame=(now:number)=>{
    ...
    if(!document.hidden){
      if(!paused){
        ...
        if(game.phase==='matchOver'&&game.tick>=(game.phaseEndsAtTick??0))log.append(world.tick+1,[ACTION,'rematch',crypto.randomUUID()]);
        world.advance(target);
      }
      if((!paused||dirty||canvas.dataset.renderer!==lastRenderer)&&now-lastDraw>=1000/30){const view=world.view()[0]!;presentation.render(view,now,defaultTheme,sprites,view.matchId);canvas.dataset.attractTick=String(world.tick);lastDraw=now;dirty=false;lastRenderer=canvas.dataset.renderer;}
    }else accumulator=0;

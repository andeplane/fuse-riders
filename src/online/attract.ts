...
export async function startAttract(initialCanvas: HTMLCanvasElement, toggle: HTMLButtonElement): Promise<()=>void> {
  ...
  const frame=(now:number)=>{
    ...
    if(!document.hidden){
      ...
      if((!paused||dirty||canvas.dataset.renderer!==lastRenderer)&&now-lastDraw>=1000/30){const view=world.view()[0]!;presentation.render(view,now,theme,view.matchId);canvas.dataset.attractTick=String(world.tick);lastDraw=now;dirty=false;lastRenderer=canvas.dataset.renderer;}
    }else accumulator=0;

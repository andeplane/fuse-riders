...
export async function startAttract(
  ...
  let paused = motion.matches,
    ...
    raf = 0,
    dirty = true,
    lastRenderer: string | undefined;
  const label = () => {
    toggle.textContent = paused ? "▶ PLAY BACKGROUND" : "Ⅱ PAUSE BACKGROUND";
    ...
    if (!document.hidden) {
      if (!paused) {
        ...
        if (
          ...
          game.tick >= (game.phaseEndsAtTick ?? 0)
        )
          log.append(world.tick + 1, [ACTION, "rematch", uuid()]);
        world.advance(target);
      }
      if (
        (!paused || dirty || canvas.dataset.renderer !== lastRenderer) &&
        now - lastDraw >= 1000 / 30
      ) {
        const view = world.view()[0]!;
        presentation.render(view, now, theme, view.matchId);
        canvas.dataset.attractTick = String(world.tick);
        lastDraw = now;
        dirty = false;
        lastRenderer = canvas.dataset.renderer;
      }
    } else accumulator = 0;

...
export async function startOnline():Promise<void>{
  ...
  title.append(node('span','FUSE'),node('span','RIDERS'));title.setAttribute('aria-label',`Fuse Riders · ${code}`);results.hidden=true;results.title='Reopen the match results';header.append(title,status,audioButton,musicButton,results,menu);
  ...
  // Only append what is new (#132): replacing every child re-inserts the card mounted above, which blurs whatever the
  // rider is already typing into — a phone keyboard closes and the keystroke in flight is dropped. Sprites load first,
  // so this is exactly the window a joiner spends filling in their name.
  app.append(canvas,sharedLobby,scoreboard,...(role==='joiner'?[]:[joinPanel]),footer,dialog);

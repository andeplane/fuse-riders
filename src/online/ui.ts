  ...
  const callbacks:Callbacks={
    ...
    state:(state,rules)=>{
      ...
      for(const p of state.players){
        ...
        const label=`${p.name} · ${p.matchScoreUnits/60} PTS · +${p.roundScoreUnits/60}${p.waitingForNextRound?' · next round':p.connected?'':' · offline'}`;if(row.label.textContent!==label)row.label.textContent=label;row.label.title=`${p.name} · ${p.matchScoreUnits/60} PTS · +${p.roundScoreUnits/60} this round`;row.label.setAttribute('aria-label',row.label.title);row.entry.style.color=p.color;row.entry.style.setProperty('--rider-color',p.color);row.entry.classList.toggle('out',!p.alive&&!['lobby','countdown'].includes(state.phase));

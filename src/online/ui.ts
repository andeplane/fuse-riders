...
export async function startOnline():Promise<void>{
  ...
  const callbacks:Callbacks={
    ...
    state:(state,rules,ack,matchId,motion)=>{
      ...
      snapshot=state;const nextRenderScope=`${runtime.transport.grant?.incarnation}:${runtime.transport.grant?.epoch}:${matchId}:${state.round}:${runtime instanceof RoomRuntime?runtime.replicationDiagnostics.alias:'local'}`;if(nextRenderScope!==renderScope)prediction.resetExternalScope();renderScope=nextRenderScope;settings=rules;if(ack>=seq){seq=ack+1;inputState.setNextSequence(seq);}if(solo)prediction.accept(state,id,ack,motion,renderScope);
      if(solo)worldBuffer.push(state,renderScope);
      ...
      for(const p of state.players){
        ...
        const label=`${p.name} · ${p.roundWins} wins${p.waitingForNextRound?' · next round':p.connected?'':' · offline'}`;if(row.label.textContent!==label)row.label.textContent=label;row.entry.style.color=p.color;row.entry.style.setProperty('--rider-color',p.color);row.entry.classList.toggle('out',!p.alive&&!['lobby','countdown'].includes(state.phase));
  ...
  setInterval(()=>{void runtime.transport.stats().then(connection=>{
    ...
    app.dataset.metrics=JSON.stringify({...connection,...(runtime instanceof RoomRuntime?{replication:runtime.replicationDiagnostics}:{}),sentBytes:runtime.transport.sentBytes,frameP95:percentile(frameTimes,.95),inputP95:percentile(inputTimes,.95),ackMs:prediction.ackMs,correction:prediction.correction,tick:snapshot?.tick??0});
  ...
  function frame(){const now=performance.now();frameTimes.push(now-previousFrame);previousFrame=now;if(frameTimes.length>300)frameTimes.shift();if(inputAt){inputTimes.push(now-inputAt);inputAt=0;if(inputTimes.length>100)inputTimes.shift();}const delayTicks=prediction.clock.presentationDelayTicks();const buffered=runtime instanceof RoomRuntime?runtime.renderSnapshot():worldBuffer.render(prediction.clock.estimate()?.tick,delayTicks);if(buffered&&(!canvas.hidden||(!sharedLobby.hidden&&!canvas.dataset.renderer))){const predicted=solo?prediction.render(buffered,id):buffered;presentation.render(predicted,now,defaultTheme,sprites,renderScope);if(responseBenchmark&&canvas.dataset.renderer?.startsWith('phaser-')&&canvas.dataset.rendererStatus!=='context-lost')sample({kind:'response-render',delayTicks:runtime instanceof RoomRuntime?(runtime.replicationDiagnostics.presentation?.delayMs??100)/50:delayTicks,...(runtime instanceof RoomRuntime?{directPresentation:runtime.replicationDiagnostics.presentation,presentationBytes:runtime.replicationDiagnostics.presentationBytes}:{}),cpuMs:performance.now()-now,frameMs:frameTimes.at(-1),clock:prediction.clock.diagnostics(),epochAt:performance.timeOrigin+performance.now(),scope:renderScope,phase:predicted.phase,tick:predicted.tick,powerupsDisabled:Object.values(settings.weights).every(weight=>weight===0),players:predicted.players.map(p=>({id:p.id,angle:p.angle,alive:p.alive,drunkUntilTick:p.drunkUntilTick,invulnerableUntilTick:p.invulnerableUntilTick,portalCooldownUntilTick:p.portalCooldownUntilTick,shielded:p.shielded}))});if(benchmark&&(benchmarkInput||now-lastBenchmarkRender>=100)){const p=predicted.players.find(p=>p.id===id);sample({kind:'prediction',renderAt:now,tick:predicted.tick,inputSeq:benchmarkInput?.seq,inputAt:benchmarkInput?.at,pose:p?{x:p.x,y:p.y,angle:p.angle}:undefined});benchmarkInput=undefined;lastBenchmarkRender=now;}}requestAnimationFrame(frame);}requestAnimationFrame(frame);

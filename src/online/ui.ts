...
export async function startOnline():Promise<void>{
  ...
  let rawStatus='';
  title.append(node('span','FUSE'),node('span','RIDERS'));title.setAttribute('aria-label',`Fuse Riders · ${code}`);results.hidden=true;results.title='Reopen the match results';header.append(title,status,statusAction,roundChip,audioButton,results,menu);
  ...
  app.replaceChildren(header,...(role==='joiner'?[]:[booting]),canvas,sharedLobby,scoreboard,joinPanel,footer,announcer,feed,hud,dialog);

...
export async function startOnline():Promise<void>{
  ...
  const callbacks:Callbacks={
    ...
    event:(event,matchId,round,tick)=>{audio.director.message({type:'event',matchId,round,tick,event});sample({kind:'event',at:performance.now(),event,matchId,round,tick});},
    state:(state,rules)=>{
      if(roomEnded)return;
      bootDone();

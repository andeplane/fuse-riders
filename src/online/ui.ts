...
export async function startOnline():Promise<void>{
  ...
  if(!code){
    ...
    const error=node('p');
    create.onclick=async()=>{create.disabled=true;try{const response=await fetch(apiUrl('/api/rooms'),{method:'POST'});const body=await response.json();if(!response.ok)throw new Error(body.error??'Could not create room');save(`fuse-room-${body.code}`,body.token);const settings=loadRoomSettings(localStorage);settings.mode=selectedMode;save(SETTINGS_KEY,JSON.stringify(settings));location.href=appUrl(`?room=${body.code}${url.searchParams.get('replication')==='actions'?'&replication=actions':''}`);}catch(e){error.textContent=String(e);create.disabled=false;}};
    join.onclick=()=>{const value=input.value.trim().toUpperCase();if(validRoomCode(value))location.href=appUrl(`?room=${value}${url.searchParams.get('replication')==='actions'?'&replication=actions':''}`);else error.textContent='Enter a room code, for example AB42';};
  ...
  if(!solo)void QRCode.toDataURL(new URL(appUrl(`?room=${code}${url.searchParams.get('replication')==='actions'?'&replication=actions':''}`),location.origin).href).then(data=>{lobbyQr.src=data;}).catch(()=>{lobbyQr.hidden=true;});
  ...
  share.onclick=async()=>{const link=new URL(appUrl(`?room=${code}${url.searchParams.get('replication')==='actions'?'&replication=actions':''}`),location.origin).href;dialogBody.replaceChildren(node('h2',`Room ${code}`),node('p',link));const qr=node('img');qr.src=await QRCode.toDataURL(link);qr.alt='Scan to join';dialogBody.append(qr);const tv=node('a','OPEN TV VIEW');tv.href=appUrl(`?room=${code}&display=1${url.searchParams.get('replication')==='actions'?'&replication=actions':''}`);tv.target='_blank';dialogBody.append(tv);dialog.showModal();};

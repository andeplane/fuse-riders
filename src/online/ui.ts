...
export async function startOnline():Promise<void>{
  ...
  const callbacks:Callbacks={
    ...
    state:(state,rules)=>{
      ...
      if(state.phase==='lobby')lastRecap='';joined=Boolean(player);if(player&&!seatTracked){seatTracked=true;track('Seat Taken',{avatarId:player.avatarId,playerCount:state.players.length});}const joining=role==='joiner'&&!joined;app.classList.toggle('joining',joining);mobileLayout.update({joined,phase:state.phase,displayOnly,host:isHost,recapReady});joinPanel.hidden=joined||displayOnly;/* Avatars are a lobby choice: the button leaves with the lobby, and a picker left open closes when the round starts. */avatarButton.hidden=!joined||state.phase!=='lobby';if(avatarButton.hidden&&dialog.open&&dialog.getAttribute('aria-label')==='Avatar')dialog.close();/* Before a seat the join form carries the avatar. */controls.hidden=!joined||displayOnly;
      ...
      if(player){if(hudAvatar!==player.avatarId){hudWho.replaceChildren(createAvatarPortrait(player.avatarId),node('b','YOU'));hudAvatar=player.avatarId;}hudFire.textContent=state.phase==='playing'&&player.alive?fireButton.textContent??'':player.alive||state.phase!=='playing'?'':'WIPED OUT';hudWins.textContent=`${player.matchScoreUnits/60} PTS · +${player.roundScoreUnits/60}`;hudRound.textContent=clock;}
    }
  };

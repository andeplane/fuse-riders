...
export interface NetSummary {
  rttP50?: number;
  rttP95?: number; // ms, over every peer link's latest echo in the window
  peers: number; // links with a measured round trip
  rollbacksPerMin: number;
  rollbackAvgTicks: number;
...
export class NetStats {
  ...
  record(m: RuntimeMetrics): void {
    const at = this.now(),
      heard = Object.values(m.heard);
    this.samples.push({
      at,
      rtts: Object.values(m.rtt),
      rollbacks: m.rollbacks,
      rollbackTicks: m.rollbackTicks,
      gap: Object.values(m.streams).some((stream) => stream.gap),
      snapshot: m.snapshotRequest,
      mismatches: m.mismatches,
      offset: m.clockTick - m.tick,
      waitingFor: m.stall.waitingFor,
      silenceMs: heard.length ? Math.min(...heard) : Infinity,
    });
    const horizon = at - STATS_WINDOW_MS;
    let drop = 0;
    while (drop < this.samples.length && this.samples[drop]!.at < horizon)
      drop++;
  ...
  summary(): NetSummary {
    ...
    const percentile = (p: number) =>
      rtts.length
        ? rtts[Math.min(rtts.length - 1, Math.floor(rtts.length * p))]
        : undefined;
    const rollbacks = Math.max(0, last.rollbacks - first.rollbacks),
      rollbackTicks = Math.max(0, last.rollbackTicks - first.rollbackTicks);
    let snapshots = 0;
    for (let index = 1; index < recent.length; index++)
      if (recent[index]!.snapshot && !recent[index - 1]!.snapshot) snapshots++;
    return {
      ...(rtts.length
        ? { rttP50: percentile(0.5), rttP95: percentile(0.95) }
        : {}),
      peers: last.rtts.length,
      rollbacksPerMin: rollbacks / minutes,
      rollbackAvgTicks: rollbacks ? rollbackTicks / rollbacks : 0,
      gapShare: recent.filter((sample) => sample.gap).length / recent.length,
      snapshotsPerMin: snapshots / minutes,
      mismatches: last.mismatches,
      clockOffsetTicks: last.offset,
      ...(last.waitingFor === undefined ? {} : { waitingFor: last.waitingFor }),
      silenceMs: last.silenceMs,
    };
  }
}
...
export function verdict(s: NetSummary): NetVerdict {
  if (s.silenceMs > 1000)
    return {
      grade: "bad",
      reason:
        s.silenceMs === Infinity
          ? "no packets from the riders yet"
          : `riders silent for ${(s.silenceMs / 1000).toFixed(1)} s`,
    };
  if (s.mismatches >= 3 || s.snapshotsPerMin > 6)
    return { grade: "bad", reason: "simulation keeps re-syncing" };
  if ((s.rttP95 ?? 0) > 250 || s.gapShare > 0.5 || s.waitingFor !== undefined)
    return {
      grade: "bad",
      reason: s.waitingFor
        ? `waiting for ${s.waitingFor}`
        : "packet loss or lag on the link",
    };
  if ((s.rttP95 ?? 0) > 120 || s.rollbackAvgTicks >= 6 || s.gapShare > 0.1)
    return { grade: "fair", reason: "link is jittery" };
...
export function formatNetStats(s: NetSummary, path: string): string {
  const v = verdict(s),
    ms = (n?: number) => (n === undefined ? "—" : `${Math.round(n)} ms`);
  return [

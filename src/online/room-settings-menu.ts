...
export function showRoomSettings(body:HTMLElement,settings:RoomSettings,solo:boolean,labels:Record<PickupType,string>,save:(draft:RoomSettings)=>boolean,close:()=>void,start:'main'|'powerups'='main'):void{
  ...
  const main=()=>{
    ...
    body.append(choices('Screen layout',draft.mode,[['devices','Full game on each device'],['shared','Shared TV + phone controls']],value=>{draft.mode=value;},solo),choices('Chain reaction',draft.chainReaction?'on':'off',[['on','Bombs set off bombs caught in the blast'],['off','Every bomb waits for its own fuse']],value=>{draft.chainReaction=value==='on';}),choices('Aim bounce',draft.aimBounce?'on':'off',[['on','Holding past full reach aims back in and out again'],['off','Holding past full reach stays at maximum']],value=>{draft.aimBounce=value==='on';}),
      choices<ArenaMapChoice>('Arena map',draft.map,[['rotate','A different map every round'],['desert',ARENA_MAP_LABELS.desert],['forest',ARENA_MAP_LABELS.forest],['city',ARENA_MAP_LABELS.city],['classic',`${ARENA_MAP_LABELS.classic} · no obstacles`],['wrap',`${ARENA_MAP_LABELS.wrap} · no walls until overtime`],['cross',`${ARENA_MAP_LABELS.cross} · the classic arena, split four ways`]],value=>{draft.map=value;}));
    body.append(element('p','Desert, Forest and City put rocks, trees and buildings on the board. Crashing into one is fatal; a bomb blast clears it away.'),
      element('p','Wrap-around has open edges: riders, shells, bullets, thrown bombs and blasts leave one side and arrive on the other. Crossed plays exactly like the classic arena, drawn shifted by half a board: the walls meet in a cross in the middle and the screen edges are open.'));
    ...
    const presets=element('div');for(const rounds of [3,5]){const button=element('button',rounds===3?'3 ROUNDS · QUICK':'5 ROUNDS · STANDARD');button.type='button';button.onclick=()=>{draft.length=rounds;length.value=String(rounds);};presets.append(button);}body.append(presets,element('p','Most points wins. +1 per opponent outlasted, +1 for the sole survivor. Ties break on round wins, then share victory.'));
    const aimLabel=element('label','Bomb aim time (seconds)'),aim=element('input');aim.type='number';aim.min=String(BOMB_MIN_CHARGE_TICKS/TICK_HZ);aim.max=String(BOMB_CHARGE_TICKS_LIMIT/TICK_HZ);aim.step=String(1/TICK_HZ);aim.required=true;aim.value=aimText;aim.setAttribute('aria-label','Bomb aim time (seconds)');aim.oninput=()=>{aimText=aim.value;if(aim.checkValidity())draft.bombChargeTicks=Math.round(Number(aim.value)*TICK_HZ);};aimLabel.append(aim);body.append(aimLabel,element('p','Time to reach maximum bomb distance. Lower values aim farther, faster, and with aim bounce on the range walks back in just as quickly.'));

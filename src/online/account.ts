...
const firebase = () => loading ??= load().catch(error => { loading = undefined; throw error; });
...
export const accountReady = async (): Promise<void> => { await firebase(); };
...
export async function signIn(): Promise<void> {
  const { auth, instance } = await firebase(), provider = new auth.GoogleAuthProvider();
...
export async function identityToken(): Promise<string | undefined> {
  ...
  try { return await (await firebase()).instance.currentUser?.getIdToken(); } catch { return undefined; }
}

export function signInFailure(error: unknown): string {
  const code = (error as { code?: unknown })?.code;
  if (code === 'auth/popup-closed-by-user' || code === 'auth/cancelled-popup-request') return '';
  if (code === 'auth/popup-blocked') return 'Allow pop-ups for this page, then try again.';
  if (code === 'auth/operation-not-allowed') return 'Sign-in is not switched on yet.';
  if (code === 'auth/network-request-failed') return 'No connection to the sign-in service.';

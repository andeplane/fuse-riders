...
export type BoundControl = ClockProbe | ClockReply | Finality | [version: 1, alias: number, kind: 'settle' | 'confirm', tick: number];
...
export function isBoundControl(raw: unknown): raw is BoundControl {
  ...
  if (raw[2] === 'settle' || raw[2] === 'confirm') return raw.length === 4 && uint32(raw[3]);

...
export interface SignalCounts { descriptions:number;candidates:number;applied:number;buffered:number;rejected:number;dropped:number }
...
export class RemoteSignal {
  ...
  readonly counts:SignalCounts={descriptions:0,candidates:0,applied:0,buffered:0,rejected:0,dropped:0};
  constructor(private readonly pc:RemotePeer,private readonly limit=64){}
  get waiting():number{return this.pending.length;}
  async describe(description:RTCSessionDescriptionInit):Promise<void> {
    this.counts.descriptions++;this.applying=true;
    try{await this.pc.setRemoteDescription(description);}finally{this.applying=false;}
    await this.flush();
  }
  async candidate(candidate:RTCIceCandidateInit):Promise<void> {
    this.counts.candidates++;
    const sdp=this.pc.remoteDescription?.sdp;
    if(this.applying||sdp===undefined||!ufragMatches(candidate,sdp)){this.buffer(candidate);return;}
    await this.apply(candidate);
  }
  private buffer(candidate:RTCIceCandidateInit):void {
    if(this.pending.length>=this.limit){this.pending.shift();this.counts.dropped++;}
    this.pending.push(candidate);this.counts.buffered++;
  }
  private async flush():Promise<void> {
    const batch=this.pending;this.pending=[];
    for(const candidate of batch){
      const sdp=this.pc.remoteDescription?.sdp;
      if(sdp!==undefined&&ufragMatches(candidate,sdp))await this.apply(candidate);
  ...
  private async apply(candidate:RTCIceCandidateInit):Promise<void> {
    try{await this.pc.addIceCandidate(candidate);this.counts.applied++;}catch{this.counts.rejected++;}
  }
}

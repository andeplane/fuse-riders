...
export function presentWorld(older: ViewSnapshot | undefined, newer: ViewSnapshot, presentationTick: number, local?: LocalRider): ViewSnapshot {
  ...
  const pose = advanceRiderPose({ x: rider.x, y: rider.y, angle: rider.angle + gravityBend(newer.gravityFields, rider, motion.turn * lead), drunkHeadingOffset: 0 }, local!.controls, { distance: motion.distance * lead, turn: motion.turn * lead, drunkHeadingOffset: 0 });

...
export class LinkRestartPolicy {
  ...
  constructor(now:number,readonly max=4,readonly intervalMs=8000){this.dueAt=now+intervalMs;}
  ...
  complete(attempt:number):boolean{if(attempt!==this.current)return false;this.inFlight=false;return true;}
}

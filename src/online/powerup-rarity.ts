...
const SCALE: Record<Rarity, number> = {
  off: 0,
  rare: 1 / 3,
  normal: 1,
  common: 3,
...
export function weightFor(
  type: PickupType,
  rarity: Rarity,
  defaults: RoomSettings["weights"],
): number {
  const base =
    defaults[type] ||
    median(
      Object.values(defaults).filter((weight) => (weight ?? 0) > 0) as number[],
    );
...
export function rarityOf(
  ...
  let best: Rarity = "normal",
    distance = Infinity;
  for (const rarity of RARITIES) {
    if (rarity === "off") continue;
    const target = weightFor(type, rarity, defaults);
    if (!target) continue;
    const gap = Math.abs(Math.log(weight / target));
    if (gap < distance) {
      distance = gap;
      best = rarity;
    }
...
function median(values: number[]): number {
  ...
  return sorted.length ? sorted[Math.floor(sorted.length / 2)]! : 1000;
}
export const POWERUP_PRESETS: Record<
  string,
  (type: PickupType, defaults: RoomSettings["weights"]) => number
> = {

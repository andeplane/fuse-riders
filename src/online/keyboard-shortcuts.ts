...
export interface ShortcutGroup {
  title: string;
  entries: readonly (readonly [keys: string, action: string])[];
}
...
export function keyboardShortcuts({
  ...
  const groups: ShortcutGroup[] = [
      ...
      entries: [
        ...(canConfigure
          ? [
              [
                mac ? "⌘P" : "Ctrl+P",
                "Configure power-ups (takes over the browser print dialog)",
              ] as const,
            ]
          : []),
    ...
    {
      title: "Left to the browser",
      entries: [
        [mac ? "⌥⌘I" : "F12 · Ctrl+Shift+I", "Developer tools"],
        [
          mac ? "⌘R" : "Ctrl+R",
          solo
            ? "Reload — a solo run starts over"
            : "Reload — a room rejoins by itself",
        ],
      ],
    },
  ];

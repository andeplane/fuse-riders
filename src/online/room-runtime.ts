...
export class RoomRuntime {
  ...
  stop(): void { this.halt(); this.transport?.close(true); }
  ...
  heldControls(id: string): { left: boolean; right: boolean } | undefined {
    const fold = this.world?.state.folds.get(id); if (!fold) return undefined;
  ...
  metrics(): RuntimeMetrics {
    ...
    const now = this.deps.now();
    return { tick: this.tick, clockTick: this.clock.tick(), rollbacks: this.world?.rollbacks ?? 0, rollbackTicks: this.world?.rollbackTicks ?? 0, rtt: Object.fromEntries([...this.members].filter(([, member]) => member.rttMs !== undefined).map(([id, member]) => [id, member.rttMs!])), heard: Object.fromEntries([...this.members].filter(([, member]) => member.lastPacketAt !== -Infinity).map(([id, member]) => [id, Math.round(now - member.lastPacketAt)])), clock: this.clock.diagnostics(), sentBytes: this.transport?.sentBytes ?? 0, snapshotRequest: this.snapshotRequest !== undefined, mismatches: this.mismatches.length, stall: this.world?.stallBound() ?? { tick: Infinity }, streams };
  }
}

...
export class TickClock {
  ...
  tick(): number {
    ...
    if (this.offset !== this.target) {
      const budget = Math.max(0, now - this.lastAdjust) / 1000 * SLEW_TICKS_PER_SECOND;
      this.offset += Math.max(-budget, Math.min(budget, this.target - this.offset));
    }
    this.lastAdjust = now;
  ...
  sample(authorityTick: number, rttMs: number): void {
    ...
    const best = this.samples.reduce((best, sample) => sample.rttMs < best.rttMs ? sample : best);
    if (Math.abs(best.offset - this.offset) > SNAP_TICKS) { this.offset = this.target = best.offset; return; }
    this.target = best.offset;
  }
  /** No usable sample for two seconds: the clock keeps running on its own rate. */
  freeRunning(): boolean { return this.t0 !== undefined && this.now() - this.lastSampleAt > SAMPLE_WINDOW_MS; }
  /** Solo pause while the page is hidden: time spent paused is removed from the clock on resume. */
  pause(): void { if (this.t0 !== undefined && this.pausedAt === undefined) this.pausedAt = this.now(); }
  resume(): void { if (this.pausedAt !== undefined && this.t0 !== undefined) { this.t0 += this.now() - this.pausedAt; this.lastAdjust = this.now(); this.pausedAt = undefined; } }
  get paused(): boolean { return this.pausedAt !== undefined; }
  diagnostics(): { offset: number; samples: number; bestRttMs?: number } {
    const best = this.samples.length ? Math.min(...this.samples.map(sample => sample.rttMs)) : undefined;
    return { offset: this.offset, samples: this.samples.length, ...(best === undefined ? {} : { bestRttMs: best }) };
  }
}

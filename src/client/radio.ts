...
export const MUSIC_TRACKS = [
  { id: 'pixel-sax-parade', title: 'Pixel Sax Parade', path: '/music/pixel-sax-parade.m4a' },
  { id: 'coin-op-swing', title: 'Coin Op Swing', path: '/music/coin-op-swing.m4a' },
  { id: 'arcade-adventure', title: 'Arcade Adventure', path: '/music/arcade-adventure.m4a' },
...
export function defaultRadio(): RadioState {
  return { track: MUSIC_TRACKS[0].id, position: 0, paused: false, loopSong: false, loopPlaylist: true, source: 'all', playlist: [] };
}

const trackIds: ReadonlySet<string> = new Set(MUSIC_TRACKS.map(track => track.id));
export const isTrackId = (value: unknown): value is TrackId => typeof value === 'string' && trackIds.has(value);
export const trackById = (id: TrackId): typeof MUSIC_TRACKS[number] => MUSIC_TRACKS.find(track => track.id === id)!;
...
export function parseRadio(raw: string | null): RadioState {
  ...
  let value: unknown;
  try { value = raw === null ? undefined : JSON.parse(raw); } catch { return state; }
  ...
  if (isTrackId(stored.track)) {
    ...
    if (typeof position === 'number' && Number.isFinite(position) && position >= 0 && position <= MAX_POSITION) state.position = position;
  }
  for (const flag of ['paused', 'loopSong', 'loopPlaylist'] as const) if (typeof stored[flag] === 'boolean') state[flag] = stored[flag];
  if (stored.source === 'all' || stored.source === 'playlist') state.source = stored.source;
  if (Array.isArray(stored.playlist)) state.playlist = [...new Set(stored.playlist.filter(isTrackId))];
  return state;
}
...
export function loadRadio(storage: SafeStorage): RadioState {
  try { return parseRadio(storage.getItem(RADIO_KEY)); } catch { return defaultRadio(); }
}
export function saveRadio(storage: SafeStorage, state: RadioState): void {
  try { storage.setItem(RADIO_KEY, JSON.stringify(state)); } catch { /* A full or blocked store keeps playing without persistence. */ }
}

/** The order tracks play in for the current source. */
export function radioQueue(state: RadioState): readonly TrackId[] {
  return state.source === 'playlist' && state.playlist.length ? state.playlist : MUSIC_TRACKS.map(track => track.id);
}
/**
 ...
 */
export function followingTrack(state: RadioState, reason: 'ended' | 'next'): TrackId | undefined {
  if (reason === 'ended' && state.loopSong) return state.track;
  const queue = radioQueue(state); const index = queue.indexOf(state.track);
  if (index < 0) return queue[0];
  if (index + 1 < queue.length) return queue[index + 1];
  const playlistEnded = reason === 'ended' && state.source === 'playlist' && state.playlist.length > 0 && !state.loopPlaylist;
  return playlistEnded ? undefined : queue[0];
}
export function precedingTrack(state: RadioState): TrackId {
  const queue = radioQueue(state); const index = queue.indexOf(state.track);
  return index < 0 ? queue[0]! : queue[(index - 1 + queue.length) % queue.length]!;
}
/** Adds a track to the end of the playlist, or removes it if already present. */
export function togglePlaylistTrack(playlist: readonly TrackId[], id: TrackId): TrackId[] {
  return playlist.includes(id) ? playlist.filter(entry => entry !== id) : [...playlist, id];
}
...
export const RADIO_SHORTCUT_HINT = 'Ctrl+A radio · Ctrl+M mute all · Ctrl+Alt+M music · Ctrl+Alt+E effects';

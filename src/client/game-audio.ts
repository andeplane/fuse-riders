...
} from "./radio-media-session.js";

export const AUDIO_SETTINGS_KEY = "fuse-riders-audio";
export const DEFAULT_VOLUME: Record<AudioChannel, number> = {
  music: 0.22,
  effects: 0.45,
};
...
export function loadAudioSettings(
  ...
  try {
    const stored: unknown = JSON.parse(
      storage.getItem(AUDIO_SETTINGS_KEY) ?? "null",
    );
...
class WebAudioSynth implements GameSynth {
  ...
  constructor(
    private readonly stateChanged: (running: boolean) => void,
    private readonly trackEnded: () => void,
    private readonly mediaPaused: () => void = () => {},
    private readonly mediaResumed: () => void = () => {},
  ) {}
    ...
    try {
      if (!this.context || this.context.state === "closed") {
        ...
        this.context.onstatechange = () =>
          this.stateChanged(this.context?.state === "running");
      }
      if (!this.effects) {
        this.effects = this.context.createGain();
        this.effects.connect(this.context.destination);
        this.gain("effects", this.levels.effects);
      }
  ...
  gain(channel: AudioChannel, value: number): void {
    ...
    if (channel === "music") {
      ...
      if (value <= 0) this.pauseElement();
      else this.startTrack();
      return;
    }
    if (this.context && this.effects)
      this.effects.gain.setTargetAtTime(value, this.context.currentTime, 0.015);
  }
  note(channel: AudioChannel, note: SynthNote): void {
    ...
    oscillator.frequency.exponentialRampToValueAtTime(
      note.endFrequency ?? note.frequency,
      end,
    );
    envelope.gain.setValueAtTime(0, start);
    envelope.gain.linearRampToValueAtTime(note.level, start + 0.006);
  ...
  music(path: string, offset: number): void {
    ...
    this.trackPath = path;
    element.volume = this.levels.music;
    ...
    if (
      ...
      if (Math.abs(element.currentTime - offset) > 0.5)
  ...
  audible(): boolean {
    ...
    return (
      !!element &&
      !!this.trackPath &&
      this.loadedPath === this.trackPath &&
      !element.paused &&
      !element.ended &&
      element.volume > 0
    );
  ...
  private createElement(): HTMLAudioElement {
    ...
    element.addEventListener("pause", () => {
      ...
      if (!element.ended && this.trackPath) this.mediaPaused();
    });
    ...
    element.addEventListener("loadedmetadata", () => {
      ...
      element.currentTime = Number.isFinite(element.duration)
        ? Math.min(this.pendingSeek, element.duration)
        : this.pendingSeek;
      this.pendingSeek = undefined;
    });
  ...
  private startTrack(): void {
    ...
    if (this.loadedPath !== this.trackPath) {
      this.loadedPath = this.trackPath;
      this.ownPause = false;
      this.ownPlay = false;
      element.src = assetUrl(this.trackPath);
    }
...
export function requestAmbientAudio(host: object = navigator): boolean {
  ...
  if (typeof session !== "object" || session === null || !("type" in session))
    return false;
  try {
    (session as { type: string }).type = "ambient";
    return true;
  } catch {
    return false;
  }
...
export function browserMediaSession(): MediaSessionPort | undefined {
  ...
  return {
    setMetadata: (track) => {
      session.metadata = new MediaMetadata(track);
    },
    ...
    setPositionState: (position) => {
      try {
        ...
        /* a rejected state, e.g. a position past the duration */
      }
    },
  };
}
...
export function createGameAudio(
  ...
  const showState = (ok: boolean) => {
    running = ok;
    const text = ok
      ? `${deviceLabel} audio enabled · test sound`
      : `Enable / resume ${deviceLabel} audio`;
  ...
  const director: AudioDirector = new AudioDirector(
    new WebAudioSynth(
      showState,
      () => director.trackEnded(),
      () => director.mediaPaused(),
      () => director.mediaResumed(),
    ),
    loadRadio(storage),
  ...
  for (const channel of CHANNELS) {
    director.setVolume(channel, settings.volume[channel]);
    director.setMuted(channel, settings.muted[channel]);
  }
  ...
  const setMuted = (channel: AudioChannel, value: boolean) => {
    settings.muted[channel] = value;
    director.setMuted(channel, value);
    store();
    for (const render of rendered) render();
  };
  const allMuted = () => settings.muted.music && settings.muted.effects;
  const setAllMuted = (value: boolean) => {
    setMuted("music", value);
    setMuted("effects", value);
  };
  const setVolume = (channel: AudioChannel, value: number) => {
    settings.volume[channel] = value;
    director.setVolume(channel, value);
    store();
  };
  ...
  const media = bindMediaSession(
    options.mediaSession ?? browserMediaSession(),
    {
      title: () => director.trackTitle,
      playing: () => director.audible(),
    ...
    radioMediaActions(
      {
        paused: () => director.state.paused,
        togglePause: () => director.togglePause(),
        previousTrack: () => director.previousTrack(),
        nextTrack: () => director.nextTrack(),
      },
      {
        muted: () => settings.muted.music,
        unmute: () => setMuted("music", false),
      },
      () => unlock(),
    ),
  );
  ...
  const unlock = (confirm = false) => {
    void director.unlock(confirm).then((ok) => {
      showState(ok);
      if (ok) director.update();
    });
  };
  ...
  const loopSong = toggle("LOOP SONG", () =>
    director.setLoopSong(!director.state.loopSong),
  );
  const loopPlaylist = toggle("LOOP PLAYLIST", () =>
    director.setLoopPlaylist(!director.state.loopPlaylist),
  );
  const sources: Record<RadioSource, HTMLButtonElement> = {
  ...
  for (const channel of CHANNELS) {
    ...
    const renderMute = () => {
      ...
      if (mute.getAttribute("aria-pressed") !== pressed)
        mute.setAttribute("aria-pressed", pressed);
    };
    rendered.add(renderMute);
    renderMute();
    mute.addEventListener("click", () =>
      setMuted(channel, !settings.muted[channel]),
    );
  ...
  const trackRow = (id: TrackId, source: RadioSource) => {
    ...
    const play = key("radio-track", name, `Play ${name}`, () =>
  ...
  const renderTime = () => {
    const text = `${formatTrackTime(director.position())} / ${formatTrackTime(director.duration())}`;
    if (time.textContent !== text) time.textContent = text;
    media.refresh();
    for (const render of rendered) render(); // The music toggle follows what is audible, which no event announces.
  };
  let listsKey = "";
  function render(): void {
    ...
    if (titleText.textContent !== director.trackTitle)
      titleText.textContent = director.trackTitle;
    ...
    flags.textContent = [
      state.loopSong ? "RPT1" : "",
      queue === state.playlist ? `LIST${state.loopPlaylist ? "⟳" : ""}` : "ALL",
    ]
      .filter(Boolean)
      .join(" ");
    const action = state.paused ? "Play" : "Pause";
    playPause.textContent = state.paused ? "▶" : "⏸";
    playPause.setAttribute("aria-label", action);
    playPause.title = action;
    lcd.classList.toggle(
      "idle",
      state.paused || settings.muted.music || !running,
    );
    loopSong.setAttribute("aria-pressed", String(state.loopSong));
    loopPlaylist.setAttribute("aria-pressed", String(state.loopPlaylist));
    for (const source of ["all", "playlist"] as const)
      sources[source].setAttribute(
        "aria-pressed",
        String(state.source === source),
      );
    ...
    if (key !== listsKey) {
      ...
      tracks.replaceChildren(
        ...MUSIC_TRACKS.map((track) => trackRow(track.id, "all")),
      );
  ...
  director.subscribe(render);
  render(); // Mute changes reach it through the director as well.
  ...
  const bindToggle = (
    ...
    const render = () => {
      ...
      const text = on() ? labels[0] : labels[1];
      if (button.textContent !== text) button.textContent = text;
      const muted = String(!on());
      if (button.dataset.muted !== muted) button.dataset.muted = muted;
    };
    rendered.add(render);
    render();
    button.addEventListener("click", () => {
  ...
  window.addEventListener(
    ...
    (event) => {
      ...
      if (shortcut === "radio")
        ...
        const channel = shortcut === "muteMusic" ? "music" : "effects";
        setMuted(channel, !settings.muted[channel]);
      }
    },
    { capture: true },
  ...
  window.addEventListener("storage", (event) => {
    ...
    const { loopSong, loopPlaylist, source, playlist } = parseRadio(
        event.newValue,
      ),
    ...
    if (
      ...
      source === state.source &&
      playlist.join() === state.playlist.join()
    )
      return;
  ...
  document.addEventListener("visibilitychange", () => {
    ...
    if (document.hidden) director.save();
    else {
      director.resume();
      unlock();
    }

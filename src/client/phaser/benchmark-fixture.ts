import { createGame, addPlayer, toSnapshot, SLOT_COLORS } from '../../shared/game.js';
...
export function visualFixture(tick: number): ViewSnapshot {
  ...
  for(let p=0;p<5;p++) addPlayer(game,{id:`p${p}`,name:`RIDER ${p+1}`,slot:p,color:SLOT_COLORS[p],avatarId:AVATARS[p].id});
  const state=toSnapshot(game);
  const phase=tick/20;
  return {...state,tick,round:1,phase:'playing',boundaryInset:35,
    players:state.players.map((player,p)=>{
      const cx=260+(p%3)*480,cy=250+Math.floor(p/3)*390,r=155;
      const angle=phase*.6+p;
      return {...player,alive:true,x:cx+Math.cos(angle)*r,y:cy+Math.sin(angle)*r,angle:angle+Math.PI/2,
        shielded:p===0,drunkUntilTick:p===1?tick+60:0,inkUntilTick:0,
        trail:Array.from({length:160},(_,i)=>{const a=angle-(160-i)*.012;return {x1:cx+Math.cos(a)*r,y1:cy+Math.sin(a)*r,x2:cx+Math.cos(a+.012)*r,y2:cy+Math.sin(a+.012)*r,createdTick:tick-160+i,expiresAtTick:tick+i};})};
    }),

...
export function mountArenaPresentation(
  ...
  const clearTimers = () => {
    cancelStartup?.(); cancelStartup = undefined;
    cancelRestore?.(); cancelRestore = undefined;
  };
  const fail = () => {
    ...
    clearTimers();
    const previous = engine; engine = undefined;
    previous?.destroy();
    canvas.dataset.rendererStatus = 'failed';
    delete canvas.dataset.rendererMetrics;
    canvas.style.opacity = '.35';
    showStatus('Graphics unavailable. The game continues while your view is paused.', true);
  };
  const paint = () => {
  ...
  const initialize = () => {
    ...
    void (async () => {
      ...
      const arena = module.createPhaserArena(canvas, {
        renderer: new URLSearchParams(location.search).get('renderer') === 'phaser-canvas' ? 'canvas' : 'auto',
  ...
  return {
    ...
    destroy() {
      if (state === 'disposed') return;
      state = 'disposed'; generation++;
      clearTimers();
      retry.onclick = null; status.remove();
      engine?.destroy(); engine = undefined; latest = undefined;
    },
  };
}

import { RIDER_SPEED, SPEED_RAMP_MAX, TICK_HZ, riderSpeedMultiplier } from '../../shared/game.js';
...
export function trailTip(player: Rider, tick: number, phase: ViewSnapshot['phase']): readonly TrailPoint[] {
  ...
  if (phase === 'playing' && player.alive && !last.detached && last.createdTick === Math.floor(tick) &&
      player.portalCooldownUntilTick <= tick && distance > 1e-6 && distance <= RIDER_SPEED * riderSpeedMultiplier(player, last.createdTick) * SPEED_RAMP_MAX / TICK_HZ + 1e-6) {

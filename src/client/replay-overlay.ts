...
export function createReplayOverlay(): ReplayOverlay {
  ...
  const release = (canvas?: HTMLCanvasElement) => {
    const target = canvas ?? zoomed;
    if (target) {
      target.style.transform = "";
      target.style.transformOrigin = "";
      target.classList.remove("replay-zoomed");
    }
    zoomed = undefined;
  };
  return {
    ...
    start(card, color) {
      ...
      title.textContent = `${card.icon}  ${card.title}`;
    ...
    update(update, canvas, world) {
      ...
      flash.style.opacity = String(update.flash * 0.85);
      if (update.stage === "play" || update.stage === "out") {
        if (update.focus && update.zoom > 1.001) {
          ...
          canvas.classList.add("replay-zoomed");
          zoomed = canvas;
        } else release(canvas);
      }

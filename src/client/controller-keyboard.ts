...
export class ControllerKeyboardBindings {
  ...
  constructor(private state:ControllerInputState,private allowed:()=>boolean,private changed:()=>void=()=>{},private used:()=>void=()=>{}){}
  down(event:KeyboardInputEvent):void {
    ...
    this.used();this.held.add(event.code);this.state.pointerDown(...binding);this.changed();
  }
  up(event:KeyboardInputEvent):void {
    if(!this.held.delete(event.code))return;
    event.preventDefault();const [id]=keys[event.code]!;
    if(this.allowed())this.state.pointerRelease(id);else this.state.pointerCancel(id);
    this.changed();
  }
  clear():void {
    for(const code of this.held)this.state.pointerCancel(keys[code]![0]);
    this.held.clear();this.changed();
  }
}

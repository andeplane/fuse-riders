...
export type WallBracket = readonly [
  number,
  number,
  number,
  number,
...
const wallDepth = (inset: number): number =>
  Math.min(18, Math.max(2, inset - 2));
...
export function arenaWall(
  ...
  return theme.rendering.pixelated
    ...
    : {
        kind: "smooth",
        rect: smoothWallRect(width, height, inset),
        strokeWidth: theme.rendering.wallWidth,
      };
}
...
export function smoothWallRect(
  width: number,
  height: number,
  inset: number,
): WallRect {
 ...
 */
export function trailStuds(
  path: readonly WallStud[],
  spacing = TRAIL_STUD_SPACING,
): WallStud[] {
  ...
  if (last && studs.length > 0)
    studs.push({ x: Math.round(last.x), y: Math.round(last.y) });
  return studs;
}

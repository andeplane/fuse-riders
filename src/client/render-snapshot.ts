 ...
 */
export function renderedSnapshot(
  ...
  if (
    ...
    tickDelta <= 0 ||
    older.snapshot.phase !== "playing" ||
    newer.snapshot.phase !== "playing"
  )
    return newer.snapshot;

  const authoritativeDuration = Math.max(
    VISUAL_PROJECTION_LIMIT_MS,
    tickDelta * VISUAL_PROJECTION_LIMIT_MS,
  );
  const projectionDuration = Math.max(
    0,
    Math.min(VISUAL_PROJECTION_LIMIT_MS, now - newer.receivedAt),
  );
  ...
  return {
    ...
    bombs: newer.snapshot.bombs.map((bomb) => {
      ...
      const motion = {
        x: bomb.x,
        y: bomb.y,
        vx: bomb.shell.vx * dt * 20,
        vy: bomb.shell.vy * dt * 20,
      };
      // As in the simulation: the obstacle a shell is inside of does not hold it.
      const obstacleWalls = newer.snapshot.obstacles
        .filter(
      ...
      advanceShell(
        ...
        [
          ...obstacleWalls,
          ...newer.snapshot.players.flatMap((player) =>
            player.id === bomb.ownerId &&
            newer.snapshot.tick - bomb.launchedTick < 6
              ? []
              : [...player.trail],
          ),
        ],
    ...
    players: newer.snapshot.players.map((player) => {
      ...
      if (
        !previous?.alive ||
        !player.alive ||
        player.portalCooldownUntilTick > previous.portalCooldownUntilTick
      )
        return player;
      ...
      return {
        ...
        ...(player.bombTarget && previous.bombTarget
          ? {
              bombTarget: {
                x: Math.max(
                  newer.snapshot.boundaryInset + 20,
                  Math.min(
                    newer.snapshot.width - newer.snapshot.boundaryInset - 20,
                    player.bombTarget.x +
                      (player.bombTarget.x - previous.bombTarget.x) * factor,
                  ),
                ),
                y: Math.max(
                  newer.snapshot.boundaryInset + 20,
                  Math.min(
                    newer.snapshot.height - newer.snapshot.boundaryInset - 20,
                    player.bombTarget.y +
                      (player.bombTarget.y - previous.bombTarget.y) * factor,
                  ),
                ),
              },
            }

 ...
 */
export function selectedTheme(search: string = location.search, store: SafeStorage = themeStore): ThemeDefinition {
  ...
  if (requested && Object.hasOwn(themes, requested)) { store.setItem(THEME_STORAGE_KEY, requested); return themes[requested as ThemeId]; }
  const stored = store.getItem(THEME_STORAGE_KEY);
  return stored && Object.hasOwn(themes, stored) ? themes[stored as ThemeId] : defaultTheme;
}

...
export class ControllerPointerBindings {
  ...
  constructor(
    ...
    private readonly buttons: ReadonlyArray<
      readonly [PointerButton, ControllerControl]
    >,
    terminalTarget: EventTarget,
    private readonly changed: () => void,
    private readonly buttonAtPoint?: (
      x: number,
      y: number,
    ) => PointerButton | undefined,
  ) {
    terminalTarget.addEventListener(
      "pointerdown",
      (event) => this.interrupted.delete((event as PointerEvent).pointerId),
      { capture: true },
    );
    for (const [button, control] of buttons) {
      ...
      button.addEventListener("pointerdown", (event) => {
        ...
        try {
          ...
          /* Window terminal events remain available. */
        }
        this.sync();
      });
      button.addEventListener("lostpointercapture", (event) => {
        const pointer = event as PointerEvent;
        if (this.owners.get(pointer.pointerId) === button) {
          this.interrupted.add(pointer.pointerId);
          this.finish(pointer.pointerId, true);
        }
      });
    }
    terminalTarget.addEventListener(
      ...
      (event) => {
        ...
        if (
          ...
          const target =
            candidate && !candidate.disabled ? candidate : undefined;
  ...
  bindKeyboard(target: EventTarget, enabled: () => boolean): void {
    ...
    target.addEventListener("keydown", (event) => {
      ...
      if (!binding || !enabled() || key.altKey || key.ctrlKey || key.metaKey)
        return;
      key.preventDefault();
  ...
  private sync(): void {
    for (const [button, control] of this.buttons)
      button.classList.toggle("active", this.state.isHeld(control));
    this.changed();
  }
}

...
export const radioArtwork = (): MediaArtwork[] => [
  { src: assetUrl(RADIO_ARTWORK_PATH), sizes: "512x512", type: "image/png" },
];
...
export function bindMediaSession(
  port: MediaSessionPort | undefined,
  radio: MediaRadio,
  actions: MediaActions,
  artwork: MediaArtwork[] = radioArtwork(),
): MediaBinding {
  ...
  const refresh = () => {
    ...
    if (radio.playing() !== playing) {
      playing = radio.playing();
      port.setPlaybackState(playing ? "playing" : "paused");
    }
    const duration = radio.duration();
    if (duration !== undefined && Number.isFinite(duration) && duration > 0) {
      known = true;
      port.setPositionState({
        duration,
        position: Math.max(0, Math.min(duration, radio.position())),
        playbackRate: 1,
      });
    } else if (known) {
      known = false;
  ...
  return {
    ...
    unbind() {
      unsubscribe();
      for (const action of ACTIONS) port.setActionHandler(action, null);
    },
  };
}
 ...
 */
export function radioMediaActions(
  ...
  return {
    play: () => {
      if (music.muted()) music.unmute();
      if (radio.paused()) radio.togglePause();
      unlock();
    },
    pause: () => {
      if (!radio.paused()) radio.togglePause();

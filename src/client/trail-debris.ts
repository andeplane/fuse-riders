...
export class TrailDebris {
  ...
  update(snapshot: ViewSnapshot, now: number, match: string): DebrisStroke[] {
    ...
    this.fragments = this.fragments.filter(
      (piece) => now - piece.born < piece.life,
    );
    const fresh = reset
      ? []
      : snapshot.blasts.filter(
          (blast) =>
    ...
    if (fresh.length) {
      ...
      for (const previous of this.trails) {
        const rider = snapshot.players.find(
        ...
        for (const segment of advanceTrail(
          ...
          if (
            [segment.x1, segment.x2].some(
              (x) => x < b || x > snapshot.width - b,
            ) ||
            [segment.y1, segment.y2].some(
              (y) => y < b || y > snapshot.height - b,
            )
          )
            continue;
          ...
          for (const blast of fresh) {
            ...
            const force =
              1 -
              Math.hypot(x - blast.circle.x, y - blast.circle.y) /
                Math.max(1, blast.circle.radius);
            if (force > pressure) {
              strongest = blast;
              pressure = force;
            }
          ...
          for (let i = 0; i < pieces; i++) {
            ...
            const px = segment.x1 + (segment.x2 - segment.x1) * t,
              py = segment.y1 + (segment.y2 - segment.y1) * t;
            const dx = px - strongest.circle.x,
              dy = py - strongest.circle.y;
            const direction =
              (Math.hypot(dx, dy) < 1
                ? this.random() * Math.PI * 2
                : Math.atan2(dy, dx)) +
              (this.random() - 0.5) * 0.65;
            const speed =
              (200 + 360 * clamp(pressure)) * (0.75 + this.random() * 0.5);
            const piece: Fragment = {
              ...
              spin: (this.random() - 0.5) * 22,
              born: now,
              life: 520 + this.random() * 300,
              color: previous.color,
            };
            // Reservoir sampling shares the budget across every rider in a dense chain reaction.
            const slot =
              candidates < this.limit
                ? candidates
                : Math.floor(this.random() * (candidates + 1));
    ...
    return this.fragments.map((piece) => {
      const age = clamp((now - piece.born) / piece.life),
        seconds = Math.max(0, now - piece.born) / 1000;
      const travel = (1 - Math.exp(-3.4 * seconds)) / 3.4;
      const x = piece.x + piece.vx * travel,
        y = piece.y + piece.vy * travel;
      const angle =
        piece.angle + (piece.spin * (1 - Math.exp(-1.8 * seconds))) / 1.8;
      const half = (piece.length * (1 - 0.65 * age ** 2)) / 2;
      return {

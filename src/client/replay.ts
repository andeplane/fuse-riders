...
import { describeMoment, rankMoments, type MomentCard } from '../shared/match-recap.js';
import { interpolateWorld } from '../online/interpolate.js';
...
export const SLOW_FACTOR = .25;
export const REPLAY_ZOOM = 1.35;
const TICK_MS = 50;

export interface ReplayClip { key: string; moment: Moment; frames: ViewSnapshot[] }
...
export class ReplayRecorder {
  ...
  record(snapshot: ViewSnapshot, matchId: string): void {
    if (matchId !== this.matchId) { this.matchId = matchId; this.clips.clear(); this.round = -1; }
    if (snapshot.round !== this.round) { this.round = snapshot.round; this.frames = []; }
    if (!Number.isInteger(snapshot.tick)) return;
    const last = this.frames[this.frames.length - 1];
    if (last && snapshot.tick <= last.tick) return;
    this.frames.push(snapshot);
    while (this.frames.length && this.frames[0]!.tick < snapshot.tick - RECORDER_KEEP_TICKS) this.frames.shift();
  }
  ...
  cut(moment: Moment, matchId: string): ReplayClip | undefined {
    ...
    if (matchId !== this.matchId || moment.round !== this.round) return undefined;
    const frames = this.frames.filter(frame => frame.tick >= moment.tick - CLIP_BEFORE_TICKS && frame.tick <= moment.tick + CLIP_AFTER_TICKS);
    const last = frames[frames.length - 1];
    if (frames.length < 8 || !last || last.tick < moment.tick) return undefined;
    const clip = { key, moment, frames };
    this.clips.set(key, clip);
    if (this.clips.size > MAX_CLIPS) this.clips.delete(this.clips.keys().next().value!);
    return clip;
  }
  clip(key: string): ReplayClip | undefined { return this.clips.get(key); }
}
...
export interface ReplayTimeline { clip: ReplayClip; /** Milliseconds into playback at which each frame is reached. */ at: number[]; playMs: number; totalMs: number; focus?: { x: number; y: number } }
...
export function buildTimeline(clip: ReplayClip): ReplayTimeline {
  ...
  for (let index = 1; index < clip.frames.length; index += 1) {
    const previous = clip.frames[index - 1]!, frame = clip.frames[index]!;
...
export function replayFrameAt(timeline: ReplayTimeline, elapsedMs: number): ReplayFrame {
  const { clip, at, playMs, focus } = timeline;
  const frames = clip.frames, first = frames[0]!, last = frames[frames.length - 1]!;
...
export function zoomOrigin(element: { width: number; height: number }, world: { width: number; height: number }, focus: { x: number; y: number }): { x: number; y: number } {
  const scale = Math.min(element.width / world.width, element.height / world.height);
  return { x: (element.width - world.width * scale) / 2 + focus.x * scale, y: (element.height - world.height * scale) / 2 + focus.y * scale };
}

/** The chyron for a clip, naming riders from the clip's own frames, with the protagonist's colour. */
export function describeClip(clip: ReplayClip): { card: MomentCard; color: string } {
  const frame = clip.frames[clip.frames.length - 1]!;
  const byId = new Map(frame.players.map(player => [player.id, player]));
  return { card: describeMoment(clip.moment, id => byId.get(id)?.name ?? id), color: byId.get(clip.moment.playerId)?.color ?? '#29dfff' };
}

export type ReplayCue = 'in' | 'impact' | 'out';
export interface ReplayUpdate extends ReplayFrame { cues: ReplayCue[]; clip: ReplayClip }

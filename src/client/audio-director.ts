...
export type RadioChoices = Pick<
  RadioState,
  "loopSong" | "loopPlaylist" | "source" | "playlist"
>;
...
export class AudioDirector {
  ...
  private musicPath = "";
  private settings = {
    music: { muted: false, volume: 0.22 },
    effects: { muted: false, volume: 0.45 },
  };
  ...
  constructor(
    private readonly synth: GameSynth,
    private readonly radio: RadioState = defaultRadio(),
    private readonly persist: (
      state: Readonly<RadioState>,
      kind: RadioSave,
    ) => void = () => {},
  ) {
  ...
  async unlock(confirm = false): Promise<boolean> {
    ...
    const ok = await this.synth.unlock();
    this.unlocked ||= ok;
    if (ok && confirm)
  ...
  get trackTitle(): string {
    return trackById(this.radio.track).title;
  }
  isMuted(channel: AudioChannel): boolean {
    return this.settings[channel].muted;
  }
  /** Live position while a track plays, otherwise the point it resumes from. */
  position(): number {
    return (
      (this.musicPath ? this.synth.position() : undefined) ??
      this.radio.position
    );
  }
  duration(): number | undefined {
    const live = this.musicPath ? this.synth.duration() : undefined;
    if (live !== undefined) this.knownDuration = live;
    return live ?? this.knownDuration;
  }
  /** True only while the media element is audibly playing this radio's track, whatever the effects context is doing. */
  audible(): boolean {
    return this.musicPath !== "" && this.synth.audible();
  }
  ...
  resume(): void {
    if (this.background) this.playBackground();
  }
  /**
   ...
   */
  setEffectsSilenced(value: boolean): void {
    this.silenced = value;
  }
  /** Next track, wrapping; also the old Next tune. */
  nextTrack(): void {
    this.select(followingTrack(this.radio, "next")!);
  }
  /** Restarts the song, or goes back a track when it has only just started. */
  previousTrack(): void {
    this.select(
      this.position() > RESTART_THRESHOLD
        ? this.radio.track
        : precedingTrack(this.radio),
    );
  }
  play(id: TrackId, source?: RadioSource): void {
    if (source) this.radio.source = source;
    this.select(id);
  }
  togglePause(): void {
  ...
  trackEnded(): void {
    ...
    this.synth.pauseMusic();
    this.musicPath = "";
    const following = followingTrack(this.radio, "ended");
    if (following) this.select(following);
    else this.select(radioQueue(this.radio)[0]!, true);
  }
  /**
  ...
  mediaPaused(): void {
    ...
    this.radio.position = this.position();
  ...
  mediaResumed(): void {
    ...
    this.radio.paused = false;
    this.update();
    this.changed();
  }
  setLoopSong(loop: boolean): void {
    this.radio.loopSong = loop;
    this.changed();
  }
  setLoopPlaylist(loop: boolean): void {
    this.radio.loopPlaylist = loop;
    this.changed();
  }
  setSource(source: RadioSource): void {
    this.radio.source = source;
    this.changed();
  }
  togglePlaylist(id: TrackId): void {
    this.radio.playlist = togglePlaylistTrack(this.radio.playlist, id);
    this.changed();
  }
  ...
  adoptChoices(choices: RadioChoices): void {
    this.radio.loopSong = choices.loopSong;
    this.radio.loopPlaylist = choices.loopPlaylist;
    this.radio.source = choices.source;
    this.radio.playlist = [...choices.playlist];
    this.emit();
  }
  ...
  setVolume(channel: AudioChannel, value: number): void {
    this.settings[channel].volume = Number.isFinite(value)
  ...
  save(): void {
    this.radio.position = this.position();
    this.persist(this.radio, "position");
  }
  private applyGain(channel: AudioChannel): void {
    const setting = this.settings[channel];
    this.synth.gain(channel, setting.muted ? 0 : setting.volume);
  }
  disconnect(): void {
    this.save();
    this.scope = "";
  ...
  message(message: ServerMessage): void {
    ...
    if (message.type === "snapshot") {
      if (message.matchId === this.matchId && message.round < this.round)
        return;
    ...
    if (
      scope !== this.scope ||
      message.tick <= this.baselineTick ||
  ...
  update(): void {
    ...
    if (path !== this.musicPath) {
      this.musicPath = path;
      this.synth.music(path, this.radio.position);
    }
  }
  private select(id: TrackId, paused = false): void {
    ...
    this.radio.track = id;
    this.radio.position = 0;
    this.radio.paused = paused;
    this.knownDuration = undefined;
    this.update();
    this.changed();
  }
  private changed(): void {
    this.radio.position = this.position();
    this.persist(this.radio, "all");
    this.emit();
  }
  private emit(): void {
  ...
  replayCue(kind: "in" | "impact" | "out"): void {
    ...
    switch (kind) {
      case "in":
        note(160, 1400, 0.42, "sawtooth");
        note(80, 700, 0.42, "triangle", 0, 0.12);
        note(1760, 1760, 0.1, "square", 0.4, 0.14);
        break;
      case "impact":
        note(220, 40, 0.45, "triangle", 0, 0.3);
        note(55, 30, 0.6, "sawtooth", 0, 0.18);
        note(1200, 300, 0.08, "square", 0, 0.12);
        break;
      case "out":
        note(1200, 180, 0.3, "sawtooth");
        note(880, 880, 0.06, "square", 0.28, 0.14);
        note(1320, 1320, 0.12, "square", 0.34, 0.16);
        break;
    }
  }
  private cue(type: string): void {
    ...
    switch (type) {
      ...
      case "bombPlaced":
        ...
        break;
      case "explosion":
        note(130, 28, 0.25, "sawtooth");
        note(68, 25, 0.3, "triangle");
        break;
      ...
      case "pickupCollected":
        note(660, 660, 0.08);
        note(990, 990, 0.1, "square", 0.08);
        note(1320, 1320, 0.13, "square", 0.16);
        break;
      case "roundEnded":
        note(523, 523, 0.12);
        note(659, 659, 0.12, "square", 0.12);
        note(784, 784, 0.22, "square", 0.24);
        break;
      case "matchEnded":
        note(784, 784, 0.2);
        note(1047, 1047, 0.35, "triangle", 0.2);
        break;
    }
  }
}

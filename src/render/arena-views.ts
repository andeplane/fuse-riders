import { wrapImages, type WrapOffset } from "../engine/view-kit.js";
 ...
 */
export function crossViews(
  worldWidth: number,
  worldHeight: number,
  screenWidth: number,
  screenHeight: number,
): ArenaView[] {
  const splitX = Math.round(screenWidth / 2),
    splitY = Math.round(screenHeight / 2);
  const columns = [
    { x: 0, width: splitX, scrollX: worldWidth / 2 },
    { x: splitX, width: screenWidth - splitX, scrollX: 0 },
  ];
  const rows = [
    { y: 0, height: splitY, scrollY: worldHeight / 2 },
    { y: splitY, height: screenHeight - splitY, scrollY: 0 },
  ];
...
export function crossScreenPoint(
  x: number,
  y: number,
  worldWidth: number,
  worldHeight: number,
  ...
  return {
    x: ((x + worldWidth / 2) % worldWidth) / worldWidth,
    y: ((y + worldHeight / 2) % worldHeight) / worldHeight,
  };
}

/** Every place something within `reach` of an open edge has to be drawn, its own first. */
export function edgeGhosts(
  width: number,
  height: number,
  x: number,
  y: number,

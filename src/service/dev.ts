...
import { createDevRoomService as createService, type DevRoomService, type DevRoomServiceOptions as NetworkOptions } from 'fuse-network-be';
...
export interface DevRoomServiceOptions extends Omit<NetworkOptions, 'httpExtension'> { identity?: IdentityVerifier }
...
export function createDevRoomService(options: DevRoomServiceOptions = {}): DevRoomService {
  ...
  return createService({ ...ROOM_LIMITS, ...options,
    httpExtension: store => createHistoryHttp(new HistoryStore(new MemoryHistoryDatabase(), store, now), options.identity ?? createIdentityVerifier(FIREBASE_PROJECT_ID)),
  });
}

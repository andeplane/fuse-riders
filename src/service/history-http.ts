...
export function createHistoryHttp(history: HistoryStore, identity: IdentityVerifier): HttpExtension {
  return {
    methods: ['PUT'], headers: ['X-Fuse-Identity'],
    async handle(req, res, clientAddress) {
      const url = new URL(req.url ?? '/', 'http://gateway');
      const route = url.pathname.match(/^\/api\/rooms\/([A-Z]{2}[0-9]{2})\/(results)$/);
      ...
      if (route?.[2] === 'results' && req.method === 'POST') {
        ...
        const reporter = await history.admit(route[1]!, bearer(req), clientAddress), body = await readJson(req);
        const header = req.headers['x-fuse-identity'], uid = typeof header === 'string' ? await identity(header) : undefined;
        json(await history.submit(reporter, body, uid)); return true;
      }
      if (url.pathname === '/api/me' && (req.method === 'GET' || req.method === 'PUT')) {
        const uid = await identity(bearer(req));
        if (!uid) { json({ error: 'Sign in first' }, 401); return true; }
        json(req.method === 'PUT' ? await history.rename(uid, await readJson(req)) : { profile: await history.profile(uid) ?? null }); return true;
      }
      if (url.pathname === '/api/me/matches' && req.method === 'GET') {
        ...
        if (cursor !== undefined && (!Number.isSafeInteger(cursor) || cursor < 0)) { json({ error: 'Invalid cursor' }, 400); return true; }
        json(await history.history(uid, cursor)); return true;
      }
      return false;
    },
  };

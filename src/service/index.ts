...
const firestore=new Firestore({projectId,databaseId:process.env.FIRESTORE_DATABASE_ID??'(default)',ignoreUndefinedProperties:true});
const pubsub=new PubSub({projectId,apiEndpoint:`${region}-pubsub.googleapis.com`});
...
const server=createServer(async(req,res)=>{
  ...
  try{
    ...
    if(url.pathname==='/api/rooms'&&req.method==='POST'){
      ...
      const code=randomBytes(5).toString('hex').toUpperCase(),token=randomBytes(32).toString('hex');await store.create(code,token);json({code,token},201);return;
    }
    const match=url.pathname.match(/^\/api\/rooms\/([A-Z0-9]{10})\/ice$/);
  ...
  }catch(error){json({error:error instanceof RoomError?error.message:'Room service unavailable'},error instanceof RoomError?error.status:503);}
});
const sockets=new WebSocketServer({noServer:true,maxPayload:32_000,perMessageDeflate:false});
server.on('upgrade',(req,socket,head)=>{
  const origin=req.headers.origin,url=new URL(req.url??'/','http://gateway'),match=url.pathname.match(/^\/api\/rooms\/([A-Z0-9]{10})\/ws$/),token=url.searchParams.get('token')??'';
  ...
  sockets.handleUpgrade(req,socket,head,ws=>{
    ...
    void gateway.connect(match[1],token,ws).then(id=>{connectionId=id;if(closed){void gateway.disconnect(id);return;}for(const data of pending)void gateway.receive(id,data);pending=[];}).catch(error=>{ws.close(error instanceof RoomError&&error.status===404?4004:4000,error instanceof RoomError?error.message:'Room service unavailable');});
  });
});
const port=Number(process.env.PORT??8080);server.listen(port,'0.0.0.0',()=>console.log(JSON.stringify({service:'fuse-riders-gateway',port,region,projectId,database:process.env.FIRESTORE_DATABASE_ID??'(default)'})));

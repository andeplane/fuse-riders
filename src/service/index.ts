...
export function startService(authClient?: AuthClient): void {
  startGcpRoomService({ serviceName: 'fuse-riders-gateway', defaultPrefix: 'fuse-preview', ...ROOM_LIMITS, ...(authClient ? { authClient } : {}),
    httpExtension: ({ store, firestore, prefix, projectId }) => createHistoryHttp(new HistoryStore(new FirestoreHistoryDatabase(firestore, prefix), store, Date.now), createIdentityVerifier(process.env.FIREBASE_PROJECT_ID ?? projectId)),
  });
}

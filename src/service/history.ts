...
const counter = (value: unknown, most = MAX_EVENTS): value is number => Number.isSafeInteger(value) && (value as number) >= 0 && (value as number) <= most;
...
function parsePlayer(raw: unknown, rounds: number): StoredPlayer | undefined {
  ...
  player.deathsByCause = Object.fromEntries(DEATHS.map(cause => [cause, deaths[cause]])) as unknown as MatchDeathCounts;
  return player;
}
...
export function parseMatchRecord(raw: unknown): MatchRecord | undefined {
  if (!plain(raw) || raw.version !== 1 || typeof raw.id !== 'string' || !/^[a-f0-9]{40}$/.test(raw.id) || typeof raw.roomCode !== 'string' || !validCode(raw.roomCode)) return;
  if (raw.status !== 'pending' && raw.status !== 'confirmed') return;
  const result = parseMatchResult(raw.result);
  if (!result || !counterLike(raw.createdAt) || (raw.endedAt !== undefined && !counterLike(raw.endedAt)) || (raw.expiresAt !== undefined && !counterLike(raw.expiresAt))) return;
  if ((raw.status === 'confirmed') !== (raw.endedAt !== undefined)) return;
  const humans = new Set(humansOf(result));
  if (!Array.isArray(raw.attesters) || raw.attesters.length > humans.size || !raw.attesters.every(id => typeof id === 'string' && humans.has(id))) return;
  if (!plain(raw.uidByPlayer) || !Object.entries(raw.uidByPlayer).every(([id, uid]) => humans.has(id) && validUid(uid))) return;
  if (!plain(raw.avatars) || !Object.entries(raw.avatars).every(([id, avatar]) => humans.has(id) && isAvatarId(avatar))) return;
  if (!Array.isArray(raw.participantUids) || raw.participantUids.length > humans.size || !raw.participantUids.every(validUid)) return;
  // Rebuilt rather than passed through, so a storage-only field (Firestore's cleanupAt) never rides along into a rewrite.
  return {
    version: 1, id: raw.id, roomCode: raw.roomCode, status: raw.status, result, attesters: [...raw.attesters] as string[],
    uidByPlayer: { ...raw.uidByPlayer } as Record<string, string>, avatars: { ...raw.avatars } as Record<string, AvatarId>, participantUids: [...raw.participantUids] as string[], createdAt: raw.createdAt,
    ...(raw.endedAt === undefined ? {} : { endedAt: raw.endedAt }), ...(raw.expiresAt === undefined ? {} : { expiresAt: raw.expiresAt }),
  };
}
const counterLike = (value: unknown): value is number => typeof value === 'number' && Number.isFinite(value) && value >= 0;

/** Where a record lives: the room's incarnation keeps a reused room code apart, the hash keeps differing results apart. */
export function matchRecordId(incarnation: string, result: MatchResult): string {
  return createHash('sha256').update(JSON.stringify([incarnation, result])).digest('hex').slice(0, 40);
}

function creditFor(uid: string, player: StoredPlayer, avatarId: AvatarId | undefined, at: number): Credit {
  return {
    uid, name: player.name, ...(avatarId === undefined ? {} : { avatarId }), at,
    totals: {
      matches: 1, wins: player.matchPlacement === 1 ? 1 : 0, roundWins: player.roundWins, eliminations: player.eliminations, bombsPlaced: player.bombsPlaced,
      pickupsCollected: player.pickupsCollected, survivalTicks: player.survivalTicks, distanceUnits: player.distanceUnits,
    },
  };
}

/** A seat proven by its room token. Only HistoryStore.admit makes one. */
export interface Reporter { code: string; rider: string; incarnation: string }
export type SubmitOutcome = { status: 'pending' | 'confirmed'; attestations: number; needed: number; linked: boolean };
/** What an account may see of a match: every rider's stats, and which seat was its own; never another rider's account. */
export interface HistoryEntry { id: string; endedAt: number; roomCode: string; you?: string; attestations: number; avatars: Record<string, AvatarId>; result: MatchResult }

export class HistoryStore {
  constructor(private database: HistoryDatabase, private rooms: RoomStore, private now: () => number) {}
  ...
  async admit(code: string, token: string, address: string): Promise<Reporter> {
    ...
    const room = await this.rooms.get(code), rider = peerId(token), member = room.members[rider];
    if (!member || member.expiresAt <= this.now()) throw new RoomError(403, 'Join the room first');
    const allowed = await this.rooms.database.allowance(digest(`results:${code}:${rider}`), this.now(), SUBMISSIONS_PER_HOUR)
      && await this.rooms.database.allowance(digest(`results-address:${address}`), this.now(), SUBMISSIONS_PER_ADDRESS_PER_HOUR);
  ...
  async submit(reporter: Reporter, body: unknown, uid: string | undefined): Promise<SubmitOutcome> {
    const { code, rider } = reporter;
    if (!plain(body) || !Object.keys(body).every(key => key === 'result' || key === 'avatarId') || (body.avatarId !== undefined && !isAvatarId(body.avatarId))) throw new RoomError(400, 'Invalid match result');
    const result = parseMatchResult(body.result), avatarId = body.avatarId;
    if (!result) throw new RoomError(400, 'Invalid match result');
    const needed = attestationsNeeded(result);
    if (!humansOf(result).includes(rider)) throw new RoomError(403, 'Only a rider in the match can report it');
    ...
    const account = uid !== undefined && await this.rooms.database.allowance(digest(`link:${uid}`), this.now(), LINKS_PER_HOUR) ? uid : undefined;
    const id = matchRecordId(reporter.incarnation, result), player = result.players.find(entry => entry.playerId === rider)!;
    return this.database.transactMatch(id, current => {
      const now = this.now();
      const match: MatchRecord = current ? structuredClone(current) : { version: 1, id, roomCode: code, status: 'pending', result, attesters: [], uidByPlayer: {}, avatars: {}, participantUids: [], createdAt: now, expiresAt: now + PENDING_TTL_MS };
      ...
      if (match.status === 'confirmed') {
        ...
        if (match.participantUids.length) delete match.expiresAt; else match.expiresAt = (match.endedAt ?? now) + GUEST_MATCH_TTL_MS;
      }
  ...
  async history(uid: string, before: number | undefined): Promise<{ profile: UserProfile | undefined; matches: HistoryEntry[] }> {
    ...
    const [profile, records] = await Promise.all([this.database.profile(uid), this.database.matchesFor(uid, before, HISTORY_PAGE)]);
    return {
      profile,
      matches: records.map(record => {
        const you = Object.entries(record.uidByPlayer).find(([, account]) => account === uid)?.[0];
        return { id: record.id, endedAt: record.endedAt ?? record.createdAt, roomCode: record.roomCode, ...(you ? { you } : {}), attestations: record.attesters.length, avatars: record.avatars, result: record.result };
      }),
    };
  }
}

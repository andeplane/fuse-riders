...
export function trailSegmentsConnect(
  a: TrailSegment,
  b: TrailSegment,
): boolean {
  return (
    a.detached?.id === b.detached?.id &&
    b.createdTick === a.createdTick + 1 &&
    a.x2 === b.x1 &&
    a.y2 === b.y1
  );
}
...
function detachRuns(
  ...
  decayStartTick: number,
  allocate: AllocatePiece,
): TrailSegment[] {
  let detached: TrailSegment["detached"];
  return segments.map((segment, index) => {
    if (!index || !trailSegmentsConnect(segments[index - 1]!, segment))
      detached = { id: allocate(), decayStartTick };
    return { ...segment, detached };
  });
}

/** Death preserves every existing piece's clock, including debris detached this tick. */
export function detachTrail(
  ...
  const flush = () => {
    result.push(
      ...detachRuns(active, tick + TRAIL_DECAY_PAUSE_TICKS, allocate),
    );
    active = [];
  };
...
export function cutTrail(
  trail: readonly TrailSegment[],
  tick: number,
  cut: (segment: TrailSegment) => readonly TrailSegment[],
  allocate: AllocatePiece,
): TrailSegment[] {
  const chunks: TrailSegment[][] = [];
  let chunk: TrailSegment[] = [],
    changed = false;
  const flush = () => {
  ...
  for (const segment of trail) {
    ...
    if (
      parts.length !== 1 ||
      parts[0]!.x1 !== segment.x1 ||
      parts[0]!.y1 !== segment.y1 ||
      parts[0]!.x2 !== segment.x2 ||
      parts[0]!.y2 !== segment.y2
    )
      changed = true;
...
export function erodeTrailPiece(
  piece: readonly TrailSegment[],
  distance: number,
): TrailSegment[] {
  ...
  for (let i = 0; i < piece.length; i++) {
    const segment = piece[i]!,
      length = lengths[i]!;
    const from = Math.max(0, distance - offset),
      to = Math.min(length, total - distance - offset);
    if (length === 0 && offset > distance && offset < total - distance)
      result.push(segment);
    if (length > 0 && to > from) {
      const at = (a: number, b: number, d: number) =>
        a + (b - a) * (d / length);
      result.push({
        ...segment,
...
export function advanceTrail(
  ...
  for (let i = 0; i < trail.length;) {
    const first = trail[i]!;
    if (!first.detached) {
      if (first.expiresAtTick > tick) result.push(first);
      i++;
      continue;
    }
    let end = i + 1;
    while (end < trail.length && trail[end]!.detached?.id === first.detached.id)
      end++;
    const piece = trail.slice(i, end);
    result.push(
      ...(tick > first.detached.decayStartTick
        ? erodeTrailPiece(
            piece,
            TRAIL_DECAY_PER_TICK *
              Math.max(
                0,
                tick - Math.max(previousTick, first.detached.decayStartTick),
              ),
          )
        : piece),
    );
...
export function boundTrail(trail: TrailSegment[]): TrailSegment[] {
  ...
  return trail
    .filter((segment) => !(segment.detached && excess-- > 0))

...
export function validRiderName(value: unknown): value is string {
  return typeof value === 'string' && value.length > 0 && value === value.trim() && Array.from(value).length <= MAX_RIDER_NAME
    && !/[\u0000-\u001f\u007f]/.test(value) && !/\p{Cs}/u.test(value);
}
...
export function suggestRiderName(text: string): string {
  const cleaned = text.replace(/[\u0000-\u001f\u007f]/g, '').replace(/\p{Cs}/gu, '').trim(), first = cleaned.split(/\s+/)[0] ?? '';

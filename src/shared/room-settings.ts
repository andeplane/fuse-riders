...
export function defaultRoomSettings(): RoomSettings {
  return { version: 1, mode: 'devices', match: 'wins', length: 3, bombChargeTicks: BOMB_MAX_CHARGE_TICKS, chainReaction: true, weights: Object.fromEntries(PICKUP_WEIGHTS.map(row => [row.type, row.weight])) };
}
export function parseRoomSettings(raw: unknown): RoomSettings | undefined {
  ...
  return { version: 1, mode: value.mode, match: value.match, length: value.length, bombChargeTicks, chainReaction, weights };
}

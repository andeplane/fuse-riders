 ...
 */
export type MomentKind = 'multiKill' | 'directHit' | 'trickShot' | 'cutOff' | 'boxedIn' | 'bombDodge' | 'ownGoal' | 'mutualDestruction';
export const MOMENT_KINDS: readonly MomentKind[] = ['multiKill', 'directHit', 'trickShot', 'cutOff', 'boxedIn', 'bombDodge', 'ownGoal', 'mutualDestruction'];
...
export interface DodgeObservation { playerId: PlayerId; ownerId: PlayerId; clearance: number }
export interface TickObservations { deaths: DeathObservation[]; dodges: DodgeObservation[] }

export function pushMoment(state: GameState, moment: Moment): boolean {
  let kept = 0;
  for (const existing of state.moments) if (existing.kind === moment.kind) kept += 1;
...
export function detectMoments(state: GameState, elapsed: number, observations: TickObservations): void {
  ...
  for (const death of observations.deaths) {
    ...
    if (victims) victims.push(death.victimId); else kills.set(owner, [death.victimId]);
    if (death.cause === 'explosion') {
      if (death.landingHit === owner) record('directHit', owner, [death.victimId], 1);
      const shell = death.shellHit;
      if (shell && shell.ownerId === owner && shell.bounces >= 1 && shell.age <= TRICK_SHOT_MAX_AGE_TICKS) record('trickShot', owner, [death.victimId], shell.bounces);
    } else if (death.cause === 'trail' && death.trailAge !== undefined) {
      if (death.trailAge <= CUT_OFF_MAX_AGE_TICKS) record('cutOff', owner, [death.victimId], death.trailAge);
      ...
      const origin = victim && victim.drunkUntilTick <= tick ? victim.trail.find(segment => segment.createdTick === tick - BOXED_IN_LOOKBACK_TICKS) : undefined;
      if (origin) {
        const displacement = hypot2(death.x - origin.x1, death.y - origin.y1);
        if (displacement < BOXED_IN_MAX_DISPLACEMENT) record('boxedIn', owner, [death.victimId], Math.round(displacement));
      }
    }
  }
  for (const [owner, victims] of kills) if (victims.length >= 2) record('multiKill', owner, bySlot(victims), victims.length);
  ...
  if (observations.deaths.length >= 2 && observations.deaths.some(death => death.owners.length > 0) && ![...state.players.values()].some(player => player.alive)) {

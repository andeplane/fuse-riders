...
export function createPortalPair(options: PortalPlacementOptions): PortalPair | undefined {
  ...
  const halfLength = Math.min(PORTAL_MAX_LENGTH, (bounds.maxY - bounds.minY) / 3) / 2;
  ...
  if (width < PORTAL_MIN_SEPARATION || height < 0 || halfLength <= 0) return undefined;
  const candidates: PortalGate[] = [];
  for (let attempt = 0; attempt < PORTAL_PLACEMENT_ATTEMPTS; attempt += 1) {
    const gate = { x: minX + random() * width, y: minY + random() * height, halfLength };
    ...
    for (let sample = 0; sample <= intervals; sample += 1) {
      if (!isSafe({ x: gate.x, y: gate.y - halfLength + sample * spacing }, clearance + spacing / 2)) { safe = false; break; }
    }
    if (!safe) continue;
    const partner = candidates.find(other => Math.abs(other.x - gate.x) >= PORTAL_MIN_SEPARATION);
    if (partner) return { id: options.id, gates: [partner, gate], expiresAtTick: options.tick + PORTAL_LIFETIME_TICKS };
...
export function fitPortalPair(pair: PortalPair, bounds: PortalBounds, riderRadius: number): PortalPair | undefined {
  const margin = PORTAL_WALL_HALF_WIDTH + riderRadius + 1;
  const maximumLength = Math.min(PORTAL_MAX_LENGTH, (bounds.maxY - bounds.minY) / 3);
  const gates: PortalGate[] = [];
  for (const gate of pair.gates) {
    if (gate.x < bounds.minX + margin || gate.x > bounds.maxX - margin) return undefined;
    ...
    gates.push({ x: gate.x, y: (top + bottom) / 2, halfLength: Math.min(bottom - top, maximumLength) / 2 });
  }
  return { ...pair, gates: [gates[0]!, gates[1]!] };
}

function circleEntry(from: PortalPoint, to: PortalPoint, center: PortalPoint, radius: number): number | undefined {
...
function entryFraction(from: PortalPoint, to: PortalPoint, gate: PortalGate, radius: number): number | undefined {
  const top = gate.y - gate.halfLength; const bottom = gate.y + gate.halfLength;
  const closestY = Math.max(top, Math.min(bottom, from.y));
  if ((from.x - gate.x) ** 2 + (from.y - closestY) ** 2 <= radius ** 2) return undefined;

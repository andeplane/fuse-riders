...
test('Fuse stacks twice, applies on the collection tick, explodes exactly on time and resets next round', () => {
  ...
  assert.equal(bombFuseTicks(-1), 40); assert.equal(bombFuseTicks(3), 20);
  for (const duration of [30, 20, 20]) {
    collect(game, 'stopwatch'); step(game, fire);
    const bomb = [...game.bombs.values()][0]!;
    assert.equal(bomb.explodeAtTick - bomb.launchedTick, duration);
    assert.equal(toSnapshot(game).players[0]!.fuseLevel, duration === 30 ? 1 : 2);
...
test('collecting Fuse leaves launched bombs and active reloads unchanged', () => {
  const game = playing(), rider = game.players.get('p0')!;
  step(game, fire);
  const bomb = [...game.bombs.values()][0]!, readyAt = rider.bombReadyAtTick;
  collect(game, 'stopwatch', 'stopwatch'); step(game, new Map());
  assert.equal(rider.fuseLevel, 2);
...
test('checkpoint recovery retains Fuse during charging and replays explosions exactly', () => {
  const game = playing(); collect(game, 'stopwatch', 'stopwatch', 'extraBomb', 'gravity'); step(game, commands('press'));
...
test('checkpoint rejects invalid or missing Fuse levels; settings enable and configure the pickup', () => {
  const game = playing(), rider = game.players.get('p0')!;
  for (const invalid of [-1, .5, 3, NaN]) {
    rider.fuseLevel = invalid; assert.equal(decodeGameState(encodeGameState(game)), undefined);
  }
  rider.fuseLevel = 0;
  const valid = encodeGameState(game);
  assert.equal(decodeGameState(valid.replace(/"fuseLevel":0,/g, '')), undefined);
  ...
  assert.equal(roomPickup(.5, { stopwatch: 1 }), 'stopwatch');
  assert.equal(roomPickup(.5, { stopwatch: 0 }), undefined);
});

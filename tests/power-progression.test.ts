...
test('trail capacity starts at six seconds and grows by 1.5 seconds per diamond up to the ceiling', () => {
  assert.equal(powerTrailLifetimeTicks(0), 120);
  assert.equal(powerTrailLifetimeTicks(4), 240);
  assert.equal(powerTrailLifetimeTicks(8), 360);
  for (let count = 1; count <= 30; count++) {
    assert.equal(powerTrailLifetimeTicks(count) - powerTrailLifetimeTicks(count - 1), 30);
  }
  assert.equal(powerTrailLifetimeTicks(30), 1020);
  assert.equal(powerTrailLifetimeTicks(31), MAX_CHECKPOINT_TRAILS);
  assert.equal(powerTrailLifetimeTicks(32), MAX_CHECKPOINT_TRAILS);
...
test('a saturated trail grows immediately with each collected diamond and expires at the extended deadline', () => {
  ...
  for (let i = 0; i < 120; i++) step(game, new Map());
  assert.equal(player.trail.length, 120);
  ...
  assert.equal(player.trail.length, 121, 'only the segment expired before collection is lost');
  assert.ok(!player.trail.some(segment => segment.createdTick === expired.createdTick));
  assert.equal(player.trail[0]!.expiresAtTick, originalDeadline + 60);
  assert.equal(player.trail.at(-1)!.expiresAtTick, game.tick + 180);
  const restored = decodeGameState(encodeGameState(game)); assert.ok(restored);
  while (game.tick < originalDeadline + 59) {
    assert.deepEqual(step(restored, new Map()), step(game, new Map()));
  }
  assert.equal(player.trail.length, 180);
...
test('an extended trail remains a collision obstacle past its old expiry', () => {
  ...
  player.trail = [{ x1: 900, y1: 500, x2: 1000, y2: 500, createdTick: game.tick - 118, expiresAtTick: game.tick + 2 }];
  game.pickups = [{ id: game.nextPickupId++, type: 'power', x: player.x, y: player.y, expiresAtTick: game.tick + 100 }];
  step(game, new Map());
...
test('trail growth resets with the round and does not extend an eliminated rider', () => {
  ...
  step(game, new Map());
  assert.equal(player.trail.at(-1)!.expiresAtTick, game.tick + 210);
  ...
  assert.equal(player.trail.at(-1)!.expiresAtTick, game.tick + 120);
});

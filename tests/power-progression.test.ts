...
test('trail capacity starts shorter and grows by the same amount per diamond', () => {
  assert.equal(powerTrailLifetimeTicks(0), 60);
  assert.equal(powerTrailLifetimeTicks(20), 160, 'twenty diamonds reach the old eight-second length');
  for (let count = 1; count <= 100; count++) {
    assert.equal(powerTrailLifetimeTicks(count) - powerTrailLifetimeTicks(count - 1), 5);
  }
...
test('a saturated trail grows immediately with each collected diamond and expires at the extended deadline', () => {
  ...
  assert.equal(player.trail.length, 60);
  ...
  assert.equal(player.trail.length, 61, 'only the segment expired before collection is lost');
  assert.ok(!player.trail.some(segment => segment.createdTick === expired.createdTick));
  assert.equal(player.trail[0]!.expiresAtTick, originalDeadline + 10);
  assert.equal(player.trail.at(-1)!.expiresAtTick, game.tick + 70);
  const restored = decodeGameState(encodeGameState(game)); assert.ok(restored);
  while (game.tick < originalDeadline + 9) {
    assert.deepEqual(step(restored, new Map()), step(game, new Map()));
  }
  assert.equal(player.trail.length, 70);
...
test('an extended trail remains a collision obstacle past its old expiry', () => {
  ...
  player.trail = [{ x1: 900, y1: 500, x2: 1000, y2: 500, createdTick: game.tick - 58, expiresAtTick: game.tick + 2 }];
  game.pickups = [{ id: game.nextPickupId++, type: 'power', x: player.x, y: player.y, expiresAtTick: game.tick + 100 }];
  step(game, new Map());
...
test('trail growth resets with the round and does not extend an eliminated rider', () => {
  ...
  step(game, new Map());
  assert.equal(player.trail.at(-1)!.expiresAtTick, game.tick + 75);
  ...
  assert.equal(player.trail.at(-1)!.expiresAtTick, game.tick + 60);
});

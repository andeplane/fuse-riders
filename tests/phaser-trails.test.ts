...
import { TrailHistoryCache, trailPaths, trailTip } from '../src/client/phaser/trails.js';

const segment = (tick: number, x1: number, y1: number, x2: number, y2: number): TrailSegment =>
  ({ x1, y1, x2, y2, createdTick: tick, expiresAtTick: tick + 160 });
const rider = () => {
  const game = createGame('trail-test', 42);
  addPlayer(game, { id: 'p', name: 'Player', slot: 0, color: '#22d3ee' });
  return { ...toSnapshot(game).players[0]!, alive: true, x: 25, y: 10, portalCooldownUntilTick: 0,
    trail: [segment(9, 0, 0, 10, 0), segment(10, 10, 0, 20, 10)] };
};

test('a continuous bend is one stroke with the exact supplied vertices', () => {
  const trail = [segment(1, 0, 0, 10, 0), segment(2, 10, 0, 20, 10), segment(3, 20, 10, 20, 20)];
  assert.deepEqual(trailPaths(trail), [[{ x: 0, y: 0 }, { x: 10, y: 0 }, { x: 20, y: 10 }, { x: 20, y: 20 }]]);
});

test('portal jumps, deleted ticks and clipped segments remain separate strokes', () => {
  const trail = [segment(1, 0, 0, 10, 0), segment(2, 100, 0, 110, 0),
    segment(4, 110, 0, 120, 0), segment(5, 125, 0, 130, 0)];
  assert.deepEqual(trailPaths(trail), trail.map(t => [{ x: t.x1, y: t.y1 }, { x: t.x2, y: t.y2 }]));
...
test('history survives fractional motion, a changing predicted tip and equivalent deserialized snapshots', () => {
  ...
  const moved = { ...player, x: 27, trail: [structuredClone(player.trail[0]!), { ...player.trail[1]!, x2: 27 }] };
  ...
  assert.equal(cache.update([structuredClone(moved)], 'epoch:match:round1').changed, false);
  assert.deepEqual(player, rider(), 'presentation must not mutate supplied state');
});
...
test('history refreshes for trail holes, clipping, expiry, death, identity, scope and reset', () => {
  ...
  for (const changed of [
    ...
    { ...player, trail: [{ ...player.trail[0]!, x2: 9 }, ...player.trail.slice(1)] },
    { ...player, alive: false }, { ...player, id: 'replacement' }, { ...player, color: '#ff0000' },
  ]) {
    const cache = new TrailHistoryCache();
    cache.update([player], 'match:1');
    assert.equal(cache.update([changed], 'match:1').changed, true);
  }
  const cache = new TrailHistoryCache(); cache.update([player], 'match:1');
  assert.equal(cache.update([player], 'match:2').changed, true);
  assert.deepEqual(cache.update([], 'match:2').strokes, []);
  cache.update([player], 'match:2'); cache.reset();
  assert.equal(cache.update([player], 'match:2').changed, true);
});

test('a fresh trail follows the interpolated head by at most one simulation step', () => {
  const player = rider();
  assert.deepEqual(trailTip(player, 10.5, 'playing'), [{ x: 10, y: 0 }, { x: 20, y: 10 }, { x: 25, y: 10 }]);
  assert.deepEqual(trailTip({ ...player, x: 27.5 }, 10.9, 'playing').at(-1), { x: 27.5, y: 10 });
  const predicted = { ...player, trail: [...player.trail, segment(10, 20, 10, 25, 10)] };
  assert.deepEqual(trailTip(predicted, 10.5, 'playing'), [{ x: 20, y: 10 }, { x: 25, y: 10 }]);
});

test('head interpolation never extends a stale, destroyed, teleported or dead trail', () => {
  const player = rider(), original = [{ x: 10, y: 0 }, { x: 20, y: 10 }];
  ...
  assert.deepEqual(trailTip({ ...player, x: 60 }, 10.5, 'playing'), original, 'farther than a fully ramped stride');
  // A plain rider's longest stride is 7.5 × 1.5 ramp = 11.25 units: the tip follows up to it and no further.
  assert.deepEqual(trailTip({ ...player, x: 31.2 }, 10.5, 'playing').at(-1), { x: 31.2, y: 10 });
  ...
  assert.deepEqual(trailTip({ ...nitro, x: 64.9 }, 10.5, 'playing').at(-1), { x: 64.9, y: 10 });

...
import {
  addPlayer, createGame, startMatch, startNextRound, step, eliminatePlayer, toSnapshot,
  COUNTDOWN_TICKS, ROUND_DRAW_TICK, SLOT_COLORS, type GameState, type PickupType,
...
function playing(): GameState {
  ...
  for (const [i, player] of [...game.players.values()].entries()) {
    player.x = 500 + 700 * i; player.y = 700; player.angle = 0; player.trail = [];
  }
  game.nextPickupSpawnTick = Number.MAX_SAFE_INTEGER;
  return game;
}

function pickup(game: GameState, x: number, y: number, type: PickupType = 'power'): number {
...
test('spawned pickups survive the old timeout and late round, then reset with the round', () => {
  ...
  if (game.phase === 'playing') { eliminatePlayer(game, 'p1'); step(game, new Map()); }
...
test('blast destruction is strictly inside 60% of the actual radius, using center distance', () => {
  for (const radius of [50, 100, 250]) {
    ...
    pickup(game, 800 + edge - .01, 400, 'star');
    const boundary = pickup(game, 800 + edge, 400, 'extraBomb');
    const fringe = pickup(game, 800 + edge + .01, 400, 'target');
    const diagonal = pickup(game, 800 + radius * .5, 400 + radius * .5);
    const outside = pickup(game, 800 + radius + 1, 400);
    bomb(game, 800, 400, radius);
    const result = step(game, new Map());
    assert.deepEqual(result.snapshot.pickups.map(p => p.id), [boundary, fringe, diagonal, outside]);
    assert.equal(result.events.filter(e => e.type === 'pickupCollected').length, 0);
...
test('chained and gravity bombs destroy pickups and replay identically from a checkpoint', () => {
  ...
  assert.deepEqual(game.pickups.map(p => p.id), [survivor]);
...
test('Target blasts destroy pickups on their release tick', () => {
  ...
  pickup(game, 800, 450);
  const result = step(game, new Map([['p0', { left: false, right: false, bomb: false,
    bombCommands: [{ action: 'press', aim: { x: .5, y: .5 } }, { action: 'release', aim: { x: .5, y: .5 } }] }]]));
...
test('collection wins before a same-tick explosion and destruction frees a spawn slot', () => {
  ...
  const result = step(game, new Map());
  assert.equal(result.events.filter(e => e.type === 'pickupCollected').length, 1);

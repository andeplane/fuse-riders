...
test('arrows share held state and Space emits exactly one press and release despite repeats',()=>{const f=fixture();f.keyboard.down(f.event('ArrowLeft'));f.keyboard.down(f.event('Space'));f.keyboard.down(f.event('Space',true));f.keyboard.down(f.event('Space'));f.keyboard.up(f.event('Space'));f.keyboard.up(f.event('Space'));f.keyboard.up(f.event('ArrowLeft'));assert.deepEqual(f.messages.map(m=>m.bombAction),[undefined,'press','release','release'],'later packets restate the finished gesture');assert.equal(f.state.hasHeld(),false);});

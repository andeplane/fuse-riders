...
import {
  ...
  type MediaTrack,
} from "../src/client/radio-media-session.ts";

const artwork = [
  { src: "/radio-artwork.png", sizes: "512x512", type: "image/png" },
];
function fixture(
  state: {
    title: string;
    ...
    position: number;
    duration?: number;
  } = { title: "Pixel Sax Parade", playing: true, position: 12, duration: 180 },
) {
  const calls: string[] = [];
  const metadata: MediaTrack[] = [];
...
test("binding publishes the track, playback state and position, and only republishes what changed", () => {
  ...
  assert.deepEqual(f.positions, [
    { duration: 180, position: 12, playbackRate: 1 },
  ]);
  f.state.position = 30;
  f.emit();
  assert.equal(f.metadata.length, 1, "same track is not republished");
  assert.equal(f.calls.length, 2, "same state is not republished");
  assert.deepEqual(f.positions.at(-1), {
    duration: 180,
    position: 30,
    playbackRate: 1,
  });
  f.state.title = "Coin Op Swing";
  f.state.playing = false;
  f.state.position = 200;
  f.emit();
  assert.equal(f.metadata.at(-1)!.title, "Coin Op Swing");
  assert.equal(f.calls.at(-1), "state:paused");
  assert.deepEqual(
    f.positions.at(-1),
    { duration: 180, position: 180, playbackRate: 1 },
...
test("an unknown length clears the position once, and a known one publishes it again", () => {
  const f = fixture();
  f.state.duration = undefined;
  f.emit();
  f.emit();
  assert.deepEqual(
    f.positions.slice(1),
    [undefined],
    "cleared once, not on every refresh",
  );
  f.state.duration = NaN;
  f.emit();
...
test("play unmutes and resumes, pause only pauses, and prev/next go to the radio", () => {
  ...
  actions.play();
  assert.deepEqual(calls, ["unmute", "toggle", "unlock"]);
  ...
  assert.deepEqual(
    calls.slice(3),
    ["unlock"],
    "play on a playing radio does not toggle it off",
  );
  actions.pause();
  actions.pause();
  assert.deepEqual(
    calls.slice(4),
    ["toggle"],
    "a second pause does not start the music again",
  );
  actions.previous();
  actions.next();
  assert.deepEqual(calls.slice(5), ["previous", "next"]);
});

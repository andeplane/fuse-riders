 ...
 */
function fixture(riders = 2, settings?: RoomSettings): { game: GameState; player: PlayerState; victim: PlayerState; input: (intent: Partial<InputIntent>) => void } {
  ...
  return {
    game, player, victim: game.players.get('p1')!,
    input: (intent) => step(game, new Map([['p0', { left: false, right: false, bomb: false, ...intent }]])),
  };
}
...
const shots = (game: GameState, playerId: string) => tally(game.shots.filter(shot => shot.shooterId === playerId).map(shot => shot.weapon));
const kills = (game: GameState, playerId: string) =>
  tally(game.shots.filter(shot => shot.shooterId === playerId).flatMap(shot => shot.kills.map(() => shot.weapon)));

/** A pull of `weapon` whose one bomb is already due, so its blast opens on the next step wherever it is put. */
function dueBomb(game: GameState, id: number, ownerId: string, x: number, y: number, weapon: Weapon, landing = false): void {
  const bomb: BombState = {
    id, ownerId, launchX: x, launchY: y, x, y, placedTick: game.tick, launchedTick: game.tick - 1,
    landsAtTick: landing ? game.tick + 1 : game.tick - 1, explodeAtTick: landing ? game.tick + 99 : game.tick + 1,
    blastRange: BOMB_BLAST_RANGE, flightPath: Array.from({ length: BOMB_FLIGHT_TICKS + 1 }, () => ({ x, y, angle: 0 })), shot: id,
  };
  game.bombs.set(id, bomb);
  game.shots.push({ shot: id, shooterId: ownerId, weapon, elapsed: 0, bombs: 1, power: 0, extraBombs: 0, fuseLevel: 0, grip: false, kills: [] });
...
test('every launch logs one shot under the powerup it spent, and its bombs name that shot', () => {
  ...
  for (const [armed, weapon, launched] of cases) {
    ...
    assert.deepEqual(shots(game, 'p0'), { [weapon]: 1 }, `${JSON.stringify(armed)} is one ${weapon} shot`);
    ...
    assert.equal(game.matchStats.get('p0')!.bombsPlaced, launched, 'bombsPlaced still counts bombs, not pulls');
    ...
    for (const bomb of bombs) assert.equal(bomb.shot, game.shots[0]!.shot, 'every bomb of the pull names the one shot');
...
test('a Target bomb released onto a rider is one shot and one kill for Target, end to end', () => {
  ...
  victim.trail = [];
  input({ bomb: true, bombCommands: [{ action: 'press' }] });
  input({ bombCommands: [{ action: 'release', aim: { x: victim.x / game.width, y: victim.y / game.height } }] });
  ...
  assert.equal(game.matchStats.get('p0')!.eliminations, 1, 'a weapon kill is one of the killer eliminations');
...
test('a blast and a direct landing both credit the weapon that fired the bomb', () => {
  for (const landing of [false, true]) {
    for (const weapon of ['bomb', 'triple', 'five', 'shell'] as const) {
      ...
      step(game, new Map());
      assert.equal(victim.alive, false, `${weapon} ${landing ? 'landing' : 'blast'} killed`);
      assert.deepEqual(kills(game, 'p0'), { [weapon]: 1 }, `${weapon} by ${landing ? 'landing' : 'blast'}`);
      assert.equal(game.shots[0]!.kills[0]!.victimId, 'p1', 'logged against the pull, naming the victim');
    }
  }
});
...
test('a rider that blows itself up records the death and credits the weapon to nobody', () => {
  ...
  assert.deepEqual(kills(game, 'p0'), {}, 'an own goal is not a kill for the weapon that caused it');
...
test('two riders blasting the same victim credit neither an elimination nor a weapon', () => {
  ...
  for (const id of ['p0', 'p1']) {
    assert.deepEqual(kills(game, id), {}, `${id} shares the blame, so it gets no credit`);
...
test('one owner, two weapons on the same victim: the lowest bomb id decides, so every replica agrees', () => {
  for (const [first, second] of [['triple', 'five'], ['five', 'triple']] as const) {
    ...
    assert.equal(game.matchStats.get('p1')!.eliminations, 1, 'still exactly one elimination');
  }
});
...
test('a shot that kills nobody stays a shot: the miss half of the measure, end to end', () => {
  ...
  assert.equal(game.matchStats.get('p0')!.bombsExploded, 1, 'the bomb did go off');
  assert.ok(player.alive && victim.alive, 'and caught nobody');
  assert.deepEqual(shots(game, 'p0'), { bomb: 1 }, 'the pull is counted all the same');
...
test('a chained bomb credits its own owner and its own weapon, as its elimination always has', () => {
  ...
  step(game, new Map());
  assert.equal(victim.alive, false);
  assert.deepEqual(kills(game, 'p1'), { five: 1 }, 'the bomb that reached the victim was p1 Five');
...
test('a decided round keeps its log until the next is decided, through a rematch, and leaves out interrupted pulls', () => {
  ...
  const { game, player, victim, input } = fixture(2, { ...defaultRoomSettings(), match: 'rounds', length: 2 });
  ...
  const strike = () => {
    ...
    player.targetBombArmed = true;
    game.bombs.forEach(bomb => { bomb.ownerId = 'p0'; bomb.shell = { vx: 0, vy: 0 }; }); // a shell does not block the next pull
    input({ bomb: true, bombCommands: [{ action: 'press' }] });
    input({ bombCommands: [{ action: 'release', aim: { x: victim.x / game.width, y: victim.y / game.height } }] });
  };
  assert.equal(decided(), undefined, 'nothing is published before any round is decided');
  strike();
  ...
  const first = decided()!;
  assert.deepEqual([first.round, first.tick], [1, game.tick], 'stamped with its round and the tick it was decided at');
  assert.deepEqual(first.shots.map(shot => [shot.weapon, shot.kills.map(kill => kill.victimId)]), [['target', ['p1']]],
    'the bomb still in the air is left out: the round ended on it, it did not miss');
  game.tick = game.phaseEndsAtTick!;
  startNextRound(game);
  assert.deepEqual(game.shots, [], 'the next round starts an empty log, because bomb ids restart with it');
  assert.deepEqual(decided(), first, 'but the decided round stays readable for the whole next round');
  while (phase() === 'countdown') step(game, new Map());
  strike();
  assert.equal(phase(), 'matchOver', 'the second round decided the match');
  assert.equal(decided()!.round, 2, 'the deciding round replaces it at matchOver');
  const second = decided();
  resetMatch(game, 'weapons-2');
  assert.deepEqual(decided(), second, 'a rematch keeps it, so a device still waiting on confirmation can report it');
...
test('a bomb naming no shot kills without a kill being logged against a guess', () => {
  ...
  step(game, new Map());
  assert.equal(victim.alive, false);
  assert.equal(game.matchStats.get('p0')!.eliminations, 1, 'the elimination is still credited');
  assert.deepEqual(kills(game, 'p0'), {}, 'but nothing is logged against a guess');
});
...
test('a decided round becomes one Kill per kill and one Miss per miss, from the shooter alone', () => {
  ...
  for (const bomb of game.bombs.values()) Object.assign(bomb, { x: 800, y: 860, landsAtTick: game.tick, explodeAtTick: game.tick + 1 });
  input({});
  ...
  Object.assign(player, { powerPickups: 3, extraBombs: 1, fuseLevel: 2, grip: true });
  input({ bomb: true, bombCommands: [{ action: 'press' }] });
  input({ bombCommands: [{ action: 'release', aim: { x: victim.x / game.width, y: victim.y / game.height } }] });
  assert.equal(victim.alive, false);
  Object.assign(player, { powerPickups: 9, extraBombs: 4, fuseLevel: 0, grip: false });
  third.gunArmed = true; Object.assign(third, { x: 1200, y: 100, angle: 0 });
  step(game, new Map([['p2', { left: false, right: false, bomb: true, bombCommands: [{ action: 'press' }] }]]));
  step(game, new Map([['p2', { left: false, right: false, bomb: false, bombCommands: [{ action: 'release' }] }]]));
  // p2 leaves, which decides the one-round match.
  eliminatePlayer(game, 'p2'); step(game, new Map());
  ...
  assert.deepEqual(mine.map(entry => entry.event), ['Miss', 'Kill'], 'one event per pull outcome, in pull order');
  assert.deepEqual(mine[0]!.properties, { weapon: 'bomb', round: 1, secondsIntoRound: mine[0]!.properties.secondsIntoRound,
    bombs: 1, power: 0, extraBombs: 0, fuseLevel: 0, grip: false, riders: 3, bots: 0 });
  assert.deepEqual(mine[1]!.properties, {
    weapon: 'target', round: 1, secondsIntoRound: mine[1]!.properties.secondsIntoRound,

...
test('clock diagnostics are read-only and cannot qualify or refresh a sample',()=>{const f=fixture();assert.equal(f.clock.diagnostics(),undefined);f.sample(0);f.sample(500);f.sample(1000);f.set(1100);const before=f.clock.diagnostics()!;assert.equal(before.rttMs,10);assert.equal(before.sampleAgeMs,100);assert.equal(before.nearbySamples,3);before.scope.round=99;f.set(1200);assert.equal(f.clock.diagnostics()!.scope.round,1);assert.equal(f.clock.diagnostics()!.sampleAgeMs,200);assert.equal(f.clock.diagnostics()!.nearbySamples,3);});

test('measured nearby boundary accepts exactly40ms but not40ms plus epsilon',()=>{const f=fixture();for(const at of [0,500,1000])f.sample(at,40);assert.equal(f.clock.presentationDelayTicks(),1);f.sample(1500,40.001);assert.equal(f.clock.presentationDelayTicks(),2);for(const at of [2000,2500,3000])f.sample(at,60);assert.equal(f.clock.presentationDelayTicks(),2);});

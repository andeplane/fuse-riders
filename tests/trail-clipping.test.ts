...
test('overtime clipping preserves portal entry and exit discontinuity', () => {
  const state = overtimeArena(); const player = state.players.get('p0')!;
  Object.assign(player, { x: 165, y: 200, angle: 0 });
  state.portalPair = { id: 'pair', gates: [{ x: 200, y: 200 }, { x: 1000, y: 300 }], expiresAtTick: state.tick + 100 };
  step(state, new Map());
  assert.equal(player.x, 1000);
  assert.equal(player.trail.at(-1)!.x2, 169);
  step(state, new Map());
  assert.equal(player.trail.length, 2);
  assert.equal(player.trail[0]!.x2, 169);
  assert.equal(player.trail[1]!.x1, 1000);
  assert.equal(player.trail[1]!.x2, 1007.5);
});

...
const segment = (
  ...
  x2: number,
  y2: number,
): TrailSegment => ({ x1, y1, x2, y2, createdTick: 12, expiresAtTick: 9999 });
...
test("trail clipping keeps closed tangent edges and corner contacts", () => {
  assert.deepEqual(
    clipTrailSegment(segment(0, 20, 100, 20), bounds),
    segment(10, 20, 90, 20),
  );
  assert.deepEqual(
    clipTrailSegment(segment(10, 0, 10, 100), bounds),
    segment(10, 20, 10, 80),
  );
  assert.deepEqual(
    clipTrailSegment(segment(0, 30, 20, 10), bounds),
    segment(10, 20, 10, 20),
  );
});
...
test("overtime trims dead and living trails before collision and snapshot, retaining disconnected pieces", () => {
  ...
  Object.assign(rider, {
    x: INITIAL_BOUNDARY_INSET + 0.5 + RIDER_RADIUS,
    y: 440,
    angle: Math.PI / 2,
  });
  owner.alive = false;
  owner.trail = [
    segment(20, 400, 20, 500),
    segment(0, 100, 100, 100),
    segment(1500, 100, 1600, 100),
  ];
  const result = step(state, new Map());
  assert.equal(
    rider.alive,
    true,
    "closed-field trail must not kill an inside rider",
  );
  ...
  assert.deepEqual(
    toView(state).players.find((player) => player.id === "p1")!.trail,
    owner.trail,
  );
  step(state, new Map());
...
test("overtime clipping preserves portal entry and exit discontinuity", () => {
  ...
  assert.equal(
    player.trail[1]!.x2,
    1012 + 7.5 * SPEED_RAMP_MAX,
    "overtime runs at the full ramped speed",
  );
});

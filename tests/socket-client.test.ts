...
test("connect() while the previous socket is still CONNECTING closes it instead of orphaning it", () => {
  ...
  assert.equal(
    sockets.length,
    2,
    "connect() while CONNECTING still opens a replacement socket",
  );
  ...
  assert.equal(
    client.socket,
    second,
    "the client now tracks only the newest socket",
  );
  assert.equal(
    first.closed,
    true,
    "the superseded socket must be closed, not left dangling",
  );

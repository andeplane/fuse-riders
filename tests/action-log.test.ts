...
test('accepted host actions replay complete lifecycle, bots, timeout cancellation and resets',()=>{
  ...
  for(let tick=0;tick<1500;tick++){
    if(tick===65)host.command('guest',{type:'input',scope:host.controlScope('guest')!,intendedTick:host.game.tick+1,seq:0,left:true,right:false,bomb:true,bombAction:'press',gesture:1});
...
test('action changes carry full applied ticks across sparse input and bomb transitions',()=>{
  ...
  for(let tick=66;tick<=72;tick++){
    if(tick===66||tick===70)host.command('guest',{type:'input',scope:host.controlScope('guest')!,intendedTick:tick,seq:tick-66,left:tick===66,right:false,bomb:tick===66,bombAction:tick===66?'press':'release',gesture:1});
    host.advance();
  }

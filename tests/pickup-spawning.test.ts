...
test("two through five living riders spawn one pickup per scheduled tick, never a batch", () => {
  for (const count of [2, 3, 4, 5]) {
    ...
    const start = game.tick,
      interval = pickupPacing(count).interval;
    assert.equal(
      game.nextPickupSpawnTick,
      start + interval,
      "initial delay uses the population too",
    );
    ...
    for (let i = 0; i < 80; i++) {
      ...
      step(game, inputs);
      assert.ok(
        game.nextPickupId - previousId <= 1,
        "at most one new pickup on a tick",
      );
...
test("elimination lowers the rate and cap without removing existing drops; spawning resumes below the cap", () => {
  ...
  game.pickups = Array.from({ length: pickupPacing(5).cap }, (_, i) => ({
  ...
  assert.deepEqual(
    ...
    ids,
    "existing drops survive the cap reduction",
  );
  assert.equal(game.nextPickupId, nextId);
  assert.equal(game.nextPickupSpawnTick, game.tick + pickupPacing(2).interval);
  const collector = game.players.get("human")!;
  for (const pickup of game.pickups.slice(0, 13)) {
  ...
  assert.equal(
    game.nextPickupId,
    nextId + 1,
    "seven remaining drops allow exactly one new drop",
  );
});

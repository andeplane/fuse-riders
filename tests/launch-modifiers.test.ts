...
import { VOLLEY_FLIGHT_STEPS, createVolleyFlightPaths, volleyAngles } from '../src/shared/launch-modifiers.ts';
...
test('creates a deterministic straight triple fan with seven points per bomb', () => {
  ...
  for (const path of paths) {
    ...
    for (let step = 1; step < path.length; step += 1) {
      assert.ok(Math.abs(Math.hypot(path[step]!.x - path[step - 1]!.x, path[step]!.y - path[step - 1]!.y) - 20) < 1e-10);
    }
  }
...
test('rejects invalid geometry', () => {
  ...
  assert.throws(() => createVolleyFlightPaths({ x: Infinity, y: 10 }, 0, 120, bounds));
  assert.throws(() => createVolleyFlightPaths({ x: 10, y: 10 }, 0, 120, { ...bounds, minX: 2000 }));
  assert.throws(() => createVolleyFlightPaths({ x: 10, y: 10 }, 0, 120, { ...bounds, minY: 2000 }));
});

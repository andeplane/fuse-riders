...
test('every pickup type has artwork in every theme', () => {
  for (const theme of Object.values(themes)) {
    const files = new Set(readdirSync(`public/themes/${theme.id}`));
...
test('no theme ships artwork for a pickup the game does not have', () => {
  ...
  for (const theme of Object.values(themes)) {
    const strays = readdirSync(`public/themes/${theme.id}`).filter(file => file.startsWith('pickup-') && !known.has(file));

...
const themeDirectory = (id: string) => fileURLToPath(new URL(`../public/themes/${id}`, import.meta.url));
...
test('every pickup type has artwork in every theme', () => {
  for (const theme of Object.values(themes)) {
    ...
    for (const type of PICKUP_TYPES) {
      assert.ok(files.has(`pickup-${type}.svg`), `${theme.id} is missing pickup-${type}.svg`);
    }
  }
});
test('no theme ships artwork for a pickup the game does not have', () => {
  ...
  for (const theme of Object.values(themes)) {
    const strays = readdirSync(themeDirectory(theme.id)).filter(file => file.startsWith('pickup-') && file.endsWith('.svg') && !known.has(file));
    assert.deepEqual(strays, [], `${theme.id} ships artwork for pickups that no longer exist`);
  }
});

...
test('fresh-input age neutralizes held controls at tick twenty',()=>{const {state,motion,predictor}=setup();motion.held={left:true,right:false};motion.appliedTick=state.tick-19;predictor.accept(state,'h',0,motion,'authority');assert.equal(predictor.predict(state.tick+1)!.angle,state.players[0]!.angle);});

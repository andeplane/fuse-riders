...
import { MAX_PORTAL_PAIRS, PORTAL_COOLDOWN_TICKS, type PortalPair } from '../src/shared/portal.js';
...
const wall = (x: number, y1 = 300, y2 = 600, tick = 0) => ({ x1: x, y1, x2: x, y2, createdTick: tick, expiresAtTick: tick + 1000 });
...
test('a shell leaves the partner gate carrying its speed and its remaining travel', () => {
  ...
  assert.ok(Math.abs(shell.x - (919 + REACH * (1 - spent))) < 1e-9, `left the partner gate at ${shell.x}`);
...
test('a shell travelling the other way enters the far gate and leaves beside the near one', () => {
  const { game } = scene();
  const shell = launchShell(game, { launchX: 930, x: 930, shell: { vx: -SHELL_SPEED, vy: 0 } });
  step(game, new Map());
  // Entry at 900 + 18, exit at 300 - 19, then the rest of the tick continuing left.
  const spent = (930 - (900 + SHELL_EDGE)) / REACH;
  assert.ok(Math.abs(shell.x - (281 - REACH * (1 - spent))) < 1e-9, `left the near gate at ${shell.x}`);
  assert.equal(shell.shell!.vx, -SHELL_SPEED, 'heading is preserved through the gate');
});
...
test('entry height maps onto a partner of a different centre and length', () => {
  ...
  game.portalPairs = [pair({ gates: [{ x: 300, y: 450, halfLength: 100 }, { x: 900, y: 300, halfLength: 50 }] })];
  const shell = launchShell(game, { launchY: 500, y: 500 });
  step(game, new Map());
  assert.equal(shell.y, 325, 'half-way down the entry gate is half-way down its partner');
  assert.ok(Math.abs(shell.x - 929.5) < 1e-9, 'and it still came out beside the partner');
});
...
test('teleporting past a rider never runs them down', () => {
  ...
  assert.equal(victim.alive, true, 'the gap between gates is not ground it covered');
});
...
test('a shell still kills what stands beyond the gate it came out of', () => {
  ...
  assert.equal(shell.portalCooldownUntilTick, game.tick + PORTAL_COOLDOWN_TICKS, 'by way of the gate, not across the field');
});
...
test('the cooldown refuses the tick before its deadline and allows the tick it falls on', () => {
  ...
  const aimBack = () => Object.assign(shell, { x: 930, y: 450, shell: { vx: -SHELL_SPEED, vy: 0 } });
  while (game.tick < deadline) {
    aimBack();
    step(game, new Map());
    if (game.tick >= deadline) break;
    assert.ok(Math.abs(shell.x - (930 - REACH)) < 1e-9, `refused at tick ${game.tick}, so it ran on: ${shell.x}`);
    assert.equal(shell.portalCooldownUntilTick, deadline, 'unchanged while refused');
  }
...
test('a shell meets a gate that is only in its way after a bounce', () => {
  ...
  Object.assign(victim, { x: 1400, y: 100, trail: [wall(400, 300, 600, game.tick)] });
  const shell = launchShell(game, { launchX: 330, x: 330, shell: { vx: SHELL_SPEED * 6, vy: 0 } });
  step(game, new Map());
  ...
  assert.ok(Math.abs(shell.x - 864) < 1e-3, `then it took the gate it had been driven into: ${shell.x}`);
  assert.equal(shell.portalCooldownUntilTick, game.tick + PORTAL_COOLDOWN_TICKS);
});
...
test('a shell bouncing off a wall after the gate keeps both outcomes', () => {
  ...
  assert.ok(Math.abs(shell.x - (right - (travelled - (right - 919)))) < 1e-6, `reflected off the far wall: ${shell.x}`);
});
...
test('a trail contact landing exactly on the gate mouth is the gate\'s', () => {
  ...
  Object.assign(victim, { x: 1400, y: 100, trail: [wall(299, 300, 600, game.tick)] });
  const shell = launchShell(game);
  step(game, new Map());
  assert.equal(shell.shell!.vx, SHELL_SPEED, 'it did not reflect and leave the partner backwards');
  assert.equal(shell.shell!.bounces, undefined, 'and the tie was not also counted as a bounce');
  assert.ok(Math.abs(shell.x - 929.5) < 1e-9, `through the gate as usual: ${shell.x}`);
});

test('a shell carries its earlier bounces through a gate', () => {
  const { game } = scene();
  const shell = launchShell(game, { shell: { vx: SHELL_SPEED, vy: 0, bounces: 4 } });
  step(game, new Map());
  assert.ok(Math.abs(shell.x - 929.5) < 1e-9, 'it went through');
  assert.equal(shell.shell!.bounces, 4, 'a gate neither counts as a bounce nor forgets the earlier ones');
});
...
test('a shell ignores a gate whose exit is fouled, and takes the same gate without the obstruction', () => {
  ...
  step(game, new Map());
  assert.ok(Math.abs(fouled.x - (270 + REACH)) < 1e-9, `passed straight through: ${fouled.x}`);
  assert.equal(fouled.portalCooldownUntilTick, undefined);

  const control = scene();
  const through = launchShell(control.game);
  step(control.game, new Map());
  assert.ok(Math.abs(through.x - 929.5) < 1e-9, 'the identical launch does portal once the corridor is clear');
});
...
test('a shell ignores a gate whose exit would fall outside the field', () => {
  ...
  game.portalPairs = [pair({ gates: [{ x: 300, y: 450, halfLength: 100 }, { x: 1600 - game.boundaryInset - 2, y: 450, halfLength: 100 }] })];
  const shell = launchShell(game);
  step(game, new Map());
  assert.ok(Math.abs(shell.x - (270 + REACH)) < 1e-9, `travelled on as though the gate were not there: ${shell.x}`);
  assert.equal(shell.portalCooldownUntilTick, undefined);

  const control = scene();
  const through = launchShell(control.game);
  step(control.game, new Map());
  assert.ok(Math.abs(through.x - 929.5) < 1e-9, 'the identical launch does portal when the partner has room');
});
...
test('a gun ray comes out of the partner gate and kills there, one tracer per segment', () => {
  ...
  assert.equal(tracers[0]!.x, 300 - GUN_EDGE, 'the first segment ends at the gate');
  ...
  assert.equal(tracers[0]!.explodeAtTick, tracers[1]!.explodeAtTick, 'both fade together');
  ...
  assert.equal(result.events.filter(event => event.type === 'bombPlaced').length, 1, 'the trigger was pulled once');
});
...
test('a body in front of a gate takes the shot instead of the gate', () => {
  ...
  assert.equal(victim.alive, true, 'and the ray never left through the partner gate');
...
test('a gun ray cuts its hole past the gate, not between the gates', () => {
  const { game, shooter, victim, bystander } = scene();
  Object.assign(shooter, { gunArmed: true });
  Object.assign(bystander, { x: 1400, y: 300, trail: [wall(600, 300, 600, game.tick)] });
  Object.assign(victim, { x: 1100, y: 450, angle: Math.PI, trail: [wall(1000, 300, 600, game.tick)] });
  step(game, new Map([['p0', press]]));
  assert.equal(victim.alive, true, 'their own body stopped the bullet short of their head');
  assert.equal(bystander.trail.filter(segment => segment.x1 === 600).length, 1, 'the trail between the gates is untouched');
  assert.equal(victim.trail.filter(segment => segment.x1 === 1000).length, 2, 'the trail past the gate is holed');
});
...
test('a cycle of gates cannot hold a gun ray', () => {
  ...
  game.portalPairs = [
    pair({ id: 'a', gates: [{ x: 300, y: 450, halfLength: 100 }, { x: 900, y: 450, halfLength: 100 }] }),
    pair({ id: 'b', gates: [{ x: 1000, y: 450, halfLength: 100 }, { x: 400, y: 450, halfLength: 100 }] }),
  ];
  step(game, new Map([['p0', press]]));
  assert.equal(game.bombs.size, 3, 'one segment up to A, one from A to B, one from B to the wall');
...
test('a projectile transit is nobody\'s portal jump, and a continuation tracer is nobody\'s bomb', () => {
  ...
  assert.equal(owner.portalTransits, was.portalTransits, 'a gate is a rider award, not a projectile one');
  assert.equal(owner.bombsPlaced, was.bombsPlaced + 1, 'one trigger pull, one placement');
});
...
test('a hopping ray folds the same from a checkpoint decoded in another map order', () => {
  ...
  const fold = (state: GameState) => {
    ...
    return canonicalRoomState({ game: state, settings: defaultRoomSettings(), folds: new Map(), bots: new Set() });
  };
  assert.deepEqual(fold(reversed), fold(restored), 'map order cannot change where a hopping ray left things');
});

...
test('bots are simulated on every replica and the same log always folds to the same hash', () => {
  ...
  assert.match(hashText('x'), /^[0-9a-f]{16}$/); assert.notEqual(hashText('a'), hashText('b')); assert.match(RULES, /^fuse-p2p-\d+$/, 'a bump is expected; snapshot.test.ts is what pins the value on the wire');

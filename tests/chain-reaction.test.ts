...
function twoBombs(chainReaction: boolean, fuse = 500): GameState {
  ...
  state.bombs.set(2, {
    ...
    x: 560,
    y: 450,
    explodeAtTick: state.tick + fuse,
    flightPath: flightPath(560, 450),
  });
  return state;
}
const explosions = (result: ReturnType<typeof step>) =>
  result.events
    .filter((event) => event.type === "explosion")
    .map((event) => (event as { bombId: number }).bombId);
...
test("with chaining off the caught bomb keeps its own fuse", () => {
  ...
  assert.equal(
    state.bombs.has(2),
    true,
    "the second bomb survives its neighbour",
  );
...
test("with chaining off a caught bomb still fires on its own fuse, it is not stranded", () => {
  ...
  assert.deepEqual(explosions(step(state, new Map())), [1]);
  assert.deepEqual(
    explosions(step(state, new Map())),
    [],
    "it does not go with its neighbour",
  );
  assert.deepEqual(
    explosions(step(state, new Map())),
    [2],
...
test("settings saved before the toggle keep chaining, and a non-boolean is refused", () => {
  ...
  assert.equal(
    parseRoomSettings(older)?.chainReaction,
    true,
    "an older payload defaults to on",
  );
  assert.equal(
    parseRoomSettings({ ...defaultRoomSettings(), chainReaction: false })
      ?.chainReaction,
    false,
    "off survives a round trip",
  );
  assert.equal(
    parseRoomSettings({ ...defaultRoomSettings(), chainReaction: "yes" }),
    undefined,
  );
});

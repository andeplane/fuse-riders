...
import {
  renderedSnapshot,
  type SnapshotFrame,
} from "../src/client/render-snapshot.js";
...
test("visual projection uses authoritative tick spacing during packet bursts", () => {
  ...
  assert.equal(
    projected.players[0]!.x,
    122.5,
    "two-tick velocity projects at most one 7.5-unit step",
  );
});
...
test("a projected shell bounces off scenery instead of sliding through it", () => {
  ...
  const shot = (obstacles: GameSnapshot["obstacles"]): number => {
    ...
    for (const frame of frames)
      Object.assign(frame.snapshot, {
        bombs: [{ ...shell, shell: { ...shell.shell } }],
        obstacles,
      });
  ...
  assert.equal(
    open,
    437.5,
    "an open lane carries the shell past where the wall would stand",
  );
  const blocked = shot([
    { id: 1, kind: "building", x: 500, y: 100, halfWidth: 60, halfHeight: 60 },
  ]);
  assert.ok(
    blocked <= 500 - 60 - 14 + 1e-6,
    `the wall turns it at its face, not at ${blocked}`,
  );
  assert.ok(
    blocked < open,
    "and it never reaches where an open lane would have carried it",
  );
});
...
test("LAN bomb preview uses fractional rider time with a one-tick cap and intact authoritative state", () => {
  const frames = [playingFrame(10, 100, 100), playingFrame(12, 101, 115)];
  for (const frame of frames)
    frame.snapshot.players[0]!.bombChargeStartedTick = 10;
  ...
  assert.equal(
    shown.presentationTick,
    12.5,
    "blast animation receives fractional world time",
  );
  assert.equal(
    bombPreviewDistance(
      player.presentationTick! - player.bombChargeStartedTick!,
    ),
    193.75,
  );
  assert.equal(
    shown.tick,
    12,
    "world and discrete effects retain the authoritative tick",
  );
  assert.equal(
    renderedSnapshot(frames, 9999)!.players[0]!.presentationTick,
    13,
  );
  assert.equal(
    renderedSnapshot(frames, 9999)!.presentationTick,
    13,
    "cosmetic time also stops after one missing tick",
  );
  assert.deepEqual(frames, original);
  const next = playingFrame(13, 151, 122.5);
  assert.equal(
    renderedSnapshot([frames[1]!, next], 176)!.players[0]!
      .bombChargeStartedTick,
    undefined,
    "release/cancel removes the preview immediately",
  );
});
...
test("visual projection freezes after 50 ms and never projects death or non-playing phases", () => {
  ...
  const lobby = playingFrame(11, 150, 107.5);
  lobby.snapshot.phase = "lobby";
...
test("visual projection does not cross round, match, membership or timestamp boundaries", () => {
  ...
  const countdown = {
    ...first,
    snapshot: { ...first.snapshot, phase: "countdown" as const },
  };
  ...
  for (const previous of [
    ...
    assert.equal(
      renderedSnapshot([previous, next], 200)!.players[0],
      next.snapshot.players[0],
    );
});
...
test("visual projection snaps a portal transit and resumes on the next ordinary movement pair", () => {
  ...
  const transit: SnapshotFrame = {
    ...transitBase,
    snapshot: {
      ...transitBase.snapshot,
      players: transitBase.snapshot.players.map((player) => ({
        ...player,
        portalCooldownUntilTick: 26,
      })),
    },
  };
  assert.equal(
    renderedSnapshot([before, transit], 200)!.players[0]!.x,
    600,
    "teleport displacement is never extrapolated",
  );
  const afterBase = playingFrame(12, 200, 607.5);
  const after: SnapshotFrame = {
    ...afterBase,
    snapshot: {
      ...afterBase.snapshot,
      players: afterBase.snapshot.players.map((player) => ({
        ...player,
        portalCooldownUntilTick: 26,
      })),
    },
  ...
  assert.equal(
    renderedSnapshot([transit, after], 250)!.players[0]!.x,
    615,
    "normal projection resumes after the transit frame",
  );
});
...
test("snapshot acceptance handles opaque match ids without lexicographic ordering", () => {
  ...
  assert.ok(
    stream.accept({ matchId: "ffff", round: 1, tick: 30, state: snapshot() }),
  );
  assert.deepEqual(stream.scope, { matchId: "ffff", round: 1, tick: 30 });
  assert.ok(
    stream.accept({ matchId: "ffff", round: 1, tick: 30, state: snapshot() }),
    "same-tick resync is accepted",
  );
  assert.equal(
    stream.accept({ matchId: "ffff", round: 1, tick: 29, state: snapshot() }),
    undefined,
  );
  assert.deepEqual(
    stream.scope,
    { matchId: "ffff", round: 1, tick: 30 },
    "rejected frame cannot rewind scope",
  );
  assert.ok(
    stream.accept({ matchId: "0000", round: 1, tick: 1, state: snapshot() }),
  );
  assert.deepEqual(stream.scope, { matchId: "0000", round: 1, tick: 1 });
  assert.equal(
    stream.accept({ matchId: "ffff", round: 99, tick: 999, state: snapshot() }),
    undefined,
  );
});
...
test("snapshot acceptance rejects an older round in the active match", () => {
  const stream = new SnapshotStream();
  assert.ok(
    stream.accept({ matchId: "match", round: 2, tick: 100, state: snapshot() }),
  );
  assert.equal(
    stream.accept({ matchId: "match", round: 1, tick: 101, state: snapshot() }),
    undefined,
  );
  assert.ok(
...
test("shell visuals advance between server ticks and reflect off walls without changing authority", () => {
  const before = playingFrame(10, 100, 100);
  const current = playingFrame(11, 150, 107.5);
  current.snapshot.bombs = [
    {
      id: 1,
      ownerId: "p1",
      launchX: 1500,
  ...
  assert.equal(
    projected.bombs[0]!.x,
    1562,
    "travels 6 units to wall and reflects remaining 4",
  );
  assert.equal(current.snapshot.bombs[0]!.x, 1560);
  assert.equal(
    renderedSnapshot([before, current], 9999)!.bombs[0]!.x,
    1552,
    "projection caps at 50ms",
  );
});
...
test("instant gun tracer remains at the resolved endpoint during visual projection", () => {
  const before = playingFrame(10, 100, 100),
    current = playingFrame(11, 150, 107.5);
  const tracer = {
    id: 1,
    ownerId: "p1",
    launchX: 100,

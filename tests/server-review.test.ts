...
test("a reconnect cancels prior charge and accepts a fresh explicit press-release pair", async () => {
  ...
  try {
    ...
    assert.equal(
      app.game.bombs.size,
      0,
      "a held input from before reconnect must not place a bomb",
    );

    replacement.send({
      type: "input",
      seq: rejoined.nextInputSeq + 1,
      left: false,
      right: false,
      bomb: false,
    });
    ...
    assert.equal(
      app.game.bombs.size,
      1,
      "fresh explicit edges launch after reconnect cancellation",
    );
  } finally {
...
test("match rematch resets scope and wins, then round-over automatically starts the next round", async () => {
  ...
  try {
    ...
    const [winner, loser] = [...app.game.players.values()].sort(
      (a, b) => a.slot - b.slot,
    );
    app.game.round = 5;
    winner!.x = 900;
    winner!.y = 600;
    loser!.x = app.game.boundaryInset + 7.1;
    loser!.y = 300;
    loser!.angle = Math.PI;
    app.advance();
    ...
    await host.take(
      "snapshot",
      (message) =>
        message.matchId !== oldMatchId && message.state.phase === "countdown",
    );
    assert.notEqual(app.game.matchId, oldMatchId);
    assert.equal(app.game.round, 1);
    assert.ok(
      [...app.game.players.values()].every((player) => player.roundWins === 0),
    );

    app.advance(60);
    const [, nextLoser] = [...app.game.players.values()].sort(
      (a, b) => a.slot - b.slot,
    );
    nextLoser!.x = app.game.boundaryInset + 7.1;
    nextLoser!.y = 300;
    nextLoser!.angle = Math.PI;
    app.advance();

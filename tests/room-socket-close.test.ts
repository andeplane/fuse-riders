...
test('expired room stops before clearing runtime and never retries a reused public code',()=>{const f=fixture();handleRoomSocketClose(4004,f.actions);handleRoomSocketClose(1006,f.actions);assert.deepEqual(f.calls,['stop','ended','terminal:Room ended — return to menu to start again']);});

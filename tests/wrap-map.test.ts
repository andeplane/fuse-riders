...
import {
  addPlayer, createGame, startMatch, step, toSnapshot,
  ARENA_HEIGHT, ARENA_WIDTH, BOMB_FUSE_TICKS, INITIAL_BOUNDARY_INSET, OVERTIME_START_TICK, SLOT_COLORS,
  type GameState, type InputIntent, type PlayerState,
} from '../src/shared/game.js';
import { BOMB_FLIGHT_TICKS } from '../src/shared/bomb-launch.js';
import { chooseArenaMap, edgesOpen, type ArenaMapId } from '../src/shared/arena-map.js';
import { splitWrappedSegment, wrapCoordinate, wrapDelta, wrapImages } from '../src/shared/wrap.js';
...
import { crossScreenPoint, crossViews, edgeGhosts } from '../src/client/arena-views.js';
...
const release: InputIntent = { ...neutral, bombCommands: [{ action: 'release' }] };
...
function scene(map: ArenaMapId = 'wrap', riders = 2): GameState {
  ...
  for (const player of game.players.values()) Object.assign(player, { x: 800, y: 150 + player.slot * 150, angle: 0, trail: [] });
  return game;
}
const rider = (game: GameState, id = 'p0'): PlayerState => game.players.get(id)!;
const place = (game: GameState, id: string, pose: Partial<PlayerState>): void => { Object.assign(rider(game, id), { trail: [], ...pose }); };
const run = (game: GameState, ticks: number, inputs = new Map<string, InputIntent>()): void => { for (let i = 0; i < ticks; i += 1) step(game, inputs); };
...
test('wrap geometry folds coordinates, takes the short way round and names the images a box needs', () => {
  ...
  assert.equal(wrapCoordinate(-1e-17, 1600), 0, 'a fold that rounds up to the far edge is the near one');
  assert.equal(wrapDelta(1590, 1600), -10);
  assert.equal(wrapDelta(-1590, 1600), 10);
  assert.equal(wrapDelta(300, 1600), 300);
  assert.deepEqual(wrapImages(1600, 900, 100, 100, 200, 200), [{ dx: 0, dy: 0 }]);
  assert.deepEqual(wrapImages(1600, 900, -10, 100, 50, 200), [{ dx: 0, dy: 0 }, { dx: 1600, dy: 0 }]);
  assert.equal(wrapImages(1600, 900, 1550, 850, 1650, 950).length, 4, 'a box over a corner shows up in all four');
});

test('a segment across an edge splits into travel-ordered pieces that end and begin on opposite edges', () => {
  const pieces = splitWrappedSegment({ x1: 1596, y1: 400, x2: 1604, y2: 400, createdTick: 5, expiresAtTick: 99 }, 1600, 900);
  assert.deepEqual(pieces.map(({ x1, x2 }) => [x1, x2]), [[1596, 1600], [0, 4]]);
  assert.ok(pieces.every(piece => piece.createdTick === 5 && piece.expiresAtTick === 99));
  const backwards = splitWrappedSegment({ x1: 3, y1: 2, x2: -5, y2: -2, createdTick: 1, expiresAtTick: 9 }, 1600, 900);
  assert.equal(backwards.length, 3, 'through a corner: this board, the one beside it, the one diagonally across');
  assert.deepEqual([backwards[0]!.x1, backwards[0]!.y1], [3, 2], 'starting where the rider was');
  assert.deepEqual([backwards.at(-1)!.x2, backwards.at(-1)!.y2], [1595, 898], 'and ending where it is now');
});
...
test('the wrap map starts with no walls, and a rider leaves one side to arrive on the other', () => {
  ...
  assert.ok(crossed.x > 0 && crossed.x < 40, `came in from the left at ${crossed.x}`);
  assert.equal(crossed.y, 400);
  const seam = crossed.trail.filter(segment => segment.x2 === ARENA_WIDTH || segment.x1 === 0);
  assert.deepEqual(seam.map(({ x1, x2 }) => [x1 < 800, x2 < 800]), [[false, false], [true, true]], 'one piece up to the edge, one on from the opposite edge');
  assert.ok(crossed.trail.every(segment => Math.abs(segment.x2 - segment.x1) < 100), 'no segment spans the board');
  assert.equal(trailPaths(crossed.trail).length, 2, 'and the renderer never joins the two sides with a line');
});
...
test('every other map keeps its walls: the same ride is fatal on classic and on cross', () => {
  for (const map of ['classic', 'cross'] as const) {
    ...
    assert.ok(events.some(event => event.type === 'playerEliminated' && event.playerId === 'p0' && event.cause === 'wall'), `${map} has a wall`);
  }
});
...
test('cross is the classic arena move for move: only the drawing differs', () => {
  const play = (map: ArenaMapId) => {
    ...
    for (let i = 0; i < 200; i += 1) step(game, new Map([...game.players.keys()].map(id => [id, bots.input(game, id)])));
...
test('a trail just past an open edge kills a rider crossing towards it', () => {
  ...
  rider(game, 'p1').trail = [{ x1: 4, y1: 300, x2: 4, y2: 500, createdTick: game.tick - 40, expiresAtTick: game.tick + 400 }];
  place(game, 'p0', { x: 1585, y: 400, angle: 0 });
  const events = [];
  for (let i = 0; i < 6 && rider(game).alive; i += 1) events.push(...step(game, new Map()).events);
  assert.ok(events.some(event => event.type === 'playerEliminated' && event.playerId === 'p0' && event.cause === 'trail'));
  const dead = rider(game);
  assert.ok(dead.x >= 0 && dead.x < 4 || dead.x > 1590, `the wreck is left at the contact, on the board: ${dead.x}`);
});
...
test('two riders meeting head-on across an open edge ram each other', () => {
  ...
  const rammed = events.filter(event => event.type === 'playerEliminated' && event.cause === 'rider').map(event => event.type === 'playerEliminated' && event.playerId);
...
test('a bomb thrown at an open edge lands on the far side, and its blast reaches back through the edge', () => {
  ...
  assert.ok(bomb.x < bomb.blastRange, `landing folded onto the board, within a blast of the edge: ${bomb.x}`);
  assert.ok(bomb.flightPath.at(-1)!.x > ARENA_WIDTH, 'while the flight path stays one straight throw');
  ...
  const events = [];
  for (let i = 0; i < BOMB_FUSE_TICKS + BOMB_FLIGHT_TICKS && !blasts.length; i += 1) {
    ...
    place(game, 'p1', { x: 1600 - (bomb.blastRange - bomb.x) + 30, y: 400, angle: Math.PI / 2 });
    events.push(...step(game, new Map()).events);
    blasts = game.blasts;
  }
  assert.equal(blasts.length, 2, 'one blast on the board, one image through the edge');
  assert.deepEqual(blasts.map(blast => blast.bombId), [bomb.id, bomb.id]);
  assert.deepEqual(blasts.map(blast => Math.round(blast.circle.x - bomb.x)), [0, ARENA_WIDTH]);
  assert.ok(events.some(event => event.type === 'playerEliminated' && event.playerId === 'p1' && event.cause === 'explosion'), 'and it kills across the edge');
  assert.equal(events.filter(event => event.type === 'explosion').length, 1, 'still one explosion');
});
...
test('a shell flies through an open edge instead of bouncing, and still bounces off a trail waiting beyond it', () => {
  ...
  run(through, 6);
  const shell = [...through.bombs.values()].find(bomb => bomb.shell && !bomb.shell.gun)!;
  ...
  rider(blocked, 'p1').trail = [{ x1: 6, y1: 300, x2: 6, y2: 500, createdTick: blocked.tick - 40, expiresAtTick: blocked.tick + 400 }];
  run(blocked, 6);
  const bounced = [...blocked.bombs.values()].find(bomb => bomb.shell && !bomb.shell.gun)!;
...
test('a bullet carries on through an open edge and hits a rider beyond it, within one board of range', () => {
  ...
  assert.ok(events.some(event => event.type === 'playerEliminated' && event.playerId === 'p1' && event.cause === 'explosion'));
  const tracers = [...game.bombs.values()].filter(bomb => bomb.shell?.gun);
  assert.deepEqual(tracers.map(tracer => [Math.round(tracer.launchX), Math.round(tracer.x)]),
    [[Math.round(rider(game).x), 1600], [0, Math.round(rider(game, 'p1').x - 5)]], 'drawn as two straight legs, the second ending on the target');
  ...
  const travelled = legs.reduce((sum, leg) => sum + Math.abs(leg.x - leg.launchX), 0);
  assert.ok(Math.abs(travelled - ARENA_WIDTH) < 1e-6, `a ray that meets nothing stops after one board, not never: ${travelled}`);
});
...
test('overtime brings the walls in from the very edge, and the round is an ordinary walled one from then on', () => {
  ...
  step(game, new Map());
  assert.ok(game.boundaryInset > 0 && game.boundaryInset < INITIAL_BOUNDARY_INSET + 1, `walls start at the edge: ${game.boundaryInset}`);
  assert.equal(edgesOpen(game), false);
  place(game, 'p0', { x: 1560, y: 400, angle: 0 });
  const events = [];
  for (let i = 0; i < 6; i += 1) events.push(...step(game, new Map()).events);
...
test('bots ride through open edges rather than turning away from them, and survive a wrap round', () => {
  ...
  for (let slot = 0; slot < 4; slot += 1) addPlayer(game, { id: `p${slot}`, name: `AI ${slot} · Hard`, slot, color: SLOT_COLORS[slot]! });
  startMatch(game);
  const bots = new BotController();
  let crossings = 0;
  for (let i = 0; i < 460 && game.phase !== 'roundOver'; i += 1) {
    const before = new Map([...game.players.values()].map(player => [player.id, player.x]));
    step(game, new Map([...game.players.keys()].map(id => [id, bots.input(game, id)])));
    for (const player of game.players.values()) if (player.alive && Math.abs(player.x - before.get(player.id)!) > ARENA_WIDTH / 2) crossings += 1;
    for (const player of game.players.values()) {
      assert.ok(player.x >= 0 && player.x <= ARENA_WIDTH && player.y >= 0 && player.y <= ARENA_HEIGHT, `${player.id} stays on the board`);
    }
  }
  assert.ok([...game.players.values()].filter(player => player.alive).length >= 2, 'most of a hard field is still riding after twenty seconds');
...
test('a wrap round survives a checkpoint, and replays to the same state afterwards', () => {
  ...
  const drive = (state: GameState, ticks: number): void => {
    for (let i = 0; i < ticks; i += 1) step(state, new Map([...state.players.keys()].map(id => [id, bots.input(state, id)])));
  };
  drive(game, 150);
  const restored = decodeGameState(encodeGameState(game));
  assert.ok(restored, 'the checkpoint is accepted');
  drive(game, 100); drive(restored!, 100);
  assert.deepEqual(toSnapshot(restored!), toSnapshot(game));
});

test('rotation still visits only the maps with scenery', () => {
  const visited = new Set(Array.from({ length: 12 }, (_, round) => chooseArenaMap('rotate', 5, round + 1)));
...
test('presentation follows a rider through the edge instead of sweeping it back across the board', () => {
  ...
  assert.ok(newer.players[0]!.x < 20, 'the rider crossed between the two ticks');
  const half = interpolateWorld(older, newer, .5).players[0]!;
  assert.ok(half.x > 1597 && half.x < 1605, `half way is half a step on, not mid-board: ${half.x}`);
  ...
  assert.ok(projected.x > newer.players[0]!.x && projected.x < newer.players[0]!.x + 10, `projected forwards by part of a step: ${projected.x}`);
});
...
test('the crossed view tiles the screen with four quarters of the world, each in the opposite corner', () => {
  ...
  assert.equal(views.reduce((area, view) => area + view.width * view.height, 0), 1001 * 563, 'no gap and no overlap, on an odd-sized canvas too');
  const [topLeft, topRight, bottomLeft, bottomRight] = views;
  assert.deepEqual([topLeft!.scrollX, topLeft!.scrollY], [800, 450], 'the top-left of the screen shows the bottom-right of the world');
  assert.deepEqual([bottomRight!.scrollX, bottomRight!.scrollY], [0, 0], 'and the bottom-right shows the top-left');
  assert.deepEqual([topRight!.x, topRight!.scrollX, bottomLeft!.y, bottomLeft!.scrollY], [topLeft!.width, 0, topLeft!.height, 0]);
  // The world's outer wall, at its corners and edges, lands on the middle of the screen: that is the cross.
  assert.deepEqual(crossScreenPoint(0, 0, 1600, 900), { x: .5, y: .5 });
  assert.deepEqual(crossScreenPoint(800, 450, 1600, 900), { x: 0, y: 0 }, 'and the middle of the world is at the screen corner');
});

...
import { addPlayer, createGame, startMatch, step, COUNTDOWN_TICKS, SLOT_COLORS, type InputIntent } from '../src/shared/game.js';
...
const canonical = (game: GameState) => canonicalRoomState({ game, settings: defaultRoomSettings(), folds: new Map(), bots: new Set() });
...
function scene() {
  ...
  Object.assign(shooter!, { x: 200, y: 450, angle: 0, trail: [], gunArmed: true });
  ...
  return { game, shooter: shooter!, target: target!, behind: behind!, spare: spare!, fire, body };
}
...
test('pickup fires on press across the arena, stops at the first head and credits one instant kill', () => {
  ...
  game.pickups = [{ id: 1, type: 'gun', x: shooter.x, y: shooter.y, expiresAtTick: game.tick + 20 }];
  step(game, new Map());
  ...
  assert.equal(result.events.some(event => event.type === 'explosion'), false);
  assert.deepEqual(game.shots[0]!.kills.map(kill => kill.victimId), ['p1']);
...
test('small body hole stops the shot before a second body and head without splash', () => {
  ...
  fire();
  assert.equal(target.alive, true); assert.equal(behind.alive, true);
  const tracer = [...game.bombs.values()][0]!;
  assert.ok(Math.abs(tracer.x - 695) < 1e-5, 'first contact includes 2px bullet and 3px trail radius');
  ...
  assert.ok(pieces[0]!.detached); assert.equal(pieces[1]!.detached, undefined);
  assert.ok(behind.trail.some(segment => segment.x1 === 1000 && segment.y1 === 300 && segment.y2 === 600));
  assert.equal(game.blasts.length, 0);
});
...
test('body hits near its own head kill; more distant hits and unrelated nearby heads do not', () => {
  for (const offset of [8, 24]) {
    ...
    Object.assign(target, { x: 700, y: 450 + offset, angle: Math.PI / 2, trail: [body(700)] });
    fire();
  ...
  fire(); assert.equal(spare.alive, true, 'another rider close to the hole is not splash damage');
});
...
test('shots follow this tick’s facing after turning, without homing, and clip to the wall', () => {
  ...
  assert.ok(Math.abs(Math.atan2(tracer.y - tracer.launchY, tracer.x - tracer.launchX) - shooter.angle) < 1e-9);
  assert.ok(tracer.x <= game.width - game.boundaryInset - 2 + 1e-6);
  assert.equal(game.blasts.length, 0);
  const endpoint = { x: tracer.x, y: tracer.y };
  step(game, new Map());
  assert.deepEqual({ x: tracer.x, y: tracer.y }, endpoint, 'tracer never flies or curves');
});

test('headshots consume shields, respect immunity and still stop before the next rider', () => {
  for (const protection of ['shielded', 'invulnerableUntilTick', 'shieldGraceUntilTick', 'portalGraceUntilTick'] as const) {
    const { game, target, behind, fire } = scene();
    Object.assign(target, { x: 900, y: 450, angle: Math.PI, [protection]: protection === 'shielded' ? true : game.tick + 20 });
    fire();
    assert.equal(target.alive, true, protection); assert.equal(behind.alive, true);
...
test('cooldown blocks press without spending Gun, and release alone cannot shoot', () => {
  ...
  fire(); assert.equal(shooter.gunArmed, true); assert.equal(game.shots.length, 0);
  shooter.bombReadyAtTick = game.tick;
  fire({ ...neutral, bombCommands: [{ action: 'release' }] });
  assert.equal(game.shots.length, 0);
  fire(); assert.equal(game.shots.length, 1);
});
...
test('simultaneous opponents fire before deaths and checkpoint replay agrees with reversed map order', () => {
  ...
  const inputs = new Map([['p0', press], ['p1', press]]);
  assert.deepEqual(step(restored, inputs), step(game, inputs));
  assert.equal(game.players.get('p0')!.alive, false); assert.equal(target.alive, false);
  assert.equal(canonical(restored), canonical(game));
  const after = decodeGameState(encodeGameState(game)); assert.ok(after);
  for (let tick = 0; tick < 5; tick++) {
    assert.deepEqual(step(after, new Map()), step(game, new Map()));
    assert.equal(canonical(after), canonical(game));
  }
  assert.equal(game.shots.flatMap(shot => shot.kills).length, 2, 'restoring tracers does not repeat damage');
});
...
test('a volley beside a wall or trail spares the shooter and spends its modifiers once', () => {
  for (const wall of [false, true]) {
    ...
    Object.assign(shooter, { fiveShotArmed: true, extraBombs: 1, y: wall ? 34 : 450 });
    target.trail = wall ? [] : [body(240)];
    fire();
    assert.equal(game.shots[0]!.bombs, 6); assert.equal(game.shots.length, 1);
    assert.equal(game.bombs.size, 6);
    assert.equal(shooter.alive, true); assert.equal(shooter.fiveShotArmed, false);
    assert.equal(shooter.extraBombs, 1); assert.equal(game.blasts.length, 0);
  }
});

test('one hole cutting multiple owners allocates detached ids in stable slot order', () => {
  const { game, target, behind, body } = scene();
  target.trail = [body(700)]; behind.trail = [body(705)];
  const restored = decodeGameState(encodeGameState(game)); assert.ok(restored);
  restored.players = new Map([...restored.players].reverse());
  const inputs = new Map([['p0', press]]);
  assert.deepEqual(step(restored, inputs), step(game, inputs));
  assert.equal(canonical(restored), canonical(game));
  assert.ok(target.trail[0]!.detached); assert.ok(behind.trail[0]!.detached);
});

test('equal body contacts retain slot priority for headshot classification', () => {
  const { game, target, behind, fire, body } = scene();
  Object.assign(target, { x: 500, y: 458, angle: Math.PI / 2, trail: [body(500, 300, 450)] });

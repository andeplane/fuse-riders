...
function inputs(
  ...pairs: Array<[string, Partial<InputIntent>]>
...
test("spawns two through five players evenly on a circle with clockwise tangent headings", () => {
  for (let count = 2; count <= 5; count += 1) {
    ...
    players.forEach((player, index) => {
      ...
      assert.ok(
        Math.abs(
          Math.hypot(player.x - state.width / 2, player.y - state.height / 2) -
            expectedRadius,
        ) < 1e-8,
      );
      ...
      assert.ok(
        Math.abs(
          Math.atan2(
            Math.sin(radial - expectedSpawn),
            Math.cos(radial - expectedSpawn),
          ),
        ) < 1e-8,
      );
      assert.ok(
        Math.abs(
          Math.atan2(
            Math.sin(player.angle - expectedSpawn - Math.PI / 2),
            Math.cos(player.angle - expectedSpawn - Math.PI / 2),
          ),
        ) < 1e-8,
      );
    });
...
test("a due blast removes an intersecting segment before trail collision", () => {
  ...
  assert.equal(
    rider.alive,
    true,
    "the newly cleared crossing is safe on the same tick",
  );
  assert.equal(
    owner.trail.some((segment) => segment.x1 === 504),
    false,
  );
  assert.ok(
    result.events.some(
      (event) => event.type === "explosion" && event.bombId === 99,
    ),
  );
});
...
test("swept movement stops at first contact with an old trail", () => {
  ...
  assert.equal(
    state.matchStats.get("p0")!.survivalTicks,
    2,
    "the countdown transition tick and fatal tick both simulate movement",
  );
  ...
  assert.equal(
    rider.trail.at(-1)!.x2,
    rider.x,
    "the fatal trail reaches the impact pose",
  );
...
test("recent self trail is ignored but becomes lethal at the exact grace boundary", () => {
  ...
  player.trail = [
    {
      ...
      x2: 503,
      y2: 400,
      createdTick: recent.tick - SELF_TRAIL_GRACE_TICKS + 2,
      expiresAtTick: recent.tick + 100,
    },
  ];
  step(recent, new Map());
  ...
  oldPlayer.y = 350;
  oldPlayer.angle = 0;
  old.players.get("p1")!.x = 900;
  old.players.get("p1")!.y = 600;
  oldPlayer.trail = [
    {
      x1: 503,
      y1: 300,
      x2: 503,
      y2: 400,
      createdTick: old.tick - SELF_TRAIL_GRACE_TICKS + 1,
      expiresAtTick: old.tick + 100,
    },
  ];
  step(old, new Map());
...
test("following riders survive when their swept paths are close but their bodies stay apart in time", () => {
  for (const reversed of [false, true]) {
    for (const boosted of [false, true]) {
      ...
      assert.ok(
        Math.abs(
          state.players.get(ahead)!.x - state.players.get(behind)!.x - 20,
        ) < 1e-9,
      );
      assert.equal(
        result.events.some((event) => event.type === "playerEliminated"),
        false,
      );
      assert.equal(state.phase, "playing");
    }
...
test("a rider crossing behind another hits only the existing trail, without killing its owner", () => {
  for (const reversed of [false, true]) {
    ...
    assert.deepEqual(
      result.events.filter((event) => event.type === "playerEliminated"),
...
test("rider body contact includes tangency but excludes a near miss", () => {
  for (const separation of [6, 6.001]) {
    ...
    Object.assign(state.players.get("p1")!, {
      x: 507.5,
      y: 350 + separation,
      angle: Math.PI,
      trail: [],
    ...
    assert.ok(
      [...state.players.values()].every(
        (player) => player.alive === separation > 6,
      ),
    );
  }
});
  ...
  for (const gap of [6, 6.001, 9]) {
    ...
    if (gap === 6) {
      assert.equal(
        rider.x,
        500,
        "contact at tick start does not advance the corpse",
      );
      assert.equal(
        rider.trail.length,
        0,
        "zero travel does not leave a zero-length segment",
      );
    }
  }
});
...
test("riders abreast just outside contact survive, though their paths overlap in space", () => {
  ...
  assert.deepEqual(
    result.events.filter((event) => event.type === "playerEliminated"),
    [],
    "a 6.5 gap is outside the 6-unit contact width at every instant",
  );
...
test("head-on swept rider collision eliminates both and produces a draw", () => {
  ...
  assert.deepEqual(
    result.events
      .filter((event) => event.type === "playerEliminated")
  ...
  assert.ok(
    result.events.some(
      (event) => event.type === "roundEnded" && event.winnerId === undefined,
    ),
  );
  // Everybody dying to each other is a highlight moment: it is announced before the round ends (ADR 043/044).
  assert.deepEqual(
    result.events.map((event) => event.type),
    ["playerEliminated", "playerEliminated", "moment", "roundEnded"],
  );
  assert.ok(
    state.roundPlacements.every(
      (placement) => placement.place === 1 && placement.scoreUnits === 0,
    ),
  );
  assert.equal(state.matchStats.get("p0")!.eliminations, 1);
  assert.equal(state.matchStats.get("p1")!.eliminations, 1);
  assert.ok(
    [...state.matchStats.values()].every(
      (entry) => entry.roundsDrawn === 1 && entry.deathsByCause.rider === 1,
    ),
  );
});
...
test("bomb input charges, launches on release, caps one live bomb, chains once, and observes cooldown", () => {
  ...
  assert.equal(
    state.bombs.size,
    0,
    "held input without an edge only continues charging",
  );
  result = step(
    state,
  ...
  assert.equal(
    firstBomb.blastRange,
    90,
    "base radius is 60% of the original 150",
  );
  ...
  assert.equal(
    firstBomb.x - firstBomb.launchX,
    BOMB_MIN_LAUNCH_DISTANCE +
      (2 * (BOMB_MAX_LAUNCH_DISTANCE - BOMB_MIN_LAUNCH_DISTANCE)) /
        BOMB_MAX_CHARGE_TICKS,
  );
  ...
  state.bombs.set(77, {
    ...
    flightPath: fixedFlightPath(
      firstBomb.x + BOMB_BLAST_RANGE / 2,
      firstBomb.y,
    ),
...
test("quick bomb action bursts launch at minimum range and cancel paths never launch", () => {
  ...
  owner.y = 450;
  owner.angle = 0;
  state.players.get("p1")!.x = 1200;
  state.players.get("p1")!.y = 700;
  step(
    state,
    inputs(["p0", { bomb: false, bombCommands: [{ action: "release" }] }]),
  );
  assert.equal(
    state.bombs.size,
    0,
    "release without accepted charge is ignored",
  );
  const launched = step(
    state,
    inputs([
  ...
  step(
    ...
    inputs([
      ...
      {
        bomb: false,
        bombCommands: [{ action: "cancel" }, { action: "release" }],
      },
    ]),
  );
  assert.equal(state.bombs.size, 0);
  ...
  for (const [bombChargeTicks, distance, aimBounce = false, holdTicks = 8] of [
    ...
    for (let tick = 1; tick < holdTicks; tick++) {
      ...
      assert.equal(
        state.bombs.size,
        0,
        "steering while charging must not launch",
      );
    }
    step(
      state,
    ...
    assert.ok(
      Math.abs(
        Math.hypot(bomb.x - bomb.launchX, bomb.y - bomb.launchY) - distance,
      ) < 1e-8,
    );
    assert.ok(
      Math.abs(bomb.x - (owner.x + Math.cos(owner.angle) * distance)) < 1e-8,
    );
    assert.ok(
...
test("charged launches cap and clamp to the active safe interior", () => {
  ...
  enterPlaying(state);
  const owner = state.players.get("p0")!;
  owner.x = state.width - state.boundaryInset - 50;
  owner.y = 450;
  owner.angle = 0;
  state.players.get("p1")!.x = 800;
  state.players.get("p1")!.y = 700;
  ...
  assert.ok(
    bomb.x - bomb.launchX < BOMB_MAX_LAUNCH_DISTANCE,
    "boundary clamp shortens the flight endpoint",
  );
...
test("a flying bomb cannot explode or chain-trigger before landing", () => {
  const state = gameWithPlayers();
  enterPlaying(state);
  state.players.get("p0")!.x = 1000;
  state.players.get("p0")!.y = 700;
  state.players.get("p1")!.x = 1200;
  state.players.get("p1")!.y = 700;
  state.bombs.set(1, {
    id: 1,
    ...
    explodeAtTick: state.tick + 1,
    blastRange: 150,
    flightPath: fixedFlightPath(600, 450),
  });
  state.bombs.set(2, {
    id: 2,
    ownerId: "p1",
...
test("points score once, five rounds end the default match, and rematch resets match points and scope", () => {
  ...
  for (let win = 1; win <= 5; win += 1) {
    ...
    assert.equal(
      result.events.filter((event) => event.type === "roundEnded").length,
      1,
    );
    if (win < 5) {
      assert.equal(state.phase, "roundOver");
  ...
  assert.ok(
    [...state.players.values()].every((player) => player.roundWins === 0),
  );
  assert.equal(
    state.leaderboard.get("p0")!.matchWins,
    1,
    "session totals survive resetMatch",
  );
  assert.ok(
    [...state.matchStats.values()].every((entry) => entry.roundsPlayed === 0),
...
test("overtime inset updates before collision and a 90-second unresolved round draws", () => {
  ...
  assert.equal(
    shrinking.boundaryInset,
    INITIAL_BOUNDARY_INSET + OVERTIME_INSET_PER_TICK,
  );

  const timeout = gameWithPlayers();
  enterPlaying(timeout);
  timeout.players.get("p0")!.x = 600;
  timeout.players.get("p0")!.y = 450;
  timeout.players.get("p1")!.x = 1000;
  timeout.players.get("p1")!.y = 450;
  ...
  assert.ok(
    timeout.roundPlacements.every(
      (placement) => placement.place === 1 && placement.scoreUnits === 0,
    ),
  );
  assert.ok(
    [...timeout.leaderboard.values()].every(
      (entry) => entry.totalScoreUnits === 0 && entry.roundsPlayed === 1,
    ),
  );
});
...
test("lifecycle commands enforce phase, capacity, identity, and connected-player guards", () => {
  ...
  assert.throws(
    () => addPlayer(state, { id: "bad", name: "bad", slot: 7, color: "red" }),
    /invalid slot/,
  );
  assert.throws(
    () =>
  ...
  assert.throws(
    () => setPlayerConnected(state, "missing", true),
    /unknown player/,
  );
  startMatch(state);
...
test("wall and explosion causes are authoritative, clipped, and blast visuals expire exactly", () => {
  ...
  explosion.bombs.set(10, {
    ...
    blastRange: 150,
    flightPath: fixedFlightPath(30, 350),
  });
  const blastResult = step(explosion, new Map());
  assert.ok(
    blastResult.events.some(
      (event) =>
        event.type === "playerEliminated" &&
        event.playerId === "p0" &&
  ...
  assert.equal(
    explosion.roundParticipants.get("p0")!.eliminatedAtTick,
    explosion.tick,
  );
...
test("a finished match can replace every seat and start a clean rematch without restarting the server", () => {
  ...
  assert.ok(
    [...state.players.values()].every(
      (player) => player.roundWins === 0 && player.alive,
    ),
  );
});
...
test("the larger arena and seeded pickup schedule replay deterministically", () => {
  ...
  assert.equal(
    first.nextPickupSpawnTick,
    first.tick + pickupPacing(2).interval,
  );
...
test("first Power pickup applies both weapon upgrades on the collection tick", () => {
  ...
  assert.equal(
    ...
    state.tick + powerReloadTicks(player.powerPickups),
  );
...
test("a star collected on the swept path rescues and reflects a wall hit, then expires sharply", () => {
  ...
  const player = state.players.get("p0")!;
  const other = state.players.get("p1")!;
  player.x = state.boundaryInset + 7.1;
  player.y = 450;
  ...
  assert.equal(
    player.alive,
    false,
    "the first unprotected sweep checks a trail containing the rider",
  );
});
...
test("persistent pickups respect the active cap and impossible safe interior stays bounded", () => {
  ...
  assert.equal(
    ...
    pickupPacing(2).cap,
    "a full board prevents further spawns",
  );
  assert.deepEqual(
    state.pickups.map((pickup) => pickup.id),
    Array.from({ length: pickupPacing(2).cap }, (_, i) => i + 1),
  );
  ...
  assert.equal(
    state.pickups.length,
    0,
    "an empty safe rectangle skips instead of looping",
  );
});
...
test("beer pickup debuffs every other living rider, refreshes, and keeps the collector unchanged", () => {
  ...
  const expectedAngle =
    (((angleBefore - turn + expectedNoise) % (Math.PI * 2)) + Math.PI * 2) %
    (Math.PI * 2);
  assert.ok(
    Math.abs(target.angle - expectedAngle) < 1e-10,
    "normal steering and deterministic wobble are added",
  );
...
test("modifier pickups arm one use, refresh without stacking, and reset between rounds", () => {
  ...
  assert.deepEqual(
    [
      state.matchStats.get("p0")!.triplePickups,
      state.matchStats.get("p0")!.shieldPickups,
    ],
    [1, 1],
  );
  state.pickups = [
...
test("orbit shields absorb a whole clustered hazard tick and grace protects until strict expiry", () => {
  ...
  assert.equal(
    shield.alive,
    false,
    "grace is expired when untilTick equals the current tick",
  );
});
...
test("two shields both break on contact while Star preserves its stored shield", () => {
  ...
  second.y = 450;
  second.angle = Math.PI;
  second.shielded = true;
  step(both, new Map());
  assert.equal(first.alive, true);
  assert.equal(second.alive, true);
  assert.equal(first.shielded, false);
  assert.equal(second.shielded, false);

  const star = gameWithPlayers();
  enterPlaying(star);
  const protectedPlayer = star.players.get("p0")!;
  const normal = star.players.get("p1")!;
  protectedPlayer.x = 500;
...
test("countdown leave is stamped, scored once, and a later join receives no prior-round award", () => {
  ...
  assert.equal(state.phase, "roundOver");
  assert.deepEqual(
    state.roundPlacements.map((placement) => [
      placement.playerId,
      placement.place,
      placement.scoreUnits,
    ]),
    [
      ["p0", 1, 2 * POINT_UNIT],
      ["p1", 2, 0],
  ...
  step(state, new Map());
  assert.equal(
    state.leaderboard.get("p0")!.totalScoreUnits,
    totalAfterRound,
    "round-over ticks cannot score twice",
  );
  ...
  assert.ok(
    !state.roundPlacements.some((placement) => placement.playerId === "late"),
  );
  removePlayer(state, "p1");
  assert.ok(
...
test("overlapping blast owners receive no speculative elimination credit", () => {
  ...
  target.angle = 0;
  state.players.get("p1")!.x = 1200;
  state.players.get("p1")!.y = 200;
  state.players.get("p2")!.x = 1200;
  state.players.get("p2")!.y = 700;
...
test("Five Shot survives Triple collection and cancellation, then launches five with one cooldown", () => {
  ...
  assert.deepEqual(
    bombs.map((b) => b.flightPath[0]!.angle),
    [-0.44, -0.22, 0, 0.22, 0.44],
  );
  assert.ok(
    bombs.every(
      (b) =>
        b.landsAtTick === bombs[0]!.landsAtTick &&
        b.explodeAtTick === bombs[0]!.explodeAtTick,
    ),
  );
...
test("radial blast hits diagonal riders, clears diagonal trails and chains diagonal bombs", () => {
  ...
  rider.angle = 0;
  state.players.get("p1")!.x = 1200;
  state.players.get("p1")!.y = 750;
  const distant = state.players.get("p2")!;
  distant.x = 1000;
  distant.y = 300;
  distant.trail = [
    { x1: 560, y1: 560, x2: 580, y2: 580, createdTick: 0, expiresAtTick: 999 },
  ];
  for (const [id, x, y] of [
    [1, 500, 500],
    [2, 400, 400],
  ...
  assert.equal(rider.alive, false);
  assert.deepEqual(
    result.events.filter((e) => e.type === "explosion").map((e) => e.bombId),
    [1, 2],
  );
  assert.equal(
    state.bombs.has(3),
    true,
    "outside the disk despite being inside the square",
  );
...
test("bomb landing hits a rider before detonation, respecting star and shield", () => {
  for (const protection of ["none", "star", "shield"] as const) {
    ...
    state.bombs.set(99, {
      ...
      blastRange: 90,
      flightPath: Array.from({ length: 7 }, (_, i) => ({
        x: 500 + i * 100,
        y: 450,
        angle: 0,
      })),
    ...
    assert.equal(
      state.blasts.length,
      0,
      "impact damage precedes the explosion",
    );
    assert.equal(
      state.bombs.has(99),
      true,
      "bomb keeps its fuse after the hit",
    );
...
test("a bomb landing inside an active blast chains immediately", () => {
  ...
  state.bombs.set(99, {
    ...
    explodeAtTick: state.tick + 40,
    blastRange: 90,
    flightPath: fixedFlightPath(800, 450),
  });
  step(state, new Map());
...
test("blast preview is harmless before the fuse, both in flight and after landing", () => {
  for (const airborne of [false, true]) {
    ...
    rider.y = 500;
    rider.angle = 0;
    state.players.get("p0")!.x = 200;
    state.players.get("p0")!.y = 200;
    state.players.get("p2")!.x = 1200;
    state.players.get("p2")!.y = 700;
    for (const player of state.players.values()) player.trail = [];
    state.bombs.set(99, {
    ...
    assert.equal(
      rider.alive,
      true,
      "inside the large preview but outside the physical bomb",
    );
    ...
    assert.equal(
      rider.alive,
      false,
      "radius becomes lethal only when bomb detonates",
    );
  }
});
...
test("bomb skips riders along flight and only snipes at its landing position", () => {
  ...
  state.bombs.set(99, {
    ...
    explodeAtTick: state.tick + 40,
    blastRange: 140,
    flightPath: [{ x: 500, y: 450, angle: 0 }, ...fixedFlightPath(600, 450)],
  });
  step(state, new Map());
  assert.equal(
    state.players.get("p1")!.alive,
    true,
    "crossing the flight path is harmless",
  );
  state.tick += 4;
  ...
  assert.equal(
    state.players.get("p2")!.alive,
    false,
    "physical landing contact snipes",
  );
  assert.equal(
    state.players.get("p3")!.alive,
    true,
    "large preview is still harmless",
  );
  assert.equal(state.bombs.get(99)!.x, 600);
  assert.equal(state.blasts.length, 0);
});
...
test("landing step never uses blast radius for contact damage", () => {
  for (const offset of [30, 50, 100, 135]) {
    ...
    assert.equal(
      state.players.get("p1")!.alive,
      true,
      `landing preview at offset ${offset} is harmless`,
    );
    assert.equal(state.blasts.length, 0);
  }
});
...
test("shell persists beyond five seconds and permits another shot after cooldown", () => {
  ...
  for (const extra of [...state.bombs.values()].filter(
    (_, index) => index !== 2,
  ))
    state.bombs.delete(extra.id);
  ...
  step(state, new Map());
  assert.ok(state.bombs.has(shell.id));
  assert.equal(state.blasts.length, 0);
  step(
    state,
    inputs(["p0", { bomb: true, bombCommands: [{ action: "press" }] }]),
  ...
  assert.equal(
    state.bombs.size,
    2,
    "the spent Five no longer widens the next pull",
  );
});
test("shell body hits once, shield absorbs it, and no blast radius is produced", () => {
  for (const shield of [false, true]) {
    ...
    step(state, new Map());
    assert.equal(state.players.get("p1")!.alive, shield);
    assert.equal(state.bombs.size, 0);
    assert.equal(state.players.get("p1")!.shielded, false);
    assert.equal(state.blasts.length, 0);
  }
});
...
test("power changes only future shots, preserves fuse timing and resets next round", () => {
  ...
  enterPlaying(state);
  const owner = state.players.get("p0")!;
  Object.assign(owner, { x: 400, y: 450, angle: 0, trail: [] });
  const other = state.players.get("p1")!;
  Object.assign(other, { x: 1000, y: 700, angle: 0, trail: [] });
  ...
  state.bombs.set(99, {
    ...
    flightPath: fixedFlightPath(700, 200),
  });
  ...
  step(
    state,
    inputs(
      ["p0", { bomb: true, bombCommands: [{ action: "press" }] }],
      ["p1", { bomb: true, bombCommands: [{ action: "press" }] }],
    ),
  );
  step(
    state,
    inputs(
      ["p0", { bomb: false, bombCommands: [{ action: "release" }] }],
      ["p1", { bomb: false, bombCommands: [{ action: "release" }] }],
    ),
  );
  const bombs = [...state.bombs.values()];
  ...
  assert.ok(
    bombs.every(
      (bomb) => bomb.explodeAtTick - bomb.launchedTick === BOMB_FUSE_TICKS,
    ),
  );
  assert.ok(
    bombs
      .filter((bomb) => bomb.ownerId === owner.id)
      .every(
        (bomb) => bomb.blastRange === powerBlastRadius(owner.powerPickups),
      ),
  );
  assert.equal(
    bombs.find((bomb) => bomb.ownerId === other.id)!.blastRange,
    BOMB_BLAST_RANGE,
  );
  assert.equal(
    toView(state).players.find((player) => player.id === owner.id)!
  ...
  state.tick = state.phaseEndsAtTick!;
  startNextRound(state);
  assert.equal(owner.powerPickups, 0);
  assert.equal(owner.reloadDurationTicks, BOMB_COOLDOWN_TICKS);
});
...
test("live shell bounces off a rider trail without damage or resetting its lifetime", () => {
  ...
  assert.equal(shell.shell!.vx, -450);
  assert.ok(shell.x < 519);
  assert.equal(shell.explodeAtTick, expires);
  assert.deepEqual(
    state.players.get("p1")!.trail.find((segment) => segment.x1 === 536),
    tail,
  );
  assert.equal(state.blasts.length, 0);
});
...
test("a landed bomb blast clears nearby trails", () => {
  ...
  Object.assign(state.players.get("p2")!, {
    x: 1200,
    y: 800,
    angle: 0,
    trail: [
  ...
  assert.equal(
    state.players.get("p2")!.trail.some((segment) => segment.y1 === 470),
    false,
  );
});
...
test("a fixed-rounds match ends on the standings leader even when another rider wins the final round", () => {
  ...
  assert.equal(
    state.roundWinnerId,
    "p1",
    "the final round still belongs to its survivor",
  );
  assert.deepEqual(state.leaderboard.get("p0"), {
    id: "p0",
    name: "Player 1",
    totalScoreUnits: 4 * POINT_UNIT,
    roundsPlayed: 3,
    roundWins: 2,
    matchWins: 1,
  });
...
test("a drawn final round still awards the fixed-rounds match to the standings leader", () => {
  ...
  assert.ok(
    result.events.some(
      (event) => event.type === "matchEnded" && event.winnerId === "p0",
    ),
  );
  assert.deepEqual(state.leaderboard.get("p0"), {
    id: "p0",
...
test("a tied match shares victory and credits both champions once", () => {
  ...
  assert.ok(
    result.events.some(
      (event) => event.type === "matchEnded" && event.winnerId === undefined,
    ),
  );
  assert.ok(
    [...state.leaderboard.values()].every(
      (entry) => entry.matchWins === 1 && entry.roundWins === 1,
    ),
  );
});

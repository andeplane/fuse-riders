...
test('gun speed is twice rider speed with a bounded nearby steering correction', () => {
  assert.equal(GUN_SPEED, RIDER_SPEED * 2);
  const v = gunVelocity(0, 0, 300, 0, [{ x: 100, y: 100 }]);
  assert.ok(Math.abs(Math.hypot(v.vx, v.vy) - 300) < 1e-9);
  assert.ok(Math.abs(Math.atan2(v.vy, v.vx) - GUN_TURN_PER_TICK) < 1e-9);
  for (const target of [{ x: 500, y: 100 }, { x: -100, y: 20 }]) assert.deepEqual(gunVelocity(0, 0, 300, 0, [target]), { vx: 300, vy: 0 });
});

...
test("each pickup adds one bomb, every subsequent shot retains it, and a new round resets it", () => {
  ...
  for (let upgrades = 1; upgrades <= 3; upgrades++) {
    ...
    assert.equal(
      game.nextBombId,
      ids,
      "upgrade does not permit independent launches during reload",
    );
    ...
    assert.equal(
      game.bombs.size,
      upgrades + 1,
      "second shot still has the upgrade",
    );
    while (game.tick < rider.bombReadyAtTick) step(game, new Map());
  }
...
test("collection while charging is immediate; cancellation preserves upgrades; cap bounds volleys", () => {
  ...
  collect(game, "extraBomb", "extraBomb");
  ...
  collect(
    capped,
    ...Array<PickupType>(MAX_EXTRA_BOMBS + 2).fill("extraBomb"),
    "five",
  );
  step(capped, fire);
...
test("even and large volleys stay symmetric, distinct and within the Five fan", () => {
  for (let count = 1; count <= MAX_VOLLEY_BOMBS; count++) {
    ...
    assert.ok(angles.every((angle) => Math.abs(angle) <= 0.44 + 1e-12));
    for (let i = 0; i < count; i++)
      assert.ok(Math.abs(angles[i]! + angles[count - 1 - i]!) < 1e-12);
  }
  assert.deepEqual(volleyAngles(0, 2), [-0.11, 0.11]);
  for (const count of [0, -1, 1.5, NaN, Infinity, MAX_VOLLEY_BOMBS + 1])
    assert.throws(() => volleyAngles(0, count));
});
...
test("Triple/Five add their temporary bonus; Power applies to upgraded volleys", () => {
  for (const temporary of ["triple", "five"] as const) {
    const game = playing(),
      rider = game.players.get("p0")!;
    collect(game, "extraBomb", "extraBomb", "power", temporary);
    step(game, fire);
...
test("checkpoint recovery replays upgraded launches and explosions exactly", () => {
  const game = playing();
  collect(game, "extraBomb", "extraBomb", "extraBomb", "gravity");
  step(game, commands("press"));
...
test("checkpoint rejects corrupt or missing counts, and default drop rate is about four percent", () => {
  const game = playing(),
    rider = game.players.get("p0")!;
  for (const invalid of [-1, 0.5, MAX_EXTRA_BOMBS + 1, NaN]) {
    rider.extraBombs = invalid;
    assert.equal(decodeGameState(encodeGameState(game)), undefined);
  }
  rider.extraBombs = 0;
  const valid = encodeGameState(game);
  assert.equal(
    decodeGameState(valid.replace(/"extraBombs":0,/g, "")),
    undefined,
  );
  assert.ok(decodeGameState(valid));
  const total = PICKUP_WEIGHTS.reduce((sum, row) => sum + row.weight, 0);
  const share =
    PICKUP_WEIGHTS.find((row) => row.type === "extraBomb")!.weight / total;
  assert.ok(share > 0.03 && share < 0.05);

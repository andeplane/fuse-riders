...
test("the pickup opens one to three black holes of bounded size inside the walls, for eight seconds", () => {
  ...
  for (let seed = 1; seed <= 40; seed += 1) {
    ...
    assert.ok(
      fields.length >= 1 && fields.length <= GRAVITY_MAX_HOLES_PER_PICKUP,
      `seed ${seed} opened ${fields.length}`,
    );
    counts.add(fields.length);
    for (const field of fields) {
      assert.ok(
        field.radius >= GRAVITY_MIN_RADIUS &&
          field.radius <= GRAVITY_MAX_RADIUS,
      );
      assert.ok(
        field.x >= state.boundaryInset &&
          field.x <= state.width - state.boundaryInset,
      );
      assert.ok(
        field.y >= state.boundaryInset &&
          field.y <= state.height - state.boundaryInset,
      );
  ...
  assert.deepEqual(
    [...counts].sort(),
    [1, 2, 3],
    "every count turns up across seeds",
  );
});
...
test("more pickups add holes up to the cap, retiring the oldest first", () => {
  const state = playing();
  for (let index = 0; index < MAX_GRAVITY_FIELDS; index += 1)
    hole(state, 100 + index, 100, 120);
  collect(state);
...
test("a rider crossing a hole follows the curve: its heading swings toward the centre and its speed is untouched", () => {
  ...
  const rider = state.players.get("p0")!,
    free = control.players.get("p0")!;
  for (let tick = 0; tick < 20; tick += 1) {
    const fromX = rider.x,
      fromY = rider.y;
    step(state, new Map());
    step(control, new Map());
    const { distance } = riderMotionStep(
      rider,
      state.tick,
      state.roundStartedTick,
    );
    assert.ok(
      Math.abs(Math.hypot(rider.x - fromX, rider.y - fromY) - distance) < 1e-9,
      "curved space bends the path, it does not drag",
    );
  }
  assert.equal(free.angle, 0, "the control rider rode straight");
  assert.ok(
    rider.angle > 0.1 && rider.angle < Math.PI,
    "the heading turned toward the hole below",
  );
...
test("a rider outside every radius is untouched", () => {
  ...
  assert.deepEqual(
    [
      state.players.get("p0")!.x,
      state.players.get("p0")!.y,
      state.players.get("p0")!.angle,
    ],
    [
      control.players.get("p0")!.x,
      control.players.get("p0")!.y,
      control.players.get("p0")!.angle,
    ],
  );
});
test("the bend is strongest near the centre, zero at the rim, zero head-on, and mirrors left to right", () => {
  ...
  const near = gravityBend([{ x: 0, y: 20, radius: 200 }], pose, turn),
    far = gravityBend([{ x: 0, y: 150, radius: 200 }], pose, turn);
  assert.ok(near > far && far > 0);
  assert.equal(
    gravityBend([{ x: 0, y: 200, radius: 200 }], pose, turn),
    0,
    "the rim",
  );
  assert.equal(
    gravityBend([{ x: 50, y: 0, radius: 200 }], pose, turn),
    0,
    "riding straight at the centre",
  );
...
test("however many holes overlap, the bend stays under the rider's own steering, so nobody is trapped in orbit", () => {
  ...
  for (const count of [1, 2, 4, 8]) {
    const bend = gravityBend(
      Array.from({ length: count }, () => ({ x: 0, y: 1, radius: 300 })),
      pose,
      turn,
    );
    assert.ok(
      bend <= GRAVITY_BEND * turn + 1e-12 && bend < turn,
      `${count} holes bent ${bend}`,
    );
  }
  // Steering flat out against a hole still turns the rider away from it.
  const state = playing();
  const rider = state.players.get("p0")!;
  hole(state, rider.x, rider.y + 60, 300);
  hole(state, rider.x, rider.y + 60, 300);
  step(
    state,
    new Map([["p0", { left: true, right: false, bomb: false }]]) as never,
  );
  assert.ok(
    rider.angle > Math.PI,
    "a left turn won against two holes on the right",
  );
});
test("a hole expires exactly", () => {
  ...
  for (let tick = 0; tick < 2; tick += 1) {
    step(state, new Map());
    assert.equal(state.gravityFields.length, 1);
  }
  step(state, new Map());
...
test("a hole caught by the closing wall is moved back inside", () => {
  ...
  assert.ok(
    ...
    "the wall actually closed, or the clamp is unexercised",
  );
  assert.ok(
    field.x >= state.boundaryInset,
    `the hole stayed at ${field.x}, outside the wall at ${state.boundaryInset}`,
  );
});
test("holes do not outlive their round", () => {
  ...
  for (
    ...
    tick < 400 &&
    !(
      state.phase === "roundOver" && state.tick >= (state.phaseEndsAtTick ?? 0)
    );
    tick += 1
  )
    step(state, new Map());
  assert.equal(
    state.gravityFields.length,
    1,
    "still there while the round winds down",
  );
...
test("a rider that falls into the core dies an ownerless wall death; skimming the hole outside it does not", () => {
  ...
  hole(
    safe,
    safe.players.get("p0")!.x,
    safe.players.get("p0")!.y + gravityCoreRadius(200) + 20,
    200,
  );
  step(safe, new Map());
...
test("a hole never opens with its core under a rider", () => {
  for (let seed = 1; seed <= 200; seed += 1) {
    ...
    for (const field of state.gravityFields)
      for (const player of state.players.values()) {
        assert.ok(
          Math.hypot(field.x - player.x, field.y - player.y) >=
            GRAVITY_CORE_SPAWN_CLEARANCE - 1e-9,
          `seed ${seed}`,
        );
      }

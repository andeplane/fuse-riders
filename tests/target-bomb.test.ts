...
function fixture() {
  ...
  other.x = 1200;
  other.y = 700;
  const input = (intent: Partial<InputIntent>) =>
    step(
      game,
      new Map([["p0", { left: false, right: false, bomb: false, ...intent }]]),
    );
...
test("target collection arms one normal-strength bomb and preserves volley upgrades", () => {
  ...
  game.pickups.push({
    ...
    x: player.x,
    y: player.y,
    expiresAtTick: game.tick + 30,
  });
  input({});
  assert.equal(player.targetBombArmed, true);
  assert.equal(game.matchStats.get("p0")!.targetPickups, 1);
  player.fiveShotArmed = true;
  player.tripleShotArmed = true;
  player.powerPickups = 2;
  input({
    bomb: true,
  ...
  assert.deepEqual(player.bombTarget, { x: 1200, y: 225 });
  input({ bombCommands: [{ action: "release", aim: { x: 0.8, y: 0.2 } }] });
  assert.equal(game.bombs.size, 0);
  const blast = game.blasts[0]!;
  assert.equal(blast.circle.x, 1280);
  assert.equal(blast.circle.y, 180);
  assert.equal(
    blast.circle.radius,
    powerBlastRadius(player.powerPickups) * 0.7,
  );
  assert.equal(
    player.bombReadyAtTick,
    game.tick + powerReloadTicks(player.powerPickups),
  );
  assert.equal(player.targetBombArmed, false);
  assert.equal(player.bombTarget, undefined);
...
test("targeting requires pickup and clamps to the current safe field", () => {
  ...
  input({
    bomb: true,
    bombCommands: [{ action: "press", aim: { x: 0, y: 1 } }],
    aim: { x: 0, y: 1 },
  });
  assert.equal(player.bombTarget, undefined);
  input({ bombCommands: [{ action: "cancel" }] });
  player.targetBombArmed = true;
  input({ bomb: true, bombCommands: [{ action: "press" }] });
  assert.deepEqual(player.bombTarget, { x: player.x + 100, y: player.y });
  game.tick = game.roundStartedTick! + 1200 + 160;
  input({ bombCommands: [{ action: "release", aim: { x: 0, y: 1 } }] });
  const bomb = game.blasts[0]!.circle;
  assert.ok(game.boundaryInset >= 100);
  assert.equal(bomb.x, game.boundaryInset + RIDER_RADIUS);
  assert.equal(bomb.y, game.height - game.boundaryInset - RIDER_RADIUS);
});
test("cancel, rejected release and death clear preview without consuming; next round resets", () => {
  ...
  assert.equal(player.bombTarget, undefined);
  assert.equal(player.targetBombArmed, true);
  player.bombReadyAtTick = game.tick + 10;
  input({ bombCommands: [{ action: "press" }, { action: "release" }] });
  assert.equal(game.bombs.size, 0);
  assert.equal(player.targetBombArmed, true);
  player.bombReadyAtTick = 0;
  input({ bomb: true, bombCommands: [{ action: "press" }] });
  eliminatePlayer(game, "p0");
  assert.equal(player.bombTarget, undefined);
  assert.equal(player.targetBombArmed, true);
  input({});
...
test("queued release uses its own aim rather than a later packet in the same server tick", () => {
  ...
  input({
    bomb: true,
    aim: { x: 0.9, y: 0.9 },
    bombCommands: buffer.drainCommands(),
  });
  assert.equal(game.blasts[0]!.circle.x, 320);
  assert.equal(game.blasts[0]!.circle.y, 180);
});
...
test("Target detonates on the release tick, with damage and protection resolved immediately", () => {
  for (const protection of ["none", "star", "shield"] as const) {
    ...
    const released = input({
      bombCommands: [
        { action: "press", aim: { x: 0.75, y: 700 / 900 } },
        { action: "release", aim: { x: 0.75, y: 700 / 900 } },
      ],
    });
    assert.equal(game.bombs.size, 0);
    assert.equal(victim.alive, protection !== "none");
    assert.equal(
      released.events.filter((event) => event.type === "explosion").length,
      1,
    );
...
test("instant Target blast chains nearby bombs in the same tick", () => {
  ...
  game.bombs.set(99, {
    ...
    placedTick: 0,
    landsAtTick: 0,
    explodeAtTick: game.tick + 100,
    blastRange: 90,
    flightPath: [{ x: 800, y: 225, angle: 0 }],
  });
  const result = input({
    bombCommands: [
      { action: "press" },
      { action: "release", aim: { x: 0.5, y: 0.25 } },
    ],
  });
  assert.equal(game.bombs.size, 0);
  assert.equal(
    result.events.filter((event) => event.type === "explosion").length,
    2,
  );
});

...
const manifestPath = fileURLToPath(new URL('../public/themes/manifest.json', import.meta.url));
const themeFile = (id: string, file: string) => fileURLToPath(new URL(`../public/themes/${id}/${file}`, import.meta.url));
...
test('the manifest inventories every pickup the game has', () => {
  for (const type of PICKUP_TYPES) {
    assert.ok(manifest.sprites[`pickup-${type}`], `manifest.json has no entry for pickup-${type}`);
  }
});
test('the manifest inventories no pickup the game lost', () => {
  const known = new Set(PICKUP_TYPES.map(type => `pickup-${type}`));
  const strays = Object.keys(manifest.sprites).filter(name => name.startsWith('pickup-') && !known.has(name));
  assert.deepEqual(strays, [], 'manifest.json still inventories pickups that no longer exist');
});
...
test('the manifest describes exactly the themes the registry defines', () => {
  assert.deepEqual(Object.keys(manifest.themes).sort(), Object.keys(themes).sort(), 'manifest.json and the themes registry disagree about which styles exist');
  for (const theme of Object.values(themes)) {
    assert.equal(manifest.themes[theme.id]?.label, theme.label, `manifest.json mislabels ${theme.id}`);
    assert.equal(manifest.themes[theme.id]?.path, `/themes/${theme.id}`, `manifest.json has the wrong path for ${theme.id}`);
  }
});
test('every file the manifest names exists in every theme', () => {
  for (const theme of Object.values(themes)) {
    for (const [name, sprite] of Object.entries(manifest.sprites)) {
      assert.ok(existsSync(themeFile(theme.id, sprite.file)), `${theme.id} is missing ${sprite.file}, named by the manifest as ${name}`);
    }
  }
});

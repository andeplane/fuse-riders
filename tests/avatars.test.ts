...
test('ten distinct avatar ids address exactly ten atlas cells', () => {
  assert.equal(AVATARS.length, 10); const cells = new Set<string>();
  AVATARS.forEach(({ id }, index) => {
    assert.ok(isAvatarId(id)); assert.deepEqual(avatarCell(id), { column: index % 5, row: Math.floor(index / 5) });
    cells.add(JSON.stringify(avatarCell(id)));
    assert.deepEqual(parseClientMessage(JSON.stringify({ type: 'join', name: 'Rider', avatarId: id })), { type: 'join', name: 'Rider', avatarId: id });
  });
  assert.equal(cells.size, 10);
  for (const invalid of [undefined, null, 0, '', 'ROBOT', 'constructor', '../bad.png', {}, []]) {
    assert.equal(isAvatarId(invalid), false); assert.deepEqual(avatarCell(invalid), { column: 0, row: 0 });
    if (invalid !== undefined) assert.equal(parseClientMessage(JSON.stringify({ type: 'join', name: 'Rider', avatarId: invalid })), null);
  }
});
...
test('default and selected heads appear in authoritative snapshots without changing slot color', () => {
  ...
  addPlayer(state, { id: 'b', name: 'B', slot: 1, color: '#ff4fa3', avatarId: 'dragon' });

...
import { createGame, addPlayer, startMatch, step, COUNTDOWN_TICKS, returnToLobby, toSnapshot, type GamePhase } from '../src/shared/game.ts';
for (const phase of ['lobby', 'countdown', 'playing', 'roundOver', 'matchOver'] satisfies GamePhase[]) {
  test(`return to lobby from ${phase} clears match state without inventing session points`, () => {
    const game = createGame('before');
    for (let slot = 0; slot < 3; slot++) addPlayer(game, { id: `p${slot}`, name: `P${slot}`, slot, color: '#fff' });
    ...
    game.portalPairs = [{ id: 'old', gates: [{ x: 200, y: 200, halfLength: 60 }, { x: 600, y: 600, halfLength: 60 }], expiresAtTick: 999 }];
    game.blasts = [{ bombId: 1, ownerId: 'p0', circle: { x: 300, y: 300, radius: 150 }, expiresAtTick: 999 }];
    game.leaderboard.get('p0')!.totalScoreUnits = 300; const ledger = structuredClone([...game.leaderboard]);
    ...
    assert.deepEqual(game.portalPairs, []); assert.equal(game.roundWinnerId, undefined); assert.equal(game.matchWinnerId, undefined); assert.equal(game.roundStartedTick, undefined); assert.equal(game.phaseEndsAtTick, undefined);
...
test('empty party can return to lobby and invalid scope is rejected before mutation', () => {
  const game = createGame('before'); assert.throws(() => returnToLobby(game, '')); assert.equal(game.matchId, 'before');
  returnToLobby(game, 'empty'); assert.equal(game.phase, 'lobby'); assert.equal(game.players.size, 0);
});

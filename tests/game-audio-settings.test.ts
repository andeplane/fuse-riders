...
import { AUDIO_SETTINGS_KEY, DEFAULT_VOLUME, loadAudioSettings, requestAmbientAudio } from '../src/client/game-audio.ts';
...
test('music can start off by default, and a stored choice overrides that default either way', () => {
  const storage = createMemoryStorage();
  assert.deepEqual(loadAudioSettings(storage, true).muted, { music: true, effects: false }, 'first visit on a phone');
  assert.deepEqual(loadAudioSettings(storage, false).muted, { music: false, effects: false }, 'first visit elsewhere');
  storage.setItem(AUDIO_SETTINGS_KEY, JSON.stringify({ muted: { music: false, effects: false } }));
  assert.deepEqual(loadAudioSettings(storage, true).muted, { music: false, effects: false }, 'a phone that turned music on keeps it on');
  storage.setItem(AUDIO_SETTINGS_KEY, JSON.stringify({ muted: { music: true, effects: false } }));
  assert.deepEqual(loadAudioSettings(storage, false).muted, { music: true, effects: false }, 'a desktop that turned music off keeps it off');
});
...
test('the audio session is asked to be ambient where the API exists and ignored where it does not', () => {
  ...
  assert.equal(requestAmbientAudio({ audioSession: session }), true); assert.equal(session.type, 'ambient');
  assert.equal(requestAmbientAudio({}), false, 'no API');
  assert.equal(requestAmbientAudio({ audioSession: null }), false, 'a null session is not an API');
  const readOnly = Object.freeze({ type: 'auto' });
  assert.equal(requestAmbientAudio({ audioSession: readOnly }), false, 'a session that rejects the type reports that instead of throwing');
});

...
import { BOMB_MAX_CHARGE_TICKS, BOMB_MAX_LAUNCH_DISTANCE, BOMB_MIN_LAUNCH_DISTANCE, bombLaunchDistance, chargeRamp } from '../src/shared/bomb-launch.js';
...
test('with bounce the range walks back to the minimum and climbs again, on a period of twice the window', () => {
  ...
  assert.equal(bombLaunchDistance(max * 3, max, true), BOMB_MAX_LAUNCH_DISTANCE, 'and climbing again');
  for (let held = 0; held <= max * 4; held += 1) {
    assert.equal(bombLaunchDistance(held, max, true), bombLaunchDistance(held + max * 2, max, true), `tick ${held} repeats one period later`);
  }
});
test('the ramp is symmetric around the peak, so the way down offers the same distances as the way up', () => {
  for (let step = 0; step <= max; step += 1) assert.equal(chargeRamp(max - step, max, true), chargeRamp(max + step, max, true));
});
test('every distance stays inside the launch range, bouncing or not', () => {
  for (const bounce of [false, true]) {
    for (let held = 0; held <= max * 5; held += 1) {
      ...
      assert.ok(distance >= BOMB_MIN_LAUNCH_DISTANCE && distance <= BOMB_MAX_LAUNCH_DISTANCE, `held ${held} bounce ${bounce} gave ${distance}`);
    }
  }
});
test('the preview follows the same ramp, including across the turnaround', () => {
  ...
  for (let held = 0; held <= max * 3; held += 1) assert.equal(bombPreviewDistance(held, max, true), bombLaunchDistance(held, max, true));

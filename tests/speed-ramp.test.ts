...
function measure(game: GameState, controls = { left: true, right: false, bomb: false }) {
  ...
  return { distance: Math.hypot(rider.x - 400, rider.y - 450), turn: Math.PI * 2 - rider.angle };
}
...
test('riders travel faster as the round goes on, with turning circles kept the same size', () => {
  ...
  assert.ok(Math.abs(early.distance - RIDER_SPEED / TICK_HZ) < .01, `an opening stride is about normal: ${early.distance}`);
  game.roundStartedTick = game.tick - OVERTIME_START_TICK / 2 + 1;
  const middle = measure(game);
  assert.ok(Math.abs(middle.distance - RIDER_SPEED / TICK_HZ * 1.25) < 1e-9, `halfway to overtime is a quarter faster: ${middle.distance}`);
...
test('the next round starts at normal speed again', () => {
  ...
  assert.ok(Math.abs(measure(game).distance - RIDER_SPEED / TICK_HZ) < .01);
});
...
test('a checkpoint restored mid-ramp replays the original world tick for tick', () => {
  ...
  for (const player of game.players.values()) player.invulnerableUntilTick = 100_000;
  const weave = (tick: number) => new Map([['p0', { left: tick % 40 < 20, right: false, bomb: false }], ['p1', { left: false, right: tick % 50 < 20, bomb: false }]]);
  ...
  for (let tick = 0; tick < 200; tick++) {
    step(game, weave(tick + 7)); step(restored, weave(tick + 7));
  }
  assert.equal(encodeGameState(restored), encodeGameState(game));
  assert.equal(game.phase, 'playing');
  assert.ok(roundSpeedMultiplier(game.tick - game.roundStartedTick!) > 1.2, 'the comparison ran well into the ramp');
});

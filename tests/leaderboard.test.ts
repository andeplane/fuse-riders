...
test("awards five-player placement points", () => {
  ...
  assert.deepEqual(
    result.map((x) => [x.playerId, x.place, x.scoreUnits]),
    [
      ["e", 1, 5 * POINT_UNIT],
      ["a", 2, 3 * POINT_UNIT],
      ["b", 3, 2 * POINT_UNIT],
...
test("does not mutate participants and rejects invalid groups", () => {
  ...
  assert.throws(() =>
    rankRound([p("a"), p("b"), p("c"), p("d"), p("e"), p("f")]),
  );
});
...
test("timeout survivors earn survival points without a win bonus", () => {
  ...
  assert.deepEqual(
    placements.map((x) => [x.playerId, x.place, x.scoreUnits]),
    [
      ["a", 1, 2 * POINT_UNIT],
      ["b", 1, 2 * POINT_UNIT],
      ["c", 3, POINT_UNIT],
  ...
  assert.equal(
    placements.reduce((sum, placement) => sum + placement.scoreUnits, 0),
    5 * POINT_UNIT,
  );
});
...
test("accumulates repeated matches and preserves departures", () => {
  ...
  assert.deepEqual(entries.get("b"), {
    id: "b",
    name: "B",

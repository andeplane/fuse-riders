...
function world(tick: number, round: number, riders: Rider[], phase: ViewSnapshot['phase'] = 'playing'): ViewSnapshot {
  return {
    ...
    players: riders.map((rider, index) => ({
      ...
      x: rider.x, y: rider.y ?? 450, angle: 0, alive: true, roundWins: 0, bombReadyAtTick: 0, trail: [], extraBombs: 0, powerPickups: 0, reloadDurationTicks: 80, invulnerableUntilTick: 0, drunkUntilTick: 0, inkUntilTick: 0,
      targetBombArmed: false, tripleShotArmed: false, fiveShotArmed: false, gravityArmed: false, boostUntilTick: 0, shielded: false, shieldGraceUntilTick: 0, portalCooldownUntilTick: 0, portalGraceUntilTick: 0,
    })),
...
test('the timeline stretches the impact, focuses the protagonist, and plays hold, bars, clip and bars back', () => {
  ...
  assert.deepEqual(buildTimeline({ ...clip, moment: moment({ playerId: 'ghost', targetIds: ['b'] }) }).focus, { x: 900, y: 450 }, 'a missing protagonist focuses the victim');
  assert.equal(buildTimeline({ ...clip, moment: moment({ playerId: 'ghost', targetIds: [] }) }).focus, undefined);
});
...
test('the director arms at the pause, plays once the aftermath frames are in, and cues in, impact and out exactly once', () => {
  ...
  const riders = (tick: number) => [{ id: 'a', x: tick * 7.5, name: 'Ada' }, { id: 'b', x: 900 }];
  for (let tick = 1; tick <= 60; tick += 1) director.observe(world(tick, 1, riders(tick)), 'm', at(tick));
  director.moment(moment({ kind: 'bombDodge', tick: 40, targetIds: ['b'], value: 5 }), 'm', 1);
  director.moment(moment({ kind: 'directHit', tick: 60 }), 'm', 1);
  director.moment(moment({ kind: 'directHit', tick: 60 }), 'm', 1);
  director.moment(moment({ kind: 'multiKill', tick: 60, value: 3, targetIds: ['b', 'c'] }), 'm', 2);
  ...
  assert.equal(director.active, true, 'armed'); assert.equal(director.current, undefined, 'but not cut yet');
  assert.equal(director.frame(at(61) + 10), undefined, 'live play stays on screen while the aftermath arrives');
  for (let tick = 62; tick < 60 + CLIP_AFTER_TICKS; tick += 1) director.observe(world(tick, 1, riders(tick), 'roundOver'), 'm', at(tick));
  assert.equal(director.current, undefined, 'one frame short');
  director.observe(world(60 + CLIP_AFTER_TICKS, 1, riders(60 + CLIP_AFTER_TICKS), 'roundOver'), 'm', at(60 + CLIP_AFTER_TICKS));
  const clip = director.current!;
  assert.equal(clip.moment.kind, 'directHit', 'the heaviest moment of this round, not the dodge and not next round\'s');
  assert.equal(clip.frames[clip.frames.length - 1]!.tick, 60 + CLIP_AFTER_TICKS, 'the clip carries the full aftermath');
  const started = at(61);
  const bars = director.frame(started + HOLD_MS)!;
  assert.deepEqual(bars.cues, ['in'], 'the hold is counted from the pause, not from the cut');
  assert.deepEqual(director.frame(started + HOLD_MS + 10)!.cues, [], 'a stage cues once');
  const timeline = buildTimeline(clip);
  const impactAt = started + HOLD_MS + BARS_IN_MS + timeline.at[clip.frames.findIndex(frame => frame.tick === 60)]!;
  ...
  assert.equal(director.active, false, 'the next countdown takes the replay with it');
  const torn = director.frame(99_060)!;
  assert.equal(torn.stage, 'done'); assert.equal(torn.clip.key, done.clip.key, 'one done frame so the screen undresses');
...
test('the hold runs out: a screen whose aftermath frames never arrive plays what it has, and one without footage plays nothing', () => {
  ...
  for (let tick = 1; tick <= 60; tick += 1) late.observe(world(tick, 1, [{ id: 'a', x: tick }, { id: 'b', x: 900 }]), 'm', tick * 50);
  late.moment(moment({ tick: 60 }), 'm', 1);
  late.observe(world(61, 1, [{ id: 'a', x: 61 }, { id: 'b', x: 900 }], 'roundOver'), 'm', 3050);
  ...
  assert.equal(cut.stage, 'in'); assert.deepEqual(cut.cues, ['in']); assert.equal(late.current!.frames[late.current!.frames.length - 1]!.tick, 61, 'cut with the footage on hand');
  const short = buildTimeline(late.current!);
  assert.deepEqual(late.frame(3050 + HOLD_MS + BARS_IN_MS + short.playMs + 1)!.cues, ['out', 'impact'], 'an impact that is the last frame still stings as the bars leave');
  ...
  assert.equal(joined.frame(3050 + HOLD_MS), undefined, 'a screen that joined at the pause has no clip to show');
  ...
  for (let tick = 61; tick <= 72; tick += 1) matchOver.observe(world(tick, 3, [{ id: 'a', x: tick }, { id: 'b', x: 900 }], 'matchOver'), 'm', tick * 50);
  assert.equal(matchOver.current?.moment.tick, 55, 'the final-round pause replays too');
  const crowded = new ReplayDirector();
  for (let round = 1; round <= 6; round += 1) crowded.moment(moment({ round, tick: round * 100 }), 'm', round);
  crowded.observe(world(601, 6, [{ id: 'a', x: 0 }]), 'm', 0);
  crowded.observe(world(602, 6, [{ id: 'a', x: 0 }]), 'other', 1);
  assert.equal(crowded.active, false, 'stale rounds and other matches are pruned without effect');

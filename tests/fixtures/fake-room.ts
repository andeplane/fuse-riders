import { RoomRuntime, type Callbacks, type RoomTransport, type RuntimeDependencies, type TransportEvents } from '../../src/online/room-runtime.js';
...
export interface NetworkOptions { loss: number; baseMs: number; jitterMs: number; reliableMs: number; oneWayMs?: (from: string, to: string) => number }
interface Delivery { at: number; order: number; deliver: () => void }
export interface Recorded { states: Frame[]; events: { event: GameEvent; matchId: string; round: number; tick: number }[]; statuses: string[]; ready: [string, boolean][]; ended: number }
...
export class FakeNetwork {
  ...
  step(ms: number): void {
    ...
    while (this.now < end) {
      this.now += 10;
      const due = this.queue.filter(item => item.at <= this.now).sort((a, b) => a.at - b.at || a.order - b.order);
  ...
  dependencies(id: string): RuntimeDependencies {
    let tokens = 0;
    return { now: () => this.now, hidden: () => this.hidden.get(id) ?? false, token: () => `${id}-token-${++tokens}`, generation: () => this.generations.get(id) ?? 1,
      schedule: callback => { this.ticks.set(id, callback); return () => { this.ticks.delete(id); }; },
      onVisibilityChange: callback => { this.visibility.set(id, callback); return () => { this.visibility.delete(id); }; } };
  }
  setHidden(id: string, hidden: boolean): void { this.hidden.set(id, hidden); this.visibility.get(id)?.(); }
  /** Create (or re-create with a new generation) a member's runtime and transport. */
  add(id: string, settings: RoomSettings, extra: { displayOnly?: boolean; humanName?: string; generation?: number } = {}): RoomRuntime {
    if (extra.generation !== undefined) this.generations.set(id, extra.generation);
    const recorded: Recorded = { states: [], events: [], statuses: [], ready: [], ended: 0 };
    this.recorded.set(id, recorded);
    const callbacks: Callbacks = { state: frame => { recorded.states.push(frame); if (recorded.states.length > 50) recorded.states.shift(); }, event: (event, matchId, round, tick) => recorded.events.push({ event, matchId, round, tick }), status: text => { if (recorded.statuses.at(-1) !== text) recorded.statuses.push(text); }, ready: (peer, host) => recorded.ready.push([peer, host]), ended: () => recorded.ended++ };
    const runtime = new RoomRuntime('AB42', settings, callbacks, { transport: events => new FakeTransport(this, id, events), displayOnly: extra.displayOnly, humanName: extra.humanName, dependencies: this.dependencies(id) });
    this.runtimes.set(id, runtime);
    return runtime;
  }
  connect(id: string): void {
    const transport = this.transports.get(id)!; transport.online = true;
    this.schedule(this.now + this.options.reliableMs, () => {
      if (!transport.online) return;
      transport.events.welcome(id, this.hostId);
      for (const [other, peer] of this.transports) if (other !== id && peer.online) { transport.events.peer(other, true); peer.events.peer(id, true); this.openLink(id, other); }
    });
  }
  ...
  disconnect(id: string): void {
    const transport = this.transports.get(id); if (!transport) return; transport.online = false; transport.links.clear();
    for (const [other, peer] of this.transports) if (other !== id && peer.online) { peer.links.delete(id); peer.events.link(id, false); peer.events.peer(id, false); }
  }
  /** Simulate a page reload: the old runtime stops, a fresh one comes back with a higher generation. */
  reload(id: string, settings: RoomSettings, extra: { displayOnly?: boolean; humanName?: string } = {}): RoomRuntime {
    this.runtimes.get(id)!.stop(); this.disconnect(id); this.transports.delete(id);
    const runtime = this.add(id, settings, { ...extra, generation: (this.generations.get(id) ?? 1) + 1 });
    runtime.start(); return runtime;
  }
  frame(id: string): Frame | undefined { return this.recorded.get(id)!.states.at(-1); }
}
...
export class FakeTransport implements RoomTransport {
  ...
  send(id: string, data: unknown): boolean { const sent = this.network.sendReliable(this.id, id, data); if (sent) this.sentBytes += JSON.stringify(data).length; return sent; }
  ...
  explain(): string { return 'fake link'; }
  async stats() { return { direct: this.links.size, relayed: 0, buffered: 0 }; }
}

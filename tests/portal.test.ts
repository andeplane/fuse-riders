...
import { createPortalPair, findPortalTransit, fitPortalPair, MAX_PORTAL_PAIRS, type PortalPair, type PortalTransitOptions } from '../src/shared/portal.ts';
...
const transit = (overrides: Partial<PortalTransitOptions> = {}) => findPortalTransit({
  pairs: [pair], tick: 30, from: { x: 150, y: 200 }, to: { x: 250, y: 200 }, heading: 0.75,
...
const other: PortalPair = { id: 'q', gates: [{ x: 500, y: 200, halfLength: 100 }, { x: 1300, y: 600, halfLength: 100 }], expiresAtTick: 200 };
...
test('swept wall entry preserves heading and relative linked height', () => {
  assert.deepEqual(transit(), { pairId: 'p', entryGateIndex: 0, entryPoint: { x: 189, y: 200 }, exitPoint: { x: 1012, y: 600 },
  ...
  assert.equal(transit({ pairs: [short], from: { x: 150, y: 250 }, to: { x: 250, y: 250 } })?.exitPoint.y, 625);
});
...
test('both directions and earliest intersected wall work', () => {
  ...
  assert.equal(transit({ pairs: [aligned], from: { x: 1100, y: 200 }, to: { x: 100, y: 200 } })?.entryGateIndex, 1);
});
...
test('cooldown, expiry, inside, stationary, moving away and short movement do not retrigger', () => {
  ...
  assert.equal(transit({ pairs: [{ ...pair, id: 'second' }], cooldownUntilTick: 31 }), undefined);
  assert.equal(transit({ tick: 200 }), undefined); assert.equal(transit({ pairs: [] }), undefined);
...
test('unsafe and out-of-field exits defer transit and safety receives actual offset exit', () => {
  ...
  transit({ isSafeExit: (point, radius, pairId) => { checks.push([point, radius, pairId]); return true; } });
...
test('several live pairs each keep their own partner, whichever wall the step meets first', () => {
  ...
  assert.deepEqual(transit({ pairs, from: { x: 150, y: 200 }, to: { x: 550, y: 200 } }),
    { pairId: 'p', entryGateIndex: 0, entryPoint: { x: 189, y: 200 }, exitPoint: { x: 1012, y: 600 },
      heading: .75, cooldownUntilTick: 45, graceUntilTick: 40 });
  // The second pair's own gate, entered leftwards, leads to the second pair's own partner.
  assert.deepEqual(transit({ pairs, from: { x: 560, y: 200 }, to: { x: 450, y: 200 } })?.exitPoint, { x: 1288, y: 600 });
  // Distance along the step decides, not position in the list.
  assert.equal(transit({ pairs: [other, pair], from: { x: 150, y: 200 }, to: { x: 550, y: 200 } })?.pairId, 'p');
});

test('an exactly tied entry resolves by list order, which replay preserves', () => {
  const twin: PortalPair = { ...pair, id: 'twin', gates: [{ ...pair.gates[0] }, { x: 1300, y: 600, halfLength: 100 }] };
...
test('an expired pair is skipped while its neighbours stay open', () => {
  ...
  assert.deepEqual(transit({ pairs, from: { x: 150, y: 200 }, to: { x: 550, y: 200 } })?.exitPoint, { x: 1312, y: 600 });
  assert.equal(transit({ pairs: pairs.map(open => ({ ...open, expiresAtTick: 30 })) }), undefined);
});

test('every pair of a full list is scanned, including the last', () => {
  const filler = Array.from({ length: MAX_PORTAL_PAIRS - 1 }, (_unused, index): PortalPair =>
    ({ id: `filler-${index}`, gates: [{ x: 700 + index, y: 800, halfLength: 20 }, { x: 1500, y: 800, halfLength: 20 }], expiresAtTick: 200 }));

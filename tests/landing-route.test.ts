...
test("JOIN ROOM trims and upper-cases the typed code, or says what a code looks like", () => {
  assert.deepEqual(joinQuery("  ab42 "), { query: "?room=AB42" });
  assert.deepEqual(joinQuery(""), {
    error: "Enter a room code, for example AB42",
  });
  assert.deepEqual(joinQuery("no way"), {
    error: "Enter a room code, for example AB42",
  });

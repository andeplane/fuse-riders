...
import { TRAIL_STUD_SPACING, arenaWall, pixelWall, smoothWallRect, trailStuds } from '../src/client/arena-wall.js';
...
test('each visual style asks for a different wall', () => {
  ...
  assert.ok(pixel.kind === 'pixel' && pixel.bricks.length > 4, 'a pixel style draws a brick run per edge');
  assert.ok(smooth.kind === 'smooth' && smooth.strokeWidth === themes['clean-neon'].rendering.wallWidth);
});
...
test('every registered style resolves to a drawable wall', () => {
  ...
  for (const theme of Object.values(themes)) {
    ...
    if (wall.kind === 'pixel') assert.ok(wall.bricks.length > 0 && wall.studs.length === 4);
...
test('bricks stay on the canvas even for a boundary thinner than the brick run', () => {
  ...
  for (const inset of [2, 6, 20]) {
    ...
    if (inset >= 20) assert.ok(bricks.length > 4, `inset ${inset} must still draw a brick run per edge`);
    for (const brick of bricks) {
      assert.ok(brick.width > 1 && brick.height > 1, `degenerate at inset ${inset}`);
      assert.ok(brick.x >= 0 && brick.y >= 0, `off-canvas at inset ${inset}: ${JSON.stringify(brick)}`);
      assert.ok(brick.x + brick.width <= 320 && brick.y + brick.height <= 200, `past the canvas at inset ${inset}`);
    }
...
test('trail studs are evenly spaced along each segment', () => {
  ...
  assert.deepEqual(trailStuds(path, 5).map(s => s.x), [0, 5, 10, 15, 20, 25, 30]);
...
test('trail studs do not crawl when expired segments drop off the front', () => {
  ...
  for (let dropped = 1; dropped <= 4; dropped++) {
    ...
    assert.deepEqual(trailStuds(trimmed), kept, `studs moved after dropping ${dropped} segment(s)`);
  }
});
...
test('trail studs ignore zero-length and single-point paths', () => {
  ...
  assert.deepEqual(trailStuds([{ x: 0, y: 0 }, { x: NaN, y: 0 }]), []);
});
...
test('every brick keeps its chip inside itself', () => {
  ...
  for (const inset of [7, 10, 20, 29, 46, 200]) {
    for (const brick of pixelWall(1600, 900, inset).bricks) {
      assert.ok(brick.chip.x >= brick.x && brick.chip.x + brick.chip.width <= brick.x + brick.width,
        `chip escaped horizontally at inset ${inset}: ${JSON.stringify(brick)}`);
      assert.ok(brick.chip.y >= brick.y && brick.chip.y + brick.chip.height <= brick.y + brick.height,
...
test('no brick is too small for its own inner detail', () => {
  ...
  for (let inset = 20; inset <= 120; inset += 0.5) {
    for (const brick of pixelWall(1600, 900, inset).bricks) {
      ...
      assert.ok(brick.width - 6 > 0 && brick.height - 6 > 0, `sliver brick at inset ${inset}: ${JSON.stringify(brick)}`);
    }
  }
});

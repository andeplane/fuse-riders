...
import { BOMB_MAX_CHARGE_TICKS, bombLaunchDistance } from '../src/shared/bomb-launch.js';
...
test('preview extends every rendered frame at 30, 60 and 120 Hz while releases retain whole ticks', () => {
  for (const hz of [30, 60, 120]) {
    ...
    for (let frame = 1; frame <= hz; frame++) {
      ...
      assert.ok(Math.abs(distance - (100 + 250 * frame / hz)) < 1e-10);
      previous = distance;
    }
  }
  ...
  assert.equal(bombLaunchDistance(0.75), 100, 'authoritative launch still rounds down');
  assert.equal(bombPreviewDistance(0.75), 109.375);
});

test('preview clamps before charge and after full charge, including invalid ages', () => {
  for (const age of [-1, NaN, Infinity, -Infinity]) assert.equal(bombPreviewDistance(age), 100);
  for (const age of [24, 24.5, 999]) assert.equal(bombPreviewDistance(age), 400);
});

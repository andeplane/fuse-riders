...
const wrap = (game: GameState) => {
  for (const player of game.players.values())
    if (player.x > 1200) Object.assign(player, { x: 100, trail: [] });
};
...
const both = (game: GameState, control: GameState) => {
  wrap(game);
  wrap(control);
  step(game, new Map());
...
const ratio = (game: GameState, control: GameState, id: string) => {
  const stride = travelled(game, id),
    plain = travelled(control, id);
...
test("Nitro doubles the collector for five seconds, on distance alone, then hands the speed back", () => {
  ...
  close(
    ratio(game, control, "p0"),
    NITRO_SPEED,
    "the next tick is at double speed",
  );
  // Steer both riders left for a tick: the Nitro rider swings through the same angle over twice the distance.
  const angleBefore = rider.angle,
    plainAngleBefore = control.players.get("p0")!.angle;
  step(game, new Map([["p0", LEFT]]));
  step(control, new Map([["p0", LEFT]]));
  close(
    rider.angle - angleBefore,
    control.players.get("p0")!.angle - plainAngleBefore,
    "steering is unchanged, so a fast rider turns wide",
  );
  assert.ok(rider.angle !== angleBefore, "the riders actually turned");
  Object.assign(rider, { angle: 0 });
  Object.assign(control.players.get("p0")!, { angle: 0 }); // back on the row for the stride measurements below
  assert.equal(
    riderMotionStep(rider, game.tick, game.roundStartedTick).turn,
    riderMotionStep(
      control.players.get("p0")!,
      control.tick,
  ...
  close(
    ratio(game, control, "p0"),
    1,
    "the deadline tick itself is ordinary speed again",
  );
  ...
  assert.equal(
    rider.alive,
    true,
    "the measurement ran on a living rider throughout",
  );
});
...
test("two Nitros stack to four times speed until the first expires, then two, then one", () => {
  ...
  close(
    ...
    "two Nitros multiply to four times",
  );
  ...
  close(
    ratio(game, control, "p0"),
    1,
    "and to ordinary speed once both are spent",
  );
});
...
test("Snail halves every living rival for five seconds and leaves the collector alone", () => {
  ...
  eliminatePlayer(control, "p2"); // a dead rider is not a rival, and must not carry the effect into its next round
  drop(game, "p0", "snail");
  step(game, new Map());
  step(control, new Map());
  ...
  step(game, new Map());
  const slowed = rival.x - before;
  const plainBefore = control.players.get("p1")!.x;
  step(control, new Map());
  ...
  close(
    ...
    1,
    "the rival is back to ordinary speed on the deadline tick",
  );
});
...
test("Snails stack on a rival, and a Snail cancels a Nitro one for one", () => {
  ...
  close(
    ratio(game, control, "p1"),
    SNAIL_SPEED * SNAIL_SPEED,
    "two Snails leave a rival at a quarter speed",
  );
  ...
  close(
    ratio(game, control, "p1"),
    SNAIL_SPEED,
    "one Nitro cancels one of two Snails",
  );
  drop(game, "p1", "nitro");
  step(game, new Map());
  step(control, new Map());
...
test("checkpoints carry the deadlines exactly and reject lists past the stack bound or holding non-integers", () => {
  ...
  assert.equal(
    ...
    undefined,
    "more deadlines than the stack bound",
  );
  assert.equal(
    corrupt((player) => {
...
test("local prediction runs the Nitro rider at its stacked speed from the snapshot alone", () => {
  ...
  assert.deepEqual(
    [view.x, view.y, view.angle],
    [rider.x, rider.y, rider.angle],
    "the predicted pose is the simulated one, four times the ordinary stride",
  );
  assert.ok(
    Math.hypot(
      view.x - snapshot.players[0]!.x,
      view.y - snapshot.players[0]!.y,
    ) >
      (3 * RIDER_SPEED) / TICK_HZ,
    "the stride is well past anything a plain or boosted rider manages",
  );
  assert.equal(
    view.trail.length,
    snapshot.players[0]!.trail.length + 1,
    "the lead segment is still drawn at that stride",
  );
});
...
test("the phone chip shows the stacked factor and the time until it next drops, and the guide explains stacking", () => {
  ...
  assert.equal(
    speedEffectLabel("NITRO", NITRO_SPEED, [100, 40], 100),
    "NITRO · --",
    "a deadline on this tick has expired",
  );
  assert.equal(
    speedEffectLabel("NITRO", NITRO_SPEED, [130, 164], 100),
    "NITRO · ×4 · 1.5s",
    "four times until the earlier deadline, not the later one",
  );
  assert.equal(
    speedEffectLabel("NITRO", NITRO_SPEED, [130, 164], 130),
    "NITRO · ×2 · 1.7s",
    "then double until the later one",
  );
  assert.equal(
    speedEffectLabel("SLOWED", SNAIL_SPEED, [150], 100),
    "SLOWED · ×0.5 · 2.5s",
  );
  assert.equal(
    speedEffectLabel("NITRO", NITRO_SPEED, [] as number[], 5),
    "NITRO · --",
    "a rider from an older bundle carries no list and reads as none in force",
  );
  const guide = (type: string) =>
    POWERUP_GUIDE.find((entry) => entry.type === type)!;
  assert.equal(guide("nitro").name, "NITRO");
  assert.match(guide("nitro").description, /2× speed for 5s.*stacks.*4×/);
  assert.equal(guide("snail").name, "SNAIL");
  assert.match(
    guide("snail").description,
    /rivals crawl at 0\.5× speed for 5s.*cancels a Nitro/,
  );
  assert.ok(
    guide("nitro").spawnsByDefault && guide("snail").spawnsByDefault,
    "both are in the default drop table",
  );
});

...
import { JOIN, ACTION, STEER, PRESENCE, PRESS, RELEASE } from '../src/shared/input-log.js';
import { encodeSnapshot, decodeSnapshot, SnapshotAssembler } from '../src/online/snapshot.js';
import { PACKET_ENTRIES } from '../src/online/stream.js';

function room(ids = ['creator', 'guest']) {
  const world = new World(createRoomState('m', defaultRoomSettings()), 'creator', 'creator');
...
function heartbeat(world: World, id: string, through: number) {
  ...
  assert.equal(world.receive(id, [], stream.lastSeq, through, through).status, 'accepted');
}
function snapshotCopy(source: World) {
  ...
  for (const stream of decoded.streams) {
    const log = copy.stream(stream.id, stream.generation, {
      seq: stream.seq, tick: copy.tick, gesture: stream.gesture,
    });
    for (let offset = 0; offset < stream.entries.length; offset += PACKET_ENTRIES) {
      const part = stream.entries.slice(offset, offset + PACKET_ENTRIES);
      assert.equal(log.receive(part, part.at(-1)![0], copy.tick, copy.tick + 60, copy.tick).status, 'accepted');
    }
  }
  return copy;
}
...
test('generation transition: a returning creator applies its new management stream', () => {
  ...
  world.advance(72);
  assert.equal(world.state.folds.get('creator')!.generation, 2,
...
test('generation transition: guest input on the PRESENCE tick is applied', () => {
  ...
  assert.equal(world.state.folds.get('guest')!.flags, 2,
...
test('snapshot replay includes the retired generation between baseline and replacement', () => {
  ...
  heartbeat(source, 'creator', 100); heartbeat(source, 'guest', 100); heartbeat(source, 'other', 64);
  source.advance(70);
...
test('snapshot completeness accounts for a missing entry sharing the last contiguous tick', () => {
  ...
  source.receive('guest', [[1, 72, STEER, 1], [3, 80, RELEASE, 1]], 3, 90, 90);
  source.advance(84);
  const copy = snapshotCopy(source);
  assert.ok(copy.tick < 72, `served ${copy.tick}: a missing entry may share the last contiguous tick, so 72 is not complete`);
  ...
  assert.equal(repairedCopy.status, 'accepted',
...
test('snapshot completeness must not fall back to speculative state after heartbeat history is lost', () => {
  ...
  source.advance(65);
  heartbeat(source, 'creator', 80); heartbeat(source, 'other', 80);
  source.receive('guest', [[2, 80, STEER, 0]], 2, 80, 80);
  source.advance(75);
  assert.equal(source.completeTick(), 65, 'the heartbeats confirmed 65 before the gap opened'); assert.ok(source.servable().tick <= 65);
  const copy = snapshotCopy(source);
  assert.equal(source.receive('guest', [[1, 70, STEER, 1]], 2, 80, 80).status, 'accepted');
  assert.equal(copy.receive('guest', [[1, 70, STEER, 1]], 2, 80, 80).status, 'accepted',

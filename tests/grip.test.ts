...
function playing() {
  ...
  for (const p of game.players.values())
    Object.assign(p, { x: 400, y: 200 + p.slot * 250, angle: 0, trail: [] });
  return game;
}
...
const steering = (right: boolean): InputIntent => ({
  left: !right,
  right,
  bomb: false,
});
...
test("GRIP increases left/right steering by 75% from the next tick without changing travel speed; neutral and cancelled steering stay straight", () => {
  for (const right of [false, true]) {
    ...
    assert.ok(
      Math.abs(p.angle - turn) < 1e-12,
      "collection tick uses ordinary steering",
    );
    const before = { ...p };
    step(game, input);
    ...
    assert.ok(
      Math.abs(Math.abs(p.angle - before.angle) - 1.75 * ungripped.turn) <
        1e-12,
    );
    assert.ok(
      Math.abs(
        Math.hypot(p.x - before.x, p.y - before.y) - ungripped.distance,
      ) < 1e-12,
    );
    for (const controls of [
      { left: false, right: false, bomb: false },
      { left: true, right: true, bomb: false },
    ]) {
      const angle = p.angle;
...
test("repeat GRIP drops remain available, including same-tick duplicates and a nearer ineligible rider", () => {
  ...
  assert.equal(
    events.filter((e) => e.type === "pickupCollected" && e.pickupId === first)
      .length,
    1,
  );
  assert.equal(
    events.filter((e) => e.type === "pickupCollected" && e.pickupId === second)
      .length,
    0,
  );
  ...
  for (const right of [true, false]) {
    ...
    for (let tick = 0; tick < 22; tick++) {
      ...
      assert.ok(
        ...
        `both riders survive tick ${tick + 1}`,
      );
      if (tick === 12) {
        const diameter = Math.abs(inner.y - innerStart);
        assert.ok(
          diameter > 58 && diameter < 64,
          `tight U-turn spans ${diameter}`,
        );
      }
    }
    ...
    assert.ok(
      (outer.y - inner.y) * sign > TRAIL_WIDTH / 2 + RIDER_CONTACT_RADIUS,
      "clearance remains after the maneuver",
    );
...
test("checkpoint restore preserves GRIP steering and pickup eligibility; corrupt flags are rejected", () => {
  ...
  for (const value of ['"yes"', "1", "null", "{}"]) {
    assert.equal(
      decodeGameState(
        encodeGameState(game).replace('"grip":true', `"grip":${value}`),
      ),
      undefined,
    );
  }
